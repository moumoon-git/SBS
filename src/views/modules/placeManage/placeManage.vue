          ...
          <el-table-column header-align="center" align="center" label="操作">
            <template slot-scope="scope">
               <span
                ...
                style="cursor:pointer;color:#02A7F0;font-size:13px;font-weight:400"
              >查看</span>
              <span
              ...
              <span
                ...
                style="cursor:pointer;color:#D9001B;font-size:13px;font-weight:400"
              >删除</span>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-container>
        ...
        <el-header style="height:120px">
          ...
          >
            ...
            <el-form-item label="地点位置" prop="locationPlace">
              <el-input
                v-model="tableDialogForm.locationPlace"
                placeholder="请选择地点位置"
            ...
            <el-form-item label="备注信息">
              <el-input
                v-model="tableDialogForm.remark"
                placeholder="请输入备注信息"
                type="textarea"
              ></el-input>
            </el-form-item>
        ...
        <el-container>
             <Location
                :address="tableDialogForm.locationPlace"
                :lngLat="[tableDialogForm.longitude, tableDialogForm.latitude]"
                :cgcsLngLatInit="[tableDialogForm.cgcsLongitude, tableDialogForm.cgcsLatitude]"
                @confirm="handleLocationConfirm"
              />
        </el-container>
      </el-container>
    </el-dialog>
    <LookMeetingTerminalDialog ref="lookMeetingTerminalDialog"></LookMeetingTerminalDialog>
  </div>
</template>
...
import LookMeetingTerminalDialog from './components/lookMeetingTerminalDialog.vue'
...
export default {
  data() {
    return {
      ...
      tableDataRule: {
        ...
        locationPlace:  [
          { required: true, message: "地点位置不能为空", trigger: "blur" }
        ]
      },
      isAll: false, //是否点击全部
  ...
  methods: {
     ...
     */
    lookInfo(id) {
      this.$refs.lookMeetingTerminalDialog.init(id)
    },
    /**
     * @description: 确认位置
     * @param {arg} 位置信息
    ...
    handleLocationConfirm(...arg) {
      this.tableDialogForm.locationPlace = arg[0]
      this.tableDialogForm.longitude = arg[1][0]
      this.tableDialogForm.latitude = arg[1][1]
      // 大地坐标系
      this.tableDialogForm.cgcsLongitude = arg[3][0];
      this.tableDialogForm.cgcsLatitude = arg[3][1];
    },
    // 新增
    addTable() {
      this.tableTitle = "新增";
    ...
    getInfo(id) {
      ...
      this.$http({
        url: this.$http.adornUrl(
          `/meeting/meetingTerminal/infor/${this.tableDialogForm.id}`
        ),
        method: "get",
    ...
    deleteData(id) {
      ...
      this.$confirm(`确定删除?`, "提示", {
        ...
        .then(() => {
          ...
          this.$http({
            url: this.$http.adornUrl("/meeting/meetingTerminal/delete"),
            method: "post",
    ...
    getTreeData() {
      this.$http({
        url: this.$http.adornUrl(
          `/meeting/meetingTerminalGroup/info/${this.treeId}`
        ),
        method: "get",
        params: this.$http.adornParams()
      }).then(({ data }) => {
        if (data && data.code === 0) {
          ...
          this.treeDialogForm.parentId =
            data.MeetingTerminalGroupEntity.parentId;
          this.treeDialogForm.name = data.MeetingTerminalGroupEntity.name;
          this.treeDialogForm.gmtCreate =
            data.MeetingTerminalGroupEntity.remark;
          this.treeVisible = true;
        } else {
          this.$message.error(data.msg);
        }
    ...
    deleteTree() {
      ...
      this.$confirm(`确定要进行删除操作?`, "提示", {
        ...
        setTimeout(() => {
          ...
          that
            .$http({
              url: that.$http.adornUrl(`/meeting/meetingTerminalGroup/delete`),
              method: "post",
              data: this.$http.adornData({
                ids: that.treeIds
              })
    ...
    getDataList() {
      ...
      this.$http({
        url: this.$http.adornUrl(`/meeting/meetingTerminal/list`),
        method: "post",
        data: this.$http.adornData({
          page: this.pageIndex,
          limit: this.pageSize,
