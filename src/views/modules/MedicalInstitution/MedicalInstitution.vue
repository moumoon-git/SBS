  ...
  <div>
    <sv-template
      ...
      :download-template-config="{
        api: '/resoure/resourcehealthdept/gettemplate'
      }"
      :delete-selected-config="isPlatformRoot ?
      { api: '/resoure/resourcehealthdept/delete' } : {}"
      :overload="overload"
    ...
    >
      ...
      <template #middle__operation__button>
        <sv-button
          v-if="isPlatformRoot"
          type="primary"
          @click="addOrEditOrViewMedicalInstitution('add')"
        >
          添加
        </sv-button>
      </template>
        ...
        >
          <template slot-scope="scope">
            <sv-table-operation
              ...
              @view="addOrEditOrViewMedicalInstitution('detail', scope.row)"
              @edit="addOrEditOrViewMedicalInstitution('update', scope.row)"
              @delete="
...
import AddOrEditOrView from './MedicalInstitution-add-update-view.vue';
...
export default {
  ...
  data() {
    return {
      downLoadUrl: 'ser/resoure/resourcehealthdept/xlsOutputError',
      // 将要删除的多个id
      deleteIds: [],
      // 是否是本平台
      isPlatformRoot: true,
      ...
      // 左边栏-树形控件配置
      treeSetting: {
        api: '/resoure/resourcehealthdeptgroup/list', // 后台接口列表
        edit: {
          add: '/resoure/resourcehealthdeptgroup/save', // 添加节点接口
          update: '/resoure/resourcehealthdeptgroup/update', // 更新树节点接口
          delete: true, // 是否可删除
      ...
      // 中间栏-请求表格数据的参数
      tableRequest: {
        api: '/resoure/resourcehealthdept/list',
        method: 'post',
        data: {
          page: 1,
          limit: 10,
      ...
      // 导入错误底部提示信息
      errorTips: [
        '1、不能在该Excel表中对机构信息进行删除或修改；',
        '2、Excel中红色字段为必填字段,黑色字段为选填字段，字段值里请不要填入特殊字符；',
        '3、分组：上下级之间用“/”隔开，且从最上级开始，例如“分组/分组1”。若导入的数据中拥有与分组中已存在的分组名相同的分组，将自动与该分组关联，若导入的分组名称在系统中不存在，系统将自动创建该分组',
        '4、所属区域：上下级区域用“/”隔开，且从最上级区域开始，例如“广州市/白云区/黄石街道”。类型字段长度不能超过32个字符。若导入的数据中拥有与区域中已存在的区域名相同的区域，将自动与该区域关联，若导入的区域名称在系统中不存在，系统将自动创建该区域',
        '5、类别和等级为固定选项的字段，提供选择，而非输入；',
        '6、名称字段长度不能超过50 个字符、地址字段长度不能超过200个字符、装备描述字段长度不能超过2000个字符、备注字段长度不能超过200个字符；',
        '7、手机号码：请直接输入手机号码，系统将自动匹配号码相同的联系人，若导入的号码在系统中不存在，系统将根据已填写的信息自动新增该联系人至“综合保障-医疗机构”的分组里；',
      ],
      // 导入错误表格必填项
      errorRequired: ['名称', '分组'],
    };
  ...
  methods: {
    ...
    getTreeAndTableData(platformId) {
      ...
      this.treeSetting.api = `/resoure/resourcehealthdeptgroup/list?platformId=${platformId}`;
    ...
    batchDelete(ids) {
      const request = {
        method: 'post',
        url: '/resoure/resourcehealthdept/delete',
        baseURL: window.SITE_CONFIG.baseUrl,
        data: {
          ids,
        },
     ...
     */
    addOrEditOrViewMedicalInstitution(type, row) {
      // 分组
     ...
     */
    viewContent(row) {
      this.$refs.viewContent.setData(row);
    },
    ...
    getTreeChildren(obj) {
      ...
      this.tableRequest.data.groupId = [...new Set(this.tableRequest.data.groupId)];
    ...
    doDeleteTreeNode() {
      this.$http({
        url: this.$http.adornUrl('/resoure/resourcehealthdeptgroup/delete'),
        method: 'post',
