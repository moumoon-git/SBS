...
function getBaseMap(pmapContainer, MAP_TYPE, mapConfig, initVis) {
    ...
    switch (MAP_TYPE) {
        ...
        case 'SC':
            ...
            break;
        case 'ShenShan':
            retData = getShenShanMap(mapContainer, initVis);
...
// 对接深汕底图
function getShenShanMap(mapContainer,initVis) {
  const maxExtent = new SuperMap.Bounds(114.72392,22.69161, 115.28440,23.05739)
  // 创建dispatchMap对象和动态图层
  const map = new SuperMap.Map(mapContainer, {
      controls: [
          new SuperMap.Control.MousePosition(), // 显示经纬度坐标
  ...
  const wmsLayer = new SuperMap.Layer.WMTS({
      name: 'shenshan202111',
      url: window.config.mapUrl.ShenShanImageWMTSMapUrl,
  ...
  const dlLayer = new SuperMap.Layer.WMTS({
      name: 'jc_2000_dll',
      url: window.config.mapUrl.ShenShanRoadMapUrl,
      layer: 'jc_2000_dll',
      style: 'default',
    ...
    dlLayer.layerType = 'img';
    dlLayer.isBaseLayer = false;

  // 背景图
  const bgLayer = new SuperMap.Layer.WMTS({
      name: 'jc_dom_gdpro_4490',
      url: window.config.mapUrl.ShenShanBGWMTSMapUrl,
      layer: 'jc_dom_gdpro_4490',
      style: 'default',
      matrixSet: 'GlobalCRS84Scale_jc_dom_gdpro_4490',
      resolutions,
      opacity: 1,
      tileOrigin: new SuperMap.LonLat(-180.0, 90.0), // 瓦片矩阵左上角
      requestEncoding: 'REST',
  ...
  const sLayer = new SuperMap.Layer.WMTS({
      name: '深汕特别合作区矢量电子地图4490政务版',
      url: window.config.mapUrl.ShenShanVectorWMTSMapUrl,
      layer: '深汕特别合作区矢量电子地图4490政务版',
      style: 'default',
      matrixSet: 'GlobalCRS84Scale_深汕特别合作区矢量电子地图4490政务版',
      resolutions,
      opacity: 1,
      tileOrigin: new SuperMap.LonLat(-180.0, 90.0), // 瓦片矩阵左上角
      requestEncoding: 'REST',
  ...
  if (initVis === '矢量') {
      ...
      wmsLayer.setVisibility(false);
      dlLayer.setVisibility(false);
      map.addLayer(bgLayer);
      map.addLayer(wmsLayer);
      map.addLayer(dlLayer);
  } else {
      sLayer.setVisibility(false);
      map.addLayer(bgLayer);
      map.addLayer(wmsLayer);
      map.addLayer(dlLayer);
      map.addLayer(sLayer);
