...
class handleMap {
    ...
    getTDTDotInfo(mapLocation, cb) {
        const promise = new Promise((resolve, reject) => {
            ...
            const data = {
                postStr: obj,
                type: 'geocode',
                tk: atob(window.config.mapConfig.tk),
            };
    ...
    coverageAddEventlist() {
        ...
        const layer = [];
        for (const key in this.allCoverage) {
            if (this.allCoverage[key].CLASS_NAME.indexOf('Markers') === -1 && this.allCoverage[key].CLASS_NAME.indexOf('HeatMapLayer') === -1) {
    ...
    dataScreening(key, condition, conditionData) {
        const promise = new Promise((resolve, reject) => {
            if (this.converge[key]) {
                ...
                const sourceData2 = layer.features.filter(ele => ele.info[condition] === conditionData);
