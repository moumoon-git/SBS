...
class handleMap {
    ...
    examineRepetition(coverageName, cb) {
        for (const key in this.allCoverage) {
            if (coverageName == key) {
                ...
                throw console.error('The coverage already exists' + key);
            }
        }
        cb && cb();
    }
        ...
        if (this.map) {
            ...
            this.map.div.onclick = function(e) {
                if (that.mapClickObj.flag) {
                    ...
                    if (window.config.mapConfig.mapBusiness === 'SJS') {
                        that.getSJSDotInfo(mapLocation, that.mapClickObj.cb);
                    } else if (window.config.mapConfig.mapBusiness === 'EGIS') {
                        that.getEGSDotInfo(mapLocation, that.mapClickObj.cb);
                    } else if (window.config.mapConfig.mapBusiness === 'TDT') {
                        that.getTDTDotInfo(mapLocation, that.mapClickObj.cb);
                    } else {
                        that.getGDDotInfo(mapLocation, that.mapClickObj.cb);
                    }
                }
            };
        } else {
    ...
    // 通过经纬度获取点信息
    acquisitionPoint(params) {
        return this.getGDDotInfo(params);
    }
    ...
    // 高德逆地理获取
    getGDDotInfo(mapLocation, cb) {
        const promise = new Promise((resolve, reject) => {
            this.tdtTrGd((mapLocation.lon || mapLocation.longitude), (mapLocation.lat || mapLocation
                .latitude)).then((res) => {
                // console.log(res);
                get.geocoder_Gd({
    ...
    getEGSDotInfo(mapLocation, cb) {
        ...
        get.geocoder_egisGetTk().then((res) => {
            const data = {
                ...
                location: `${mapLocation.lon},${mapLocation.lat}`,
            };
    ...
    setCoverTextVis(flag) {
        const promise = new Promise((resolve, reject) => {
            let className = this.mapEl.className;
            if (flag) {
                // 显示
    ...
    setCoverTextColor(color) {
        const promise = new Promise((resolve, reject) => {
            ...
            // 显示
            var elArr = this.mapEl.querySelectorAll('text');
    ...
    calculateLineDistance(start, end) {
         ...
         * @param {*}
         * @description: 要先将坐标转换为高德之后再计算, 1-10KM偏差为1%,1000KM偏差0.0000002% 数值越大偏差越小
         * @LastEditors: Seven
        ...
        return new Promise((resolve, reject) => {
            ...
            var from = turf.point([start.longitude, start.latitude]);
            var to = turf.point([end.longitude, end.latitude]);
        ...
        const promise = new Promise((resolve, reject) => {
            if (this.allCoverage[coverageName]) {
                ...
                if (coverage.id.toLowerCase().indexOf('vector') !== -1) {
                    if (coverage.features && coverage.features.length !== 0 && Array.isArray(coverage
                            .features)) {
                        arr = coverage.features;
                    }
                } else if (coverage.id.toLowerCase().indexOf('markers') !== -1) {
                    if (coverage.markers && coverage.markers.length !== 0 && Array.isArray(coverage
                            .markers)) {
                ...
                this.map.layers.forEach((ele, index) => {
                    if (ele.name === coverageName) {
                        if (ele.CLASS_NAME === 'SuperMap.Layer.Markers') {
                            ...
                            ele.clearMarkers();
                        } else if (ele.CLASS_NAME === 'SuperMap.Layer.ClusterLayer' || ele
        ...
        const promise = new Promise((resolve, reject) => {
            if (this.map) {
                this.getMapMaxZoom().then((res) => {
                    this.map.setCenter(new SuperMap.LonLat(data.longitude, data.latitude), zoom || res);
                    resolve();
                })
            } else {
                reject();
    ...
    searchPOIdata(key, type = '') {
        const promise = new Promise((resolve, reject) => {
            if (this.map) {
                ...
                if (this.mapType.type === 'EGIS') {
                    this.getExtent().then((res) => {
                        const bounds = `${res.right},${res.bottom},${res.left},${res.top}`
                        integration.mapPOISearch(key, mapTypeStr, bounds, obj).then((res) => {
                            console.log(res);
                            resolve(res);
                        });
                    });
                } else {
                    integration.mapPOISearch(key, mapTypeStr, '', '', obj).then((res) => {
                        console.log(res);
                        resolve(res);
                    });
                }
    ...
    calculateAreas(geometry) {
        const promise = new Promise((resolve, reject) => {
            // 获得图层几何对象
            if (geometry.id.indexOf('SuperMap.Geometry.Polygon') === -1 && geometry.id.indexOf(
                    'SuperMap.Geometry.GeoPolygonEx') === -1) {
                reject();
                throw new Error('feature is not Polygon');
            }
            let obj = JSON.parse(geometry.toJSON());
            let polygonData = obj.controlPoints.reduce((pre, ele) => {
                let arr = [ele.x, ele.y];
                pre.push(arr);
                return pre;
            }, []);
            polygonData.push(polygonData[0]);
            var polygon = turf.polygon([polygonData]);
    ...
    pathPlanning(obj, type) {
        const promise = new Promise((resolve, reject) => {
            ...
            let mapType = window.config.mapConfig.mapBusiness;
            if (mapType === 'GD') {
                let origin = obj.origin.split(',');
                let destination = obj.destination.split(',');
                Promise.all([
        ...
        const promise = new Promise((resolve, reject) => {
            data.map(e => {
                e.steps.map(x => {
                    ...
                    const handleFun = lonlatArrStr.reduce((pre,
                        ele) => {
                        const splitArr = ele.split(',');
                        pre.push(this.gdTrTdt(Number(
                                splitArr[0]),
                            Number(splitArr[1])
                        ));
                        return pre;
    ...
    creatTempMark(data, addEventlist = '') {
        const promise = new Promise((resolve, reject) => {
            ...
            let offset = new SuperMap.Pixel(-(size.w / (data.offsetX ? data.offsetX : 2)), -size.h / (data.offsetY ? data.offsetY : 1.65));
            const icon = new SuperMap.Icon((data.src ? data.src : this.markImg), size, offset);
            const tempMarker = new SuperMap.Marker(new SuperMap.LonLat(data.longitude, data.latitude),
                icon);
            tempMarker.data = data;
            let obj = {};
            // label有值的话渲染文字
            if (data.label && tempMarker.icon && tempMarker.icon.imageDiv) {
                let el = tempMarker.icon.imageDiv;
                let imgEl = el.getElementsByTagName('img');
    ...
    dataScreening(key, condition, conditionData) {
        const promise = new Promise((resolve, reject) => {
            if (this.converge[key]) {
                ...
                const sourceData = layer.clusterPoints.filter((ele) => ele.info[condition] ===
                    conditionData);
    ...
    createdVectorCoverage(coverageName, style) {
        const promise = new Promise((resolve, reject) => {
            ...
            this.examineRepetition(coverageName, () => {
                ...
                // 拖拽的图层
                that.dragFeature[coverageName] = new SuperMap.Control.DragFeature(that.allCoverage[
                    coverageName]);
                that.plottingEdit[coverageName] = new SuperMap.Control.PlottingEdit(that
                    .allCoverage[coverageName]);
                that.map.addLayer(that.allCoverage[coverageName]);
        ...
        const promise = new Promise((resolve, reject) => {
            ...
            if (this.allCoverage[coverageName].CLASS_NAME.toLowerCase().indexOf('vector') != -1) {
                if (str === 'Path' ||
                    ...
                    str === 'DoubleArrow' ||
                    str === 'DiagonalArrow') {
                    const edit = new SuperMap.Control.DrawFeature(this.allCoverage[coverageName], eval(
                        `SuperMap.Handler.${str}`));
                    // console.log(edit);
    ...
    // 绘制圆圈
    vectorDrawCir(coverageName, data, style) {
        const promise = new Promise((resolve, reject) => {
            if (this.allCoverage[coverageName]) {
                const datas = data;
            ...
            if (this.allCoverage[coverageName]) {
                ...
                arr.forEach((ele, index) => {
                    if (ele.name &&
                        ...
                        (ele.id.indexOf('ModifyFeature') != -1 ||
                            ele.id.indexOf('DrawFeature') != -1)) {
                        // console.log(index);
        ...
        const promise = new Promise((resolve, reject) => {
            if (that && that.geometry) {
                ...
                if (editFlag === true) {
                    ...
                    id = document.getElementById(elID) ? elID : document.getElementById(parentId) ?
                        parentId : elID;
                }
    ...
    setPoint(coverageName, data) {
        let promise = new Promise((resolve, reject) => {
            if (this.allCoverage[coverageName]) {
                // 生成点对象 lon, lat
                let point = new SuperMap.Geometry.Point(data.lon, data.lat);
                // 生成图形对象 在这里时可以设置style
                let vetor = new SuperMap.Feature.Vector(point);
