<template>
  <div class="location">
    ...
    <div class="location__map" ref="map" id="superMap"></div>
    <div class="location__address">
      {{ currentAddress }}
      <sv-button class="location__confirm" @click="confirm" v-if="!displayOnly"
        >确定位置</sv-button
      >
    </div>
  </div>
...
<script>
import handleMap from './mapJs/handleMapFun.js';
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      const map = (this.currentMap = new handleMap().init('superMap', '矢量'));
      map.setCentent(
        // 初始中心点
        {
      ...
      if (!this.displayOnly) {
        map.clickDroppoint((res) => {
          ...
          this.currentLngLat = this.specialProject.includes(window.config.project) ? [res.mapLocation.transFromLon, res.mapLocation.transFromLat] : [res.mapLocation.lon, res.mapLocation.lat];
          // 大地2000坐标
          this.cgcsLngLat = [res.mapLocation.lon, res.mapLocation.lat]
    ...
    async handleClickSearchResult(res) {
      ...
      // 大地2000坐标
      this.cgcsLngLat = res.location.split(',')
      if(this.specialProject.includes(window.config.project)) {
        this.currentLngLat = res.transFromLonLat.split(',')
      }else {
  ...
  mounted() {
    ...
    this.currentLngLat =
      this.lngLat[0] && this.lngLat[1]
        ? this.lngLat
        : [this.$cookie.get('longitude'), this.$cookie.get('latitude')];
    this.currentAddress = this.address ? this.address : '';
    this.cgcsLngLat = (this.cgcsLngLatInit[0] && this.cgcsLngLatInit[1]) ? this.cgcsLngLatInit : [];
    this.$nextTick(() => {
      this.init();
    });
  },
