  ...
  <div>
    <sv-template
      ...
      :download-template-config="{
        api: '/resoure/resourcelaw/gettemplate'
      }"
      :delete-selected-config="isPlatformRoot ?
        { api: '/resoure/resourcelaw/delete' } : {}"
      :overload="overload"
        ...
        >
          <template slot-scope="scope">
            <sv-table-operation
              ...
              @view="addOrEditLaw('detail', scope.row)"
              @edit="addOrEditLaw('update', scope.row)"
              @delete="
...
import AddOrEditOrView from './Law-add-update-view.vue';
  ...
  data() {
    return {
      ...
      treeSetting: {
        ...
        edit: {
          add: '/resoure/resourcelawgroup/save', // 添加节点接口
          update: '/resoure/resourcelawgroup/update', // 更新树节点接口
          delete: true, // 是否可删除
      ...
      // 导入错误表格必填项
      errorRequired: ['法规名称', '版本号', '分组', '法规内容'],
    };
  },
  methods: {
    /**
    ...
    getTreeAndTableData(platformId) {
      ...
      this.treeSetting.api = `/resoure/resourcelawgroup/list?platformId=${platformId}`;
    ...
    batchDelete(ids) {
      const request = {
        method: 'post',
        url: '/resoure/resourcelaw/delete',
        baseURL: window.SITE_CONFIG.baseUrl,
        data: {
          ids,
        },
    ...
    doDeleteTreeNode() {
      this.$http({
        url: this.$http.adornUrl('/resoure/resourcelawgroup/delete'),
        method: 'post',
