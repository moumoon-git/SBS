<template>
  <el-dialog :title="title" :visible.sync="visible" width="1100px" center append-to-body :close-on-click-modal="false" :modal="false">
    ...
    <el-row :gutter="20">
      ...
      <el-col :span="8">
        ...
        <div class="candidate">
          <el-checkbox :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange" v-if="isShowAll">全选</el-checkbox>
          <el-checkbox-group v-model="nameList" @change="handleCheckedCitiesChange">
            <el-checkbox
      ...
      <el-col :span="8">
        <div class="candidate" style="height: 540px;">
          ...
          <div style="padding-left: 10px;margin-top: 10px;text-align: left; overflow: auto;height: 400px;">
            <!-- <el-tag
...
export default {
  ...
  methods: {
    ...
    handleCheckAllChange(val) {
      console.log('list',this.nameList,this.personList)
      console.log('val'+val)
      if(val) {
           
           this.personList.forEach(item => {
           if(this.nameList.indexOf(item.id) === -1) {
           this.nameList.push(item.id)
           }
        })
        let chooseIdList = []
        ...
        for (let i in this.personList) {
            ...
            if(chooseIdList.indexOf(this.personList[i].id) === -1) {
        ...
        this.nameList = []
        for(let i in this.personList) {
        for (let j in this.chooseList) {
          if (this.chooseList[j].id == this.personList[i].id) {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          for(let i in this.chooseList) {
            if(this.nameList.indexOf(this.chooseList[i].id) === -1) {
              this.nameList.push(this.chooseList[i].id)
            }
          }
          ...
          for(let i in idList) {
               if(chooseIdList.indexOf(idList[i]) === -1) {
                 isAllChecked = true
                 this.checkAll = false
               }else {
