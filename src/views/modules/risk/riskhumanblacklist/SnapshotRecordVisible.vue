<template>
  <el-dialog class="infoclass" title="抓拍信息" :close-on-click-modal="false" :visible.sync="visible" width="780px !important">
    <div class="cardBox clearfloat" style="margin-left: 100px;">
      <div class="title">
        <span>识别结果</span>
      </div>
      ...
      <div class="textBox">
        ...
        <p>性别:{{dataForm.sex === 0 ? "男": dataForm.sex == 1? "女":"未知" }}</p>
      ...
      <el-timeline-item v-for="(item,index) in dataForm.riskHumanAlarmEntities" :key="index" timestamp="2018/4/12" placement="top">
        <el-card>
          <div class="cardBox">
            ...
            <img style="width: 220px; height: 200px" class="blackListFaceImg" :src="item.bkgPicUrl" fit="fill">
            <img style="width: 220px; height: 200px" class="blackListFaceImg" :src="item.facePicUrl" fit="fill">
            <!-- <el-image style="width: 220px; height: 200px" class="blackListFaceImg" :src="item.facePicUrl" fit="fill"></!-->
            <div class="boxFooter">
              <span style="text-align: left;">卡口地址:{{item.cameraName}}</span>
            </div>
          </div>
        </el-card>
      </el-timeline-item>
