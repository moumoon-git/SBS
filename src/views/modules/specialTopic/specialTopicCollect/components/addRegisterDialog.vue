<template>
  <div class="addRegisterDialog">
    <el-dialog title="新增专题采集" :visible.sync="dialogVisible" width="1100px" :close-on-click-modal="false">
          ...
          <div class="centerForm">
            ...
            >
              <!-- <div class="radioWrapper" v-if="item2.topicName == '单选题'">
                <div class="questionTitle">{{ item2.title }}</div>
                <div
                  class="radioContent"
              ...
              <div class="radioWrapper" v-if="item2.topicName === '单选题'">
                ...
                <el-radio-group v-model="item2.answer" style="display: flex;margin-left: 17px;">
                  <div
                    v-for="(selectItem, selectIndex) in item2.fkSelectList"
                    :key="selectIndex"
                  >
                    <el-radio :label="selectItem.id">
                      {{ selectItem.content }}
                      <input
                        v-if="
                          selectItem.ifInput === '1' &&
                            item2.answer == selectItem.id
                        "
                        type="text"
                        class="content"
              ...
              >
                ...
                <input type="text" value="" v-model="item2.answer" />
              </div>
              <div
              ...
              >
                ...
                <sv-upload-img v-model="item2.answer"/>
              </div>
               </div>
            </div>
          </div>
      ...
      <div class="buttonContainer">
        <div @click="addRegister" class="saveButton" type="primary"
          >保存</div
        >
...
export default {
  ...
  methods: {
    ...
    init(id) {
      ...
      this.$http({
        url: this.$http.adornUrl(
          `/fk/fkTemplateInstance/getTemplate?taskId=${id}`
        ),
        method: "post",
        data: this.$http.adornData({})
      }).then(({ data }) => {
        console.log("点击新增登记获得的数据为", data);
        var categoryArr = data.data.cataEntityList;
        categoryArr.forEach((categoryItem, index) => {
          categoryItem.questionEntityList.forEach((questionItem, index) => {
            if (
              questionItem.topicName == "多选题" ||
              questionItem.topicName == "混答+单选" ||
              questionItem.topicName == "混答+多选" ||
              questionItem.topicName == "图片"
            ) {
              questionItem.answer = [];
            } else {
              console.log("测试测试", questionItem);
              questionItem.answer = "";
            }
          });
        });
    ...
    addRegister() {
      ...
      console.log('sss',this.categoryList)
      this.categoryList.forEach((categoryItem, index) => {
        categoryItem.questionEntityList.forEach((questionItem, index) => {
          console.log('questionItem',questionItem)
            if(questionItem.isAnswer == '1') {
               hasContent = Array.isArray(questionItem.answer) ? questionItem.answer.length == 0 : questionItem.answer == ''
              if(hasContent) {
      ...
      this.categoryList.forEach((categoryItem, index) => {
        categoryItem.questionEntityList.forEach((questionItem, index) => {
          ...
          if (questionItem.topicName == "多选题") {
            console.log('questionItem',questionItem.answer)
            var answerItem = {
              questionId: questionItem.id,
              answer: questionItem.answer.join(",")
            };
          }else if(questionItem.topicName == "图片") {
            var answerItem = {
              questionId: questionItem.id,
              answer: JSON.stringify(questionItem.answer)
            };
          } else {
            var answerItem = {
              questionId: questionItem.id,
      ...
      this.$http({
        url: this.$http.adornUrl(`/fk/fkTemplateInstance/save`),
        method: "post",
        data: this.$http.adornData({
