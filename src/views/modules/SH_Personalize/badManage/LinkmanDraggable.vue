<template>
  <el-dialog :title="title" :visible.sync="visible" width="700px" center append-to-body :close-on-click-modal="false">
...
export default {
  ...
  props: ['dynamicTag','gridData','checkData'],
  data () {
    const data = [
      {
        id: 0,
  ...
  methods: {
      ...
      if (index == -1) {
        ...
        for (let i in this.gridData) {
          if (this.gridData[i].id == id) {
            let data = JSON.parse(JSON.stringify(this.gridData[i])) // 复制姓名列表中的对象保存在选中人对象数组中
    ...
    handleAddContacts (url, title, name) {
      ...
      if (this.gridData.length>0) {
        let checkData = ''
        if (typeof(this.checkData)=='number') {
          checkData = this.checkData
        }else if ((typeof(this.checkData)=='array' && this.checkData.length>0)){
