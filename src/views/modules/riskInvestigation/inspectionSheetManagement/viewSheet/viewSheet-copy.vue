    ...
    <div class="main-content">
      ...
      <div class="right-side">
        <el-collapse v-model="activeNames">
          <!-- 参与人员 dataList.allContactor.length - copyManListLength-->
          <el-collapse-item name="participants">
            <template slot="title">
              工单处置人员（共{{
                dataList.allContactor ? dataList.allContactor.length - copyManListLength : ''
              }}人）
              <span
            ...
            <div style="position: relative">
              ...
              <span style="position: relative">
                <span class="view-tag view-tag-responsibility">
                  ...
                  （{{
                       dataList.allContactor
                        ? dataList.allContactor[0].mailGroupName
                        : ''
                  }}
                  {{
              ...
              <!-- 参与人 -->
              <span
                v-for="(item, index) in dataList.allContactor"
                :key="index"
              >
                <span
                  v-if="item.type == 2"
...
export default {
  ...
  methods: {
    init(id) {
      this.$http({
        url: this.$http.adornUrl(`/ottffemergency/ottffemergencyinfo/saveAndSendWorkSheet/${id}`),
        method: 'post',
    ...
    getSingleData(id) {
      ...
      this.$http.get(url).then(
        (res) => {
          ...
          let count = 0;
          for (const item of that.viewForm.allContactorList) {
            if (item.type == 3) {
              count++;
            }
          }
