  ...
  <div class="treebox">
           ...
           <div class="warning-tree-search">
            ...
            >
              ...
              <img class="taskTree-add"  src="../../../../assets/img/addTask.svg" @click="addTree" >
          </div>
    

    <el-dialog title="修改检查项目分组" :close-on-click-modal="false" :visible.sync="visible">
    ...
    <div class="treeClass">
     <!-- <div
       @click="getAll()"
        :class="selectAll?'selectAllColor':''"
        style="paddingLeft:46px;cursor: pointer;color:#333333;height:45px;lineHeight:45px"
      >全部</div>-->
      <el-tree
      ...
      >
        ...
        <span style="position:absolute;right:0px;width:18px;padding-top:15px">
           ...
           <div v-if="hoverIndex==node.id" 
               @click="deleteTree()" 
               class="img1-hover"></div>
          <div   v-if="hoverIndex==node.id"  @click="updateTree()"  class="img2-hover"></div>
...
export default {
  ...
  props: ["list", "listProps", "Treeexpansion", "type", "syncProp","boxVisible"],
  data() {
    return {
      hoverIndex:'',
  ...
  methods: {
    ...
    getAll() {
      this.currentNode = "";
      // document.getElementsByClassName('el-tree-node__content')[0].setAttribute('style','background-color: #ffffff;border-left: none')
      this.selectAll = true;
      // $(
      //   ".el-tree--highlight-current .el-tree-node.is-current>.el-tree-node__content"
      // ).css({ "background-color": "#ffffff", "border-left": "none" });
    ...
    //点击树形更新表格
    handleNodeClick(obj,a) {
      // this.$emit("handleNodeClick", obj);

 
      // this.selectAll = false;
      // $('el-tree-node is-current is-focusable .el-tree-node__content').css({"background-color":"#ebf6ff","border-left":"2px solid #0091ff"})
   ...
   //   if (this.type) {
     //   setTimeout(() => {
      //    $(
       ...
       //   ).prepend(
      //      `<span style="position:absolute;right:45px;width:18px" class="opearation" >
     ...
     //             <img src="${require("../../../../assets/img/cmTreeEdit.svg")}" class="bianji"/>
      ...
      //            <img src="${require("../../../../assets/img/plus.svg")}" class="add"/}">
        ...
        //  );
          ...
          //   `<img src="${require("../../../../assets/img/eye.svg")}" class="add"/}">
       ...
       //   $(".bianji").click(() => {
        ...
        //          console.log("Dafffffffff",this.key,obj)
         //   this.updateTree();
      //    });
      //    $(".del").click(() => {
  ...
  created() {
    console.log("shujjjjjjdddddddd",this.list);
  },
  mounted(){
  ...
  watch: {
    currentNode: {
      handler(newV, oldV) {
        this.$emit("update:syncProp", newV);
      },
      deep: true,
    },
  },
