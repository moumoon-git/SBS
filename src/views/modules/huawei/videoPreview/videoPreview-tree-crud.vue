<template>
  <div class="contairdaDada">
    <el-form class="formStyle" ref="form" :model="form" label-width="80px">
      ...
      <p>过程描述</p>
      <el-input type="textarea" :rows="6" placeholder="请输入内容" v-model="form.desc" resize="none"></el-input>
      <p>通报人员</p>
      <!-- <el-input
        type="textarea"
        :rows="6"
      ...
      <el-row>
        <el-input @input="ttt" v-model="workPlace" style="width:80%;margin-bottom:1vh" placeholder="请输入定点或手动定位"></el-input>
    ...
    <location ref="location" :workPlace="workPlace" @updateLocation="updateLocation"></location>
  </div>
</template>
    ...
    uploadSectionFile(param) {
      ...
      this.$http({
        ...
        .then(({ data }) => {
          ...
          if (data && data.code === 0) {
            this.imageUrl.push({ name: param.file.name, url: data.filePath });
          } else {
          ...
          .then(() => {
            this.$http({
              url: this.$http.adornUrl(
                "/weChatHandleTaskPC/monitoringSendSingle"
              ),
              method: "POST",
