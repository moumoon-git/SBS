    ...
    <el-dialog title="请选择要添加的组" :visible.sync="dialogVisible" width="943px" center>
        <el-row :gutter="20">
                ...
                <div class="addData">
                      <el-checkbox :indeterminate="isBayonet" v-model="checkbayonetAll" @change="bayonetCheckAllChange">全选</el-checkbox>
                        ...
                        <el-checkbox-group v-model="checkBayonetData" @change="bayonetCheckedCitiesChange">
                            <div v-for="(city,index) in bayonetData" :key="index">
            ...
            <el-col :span="8">  
              ...
              <div class="addData">
                <ul class="humanData">
                  <li v-for="(item,index) in checkHumanDataObj" :key="index" class="oneHumanData">
                      {{item.name}}
                  </li>
                  <li v-for="(item,index) in checkBayonetDataObj" :key="index" class="oneHumanData">
                      {{item.name}}
                  </li>
                </ul>
              </div>
  ...
  methods: {
    ...
    init(id,type) {
     ...
     humanCheckAllChange(val) {
        this.checkHumanData = val ? this.humanData : [];
        this.isHuman = false;
      },
     ...
     bayonetCheckAllChange(val) {
        this.checkBayonetData = val ? this.bayonetData : [];
