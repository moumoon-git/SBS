<template>
  ...
  <div class="mod-config">
    <el-row :gutter="20">
      ...
      >
        ...
        <el-form-item>
          ...
          <el-button
            v-if="isAuth('risk:riskvehiclealarm:save')"
            type="primary"
            @click="addOrUpdateHandle()"
          >新增</el-button>
          <el-button
            v-if="isAuth('risk:riskvehiclealarm:delete')"
            type="danger"
            @click="deleteHandle()"
      ...
      <el-col :span="18">
        <el-table
          ...
          style="width: 100%;min-width:959px;height:682px;"
        >
          ...
          <el-table-column prop="longitude" header-align="center" align="center" label="经度" width="110"></el-table-column>
          <el-table-column prop="latitude" header-align="center" align="center" label="纬度" width="110"></el-table-column>
          ...
          <el-table-column prop="status" header-align="center" align="center" label="状态" width="70"></el-table-column>
    ...
    <!-- 弹窗, 新增 / 修改 -->
    <add-or-update v-if="addOrUpdateVisible" ref="addOrUpdate" @refreshDataList="getDatatree()" @getDataList="getDataList"></add-or-update>
    ...
    <tree-show-or-updata
      @refreshEventTypeDataList="getDatatree"
      v-if="TreeShowOrUpdataVisible"
      ref="treeShowOrUpdata"
    ></tree-show-or-updata>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    deleteHandle(id) {
      ...
      this.$confirm(`确定${id ? "删除" : "批量删除"}所选相机?`, "提示", {
        ...
        this.$http({
          url: this.$http.adornUrl(
            "/vcm/vcmBayonetInfo/deleteCameraByBidAndCid"
          ),
          method: "post",
    ...
    deleteEventType(data) {
      ...
      if (this.checkedIds) {
        var that = this;
        this.$confirm(
          `确定对卡口${that.bayonetData.name}以及所属子分组进行[删除]操作?`,
          "提示",
          {
            confirmButtonText: "确定",
            cancelButtonText: "取消",
          ...
          that
            .$http({
              url: that.$http.adornUrl(`/vcm/vcmBayonetInfo/delete`),
              method: "POST",
              data: this.$http.adornData({
                ids: that.checkedIds
              })
