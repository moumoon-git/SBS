  ...
  methods: {
    ...
    getAllData(status) {
      ...
      let eventInfoConditionEntity = {
        ...
        intelligentType: this.eventTypeId !== "" ? this.eventTypeId : undefined,
        // originalType: '',
        limit: this.pageSize,
        page: this.pageIndex,
      };
      ...
      this.$http({
        ...
        if (data.code === 0) {
          ...
          this.tableDataDisplay.forEach((item) => {
            // console.log(item.pievcUrl.split(","));
          });
          this.tableDataDisplay.map((v) => {
            this.eventtypelist.forEach((item) => {
              if (v.intelligentType === item.code) {
                // console.log('类型中文', item.name);
                // eslint-disable-next-line no-param-reassign
                v.intelligentType = item.name;
              }
            });
