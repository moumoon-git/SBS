      ...
      <el-header style="height:120px">
        <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="120px">
          ...
          <el-form-item label="分组描述">
            <el-input v-model="dataForm.typeDescribe" v-bind:readonly="type===1"></el-input>
          </el-form-item>
        </el-form>
      </el-header>
    ...
    gettingfocus (event) {
      ...
      this.$http({
        ...
        .then(({ data }) => {
          ...
          data.list.splice(0, 0, { id: 0, name: '顶级分组节点' })
        ...
        if (valid && (this.type === 2 || this.type === 3)) {
          this.$http({
            url: this.$http.adornUrl(
              `/notice/noticetype/${this.type === 2 ? 'save' : 'update'}`
            ),
            method: 'post',
            data: this.$http.adornData({
              ...
              typeDescribe: this.dataForm.typeDescribe,
            })
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.visible = false
              ...
              this.$emit('refreshEventTypeTreeDataList')
              this.$emit('close')
            } else {
