  ...
  <div class="PersonDeclare">
     <div style="margin-bottom:10px;">
      ...
      <el-button
        ...
        style="margin-left:0"
        class="searchMargin"
        v-if="isAuth('perpass:passRegister:save')"
      >新增</el-button>
      ...
      >
        <el-tooltip placement="top" effect="light" v-if="isAuth('perpass:perpassRegister:excel')">
          <div slot="content">
            请填写我们提供
            <br />的模板进行导入
          </div>
      ...
      <el-button
        ...
        class="buttonStyle searchMargin"
        v-if="isAuth('perpass:perpassRegister:excel')"
      >导出模板</el-button> -->
      <el-button
        @click="openView('','删除')"
        ...
        style="margin-left:0"
        class="searchMargin"
        v-if="isAuth('perpass:perpassRegister:deletePc')"
      >批量删除</el-button>
      <!-- <el-button>高级</el-button> -->
     </el-form-item>
      </el-form>
    </div>
    <div class="gongneng" v-if="isAuth('perpass:perpassRegister:updateLockStatusPc')">
      <div>
        核验口令：{{checkCode}}&nbsp;&nbsp;
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <div
                style="display: flex;"
                :style="{'justify-content': (isAuth('perpass:passRegister:update')||isAuth('perpass:passRegister:updateLockStatusPc')||isAuth('perpass:perpassRegister:deletePc'))?'space-between':'space-around'}"
              >
                <el-tooltip class="item" effect="dark" content="查看" placement="top">
                  <span class="see" @click="seeHandle(scope.row.id,'查看')"></span>
                </el-tooltip>
                <el-tooltip
                  v-if="isAuth('perpass:perpassRegister:update')"
                  class="item"
                  effect="dark"
                  content="编辑"
                  placement="top"
                ...
                <el-tooltip
                  v-if="isAuth('perpass:perpassRegister:updateLockStatusPc') && (scope.row.lockStatusString==='待审核' || scope.row.lockStatusString==='重新提交')"
                  class="item"
                  effect="dark"
                  content="审核"
                  placement="top"
...
export default {
  ...
  data() {
    return {
      ...
      documenttoken: `${httpaddress}/perpass/perpassRegister/xlsInput?token=${this.$cookie.get(
        "token"
      )}`,
      filename: "",
  ...
  mounted() {
    ...
    bus.$on("getCheckCharge", val => {
      if (val.length > 0) {
        this.$http({
          url: this.$http.adornUrl(`/pass/passCode/sendCodeToContactorPer`),
          method: "post",
  ...
  methods: {
    ...
    getInporInfo(){
      this.$http({
        url: this.$http.adornUrl(
          `/sys/config/infoByKey?key=LF_TX_CONTENT`
        ),
        method: "get",
      }).then(({ data }) => {
    ...
    handleTaskType() {
      this.$http({
        url: this.$http.adornUrl(`/perpass/perpassRegister/getsonPlatform`),
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    getLimit() {
      this.$http({
        url: this.$http.adornUrl(
          `/perpass/perpassRegister/hideCode?platformId=${this.platformId}`
        ),
        method: "get"
      }).then(({ data }) => {
        // console.log(data)
    ...
    getVerify(type) {
      this.$http({
        url: this.$http.adornUrl("/pass/passCode/getCodePer"),
        method: "post"
      }).then(({ data }) => {
        console.log(data);
        if (data.msg === "success") {
    ...
    getDataList() {
      ...
      this.$http({
        url: this.$http.adornUrl("/perpass/perpassRegister/list"),
        method: "get",
    ...
    deleteHandle() {
      ...
      this.$http({
        url: this.$http.adornUrl("/perpass/perpassRegister/delete"),
        method: "post",
    ...
    // 导出模板
    downloadtemplate() {
      window.location.href = `${httpaddress}/perpass/perpassRegister/gettemplate?token=${Cookies.get(
        "token"
      )}`;
    }
  },
