  ...
  <div class="ElectronicPassSee">
          ...
          <div style="display:flex;">
            ...
            <div style="display:flex;flex-direction:column;justify-content:space-between;">
              ...
              <p>
                ...
                {{detailData.nativePlaceString?detailData.nativePlaceString:"无"}}
              </p>
              <p>
    ...
    <el-row :gutter="20" style="background-color: white;">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="8">
          <div class="updateRecord" v-if="title!=='审核'&&updateRecord.length>0">
            <p class="updateTitle">更新记录</p>
            <ul class="updateProcess">
              <li
                class="oneUpdateProcess"
                :class="{'checkStyle':idx==index}"
                v-for="(item,index) in updateRecord"
                :key="item.id"
                @click="editReport(index,item.id)"
              >
                <div class="updateLeft textHiding">
                  <p class="updateWay">更新方式：{{JSON.parse(item.newValue).createUserId?'后台修改':'微信修改'}}</p>
                  <p
                    class="modifier"
                    v-if="item.name"
                  >修改人：{{JSON.parse(item.newValue).createUserId}}</p>
                </div>
                <div
                  class="updateTime textHiding"
                >更新时间：{{JSON.parse(item.newValue).gmtModified | formatDate}}</div>
              </li>
            </ul>
          </div>
        </el-col>
...
export default {
  ...
  methods: {
    ...
    getDetailData() {
      this.$http({
        url: this.$http.adornUrl("/perpass/perpassRegister/getOneAndLink"),
        method: "get",
        ...
        if (data && data.code === 0) {
          ...
          this.visitotDetail(data.data);
          // this.trafficRecord = data.data.passRegisterHistoryList
          this.updateRecord = data.data.perChangeEntityList;
    ...
    visitotDetail(response) {
      this.detailData = response;
      this.registeinfor = response.epiRegisterTikuEntity.epiQuestionEntityList;
      ...
      this.registeinfor.map(v => {
        if (v.topicName === "单选题" && v.answer) {
          ...
          if (v.answer.indexOf("#") === -1) {
            ...
            v.answer =
              content +
              v.answer.substring(v.answer.indexOf("#") + 2, v.answer.length);
          }
        }
      });
    },
    ...
    getPicture() {
      this.$http({
        url: this.$http.adornUrl(
          `/perpass/perpassRegister/output-passEpiEmployeeToPicture?id=${this.electronicId}`
        ),
        method: "get",
        responseType: "blob"
      }).then(({ data }) => {
    ...
    preserve(type) {
      this.$http({
        url: this.$http.adornUrl(`/perpass/perpassRegister/updateLockStatusPc`),
        method: "post",
        ...
        if (data && data.code === 0) {
          this.$message({
            ...
            onClose: () => {
              this.$router.push({
                path: "/Troubleshooting/PersonDeclare/PersonDeclare"
              });
            }
          });
        } else {
    ...
    sendSMS(){
      ...
      this.$nextTick(()=>{
        this.$refs.SendSMSView.init(this.detailData.id,'/perpass/perpassRegister/sendRegisterMsg')
      })
    },
    // 下载通行证
    download() {
      window.location.href = `${
        this.baseUrl
      }perpass/perpassRegister/output-passEpiEmployeeToPicture?id=${
        this.electronicId
    ...
    editReport(index, id) {
      this.idx = index;
      this.visitotDetail(JSON.parse(this.updateRecord[index].newValue));
    },
    // 查看审核记录
    seeRecord(){
      this.AuditRecordView = true
      this.$nextTick(()=>{
        this.$refs.AuditRecordView.init(3,this.electronicId)
      })
    }
  },
  components: {
