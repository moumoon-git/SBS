...
// import SelectContacts from '../../DeviceAdmin/selectContacts.vue';
    ...
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.currentTreeNode) {
          this.dataForm.articleStorehouseName = this.currentTreeNode.name;
          this.dataForm.articleStorehouseId = this.currentTreeNode.id;
        }
        if (this.dataForm.id) {
          this.$http({
    ...
    articleTypeGettingFocus(event) {
      ...
      this.$http({
        ...
        if (data.data && data.data.code === 0) {
          // data.data.tree.splice(0,0, {id:0,name: '顶级分组节点'})

          // this.articleTypeList = treeDataTranslate(data.data.tree, "id");
