<template>
  ...
  <div class="riskhumanblacklist">
    <div style="height:80px;margin: -20px;padding: 0 20px;" class="header">
      <el-input style="width:300px;float: left;" v-model="dataForm.key" placeholder="按名称搜索" clearable></el-input>
      <el-button @click="getDataList()" style="margin-left:5px;background-color：rgb(64, 158, 255)">查询</el-button>
      <el-button v-if="isAuth('risk:riskhumanblacklist:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>
      <el-button @click="addToTreedialogVisible=true" style="background-color：rgb(64, 158, 255)">新增黑名单到组</el-button>
      <el-button @click="blacklistSynchronization" style="background-color：rgb(64, 158, 255)">人员黑名单同步</el-button>
        ...
        <div class="list">
          ...
          <div class="colStaff" v-for="item in dataList" :key="item.id">
            <div class="left" style="width:135px;">
              ...
              <img v-if="item.imageUrl" :src="item.imageUrl.indexOf('http')===-1?baseUrl+item.imageUrl:item.imageUrl" style="width:135px;height:100%" alt="">
              <img v-else :src="item.imageUrl" style="width:135px;height:100%" alt="">
            </div>
            <div class="right">
              <ul>
                <li>
    ...
    <!-- 监管类型 新增 / 修改 -->
    <tree-add-or-updata @refreshEventTypeDataList="getDatatree" v-if="TreeAddOrUpdataVisible" ref="treeAddOrUpdata" @reftreeDataList="getDatatree"></tree-add-or-updata>
    <snapshot-record v-if="SnapshotRecordVisible" ref="SnapshotRecord" @refreshDataList="getDataList"></snapshot-record>
    ...
    <el-dialog title="新增黑名单到组" :visible.sync="addToTreedialogVisible" width="40%">
      <el-form  :modal="addToTreeObject" ref="addToTreeObject">
        <el-form-item label="请选择分组" prop="eventTypeParentName" >
          ...
          <el-input v-model="addToTreeObject.treeName" @focus="gettingfocus" v-popover:eventTypeListPopover placeholder="选择分组" :readonly="true"></el-input>
        ...
        <el-form-item label="选择人员黑名单" >
          <el-input size="mini" placeholder="请选择人员黑名单"  @focus="handelVerify('addToTree')" v-model="addToTreeObject.blackNames"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer" >
  ...
  methods: {
      ...
      this.$http({
        ...
        .then(({ data }) => {
          if (data && data.code === 0) {
              ...
              data.data.splice(0, 0, { id: 0, name: '顶级分组节点',ancestors:'0'})
    ...
    handelLinkdata ({ linkman, name }) {
      ...
      linkman.forEach((item)=>{
        ...
        this.addToTreeObject.blackNames=this.addToTreeObject.blackNames+' '+item.name
      })
    },
    // 分组修改提交
    submittingbygroups () {
    ...
    exportExcel () {
      ...
      // )
      window.location.href = `${httpaddress}//risk/riskhumanblacklist/get-template?token=${this.$cookie.get('token')}`
    },
    // 自定义文件上传行为
          ...
          data.page.records.forEach((item, index, arr) => {
            ...
            arr[index]['credentialsName'] =
              ...
              if(item.facePicUrl&&item.facePicUrl.search("http")!=-1){
                ...
                arr[index]['imageUrl'] =
                window.SITE_CONFIG.cloudUrl + item.facePicUrl
              }
