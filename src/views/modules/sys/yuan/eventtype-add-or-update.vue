<template>
  <div>
    <el-dialog
      :title="type===1 ? '查看事件类型' :type===2?'新增事件类型':'修改事件类型'"
      ...
      <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
        ...
        <el-form-item label="所属类型" prop="parentName" v-if="type===2">
          <!--       <el-popover
                  ...
                  </el-popover> v-popover:eventTypeListPopover   -->
          <span @click="selectEventType">  <el-input v-model="dataForm.parentName" placeholder="所属类型"
                                                     :readonly="true"></el-input></span>
        </el-form-item>
    ...
    <!--选择事件类型-->
    <common-set v-if="showCommonSet" ref="commonSet" @onSubmit="onSubmit"></common-set>
  </div>
</template>
    ...
    methods: {
      ...
      init (id, currenTreeObject, type) {
        ...
        this.$nextTick(() => {
          ...
          if (type === 2) {this.visible = true
           /* this.$http({
              url: this.$http.adornUrl('/event/eventType/list'),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              console.log('/sys/platform/lists', data)
      ...
      selectEventType () {
        ...
        this.$nextTick(() => {
          this.$refs.commonSet.init(`/event/eventType/list`)
        })
      },
      //选择事件类型后
      onSubmit (obj) {
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid && (this.type === 2 || this.type === 3)) {
            ...
            this.$http({
              url: this.$http.adornUrl(`/event/eventType/${this.type === 2 ? 'save' : 'update'}`),
              method: 'post',
