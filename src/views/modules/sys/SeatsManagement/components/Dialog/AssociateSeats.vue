<!-- 弹窗--坐席批量关联平台 -->
<template>
  <div class="associate-seat-dialog">
    <sv-dialog ref="dialog" :title="title" :modal="true" @cancel="handleCancel">
      <div class="seat-dialog-body">
        <!-- 已选坐席 -->
        <div class="selected-seats">
          <div class="selected-seats_header">已选</div>
          <ul class="selected-seats-list">
            <li v-for="item in seatList" :key="item.id">
              {{ '坐席号：' + item.seatNumber }}
            </li>
          </ul>
        </div>

        <el-form :model="formData">
          <el-form-item label="关联平台">
            <el-cascader
              v-model="formData.platformId"
              :options="platformData"
              :props="platformProps"
              :show-all-levels="false"
              filterable
              clearable
              style="width: 280px"
              @change="handleChangeAssociatedPlatform"
            />
          </el-form-item>
        </el-form>
      </div>
      <template #footer>
        <div class="seats-dialog-footer">
          <sv-button>取消</sv-button>
          <sv-button type="primary" @click="associateAssociate">
            关联
          </sv-button>
        </div>
      </template>
    </sv-dialog>
  </div>
</template>
<script>
import $seat from '../../assets/js/seatManagementApi.js';

export default {
  name: 'AssociateSeatDialog',
  components: {
  },
  props: {
    title: {
      type: String,
      default: '批量关联坐席号',
    },
    seatList: {
      type: Array,
      default: () => [],
    },
    // 平台信息
    platformData: {
      type: Array,
      default: () => [],
    },
    // 配置选项
    platformProps: {
      type: Object,
      default: () => ({}),
    },
  },
  data() {
    return {
      formData: {
        platformId: '', // 坐席Id
      },
    };
  },
  mounted() {
    this.$refs.dialog.visible = true;
  },
  methods: {
    /**
     * @description 改变监控终端时触发
     */
    handleChangeAssociatedPlatform(val) {
      this.formData.platformId = val;
    },
    /**
     * @description 关闭弹窗
     */
    handleCancel() {
      this.$emit('cancel');
    },
    /**
     * @description 批量关联
     */
    associateAssociate() {
      const data = {
        ids: this.seatList.map((item) => item.id), // 坐席id
        platFormId:
          this.formData.platformId[
            this.formData.platformId.length - 1
          ], // 平台Id
      };
      $seat.batchUpdate(data, this.associateAssociateCallback);
    },
    /**
     * @description 批量关联回调
     */
    associateAssociateCallback(type, data) {
      if (type === 'success') {
        this.$message.success(
          '批量关联坐席成功',
        );
        this.$refs.dialog.visible = false;
        this.$emit('success');
      } else {
        this.$message.error(
          `批量关联坐席失败，错误代码：${data.code}，错误信息：${data.msg}`,
        );
      }
    },
  },
};
</script>

<style lang="scss">
.associate-seat-dialog {
  .seat-dialog-body {
    width: 450px;
    padding: 10px 40px;

    // 已选坐席
    .selected-seats {
      width: 370px;
      height: 150px;
      background: #f2f2f2;
      border-radius: 3px;
      border: 1px solid #dddee1;

      .selected-seats_header {
        height: 20px;
        // font-size: 12px;
        font-weight: 400;
        color: #333333;
        line-height: 20px;
        text-align: center;
      }

      .selected-seats-list {
        list-style: none;
        padding: 10px;
        overflow-y: scroll;
        height: calc(100% - 20px);

        // 定义滚动条样式
        &::-webkit-scrollbar {
          width: 0px;
          height: 5px;
          border-radius: 10px;
        }
        // 定义滚动条轨道 内阴影+圆角
        &::-webkit-scrollbar-track {
          box-shadow: inset 0 0 6px rgba(0, 0, 0, 30%);
          border-radius: 10px;
          background-color: rgba(100, 104, 105, 1);
        }
        // 定义滑块 内阴影+圆角
        &::-webkit-scrollbar-thumb {
          border-radius: 10px;
          box-shadow: inset 0 0 6px rgba(0, 0, 0, 30%);
          background-color: rgba(86, 233, 255, 1);
        }

        & > li {
          height: 30px;
          line-height: 30px;
          list-style: none;
          font-family: PingFangSC-Regular, PingFang SC;
          font-weight: 400;
          color: #999999;
          // font-size: 12px;
        }
      }
    }

    .el-form {
      margin: 14px 0 4px 0;

      .el-form-item {
        margin-bottom: 0;
      }

      .el-form-item__label {
        // font-size: 12px;
        color: #555555;
      }
    }
  }
  // 底部
  .seats-dialog-footer {
    height: 60px;
    display: flex;
    justify-content: flex-end;
    align-items: center;
    padding: 0 20px;
    border-top: 1px solid #dddee1;

    .sv-button {
      padding: 0 14px;
      margin-left: 10px;
    }
  }
}
</style>
