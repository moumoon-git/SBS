<template>
  <div class="allContact">
    ...
    <div class="wrap">
      <div class="outLine">
        <input class="search" placeholder="姓名、单位、职位、号码检索" v-model="key" @change="searchContact"/>
      </div>
      <!-- 不使用树形的复选框-->
      <div class="checkWrap">
            ...
            >
              <li class="main-video" v-for="(city,index) in tableList" :key="index">
                <div class="audio-name">
                  <el-checkbox :label="city" style="width:100%" @change="show(city)">
                    <div class="oneCheck">
                      <div class="zhiwu">
                        <p>{{city.name}}</p>
                        <p>{{city.position}}</p>
                ...
                <div class="audio-body" ref="child" style="display:none;">
                    <div class="top">
                        <img v-if="city.images" :src="city.images.indexOf('http') === -1?baseUrl+city.images:city.images" alt="" class="avatar">
                        <img v-else :src="images" alt="" class="avatar">
...
export default {
  ...
  methods: {
    handleCheckAllChange(val) {
      ...
      this.checkAll = val;
      this.$emit("allSelectData",this.checkedCities)
