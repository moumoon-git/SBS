            ...
            <div class="resolution_content_left">
              <div class="resolution_content_left_top">
                ...
                <div class="resolution_content_left_top_top_bottom">
                  <span v-if="btnFlag">原传真（共{{pageFax&&pageFax.length}}页）</span>
...
<script>
    export default {
        ...
        methods:{
          ...
          faxmergePdf (arr) {
              if (arr.length!=0) {
                  ...
                  this.$http({
                      ...
                      if (res.data.data) {
                          this.newRawFaxPath = window.SITE_CONFIG.cloudUrl+'/fileupload/'+res.data.data
                          this.rawFaxPath = this.newRawFaxPath
                          this.mianContentFaxPath = this.newRawFaxPath
                          this.btnFlag = false
                          this.splitPdf(res.data.data,0,'newpageFax') // 勾选拆分后的传真合并，合并成功后，将合并传真再次拆分。这时不需要传faxId author:hexinting date:2021-04-02
                      }
                  })
              }
          ...
          /**@param {String/Number} type 类型 区分选中样式。0：无勾选框；1：; 2: 无勾选状态；3：勾选状态
          /**@param {String} str newpageFax 新传真路径（合并后的传真） /oldpageFax 原传真路径
          /**@param {String} faxId 传真ID  选中的拆分传真合并成新传真后，将新传真再次拆分，这时不需要传faxId。原传真拆分需要传真ID
           * @lastEditor hexinting
           * @lastEditedDate 2021-04-02
           * @description 拆分传真
           */
          splitPdf (fileName,type,str,faxId) {
              // bug11192 选中的拆分传真合并成新传真后，将新传真再次拆分，这时不需要传faxId。原传真拆分需要传真ID
              let data = faxId ? {
                  faxId,
                  fileName,
                  isSingle:true,
              ...
              this.$http({
                method: 'post',
                headers: {},
                url: window.g.ApiUrl + '/fax/splitPdf',
                data,
            }).then((res) => {
                  if (res.data.data){
                      if (res.data.data.length!=0){
                          ...
                          res.data.data.forEach(ele=>{
                              let obj = {
                                  name:ele.fileUrl,
                                  src: window.SITE_CONFIG.cloudUrl +"/fileupload" +ele.fileUrl,
                                  type
                              }
                              arr.push(obj)
                          })
        ...
        watch:{
            oldFaxPath:function (newVale) {
                if (newVale) {
                    // this.rawFaxPath = newVale
                    let fileName = this.oldFaxPath.substr(this.oldFaxPath.lastIndexOf('/')+1,);//72c0d9d068b34cbabe812ea171f0bf94.pdf
                    this.splitPdf(fileName,'2','oldpageFax', this.faxId) // 选中的拆分传真合并成新传真后，将新传真再次拆分，这时不需要传faxId。原传真拆分需要传真ID
                }
            }
        }
    }
