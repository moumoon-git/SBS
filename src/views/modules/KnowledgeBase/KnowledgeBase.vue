  ...
  <div>
    <sv-template
      ...
      :import-data-config="isPlatformRoot ? {
        api: '/resoure/resourceknowledge/xlsInput',
        overload: handleAvatarSuccess,
      }: {}"
      :export-select-config="{
        api: '/resoure/resourceknowledge/xlsOutputSelect',
      ...
      :download-template-config="{
        api: '/resoure/resourceknowledge/gettemplate'
      }"
      :delete-selected-config="isPlatformRoot ?
        { api: '/resoure/resourceknowledge/delete' } : {}"
      :overload="overload"
    ...
    >
      ...
      <template #middle__operation__button>
        <sv-button
          v-if="isPlatformRoot"
          type="primary"
          @click="AddOrEditOrViewKnowledgeBase('add')"
        >
          添加
        </sv-button>
      </template>
        ...
        >
          <template slot-scope="scope">
            <sv-table-operation
              ...
              @view="AddOrEditOrViewKnowledgeBase('detail', scope.row)"
              @edit="AddOrEditOrViewKnowledgeBase('update', scope.row)"
              @delete="
...
import AddOrEditOrView from './KnowledgeBase-add-update-view.vue';
  ...
  data() {
    return {
      ...
      treeSetting: {
        ...
        edit: {
          add: '/resoure/resourceknowledgegroup/save', // 添加节点接口
          update: '/resoure/resourceknowledgegroup/update', // 更新树节点接口
          delete: true, // 是否可删除
      ...
      // TODO 给获取表格数据添加接口
      tableRequest: {
        api: '/resoure/resourceknowledge/list',
        method: 'post',
        data: {
          page: 1,
          limit: 10,
      ...
      // 导入错误底部提示信息
      errorTips: [
        '1、不能在该Excel表中对知识库信息进行删除或修改；',
        '2、Excel中红色字段为必填字段,黑色字段为选填字段，字段值里请不要填入特殊字符；',
        '3、文档类别：上下级之间用“/”隔开，且从最上级开始，例如“突发事件/突发事件1”。若导入的数据中拥有与文档类别中已存在的文档类别名相同的文档类别，将自动与文档类别关联，若导入的文档类别名称在系统中不存在，系统将自动创建该文档类别',
        '4、标题字段长度不能超过100个字符、文件号字段长度不能超过20个字符、关键字字段长度不能超过50个字符、 制定单位长度不能超过20个字符',
      ],
      // 导入错误表格必填项
      errorRequired: ['标题', '文档类别'],
    };
  },
  methods: {
    /**
    ...
    getTreeAndTableData(platformId) {
      ...
      this.treeSetting.api = `/resoure/resourceknowledgegroup/list?platformId=${platformId}`;
    ...
    getTableList(params) {
      return new Promise((resolve) => {
        ...
        if (params && params.data) {
          data.groupId = params.data.groupId;
        } else {
    ...
    batchDelete(ids) {
      const request = {
        method: 'post',
        url: '/resoure/resourceknowledge/delete',
        baseURL: window.SITE_CONFIG.baseUrl,
        data: {
          ids,
        },
      ...
      this.$http(request)
        .then((response) => {
          if (response.data.code === 0) {
            // 提交成功后，刷新表格
            this.tableRequest.data.limit = this.$refs.template.paginationConfig.pageSize = 10;
            this.tableRequest.data.page = this.$refs.template.paginationConfig.currentIndex = 1;
            this.$refs.template.getTableList();
          } else {
     ...
     */
    AddOrEditOrViewKnowledgeBase(type, row) {
      // 分组
    ...
    doDeleteTreeNode() {
      this.$http({
        url: this.$http.adornUrl('/resoure/resourceknowledgegroup/delete'),
        method: 'post',
