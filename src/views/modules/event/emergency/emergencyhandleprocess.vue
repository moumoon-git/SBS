  ...
  >
    <!-- <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('emergency:emergencyhandleprocess:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>
        <el-button v-if="isAuth('emergency:emergencyhandleprocess:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
      </el-form-item>
    </el-form>
    <el-table
      :data="dataList"
      ...
      <el-table-column
        ...
        header-align="center"
        align="center"
        label="操作记录（电话、短信才会用到）">
      </el-table-column>
      <el-table-column
        prop="dataSourceId"
        header-align="center"
        align="center"
        label="当type=1时:电话记录ID，type=2时:短信记录ID">
      </el-table-column>
      <el-table-column
        prop="type"
        header-align="center"
        align="center"
        label="类型(0要勤登记 1电话记录  2短信记录  3处置过程)">
      </el-table-column>
      <el-table-column
        prop="createTime"
        header-align="center"
    ...
    <div style="width: 80%; height: 36px; margin: auto; position: relative">
      <el-button
        v-if="$route.query.operationStatus != '已办结'"
      ...
      <!-- <el-button
        style="height: 36px; position: absolute; right: -100px"
        type="primary"
        size="small"
        @click="exportDocx"
      >
    ...
    <el-timeline style="width: 80%; margin: auto; margin-right: 100px">
      ...
      >
        <el-card>
          ...
          <div
            v-if="item.type == 3 && $route.query.operationStatus != '已办结'"
            style="
              width: 100%;
              position: relative;
              height: 35px;
...
export default {
  ...
  methods: {
    ...
    //     const creatTable = new Docx.Table({
    //       width: {
    //         size: 9000,
    ...
    //                   text: item.type === 0 ? '要情登记' : (item.type === 1 ? '电话记录' : (item.type === 2 ? '短信记录' : '处置过程')),
    //                   alignment: Docx.AlignmentType.CENTER,
    //                 }),
    //               ],
    //               verticalAlign: Docx.VerticalAlign.CENTER,
    ...
    //     const date = new Date();
    //     const now = `${date.getFullYear()}${date.getMonth() > 8 ? date.getMonth() + 1 : `0${date.getMonth() + 1}`}${date.getDate() > 9 ? date.getDate() : `0${date.getDate()}`}${date.getHours() > 9 ? date.getHours() : `0${date.getHours()}`}${date.getMinutes() > 9 ? date.getMinutes() : `0${date.getMinutes()}`}`;
    //     saveAs(blob, `${this.$parent.dataForm.title}处置过程${now}.docx`);
    //   });
    // },
    // 获取数据列表
    getDataList() {
      ...
      this.$http({
        url: this.$http.adornUrl(
          `/emergency/emergencyhandleprocess/list?emergencyInfoId=${this.$route.query.id}`,
        ),
        method: 'get',
    ...
    deleteProcessItem(id) {
      this.$confirm('确定是否删除该处置过程?', {
        ...
        .then(() => {
          this.$http({
            url: this.$http.adornUrl(
              '/emergency/emergencyhandleprocess/delete',
            ),
            method: 'post',
    ...
    deleteHandle(id) {
      ...
      this.$confirm(
        ...
        this.$http({
          url: this.$http.adornUrl('/emergency/emergencyhandleprocess/delete'),
          method: 'post',
            ...
            url: this.$http.adornUrl(
              `${
                this.updateProcessDialog
                  ? '/emergency/emergencyhandleprocess/update'
                  : '/emergency/emergencyhandleprocess/save'
              } `,
            ),
            method: 'post',
            data: this.$http.adornData({
