  ...
  >
    ...
    <el-form ref="eventDetailForm" :model="eventDetailForm" label-width="80px">
      ...
      <el-form-item label="事件状态">
        <el-radio-group v-model="eventDetailForm.status" size="small">
      ...
      <el-form-item label="是否关闭" >
        <el-radio-group v-model="eventDetailForm.isClose" size="small">
          <el-radio :label="1" style="margin-right: 0px; margin-left: 20px;">是</el-radio>
      ...
      init(row) {
        ...
        this.$http({
          ...
          if (!this.eventDetailForm.id) {
            ...
            //编辑时默认选中的事件类型ID
            //this.eventDetailForm.eventTypeId = row.eventType;
          ...
          if (valid) {
            ...
            if(serviceObjectId != null && serviceObjectId != "" && serviceObjectId !=0){
                ...
                this.$confirm('该事件存在关联预案, 是否立即启动?', '提示', {
                  ...
                  confirmButtonText: '立即启动预案',
                  cancelButtonText: '稍后手动启动',
                  type: 'warning'
                }).then(() => {
                  //自动触发
                  this.automaticStartEvent();

                }).catch(action => {

                  if(action === 'cancel'){
                    //手动触发
                    this.manualStartEvent();
                  }else{
              ...
              this.$confirm('该事件没有关联服务对象, 是否继续保存编辑?', '提示', {
                distinguishCancelAndClose: true,
