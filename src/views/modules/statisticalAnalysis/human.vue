        ...
        >
          ...
          <ul style="list-style-type:none"
            ...
            <li :id="'camera'+data.cameraId" class="cameraNameTag"
                ...
                @click="clickPlatform(data.cameraId,data.indexCode)">【{{index+1}}】{{data.cameraName}}</li>
          </ul>
            ...
            <el-row>
              <el-col :span="14">
                <el-row style="margin-top: 10px;">
                  ...
                  <!--swiper切换-->
                  <el-col :span="20">
                    <div class="swiperHuman" style="width: 100%;">
        ...
        <div class="vehicle-snap-list" :key="index" v-for="(item,index) in vehicleRankingList">
          <el-row style="background: white;margin: 10px;padding: 5px;">
            ...
            <el-col :span="13">
              ...
              <div>{{item.credentialsNum.replace(/^(.{6})(?:\d+)(.{4})$/, "$1****$2")}}</div>
            </el-col>
            <el-col :span="6" style="">
        ...
        <div class="camera-snap-list" v-for="(item,index) in cameraRankingList">
          <ul style="list-style-type:none">
            ...
            <li class="platformNameTag"
                style="padding: 4px;margin-right: 20px;background: #ffffff;">【{{index+1}}】{{item.name}}</li>
          </ul>
        </div>
      </div>
...
import $ from "jquery";
import AddOrUpdate from "../risk/riskhuamnalarminfo/riskhuamnalarminfo-add-or-update";
...
export default {
  ...
  methods: {
    ...
    getCameraDataList(){
      this.$http({
        url: this.$http.adornUrl(`/statisticApi/humanAlarmApi/humanCameraTotalRankList`),
        method: 'get',
    ...
    //点击li
    clickPlatform(cameraId,indexCode){
      // console.log("点击li==cameraId===crossingId",cameraId,indexCode)
    ...
    initPanelData(){
      ...
      this.$http({
        url: this.$http.adornUrl("/statisticApi/humanAlarmApi/humanTypeTotalRankList"),
        method: "get",
        params: this.$http.adornParams({
          platformId: this.platformId,
          indexCode: this.indexCode,//摄像机路口标志
    ...
    initLineData() {
      ...
      this.$http({
        url: this.$http.adornUrl("/statisticApi/humanAlarmApi/humanStatisticsByTime"),
        method: "get",
        params: this.$http.adornParams({
    ...
    initTableData(){
      ...
      this.$http({
        url: this.$http.adornUrl("/statisticApi/humanAlarmApi/humanListByIndexCode"),
        method: "get",
        params: this.$http.adornParams({
          platformId: this.platformId,
          indexCode: this.indexCode,
        ...
        for(let i=0; i<data.list.length; i++){
          for(let j=0; j<this.vehicleTypeList.length; j++){
            if(data.list[i].controlTypeId === this.vehicleTypeList[j].id){
              let obj = {
                'id':data.list[i].id,
                'humanId':data.list[i].humanId,
                'credentialsNum': data.list[i].credentialsNum,
                'indexCode':data.list[i].indexCode,
                'humanName':data.list[i].humanName,
    ...
    vehicleRanking(){
      this.$http({
        url: this.$http.adornUrl("/statisticApi/humanAlarmApi/humanRankList"),
        method: "get",
        params: this.$http.adornParams({
        ...
        // console.log("人员排行版", data);
        this.vehicleRankingList = data.data;
        ...
        //       let obj = {
        //         'num':data.data[i].num,
        //         'credentialsNum':data.data[i].credentialsNum,
        //         'controlTypeId':data.data[i].controlTypeId,
        //         'controlName':this.vehicleTypeList[j].name
        //       };
    ...
    cameraRankinig(){
      this.$http({
        url: this.$http.adornUrl("/risk/riskcamera/queryHumanCameraRank"),
        method: "get",
        params: this.$http.adornParams({
          platformId: this.platformId,
          controlTypeId: this.vehicleTypeId,
...
<style lang="scss">
  ...
  .humanPlatformGroup::-webkit-scrollbar {
    width: 10px;
    height: 1px;
  }
  .humanPlatformGroup::-webkit-scrollbar-thumb {
    border-radius: 10px;
    background-color: rgba(52, 163, 242, 0.48);
  }
  .humanPlatformGroup::-webkit-scrollbar-track {
