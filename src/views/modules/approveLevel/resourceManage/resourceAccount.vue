      ...
      <div class="zygl-content-table">
        ...
        >
          ...
          <el-table-column align="center" prop="accountType" label="状态">
            <template slot-scope="scope">
              <span v-if="scope.row.accountType == 1">在用</span>
              <span
      ...
      <div class="approve-edit-container">
        ...
        <div v-if="activeName == 'first'">
          <div>
            ...
            <label style="position: absolute; left: 12vw"
              >车牌品牌及型号:</label
            >
        ...
        <div v-if="activeName == 'second'">
          <div>
            ...
            <label style="position: absolute; left: 12vw">地址:</label>
            <label style="position: absolute; left: 15.4vw">{{
              detailData.place
            }}</label>
          </div>
          <div>
            ...
            <label>{{ detailData.townName }}</label>
            <label style="position: absolute; left: 12vw">所属村居:</label>
            <label style="position: absolute; left: 17vw">{{
              detailData.platformName
            }}</label>
            <label style="position: absolute; left: 35vw">责任人:</label>
            <label style="position: absolute; left: 37.5vw">{{
              detailData.responsible
            }}</label>
            <label style="position: absolute; left: 41vw">联系电话:</label>
            <label style="position: absolute; left: 43vw">{{
              detailData.phone
            }}</label>
          </div>
        </div>
...
<script>
import ResourceAddUpdate from "./resourceAccount_addOrUpdate";
...
export default {
  ...
  data() {
    return {
      ...
      documenttoken: `${httpaddress}/approve/approveCarAccount/xlsInput?token=${this.$cookie.get(
        "token"
      )}`,
      fileName: "",
      nowPlatForm: "",
  ...
  methods: {
    ...
    showApprove(id) {
      ...
      let url;
      if (this.activeName == "first") {
        url = `/approve/approveCarAccount/info/${id}`;
      } else {
        url = `/approve/approveBoxAccount/info/${id}`;
      }
      this.$http({
        url: this.$http.adornUrl(url),
        method: "get",
    ...
    getDataList() {
      ...
      this.$http({
        url: this.$http.adornUrl(`/approve/approveCarAccount/list`),
        method: "post",
    ...
    getDataList2() {
      ...
      this.$http({
        url: this.$http.adornUrl(`/approve/approveBoxAccount/list`),
        method: "post",
        data: this.$http.adornData(request),
      }).then(({ data }) => {
        console.log("数据列表data", data);
    ...
    deleteHandle(row) {
      ...
      this.$confirm(
        ...
        .then(() => {
          let url;
          if (this.activeName == "first") {
            url = "/approve/approveCarAccount/delete";
          }
          if (this.activeName == "second") {
            url = "/approve/approveBoxAccount/delete";
          }
          this.$http({
            url: this.$http.adornUrl(url),
            method: "post",
    ...
    uploadSectionFile(param) {
      ...
      let url;
      if (this.activeName == "first") {
        url = "/approve/approveCarAccount/xlsInput";
      }
      if (this.activeName == "second") {
        url = "/approve/approveBoxAccount/xlsInput";
      }
    ...
    exportSelectAll() {
      ...
      if (this.activeName == "first") {
        window.location.href = `${httpaddress}/approve/approveCarAccount/xlsOutputSelect?checkedIds=${checkedIds}
        &token=${Cookies.get("token")}`;
      }
      if (this.activeName == "second") {
        window.location.href = `${httpaddress}/approve/approveBoxAccount/xlsOutputSelect?checkedIds=${checkedIds}
        &token=${Cookies.get("token")}`;
      }
    },
      ...
      if (this.activeName == "first") {
        window.open(
          window.SITE_CONFIG["baseUrl"] +
            `/approve/approveCarAccount/xlsOutputCondition?carSource=${
        ...
        window.open(
          window.SITE_CONFIG["baseUrl"] +
            `/approve/approveBoxAccount/xlsOutputCondition?townName=${
    ...
    downloadtemplate() {
      ...
      if (this.activeName == "first") {
        window.location.href = `${httpaddress}/approve/approveCarAccount/gettemplate?token=${Cookies.get(
          "token"
        )}`;
      } else {
        window.location.href = `${httpaddress}/approve/approveBoxAccount/gettemplate?token=${Cookies.get(
          "token"
        )}`;
      }
    },
    ...
    handleTabClick(tab, event) {
      ...
      if (this.activeName == "second") {
        this.documenttoken = `${httpaddress}/fk/fkTemplate/xlsInput?token=${this.$cookie.get(
          "token"
        )}`;
        this.getDataList2();
        this.getCount();
        ...
        this.documenttoken = `${httpaddress}/approve/approveCarAccount/xlsInput?token=${this.$cookie.get(
          "token"
        )}`;
        this.getDataList();
        this.getCount();
