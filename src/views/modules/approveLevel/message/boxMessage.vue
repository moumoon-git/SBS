...
<script>
import ResourceAddUpdate from '../resourceManage/resource-add-update';
...
export default {
  ...
  methods: {
    ...
    // 离职申请确定
    approveSubmit() {
      const url = '/approve/approveBoxAccount/updateAccountType';
      const manageId = -600;
      this.$http({
    ...
    showApprove(id) {
      ...
      this.getPlatform();
      this.dataId = id;
      const url = `/approve/approveBoxAccount/info/${id}`;
      this.$http({
        url: this.$http.adornUrl(url),
        method: 'get',
      }).then((data) => {
    ...
    getDataList() {
      const request = {
        ...
        townName: this.dataForm.value,
        platformName: this.dataForm.value2,
        groupIds: this.treeIds,
        accountType: this.dataForm.entryStatus,
      };
      this.$http({
        url: this.$http.adornUrl('/approve/approveBoxEntity/listByPlatform'),
        method: 'post',
    ...
    deleteHandle(row) {
      ...
      this.$confirm(
        ...
        .then(() => {
          const url = '/approve/approveBoxAccount/delete';
          this.$http({
            url: this.$http.adornUrl(url),
            method: 'post',
    ...
    exportSelectAll() {
      ...
      this.$http({
        url: this.$http.adornUrl(
          '/approve/approveBoxEntity/xlsOutputSelectInstance',
        ),
        method: 'post',
        data: this.$http.adornData({
    ...
    exportSearchAll() {
      this.$http({
        url: this.$http.adornUrl(
          '/approve/approveBoxEntity/xlsOutputConditionInstance',
        ),
        method: 'post',
        data: this.$http.adornData({
          page: this.pageIndex,
