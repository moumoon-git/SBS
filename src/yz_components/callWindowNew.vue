    ...
    <div align="center" style="margin-top:-20px;">
      <div class="call-menu-box">
        ...
        <div v-if="myContent==''" class="myscroll-box" :class="{'oy-active':pid==0}">
          <div
            class="call-menu-box-main"
            v-for="(item,index) in myList"
            :key="index"
            @click="getData(item)"
          >{{item.name}}</div>
        </div>
        <div v-else class="mycontent-box myscroll-box">{{myContent}}</div>
      </div>
      ...
      <el-row style="margin:10px;">
        <img
          callWindowNew
          :src="userCall.picture?(userCall.picture.search('http')==-1?($window.g.ApiUrl+userCall.picture):userCall.picture):'static/img/headless.png'"
      ...
      <el-row style="margin-top: 50px">
        ...
        <el-button
          v-if="callClass=='callin'||callClass=='callon'||callClass=='callheld'||callClass=='callout'"
          type="danger"
          @click="hangup"
        >挂断</el-button>
      </el-row>
    ...
    methods: {
        ...
        getEventOptions() {
            this.$api.allEventlist({}).then(res => {
                if (res.data) this.eventOptions = res.data;
            });
        },
        callIsVisibleChange() {
            ...
            this.$msgbox({
                title: '消息',
                message: h('p', null, [h('span', null, '请选择是否挂断通话')]),
