          ...
          <div>
            <el-row>
              <el-col :span="24">
                ...
                >
                  <i class="el-icon-warning"></i> 24小时未读消息({{
                    smsAnswerList.length +
                    phoneAnswerList.length +
                    faxAnswerList.length +
                    eventAnswerList.length
...
export default {
  ...
  data() {
    return {
      ...
      settingss: this.$store.state.autoCallSettings,
  ...
  methods: {
    ...
    clickAnswerShow(type, title) {
      if (title === "事件") {
        console.log(121321);
        this.$router.push({ path: "/eventRegistration", query: { type } });
        return;
      }
    ...
    // 打开大屏
    toBigScreen() {
      window.open("http://172.26.72.34:9090");
    },
    shrink() {
      this.isShow = false;
      this.$emit("menuClose");
    ...
    setting() {
      ...
      setTimeout(() => {
        this.$refs.settingDialog.$refs.reportUnit.getTreeData();
      }, 100)
    },
    handleClose() {
