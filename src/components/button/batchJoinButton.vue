<template>
  ...
  >
    <el-button slot="trigger" icon="iconfont icon-piliangjiaru" type="primary">批量加入
    </el-button>

    <el-dialog title="导入情况" :visible.sync="dialogVisible" center width="600px" @close="closeDialog"
               :close-on-click-modal="false">
      <div style="text-align: left;">导入情况:{{SituationMessage}}</div>
      <el-button type="primary" @click='failsBtn' plain size="mini"
                 style="text-align: left;margin-top: 10px;margin-bottom: 10px;">错误详情
      </el-button>
      <ul style="padding: 0;margin: 0;text-align: left;" v-if="show">
        <li v-for="(fails,index) in failsData" style="text-align: left;" :key="index">第{{fails.row}}行,{{fails.msg}}</li>
      </ul>
    ...
    methods: {
      ...
      httpRequest(item) {
        ...
        this.$api.importFromContactorExcel(formData).then((res) => {
          ...
          this.SituationMessage = "格式正确:" + res.data.successNum + "条," + "格式错误:" + fails.length + "条";
