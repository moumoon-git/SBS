<template>
  <div class="allContact">
    ...
    <div class="wrap">
      ...
      <el-input placeholder="请输入关键字搜索" class="inp" @input="searchContact" v-model="key"></el-input>
      <!-- 不使用树形的复选框-->
      <div class="checkWrap">
        <aside class="checkList">
          <el-checkbox
            ...
            style="border:none;padding-left:10px;background:rgba(251,252,255,1);width:94%;height:33px;line-height:0px;"
            ...
            >
              ...
              >
                <div class="audio-name">
                  <el-checkbox :label="city" style="width:100%" v-if="city.id!==disableId">
                    <div class="oneCheck">
                      <div class="zhiwu">
                        <p>{{city.name}}</p>
                ...
                <el-dropdown
                  ...
                  v-if="city.homeTel||city.mobile||city.number||city.officeTel||city.otherTel||city.otherTel2||city.temobile"
                >
                  ...
                  <el-dropdown-menu
                    ...
                    v-if="city.homeTel||city.mobile||city.number||city.officeTel||city.otherTel||city.otherTel2||city.temobile"
                  >
                    <el-dropdown-item v-if="city.homeTel">
                      <span>应急电话 {{city.homeTel}}</span>
                      <i
                        class="iconfont icon-bohao"
                        style="color: #409EFF;cursor: pointer;"
                        @click="talkCall(city,city.homeTel)"
                      ></i>
                    </el-dropdown-item>
                    <el-dropdown-item v-if="city.number">
                      <span>手机2 {{city.number}}</span>
                      <i
                        class="iconfont icon-bohao"
                        style="color: #409EFF;cursor: pointer;"
                        @click="talkCall(city,city.number)"
                      ></i>
                    </el-dropdown-item>
                    <el-dropdown-item v-if="city.officeTel">
                      <span>办公电话 {{city.officeTel}}</span>
                      <i
                        class="iconfont icon-bohao"
                        style="color: #409EFF;cursor: pointer;"
                        @click="talkCall(city,city.officeTel)"
                      ></i>
                    </el-dropdown-item>
                    <el-dropdown-item v-if="city.otherTel">
                      <span>备用号码 {{city.otherTel}}</span>
                      <i
                        class="iconfont icon-bohao"
                        style="color: #409EFF;cursor: pointer;"
                        @click="talkCall(city,city.otherTel)"
                      ></i>
                    </el-dropdown-item>
                    <el-dropdown-item v-if="city.otherTel2">
                      <span>其他电话 {{city.otherTel2}}</span>
                      <i
                        class="iconfont icon-bohao"
                        style="color: #409EFF;cursor: pointer;"
                        @click="talkCall(city,city.otherTel2)"
                      ></i>
                    </el-dropdown-item>
                    <el-dropdown-item v-if="city.temobile">
                      <span>手机3 {{city.temobile}}</span>
                      <i
                        class="iconfont icon-bohao"
                        style="color: #409EFF;cursor: pointer;"
                        @click="talkCall(city,city.temobile)"
                      ></i>
                    </el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
                <div class="audio-body" ref="child" style="display:none;">
                  <div class="top">
                    ...
                    <div class="mainInfo" style="width: 80%;">
                      ...
                      >{{city.workUnit?city.workUnit:"无"}}</p>
                    </div>
                  </div>
                  <div>
...
export default {
        ...
        dataList(newV, oldV) {
            ...
            if (this.groupCheckData.length > 0) {
                this.groupCheckData.map(v => {
                    ...
                    checkExist.push(
                        this.tableList.find(item => v.id === item.id)
                    );
                });
                if (checkExist.length > 0) {
                    filterData = this.$emit(
                        'delRepy',
                        checkExist
                    ).$listeners.delRepy(checkExist);
                }
            ...
            console.log('AllContactor this.tableList', this.tableList);
        },
    ...
    methods: {
        handleCheckAllChange(val) {
            ...
            console.log('短信还是传真:', this.smsOrFax);
            if (val) {
        ...
        handleCheckedCitiesChange(value) {
            ...
            if (this.smsOrFax === 'fax') {
                ...
                data = value.filter(v => v.number || v.mobile);
                value.forEach(item => {
                    if (!item.number && !item.mobile) {
                        contactorNameArr.push(item.name || item.contactor);
                        contactorNameString = contactorNameArr.join();
            ...
            // let data = value.filter(v => v.number)
