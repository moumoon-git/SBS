        ...
        <div style="padding: 20px">
          ...
          >
            <!-- <div class="item audioToTxt">
              ...
              <img src="/static/img/phoneCallImg/audioToText_mini.jpg" @click="audioToTxt">
            </div> -->
          ...
          <div class="bottomPlan" v-if="isCallPhone == 'call'">
            <el-button type="success" size="mini" @click="answerPhone(),$window.decodeURIComponent($refs.audio.src).includes('电话')&&$refs.audio.pause()"
              >接听</el-button
            >
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      conn: "getCallConn",
      getAudioText: "getAudioText", // 获取语音转文字
    }),
    selectEventStyle() {
  ...
  watch: {
    isCallPhone: {
      handler: function (newVal, oldVal) {
        console.log("监听-isCallPhone:", newVal, oldVal);
      },
    },
  },
  data() {
  ...
  methods: {
    ...
    audioDownload() {
      ...
      eleLink.href = this.audios[0].url;
      // 触发点击
    ...
    hangUp() {
      ...
      this.$confirm("是否结束当前通话?", "提示", {
        ...
        this.$window.decodeURIComponent($refs.audio.src).includes('电话')&&this.$refs.audio.pause()
        if (self.HWICP_Version == "HWICP-19.0") {
          this.$icp19.DropCall(Number(agent_id), Number(self.conn), "");
