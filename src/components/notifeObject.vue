 ...
 * @LastEditTime: 2021-04-27 10:04:03
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \xuchuangxing\SJSEOS\src\view\dutyManagement\RollCallOnDuty\selectUnit.vue
-->
<template>
  <div class="notifeObject">
    <el-dialog
      :title="notifeObjectTitle"
      :visible.sync="dialogVisible"
      :before-close="handleClose"
      :close-on-click-modal="false"
      append-to-body
      width="400px"
      custom-class="notifeObject"
    >
      <!-- <el-radio-group v-model="notifeObjectId">
        <el-radio-button
          v-for="(item, index) in notifeObjectList"
          :label="item.id"
          :key="index"
          >{{ item.name }}
        </el-radio-button>
...
export default {
  ...
  data() {
    return {
      ...
      reportingUnitData: [],
      allUnitData: [],
      notifeObjectList: [{ name: "选择单位", id: 1 }], // 所有标签页信息
      notifeObjectId: 1 // 标签页默认选中 选择单位
    };
  },
  mounted() {
  ...
  methods: {
    ...
    saveDepartment() {
      ...
      this.$emit("udpateNotifeObject", allDepartment);
      this.handleClose();
...
/deep/ .notifeObject .el-radio-button__inner {
  padding: 14px 22px;
  font-size: 14px;
  color: #999999;
  border: none;
...
/deep/ .notifeObject .is-active .el-radio-button__inner {
  border-bottom: 2px #0091ff solid;
  color: #0091ff;
  border-radius: 0;
}

/deep/ .notifeObject .el-radio-button:first-child .el-radio-button__inner {
  border-left: 0;
}
</style>
