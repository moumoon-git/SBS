...
<!--        <div style=" position: absolute; top: 34px; left: 30px;">{{menu.displayname}}</div>-->
  ...
  <el-menu-item :index="menu.name" v-else-if="menu.child.length==0">
    <i v-if="isShow" :class="'icon-style '+ menu.resourceLogo"></i>
    <div v-if="isShow==false" :class="menu.resourceLogo+ '-circle'" >
      <div :class="menu.resourceLogo+ '-shrink'" ></div>
<!--      <div style=" position: absolute; top: 34px; left: 30px;" >{{menu.displayname}}</div>-->
...
<style scoped>
  ...
  .menuName:active, .menuName:focus, .menuName:hover, .menuName:visited, .menuName:focus-within {
    color: #ffffff;
  }
  .el-menu-item:hover {
    /* color: #479DFF; */
  ...
  .el-menu--collapse > .el-menu-item span, .el-menu--collapse > .el-submenu > .el-submenu__title spanel-submenu__title span {
    height: 0;
    width: 0;
    overflow: hidden;
    visibility: hidden;
