<template>
  <div class="sv-select-contact">
    <template v-if="!displayOnly">
      ...
      >
        {{ buttonText ? buttonText : `选择${title}` }}
      </sv-button>
      <div v-if="isDisplay">
        <span
  ...
  methods: {
    ...
    getContactList() {
      ...
      const request = {
        ...
        data: {
          ...
          groupId: this.currentGroupId ? this.currentGroupId : [],
          groupIds: this.currentGroupId ? this.currentGroupId : [],
        },
      };
    ...
    confirm() {
      ...
      if (this.multiple) {
        ...
        this.$emit('update', this.selectedResult[0].id, JSON.parse(JSON.stringify(this.selectedResult[0].raw || this.selectedResult[0])));
        this.displayResult = [...this.selectedResult];
      }
      // 初始化
