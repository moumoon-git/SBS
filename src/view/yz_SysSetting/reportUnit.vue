<template>
  <div>
    <div style="text-align: left">
      <el-button type="primary" @click='showDialog("新增报告单位")'>新增</el-button>
      <el-button type="primary" @click='showDialog("修改报告单位")'>修改</el-button>
      <el-button type="primary" @click='showDialog("删除报告单位")'>删除</el-button>
    </div>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer" v-if='title == "新增报告单位"'>
        <el-button @click="save('保存并新增')">保存并新增</el-button>
        <el-button type="primary" @click="save('保存并关闭')">保存并关闭</el-button>
        <el-button @click="dialogVisible = false">关闭</el-button>
      </span>
      <span slot="footer" class="dialog-footer" v-if='title == "修改报告单位"'>
    ...
    export default {
      ...
      data() {
        return {
          ...
          breadList:['报告单位'],//获取当前树节点和其所有父级节点的lable
          nodeContent:{'id':0,'name':'报告单位'},//保存当前选择节点的内容,默认选中第一个
          treeKey:[0],
          title:'',
          dialogVisible:false,
          outerVisible:false,
      ...
      methods: {
         ...
         * @lastDate 2021-02-04
         * @description 保存报送单位
         * @param {String} title: 新增报告单位、修改报告单位、删除报告单位
         */
        showDialog(title){
          this.title = title;
          if(title == '新增报告单位'){
          ...
          } else if(title == '修改报告单位') {
            if(this.nodeContent.name == '报告单位') {
              this.$message({
              ...
              this.$api.unitSetting(data).then(res => {
                ...
                bus.$emit('updateReportUnit'); // 更新报告单位。若当前事件接报页面打开，则要更新事件接报页面的报告单位
              })
            }
          } else if(title == '删除报告单位') {
            if(this.nodeContent.name == '报告单位'){
                this.$message({
                  message: '对此节点不可进行修改和删除操作',
                  type: 'warning'
            ...
            this.$api.unitSetting(data).then((res) => {
              ...
              if(res.errorcode == 0){
                ...
                this.breadList = ['报告单位'],//获取当前树节点和其所有父级节点的lable
                this.nodeContent = {'id':0,'name':'报告单位'},//保存当前选择节点的内容,默认选中第一个
                this.getTreeData();
                bus.$emit('updateReportUnit'); // 更新报告单位。若当前事件接报页面打开，则要更新事件接报页面的报告单位
              } else {
        ...
        addMans(checkboxGroup){
          ...
          let data={"ids":checkboxGroup};
          console.log('数据：', data)
          this.$http({
            url: window.SITE_CONFIG['baseUrl'] + "/mail/mailcontactor/listByIds",
            method: "post",
            data,
          }).then((res) => {
            console.log(res)
         ...
         * @lastDate 2021-02-04
         * @description 保存报送单位
         * @param {String} button: 保存并新增、保存并关闭、保存修改
         */
        save(button){
