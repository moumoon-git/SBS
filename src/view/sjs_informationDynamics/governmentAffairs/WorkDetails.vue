        ...
        <el-row>
          ...
          <el-col :span="12" style="overflow: hidden;padding-right:10px;" :title="detailData.principal">
            <template>
              ...
              <el-input style="width: 200px" v-model="detailData.principal" placeholder="暂无负责人" disabled></el-input>
            </template>
          </el-col>
          <el-col :span="12" style="overflow: hidden;padding-right:10px;">
            <template>
              <span class="detailDescTitle">执行时间</span>
              <el-input style="width: 200px" v-model="detailData.executeTime" placeholder="暂无执行时间" disabled></el-input>
            </template>
          </el-col>
        </el-row>
...
<script>
  import attachmentPopup from "@/view/sjs_informationDynamics/governmentAffairs/attachmentPopup";
        ...
        .then(res=>{
          ...
          for (const key in this.detailData) {
            if (res.data.hasOwnProperty(key)) {
              ...
              // 执行时间显示到分钟
              if(res.data['executeTime'] == '' || res.data['executeTime'] == null){
                this.detailData['executeTime'] = ''
              }else{
          ...
          this.$emit('updateAffairsDetail', res.data);
        })
      },
      //点击显示附件下载框
      handleShowAtt() {
