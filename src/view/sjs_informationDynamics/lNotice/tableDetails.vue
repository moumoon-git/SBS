<template>
  <div class="details">
    ...
    <template v-if="id">
      <el-row class="title">
        <div style="float: left; font-size: 15px; vertical-align: middle">
          上屏显示
          <el-switch
            v-model="screenDisplay"
            @change="onScreen"
...
export default {
  ...
  methods: {
    showData(row) {
      ...
      // this.onScreen()
      //   this.flag = false;
      //   this.screenDisplay = row.onScreen ? true : false;
    ...
    getAtt(id) {
      ...
      this.$api.appDutyInformationReleasDetail({ id }).then((res) => {
        ...
        this.unit = detailData.unit;
        this.principal = detailData.extend1;
        this.time = detailData.time;
        this.status = detailData.extend2;
        this.type = detailData.type;
        this.flag = false;
        this.screenDisplay = detailData.onScreen ? true : false;
        // this.screenDisplay = true;
        // this.onScreen()
        this.content = detailData.content.split("\\n"); // lastEditor:xuchuangxing lastUpdateTime:2021/1/25 \\n 才能分割后端返回的\n
      });
    },
    //点击显示附件下载框
    handleShowAtt() {
    ...
    /**
     * @description: 上屏显示接口调用
     * @author: xuchuangxing
     * @lastUpdateTime : 2021/1/7 14:22
     * @param {*}
     * @return {*}
     */
    onScreen: debounce(function () {
      console.log(this.id,this.screenDisplay)
      let data = {
        id: this.id,
        onScreen: this.screenDisplay ? 1 : 0,
      };
      this.$api.noticeOnscreen(data).then((res) => {
        if (res.errorcode != 0) {
          this.$message({
            message: res.msg,
            type: "error",
