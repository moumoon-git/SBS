    ...
    submit: debounce(function (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        ...
        if (this.title.indexOf("新增") !== -1) {
          ...
          // this.$api.memoAdd(data)
          axios({ //备忘录，添加
            url: window.SITE_CONFIG['event'] + '/eos/appDutyInformationReleas/memo/add',
            method: 'post',
            data
          })
          .then((res) => {
        ...
        if (this.title.indexOf("修改") !== -1) {
          ...
          this.$confirm("是否确认保存当前修改?", "提示", {
            ...
            .then(() => {
              // this.$api.memoUpdate(data)
              axios({ //备忘录，修改
                url: window.SITE_CONFIG['event'] + '/eos/appDutyInformationReleas/memo/update',
                method: 'post',
                data
              })
              .then((res) => {
