          ...
          <el-main style="margin-top:16px;margin-left:16px;">
            <div class="tableContainer">
              <el-table ref="singleTable" :data="tableData" highlight-current-row
                ...
                <el-table-column min-width="160" v-for="(item,key) in tableColumn" :key="key" :label="item.label" :prop="item.prop"
...
<script>
  ...
  import TableDetails from "@/view/sjs_informationDynamics/lEmergency/tableDetails";
  import workPopup from "@/view/sjs_informationDynamics/lEmergency/workPopup";
  ...
  export default {
    ...
    methods:{
      ...
      getTableData(){
        ...
        // this.$api.memoList(data)
        axios({ //备忘录，查找表格
          url: window.SITE_CONFIG['event'] + '/eos/appDutyInformationReleas/memo/list',
          method: 'post',
          data
        })
        .then(res=>{
      ...
      handleAdd() {
        this.$nextTick(() => {
          ...
          this.$refs.WorkPopup.title = "新增备忘录";
      ...
      handleEdit() {
        ...
        this.$refs.WorkPopup.isDialogVisible = true;
        this.$refs.WorkPopup.title = "修改备忘录";
      ...
      handleDelete() {
        if (!this.rowData) {
          ...
          this.$confirm('删除当前信息?', '提示', {
            ...
            // this.$api.memoDelete(data)
            axios({ //备忘录，删除
              url: window.SITE_CONFIG['event'] + '/eos/appDutyInformationReleas/memo/delete',
              method: 'post',
              data
            })
            .then(res => {
