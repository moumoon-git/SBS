...
import voiceText from './comonnTemplat/voiceText';
import tailorAudio from './comonnTemplat/tailorAudio';
import bigTemplate from './comonnTemplat/bigTemplate';
// import phoneHistory from '../../../History/phoneHistory/phoneHistory';
...
export default {
  ...
  methods: {
    ...
    getRequeryCell() {
      ...
      this.$api.getUnhandledPhone(data).then((res) => {
        if (res.errorcode == 0) {
          ...
          this.$refs.bigTemplate.state = 'phone';
          this.$refs.bigTemplate.footerPageObj.total = res.data.total;
          console.log(this.$refs.bigTemplate.footerPageObj.allData);
        } else {
          this.$message({
            message: res.msg,
            type: 'error',
    ...
    async TurnTheText(res) {
      ...
      const { src, obj } = res;
      // this.$refs.SpeechText.textarea = obj.remark || obj.text || '';
      this.$refs.voiceTextRef.textarea = obj.remark || obj.text || '';
      if (!src || src === '') {
        ...
        const response = await allApiRequest(obj.url, { id: obj.id });
        if (response) {
    ...
    getEventList() {
      ...
      this.$api.eventHistory(data).then((res) => {
        if (res.errorcode == 0) {
          this.$refs.bigTemplate.eventOptions = res.data.data;
        } else {
          this.$message({
            message: res.msg,
            type: 'error',
