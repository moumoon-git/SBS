<template>
  <div style="margin: 30px 20px;">
      <el-row >
        <el-col :span="13">
          <el-tabs class="tabsClass" v-model="leftTabsType" @tab-click="tabClick">
                    ...
                    <div style="display: flex;">
                      <div style="flex: 5">
                       <!-- <el-select size="small" v-model="queryFaxAllSelect" placeholder="请选择">
                        ...
                        <el-input type="input" size="small"  placeholder="" v-model="queryFaxAllSelect"></el-input>
                      </div>
                      <div style="flex: 2">
                        <el-button   type="primary" size="small" @click='getUnhandledFax'>查询</el-button>
                      </div>
                    </div>
                    ...
                    <div v-loading="treeLoading">
                      <el-scrollbar style="height: 100%">
                        <div :style="listHeight">
                          <div  style="margin-top: 10px;text-align: left" v-for="(item,index) in  list" :key="item.id" @click="changeItem(item,index)">
                            <el-card  :class="{ cardModelDiv:changeIndex == index}" shadow="hover" :body-style="bodyStyle">
                              <div style="display: flex;margin-bottom: 15px;">
                                ...
                                <div style="color: #F66E6E;background:rgb(254,237,237);border-radius: 10%;padding: 3px 5px;font-size: 12px;" v-if="editIndex != item.id">{{item.status}}</div>
                              ...
                              <div style="font-size: 12px">
                                <ul class="parameterTop">
                                  <li>单位</li>
                                  <li>标题</li>
                                  <li>关联事件</li>
                                </ul>

                                <ul class="parameterBottom">
                                  <li>
                                    <el-tooltip class="item"  :content="`${item.unit?item.unit:'暂无~'}`" placement="bottom" effect="light">
                                      <div class="none">{{item.unit}}</div>
                                    </el-tooltip>
                                  </li>
                                  <li>
                                    <el-tooltip class="item"  :content="`${item.title?item.title:'暂无~'}`" placement="bottom" effect="light">
                                      <div class="none">{{item.title}}</div>
                                    </el-tooltip>
                                  </li>
                                  <li>
                                    <el-tooltip class="item"  :content="`${item.event?item.event:'暂无~'}`" placement="bottom" effect="light">
                                      <div class="none">{{item.event}}</div>
                                    </el-tooltip>
                                  </li>
                                </ul>
                              </div>
                ...
                <el-card class="box-card cardCalss" shadow="hover">
                  <el-scrollbar style="height: 100%">
                    ...
                    <div style="display: flex;">
                      ...
                      <div style="flex: 2">
                        <el-button  size="small"  type="primary" @click='getUnhandledPhone'>查询</el-button>
                      </div>
                    </div>
                          ...
                          <div  style="margin-top: 10px;text-align: left" v-for="(item,index) in  list" :key="item.id" @click="clickPhoneList(item,index)">
                            <el-card  :class="{ cardModelDiv:changeIndex == index}" shadow="hover" :body-style="bodyStyle">
                              ...
                              <div style="font-size: 12px">
                                ...
                                <div style="display: flex;align-items: center;"><i style="background: url('/static/img/svg/blue.svg')  no-repeat;width: 14px;height: 14px;
                                display: inline-block;background-size: 15px 13px;"></i><span style="padding-left: 10px;color: #ddd">(呼入)</span></div>
                              </div>
                ...
                <el-card class="box-card cardCalss" shadow="hover">
                  <el-scrollbar style="height: 100%">
                    ...
                    <div style="display: flex;">
                      ...
                      <div style="flex: 2">
                        <el-button  size="small"  type="primary" @click='getUnhandledSms'>查询</el-button>
                      </div>
                    </div>
        ...
        <el-col :span="1">
          <el-scrollbar style="height: 100%">
            <div style=" display: flex;flex-direction:column;" >
              <div class="messageDiv" style="min-height: 360px" :style="messageCutHight" @click="InformationDisposal" :class="{messsagColor:changeRed == '信息处置'}">
                <div class="messageCut">信息处置</div>
              </div>
              <div class="messageDiv" style="min-height: 360px"   :style="messageCutHight" @click="eventEntering" :class="{messsagColor:changeRed == '事件录入'}">
                <div class="messageCut">事件录入</div>
              </div>
            </div>
          </el-scrollbar>
        </el-col>
        <el-col :span="10">
          <el-card class="box-card cardCalss" shadow="hover" v-if="InformationDisposalShow" >
            <div style="width: 100%;height: 30px;line-height: 30px;text-align: center;background-color: cornflowerblue;color: white;">信息处理</div>
            <el-scrollbar style="height: 100%;">
              <el-tabs v-model="activeAllModel"  style="padding-bottom: 38px;min-height: 610px"  :style="rightHeightStyle" >
              <el-tab-pane label="传真" name="传真">
                <el-scrollbar style="height: 100%">
                  <new-fax-model ref="newFaxModel" :key="rushFaxData" @rushContent="rushContent"></new-fax-model>
                </el-scrollbar>
              </el-tab-pane>
              <el-tab-pane label="电话" name="电话">
                <el-scrollbar style="height: 100%">
          ...
          <el-card class="box-card cardCalss" shadow="hover"
                   v-if="eventEnteringShow" style="position: relative"

          >
            <el-scrollbar style="height: 100%;">
              <div style="padding-bottom: 38px;min-height: 610px" :style="saveEntering">
                  <el-form size="small"   :model="ruleForm" :rules="rules" ref="ruleForm" @submit.native.prevent
                           label-position="left" style="padding-bottom: 70px;" :hide-required-asterisk="asterisk" >
                    <el-row :gutter="gutter" id="serial">
                      <el-col :span="0">
                        <el-form-item label="事件编号:" >
                          <el-input type="text" disabled v-model.trim="ruleForm.serialNumber " show-word-limit>
                          </el-input>
                        </el-form-item>
                      </el-col>
                    </el-row>
                    ...
                    <el-row :gutter="gutter">
                      <el-col :span="12">
                        <el-form-item label="事发区域:">
                          <el-input type="text" disabled v-model.trim="ruleForm.incidenArea" show-word-limit :title="ruleForm.incidenArea">
                            <template slot="append">
                              <el-button style="letter-spacing: 6px" @click="eventScene">区域</el-button>
                            </template>
                          </el-input>
                        </el-form-item>
                      </el-col>
                      <el-col :span="12">
                        <el-form-item label="事发地点:">
                          <el-input type="text" disabled v-model.trim="ruleForm.incidentSite" show-word-limit :title="ruleForm.incidentSite">
                            <template slot="append">
                              <el-button style="letter-spacing: 6px" @click="location">地点</el-button>
                            </template>
                          </el-input>
                        </el-form-item>
                      </el-col>
                    ...
                    <el-row :gutter="gutter">
                      <el-col :span="12">
                        <el-form-item label="报送单位:" >
                          <el-input type="text" disabled v-model.trim="ruleForm.submittedUnit" show-word-limit :title="ruleForm.submittedUnit">
                            <template slot="append">
                    ...
                    <el-row :gutter="gutter">
                      <el-col :span="12">
                        <el-form-item label="事件类型:" >
                          <el-input type="text" disabled v-model.trim="ruleForm.incidenType" show-word-limit :title="ruleForm.incidenType">
                            <template slot="append">
                    ...
                    <el-row :gutter="gutter" v-if="eventsParameters==true">
                      <el-col :span="24">
                        ...
                        <el-form-item id="itemCalss" style="background:#F9FBFF;padding: 5px 0;border-radius:3px">
                          <el-col :span="24">
                            <div style="display: flex;justify-content: center">
                              <div style="display: flex;flex-wrap: wrap; flex-direction: row;justify-content: flex-start;align-items: center;width: 100%;">
                                <div v-for="(item,index) in parameter" style="width: 25%;" :key="index">
                                  <div v-if="item.type=='数字'">
                                    <div :title="`${item.name}${item.unit?`(${item.unit})`:item.unit}`" style=" width: 150px;overflow: hidden;white-space: nowrap;text-overflow:ellipsis;"  >
                                      <span v-if="item.unit!==''">{{item.name}}({{item.unit}}):</span>
                                      <span v-if="item.unit==''">{{item.name}}:</span>
                                    </div>
                                    <el-input-number style="width: 150px;"  size="mini" v-model="item.value" :min="0" label="描述文字"></el-input-number>
                                  </div>
                                  <div v-if="item.type=='文本'" style="width: 150px;">
                                    ...
                                    <el-input style="width: 150px;" type="text" size="mini" v-model="item.value" show-word-limit></el-input>
                                  </div>
                                  <div v-if="item.type=='选择'">
                                    ...
                                    <el-select style="width: 150px;" size="mini" v-model="item.value" placeholder="请选择">
                                      <el-option
                    ...
                    <div v-else-if="eventsParameters==false">
                    </div>
                      ...
                      <el-col :span="24" id="baowen" style="padding-left:0"> 
                        <el-form-item label="突发事件报文:"  style="margin: 0 0 5px 0;text-align: right">
                          <div style="display: flex;justify-content: flex-end">
                            <!--                <div>突发事件报文</div>-->
                            <div>
                              <el-button type="primary" size="mini" @click="eventMessage">生成突发事件报文id</el-button>
                            </div>
                          </div>
                        </el-form-item>
                        <el-form-item  id="baowenNeiRon" >
                          <el-input type="textarea" v-model.trim="ruleForm.allCase" :autosize="{ minRows:8, maxRows: 8}"
                    ...
                    <el-row :gutter="gutter">
                      <el-col :span="24" style="padding-right: 20px">
                        <el-form-item label="上报情况:">
                          <el-input type="text" v-model.trim="ruleForm.reportSituation" show-word-limit></el-input>
                        </el-form-item>
                      </el-col>
                    </el-row>
                  </el-form>
              ...
              <div style="display: flex;align-items: center">
                <div style="flex: 1;display: flex;justify-content: flex-start;margin-left: 10px">
                  <el-button type="primary" size="small" @click="saveEvent('ruleForm')">保存事件</el-button>
                </div>
                <div style="flex: 1;display: flex;justify-content: flex-end;margin-right: 10px">
                  <el-button type="primary" size="small" ><router-link style="color: white" to='/yangZhouEventReports'>进入事件处置</router-link></el-button>
                </div>
              </div>
            </el-scrollbar>
...
<script>
    ...
    import newPhoneModel from '@/view/eventManagement/eventRegistration/newModel/newPhoneModel'
    import audioCompName from './audioHistory'
    ...
    export default {
        ...
        components:{
          newFaxModel,newSmsModel,newPhoneModel,audioCompName,Location,addDialog,audioPlan
          // ,audioCompName
        },
        data(){
          return{
                ...
                this.$api.templateList(data).then(res => {
                    if (res.errorcode == 0) {
                        ...
                        res.data.data.forEach((item, i) => {
                            ...
                            if (this.ruleForm.eventTitle !== undefined && this.ruleForm.eventTitle !== null && this.ruleForm.eventTitle !== "") {
                                addClass = addClass.replace(
                                    "【事件标题】",
                                    this.ruleForm.eventTitle
                                );
                            }
                            if (this.ruleForm.reportedTime !== undefined && this.ruleForm.reportedTime !== null && this.ruleForm.reportedTime !== "") {
                                addClass = addClass.replace(
                                    "【接报时间】",
                                    this.ruleForm.reportedTime
                                );
                            }
                            if (this.ruleForm.incidentTime !== undefined && this.ruleForm.incidentTime !== null && this.ruleForm.incidentTime !== "") {
                                addClass = addClass.replace(
                                    "【事发时间】",
                                    this.ruleForm.incidentTime
                                );
                            }
                            if (this.ruleForm.incidenArea !== undefined && this.ruleForm.incidenArea !== null && this.ruleForm.incidenArea !== "") {
                                addClass = addClass.replace(
                                    "【事发区域】",
                                    this.ruleForm.incidenArea
                                );
                            }
                            if (this.ruleForm.incidentSite !== undefined && this.ruleForm.incidentSite !== null && this.ruleForm.incidentSite !== "") {
                                addClass = addClass.replace(
                                    "【事发地点】",
                                    this.ruleForm.incidentSite
                                );
                            }
                            if (this.ruleForm.submittedUnit !== undefined && this.ruleForm.submittedUnit !== null && this.ruleForm.submittedUnit !== "") {
                                addClass = addClass.replace(
                                    "【报送单位】",
                                    this.ruleForm.submittedUnit
                                );
                            }
                            if (this.ruleForm.incidenType !== undefined && this.ruleForm.incidenType !== null && this.ruleForm.incidenType !== "") {
                                addClass = addClass.replace(
                                    "【事件类型】",
                                    this.ruleForm.incidenType
                                );
                            }
                            if (this.ruleForm.speakerName !== undefined && this.ruleForm.speakerName !== null && this.ruleForm.speakerName !== "") {
                                addClass = addClass.replace(
                                    "【报告人-姓名】",
                                    this.ruleForm.speakerName
                                );
                            }
                            if (this.ruleForm.speakerPhone !== undefined && this.ruleForm.speakerPhone !== null && this.ruleForm.speakerPhone !== "") {
                                addClass = addClass.replace(
                                    "【报送人-电话】",
                                    this.ruleForm.speakerPhone
                                );
...
<style scoped>
 ...
 /* >>> .el-tabs__item{
  ...
  .parameterTop li,.parameterBottom li{
    list-style: none;
    width: 33.333%;
    height: 30px;
  }
