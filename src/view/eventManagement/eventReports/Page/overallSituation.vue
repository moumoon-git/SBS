    ...
    <div class="formContent">
      ...
      >
        ...
        <el-row :gutter="gutter">
          ...
          <el-col :span="12">
            <el-form-item label="首报时间:" prop="reportedTime">
              <el-date-picker
                style="width: 100%"
  ...
  data() {
    return {
      ...
      ruleForm: {
        ...
        incidentSite: "",
        submissionMethod: "",
        // accidentSituation: "", //事故情况 // bug8540 事故情况要删除 author:hexinting date:20201120
        incidenType: [],
        // incidentRank: '',
        deathToll: "",
        bruiseToll: "",
    ...
    parameter: {
      handler(nV, oV) {
        ...
        if (
          ...
          console.log(
            "参数：深度遍历是否一样：",
            this.deepCompare(this.paramsData, nV)
          );
          // console.log("旧", JSON.stringify(oV));
          // console.log("新", JSON.stringify(nV));
          let isSame = this.deepCompare(nV, this.paramsData); // 深度遍历参数是否一样
  ...
  methods: {
      ...
      if (eventId == "" || eventId == null || eventId == undefined) {
        ...
        this.$api.selectLastEvent({}).then((res) => {
          if (res.errorcode == "0") {
            ...
            let newTitle = res.data.event.title; // 事件id
            let caseID = res.data.event.caseClassId; // 事件类型id
            let caseName = res.data.event.caseClass; // 事件类型
            let areaId = res.data.event.areaId; // 辖区范围id
            let area = res.data.event.area; // 辖区范围
            ...
            sessionStorage.setItem("newTitle", newTitle); //本地存储最新事件的newTitle
            ...
            this.ruleForm.incidenArea = res.data.event.areaId; //辖区范围
            ...
            this.dataALL.incidenArea = res.data.event.areaId; //辖区范围
            ...
            // 获取辖区范围的id。因为element-UI组件中，级联组件需要每一层id组成一个数组。数据库保存辖区范围时，只保存了最后一个子级的id。
            this.getIncidenAreaId(res.data.event.areaId); // 获取辖区范围的id
            ...
            // 报送方式为三级联动的数据时，事件类型不存在，先清除上次新增的数据再自行添加数据，在隐藏
              ...
              if (res.errorcode == "0") {
                ...
                if (this.titleType == "登记") {
                  ...
                  this.$api.ok(upData).then((res) => {
                    if (res.errorcode == "0") {
                      this.parameter = res.data;
                      this.paramsData = res.data; //重新赋值数据库数据做对比
                    } else {
                      //this.$message.error(res.msg)
                    }
                  });
      ...
      this.$confirm("新增将清空以上的内容", "是否新增?", {
        ...
        .then(() => {
          ...
          (this.ruleForm.eventTitle = ""),
            ...
            (this.ruleForm.disappearToll = ""),
            (this.ruleForm.reportSituation = ""),
            // (this.ruleForm.beOnDuty = ""), //清除值班人员的值
            (this.ruleForm.beOnDuty = sessionStorage.getItem("displayname")), //初始化值班员
            (this.ruleForm.leaderOnDuty = ""), //清除值班领导的值
            (this.ruleForm.allCase = "");
          this.ruleForm.serialNumber = "";
          (this.ruleForm.speakerName = ""),
            (this.ruleForm.speakerPhone = ""),
            (this.ruleForm.timeOfWrittenSubmission = ""),
            // (this.ruleForm.accidentSituation = ""),  //事故情况 // bug8540 事故情况要删除 author:hexinting date:20201120
            (this.ruleForm.submissionMethod = ""),
            (this.parameter = []);
          this.test = "";
          this.latitude = "";
          this.longitude = "";
    ...
    getIncidenT() {
      ...
      if (this.ruleForm.incidenType.length === 1) {
          ...
          for (let i = 0; i < this.eventTypeData.length; i++) {
            if (this.eventTypeData[i].children) {
              if (
                ...
                area1 = this.eventTypeData[i].name;
                area2 = this.eventTypeData[i].children.filter(
                  (item) => item.id === this.ruleForm.incidenType[0]
                )[0].name;
                incidenType = area1 + "/" + area2;
                ...
                for (
                  ...
                  if (this.eventTypeData[i].children[j].children) {
                    if (
                      ...
                      area1 = this.eventTypeData[i].name;
                      area2 = this.eventTypeData[i].children[j].name;
                      area3 = this.eventTypeData[i].children[j].children.filter(
                        (item) => item.id === this.ruleForm.incidenType[0]
                      )[0].name;
                      incidenType = area1 + "/" + area2 + "/" + area3;
        ...
        let area1 = this.eventTypeData.filter(
          (item) => item.id === this.ruleForm.incidenType[0]
        )[0].name;
        let area2 = "";
        for (let i = 0; i < this.eventTypeData.length; i++) {
      ...
      // console.log(incidenType);
      return incidenType;
    },
    // 根据id拼接辖区名字,只获取最后一个地区
    getIncidenArea() {
      let incidenArea = "";
    ...
    generateTheTitle() {
      ...
      // 事件类型
      let incidenType = this.getIncidenT();
      /* if (this.ruleForm.incidenType.length > 0) {
