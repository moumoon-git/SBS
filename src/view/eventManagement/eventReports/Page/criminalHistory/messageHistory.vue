...
<template>
  <div>
        <div style="display:flex;text-align: left;padding: 0 10px;height: 40px;line-height: 40px">
          <label>报送单位:</label>
          <el-select v-model="value" clearable placeholder="请选择"  @change="chaneMsg">
            <el-option
...
<!--        </div>-->
        <div class="myTable">
          <my-table :tableColumn="tableColumn" :tableData="tableData" @handleCurrentChange="handleCurrentChange"></my-table>
        </div>

        <div style="margin-top: 20px">
          <el-button type="primary" size="small" @click="addAdj">添加到批示</el-button>
        </div>
...
<script>
    ...
    export default {
        ...
        methods:{
          ...
          addAdj(){//添加到批示
            if(this.tabRowData==''||this.tabRowData==[]||this.tabRowData==null){
              this.$message({
