<template>
  ...
  >
    ...
    <el-table-column min-width="60" label="通讯方式" :resizable="false" :row-style="{textAlign:'center'}">
      <template slot-scope="scope" >
        <div style="display: flex;justify-content: space-around;font-size: 16px;">
          <i v-if="showPhone"   class="myIcon yf-icon-phone" @click="handleDialPhone(scope.row,scope.$index)"></i>
          <i v-if="showoutline"    class="myIcon yf-icon-sms" @click="handleDialsSpecialPlane(scope.row)"></i>
...
<style scoped>
  @import url('../../../../assets/iconYf/style.css');
