        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="姓名:" prop="name">
              <el-input type="text" v-model="ruleForm.name" placeholder="请输入姓名" :maxlength="10" show-word-limit clearable></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-form-item label="单位:" prop="leaderWorkunit">
              <el-input type="text" v-model="ruleForm.leaderWorkunit" placeholder="请输入单位" :maxlength="15" show-word-limit clearable></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-form-item label="职务:" prop="leaderPosition">
              <el-input type="text" v-model="ruleForm.leaderPosition" placeholder="请输入职务" :maxlength="15" show-word-limit clearable></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          <el-col :span="24" style="float: right">
            <el-button
              type="primary"
              size="small"
              @click="inquireBtnDisabled('ruleForm')"
    ...
    <div>
      <el-collapse v-model="instruction" accordion>
        <div v-for="item in tableData" :key="item.id">
          <el-collapse-item style="width: 100%;font-size: 17px;font-weight: 550; margin-bottom: 5px;position: relative;">
            <template slot="title">
              <span style="margin-left: 10px;border-radius:3px;color:white;font-weight: 700;display:inline-block;width:43px;height:28px;text-align:center;line-height: 28px;" 
              :class="item.approvaltype == '批示' ? 'pishiBackColor' : 'zhishiBackColor'">{{item.approvaltype}}</span>
      ...
      <el-form>
        ...
        >
          ...
          <div
            ...
            @click="$window.open(`${fileUrl}/eos/event/sum/downloadFile?filePath=${wordPath}&fileName=${wordName}`)"
          >下载word</div>
          <div
            style="    position: absolute;
            right: 39%;
            ...
            color: white;
            background: rgb(0, 145, 255);"
          ><a :href="wordUrl" style="text-decoration:none;color: white;">在线编辑</a></div>
        </div>
      </el-form>
    </el-dialog>
...
import phoneDialog from '@/view/eventManagement/informationDisposal/communicationHistory/phone/phoneIndex';
import smsDialog from '@/view/integratedCommunication/icpPhone/icpPhonePage/smsStep/smsRightLeader';
import faxDialog from '@/view/eventManagement/newEventReports/page/faxDialog';
...
export default {
    ...
    methods: {
        ...
        copyComments(item) {
            var txtEl = document.createElement('textarea'); // 新建文本框
            txtEl.value = item.approval; // 添加值
            document.body.appendChild(txtEl); // 用浏览器的复制
            txtEl.select(); // 选择对象
        ...
        sms(newV) {
            ...
            this.isSmsShow = true;
            this.isFaxShow = false;
            this.$refs.smsDialog.msgForm1.signature = "市应急管理局值班室"
        },
        fax(item) {
            ...
            this.pdfPath = item.attachmentList[1].path
            this.wordPath = item.attachmentList[0].path
            this.pdfSendName = item.attachmentList[1].name
            this.newPdfPathDown = (this.pdfPath?this.pdfPath:"") + '?timeStamp=' + new Date().getTime()
            this.wordName = item.attachmentList[0].name
