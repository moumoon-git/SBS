...
<script>
  ...
  import TableDetails from "@/view/yz_informationDynamics/lEmergency/tableDetails";
  import workPopup from "@/view/yz_informationDynamics/lEmergency/workPopup";
  ...
  export default {
    ...
    methods:{
      ...
      getTableData(){
        ...
        this.$api.memoList(data).then(res=>{
          if(res.errorcode!=0){
            this.$message({
              message:res.msg,
              type:"error"
          ...
          if(window.g.IsYangZhouProject){
            for(var i in res.data.data){
              this.tableData[i].remindTime = res.data.data[i].remindTime.substring(0,res.data.data[i].remindTime.lastIndexOf(':'))
            }
          }
          //默认显示第一条数据
          this.$nextTick(()=>{
      ...
      handleDelete() {
        if (!this.rowData) {
          ...
          this.$confirm('删除当前信息?', '提示', {
            ...
            this.$api.memoDelete(data).then(res => {
              if (res.errorcode != 0) {
                this.$message({
                  message: res.msg,
                  type: "error"
