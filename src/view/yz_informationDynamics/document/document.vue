  ...
  <div class="document_box">
    <div class="document_main">
      <div class="document_left">
        <div class="let_top">
          <el-button type="primary" size="small" @click="treeAdd">新增</el-button>
          <el-button type="primary" size="small" @click="treeUpdat">修改</el-button>
          <el-button type="primary" size="small" @click="treeDelet">删除</el-button>
        </div>
        <div class="left_bottom" >
          <div style="width: 100%;text-align: center">
            <el-alert style="position: absolute;bottom: 0"  title="温馨提示" type="info" description="可拖动修改同级分组排序" show-icon></el-alert>
          </div>

          <div style="width: 100%;">
            <el-scrollbar style="height: 100%">
              <el-tree class="filter-tree" :filter-node-method="filterNode" :data="treeData" highlight-current :props="defaultProps"
                       @node-click="handleNodeClick" ref="tree" accordion :default-expanded-keys="[indexKey]" node-key="id"
                       :render-content="renderContent" draggable @node-drop="handleDrop"
      ...
      <div class="document_right">
        <div class="right_top">
            <div class="right_top_button">
              ...
              <el-button type="primary" size="small" @click="tableUpdate">修改</el-button>
              <el-button type="primary" size="small" @click="tableDelet">删除</el-button>
            </div>
            <div class="right_bottom_search">
              <el-col :span="18"><el-input type="text" size="small" v-model="searchText" placeholder="输入标题/关键字/文件号/制定单位"></el-input></el-col>
              <el-col :span="6"><el-button type="primary" size="small" @click="findTable">查找</el-button></el-col>
            </div>
        </div>
            ...
            <el-table
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                    <el-button type="text" icon="el-icon-download" style="font-size:20px" @click="handleShowAtt(scope.row.id)"></el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-scrollbar>
    ...
    <el-dialog :visible="dialogVisible" :title="title" width="480px"  @close="closeOuterSphere" :close-on-click-modal="false" >
      ...
      <el-button type="primary" @click="submitForm(2)" v-show="title=='修改分组'" :disabled="isSameName">保存并修改</el-button>
    </el-dialog>
    ...
    <el-dialog width="400px" :visible.sync="innerVisible" append-to-body :close-on-click-modal="false">
      <el-row >
        ...
        <el-col :span="4">
          <el-button type="primary" style="width:100%;padding-right:0;padding-left:0;" @click="filterTexts">搜索</el-button>
        </el-col>
      ...
      <el-scrollbar>
        <el-tree
          ...
          ref="tree" node-key="id"  highlight-current>
        </el-tree>
      </el-scrollbar>
    ...
    <!--表格新增||修改弹窗-->
    <work-popup ref="WorkPopup" @getTableData="getTableData" @selectTreeType="selectTreeType"></work-popup>
...
<script>
    import workPopup from "@/view/yz_informationDynamics/document/tableDialog";
    import attachmentPopup from "@/view/yz_informationDynamics/attachmentPopup";
    export default {
        name: "document",
        components:{
            workPopup,
        ...
        methods: {
            ...
            async isDuplicateName(){
                return new Promise((resolve, reject) => {
                    if (!this.parentID) { 
                        if (this.treeData.filter(item => item.name === this.ruleForm.groupName).length > 0) {
                            this.$message({
                                type: 'warning',
                        ...
                        data.forEach(item => {
                            if (item.id === parentId) {
                                ...
                                if (item.children.length > 0) {
                                    console.log('二级')
                                    console.log(item.children.filter(el => el.name === groupName))
                                    if (item.children.filter(el => el.name === groupName).length > 0) {
                                        that.$message({
                                            type: 'warning',
                                            message: '您输入的名称出现重复，请重新输入'
                                        })
            ...
            // 新增||修改保存时
            async submitForm(key){//key 1为新增 2为修改
            ...
            //内层确认按钮
            affirmDialog() {
                if(this.currentGroupParent.name!=''||this.currentGroupParent.id!=''){
            ...
            treeDelet(){
                ...
                this.$confirm('确定删除该分组?', '提示', {
                    ...
                    this.$api.deleteClasslistClass({id:this.treeNodeData.id}).then(res => {
                        if (res.errorcode == 0) {
                            this.$message({
                                message:'删除成功',
                                type: "success"
            ...
            handleNodeClick(data, node) {
                ...
                this.parentID = node.parent.data.id==undefined?0:node.parent.data.id; //父级ID
                this.parentName = node.parent.data.name==undefined?'':node.parent.data.name; //父级名称
                this.getTableData()
            ...
            handleShowAtt(id) {
                this.$nextTick(() => {
                    ...
                    setTimeout(() => {
                        ...
                        this.$refs.AttachmentPopup.showAtt(this.att);
                    }, 200);
                })
            },
            // 下载传真文件
            fileDownLoad(url) {
                window.open(
                    `${window.g.ApiUrl}/eos/communication/pdf/download?filename=${url}`+`&Authorization=${localStorage.getItem("token")? localStorage.getItem("token"): ""}`
                )
            },
        }
    }
