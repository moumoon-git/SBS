      ...
      <div class="left">
        ...
        <div style="background: white; height: 735px; padding-top: 10px">
          <el-form label-width="80px" style="height: 50px">
            ...
            >
              <div style="display: inline-block; transform: translateY(-10px)">
                <!-- 深圳需求去除 lastEditor:xuchuangxing -->
                <!-- <span class="sty">收/发</span> -->
                <!-- <el-select style="width: 90px;" class="sty" v-model="nowType" clearable>
                    <el-option v-for="(item,key) in typeList" :key="key" :label="item.name" :value="item.id">
                    </el-option>
                  </el-select> -->
              </div>
              <div style="display: inline-block; transform: translateY(-10px)">
            ...
            >
              ...
              <div
                style="display: inline-block; padding-right: 10px; float: right"
              >
                <el-date-picker
                  style="width: 281px"
                  v-model="date"
            ...
            <!-- 接收记录 -->
            <div
              v-if="statusActive == 'receiveHistory'"
              style="
                display: inline-block;
                padding-left: 33px;
                white-space: nowrap;
            ...
            >
              ...
              <div style="display: inline-block; transform: translateY(-10px)">
                ...
                >
                  <el-option
                    v-for="(item, key) in receiveStatusOptions"
                    :key="key"
                    :label="item.name"
                    :value="item.id"
                  >
          ...
          <msg-all
            ...
            :size="allSize"
            style="padding-top: 0; max-height: 640px; overflow: auto"
          ></msg-all>
          <el-pagination
            v-if="statusActive == 'msgAll'"
            background
      ...
      <div class="right">
        ...
        >
          ...
          <div class="box-card" v-for="(item, key) in smsList" :key="key">
            ...
            <span style="margin-left: 28px">{{ item.sendTime }}</span>
            <el-button
...
import { recordToText } from '../../../assets/interface/interface';
...
export default {
  ...
  methods: {
     ...
     */
    async changeALL(row) {
      console.log("row,row.content,row.smsId", row, row.content, row["smsId"]);
      let content = row["content"];
      let smsId = row["smsId"];
      this.textareaAll = content;
      // 根据短信ID获取事件ID
      let eventId = await this.getEventBySmsId(smsId);
      let eventName = row["eventName"];
      console.log("---事件id：", eventId);
      // let eventId = row['eventId']
      let comUnit = row["workUnit"];
      let direct = row["direct"];
      let dealName = row["contactor"]; //报送人
      let dealPhone = row["call"]; //报送人号码
      //防止多次点击
      if (this.smsId == smsId) {
        console.log("this.smsId和smsId相等", this.smsId, smsId);
        return;
      }
      this.checked = false;
      this.isSend = row.direct == "发送";

      console.log("this.textareaAll", this.textareaAll);
    ...
    getStatusOptions(iconRead) {
      ...
      this.$api.statusList(data).then((res) => {
        ...
        this.receiveStatusOptions = res.data.filter(item=>{
          return item.name == "未读" || item.name == "已读"
        })
        //重置状态
        this.nowStatus = "";
        if (iconRead) {
    ...
    lookupAll(page, iconRead) {
      ...
      if(this.statusActive == 'receiveHistory' && this.nowStatus == ""){
        this.nowStatus = "6"
      }
