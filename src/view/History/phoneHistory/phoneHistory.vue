...
<template>
  <div class="phoneHistory" style="margin: 0 10px">
    <el-row>
      ...
      <el-col :span="17" class="left_box">
        ...
        <div style="background: white; height: 79vh">
            ...
            >
              ...
              <template slot="directs" slot-scope="data">
                <div style="display: flex; align-items: center">
                  <div
                    class="Box1 commonBox"
                    v-show="data.slotData.direct === '已接'"
                  ></div>
                  <div
                    class="Box2 commonBox"
                    v-show="data.slotData.direct === '已拨'"
                  ></div>
                  <div
                    class="Box3 commonBox"
                    v-show="data.slotData.direct === '未接'"
                  ></div>
              ...
              <template slot="unitPost" slot-scope="data">
                {{ data.slotData.eventDealDepartment }}
                <br />
          ...
          <div
            class="my_pagination"
            style="background: #fff; text-align: right; padding: 14px 0"
          >
            <myPagination
            ...
            <el-row type="flex" justify="space-between">
              <el-col :span="9">
                ...
                >
                  <el-checkbox
                    :disabled="
                      !rigth_form.eventId || currentRow.direct == '已拨'
                    "
    ...
    <tailorAudio
      ref="tailorAudio"
      v-if="tailorAudioFlag"
      @tailorAudioClose="tailorAudioClose"
    ></tailorAudio>
    <!-- 语音转文字   -->
    <SpeechText
      ref="SpeechText"
...
import tailorAudio from "@/view/eventManagement/informationDisposal/communicationHistory/comonnTemplat/tailorAudio";
import SpeechText from "../../eventManagement/informationDisposal/communicationHistory/comonnTemplat/SpeechText";
  ...
  methods: {
    ...
    save() {
      ...
      let okData = {
        ...
        dealName:
          this.rigth_form.dealName == "" ? "-" : this.rigth_form.dealName, //报送人
      };
      // 更新电话历史
      this.$api.ok(okData).then((res) => {
        console.log(okData, 66);
        if (res.errorcode != 0) {
          this.$message({
            message: res.msg,
            type: "error",
    ...
    getTableData() {
      ...
      this.$api
        ...
        .then((res) => {
          ...
          const _index = this.tableData.findIndex(
            (el) => el.id === this.currentRow.id
          );
    ...
    //语音转文字 注释原因：语音转文字改为使用第三方服务 hexinting
    // TurnTheText(row) {
    //   alert(1)
    ...
    async TurnTheText(src, res) {
      ...
      // console.log('!!!!----', src, res)
      // this.$store.commit('setTurnToTextLoading', {loading: true})
      // this.$store.commit('setAudioText', [])
      // this.$refs.SpeechText.textarea = ''
      // this.SpeechTextFlag = true
      // if (res.recordText) {
      //   this.$refs.SpeechText.textarea = res.recordText
    ...
    rowClick(row) {
      ...
      this.rigth_form.startTime = row.startTime;
      this.rigth_form.checked = false;
