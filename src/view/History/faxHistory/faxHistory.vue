    ...
    <el-row style="margin: 0 10px">
      ...
      <el-col class="left_box" :span="16" style="background-color: #fff">
        ...
        >
          ...
          <el-tab-pane label="接收记录" name="receiveHistory">
            <receive-fax
              v-if="TabsActiveName === 'receiveHistory'"
              ref="childrenRef"
              @clickRowData="clickRowData"
            ></receive-fax>
          </el-tab-pane>
      ...
      >
        <div class="send_right_box">
          ...
          <div class="right_box">
            <img
              v-if="iframeUrl && TabsActiveName !== '回收站'"
              @click="fullScreenDialog = true"
              style="
                width: 20px;
                height: 20px;
            ...
            <div class="iframe">
              ...
              <div
                v-if="!iframeUrl"
                :style="
                  TabsActiveName === '回收站' ? 'height:655px' : 'height:600px'
                "
              >
                <img src="../../../assets/phoneHistoryIcon/faxBack.png" />
              </div>
              <div v-else>
                ...
                <div v-else>
                  <iframe
                    ...
                    border="0"
                    marginwidth="0"
                    :height="TabsActiveName !== '回收站' ? '590px' : '680px'"
                    marginheight="0"
                    scrolling="no"
                    allowtransparency="yes"
                  ></iframe>
              ...
              <template v-if="TabsActiveName !== '回收站'">
                <div style="text-align: center; padding-top: 10px">
                  <el-button
                    size="small"
...
import receiveFax from "./children/receiveFax";
...
export default {
  ...
  data() {
    return {
      isIframeUrlNotFound: 0, // 传真是否能找到，0能1不能
      resolutionKey: "",
      timerDate: "",
      ...
      isSequentialRotationIn: false, // 是否显示顺序旋转
      visible: false,
  ...
  methods: {
    ...
    async clickRowData(row, msg) {
      ...
      this.btnDisabled = true;
      if (
        this.TabsActiveName === "发送记录" ||
        this.TabsActiveName === "receiveHistory"
      ) {
        ...
        if (this.TabsActiveName === "receiveHistory") {
          this.eventTitle = row.eventTitle;
        }
        if (msg === "left" && row.faxFile) {
          this.iframeUrl = "";
          // 深圳独有问题：深圳传真格式直接赋值的话会直接下载。不能乱同步或修改
          // this.iframeUrl =
          // window.g.ApiUrl + "/fax/" + row.faxFile
          this.faxID = row.id;
        }
        if (msg === "left" && !row.faxFile) {
          this.iframeUrl = "";
        }
        // return; 不注释的话iframeUrl置空后就直接退出了
      }
      ...
      if (this.TabsActiveName === "联系人记录") {
        ...
        this.eventID = ""; //将发送记录/联系人记录的关联事件Id清空
    ...
    // 图文识别
    faxToText() {
      // 确保点击其他图文识别把上次的要识别的图片清空，重新获取识别图片，保证不会用上次的图片识别成文字
    ...
    activeResolution() {
      if (this.iframeUrl) {
        ...
        this.$refs.resolution.rawFaxPath = this.iframeUrl;
