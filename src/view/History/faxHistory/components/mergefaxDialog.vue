...
<div>
    <!-- 传真预览对话框 -->
    <el-dialog :visible.sync="isMergeFaxDialog" width="1126px" class="dialog-style" :close-on-click-modal="false" @open="faxMergeOpen" @close="faxMergeClose" top="2vh">
                            ...
                            <div style="height: 550px;">
                                <el-scrollbar style="height: 100%">
                                    <div style="margin-top: 10px;text-align: left;" v-for="(item,index) in  faxList" :key="index" @click="faxListIndexClick(item,index)">
                                        <!--<el-card  :class="{ cardModelDiv:changeIndex == index}"-->
                                        <!--shadow="hover" :body-style="bodyStyle"-->
                                        <!--style="box-shadow:0 11px 56px -11px rgba(54, 121, 225, 0.16)">-->
                                        <!--<div slot="header" >-->
                                        <!--<div style="font-weight:bold;font-size:17px;color:#555555;">{{`${item.unit?item.unit:item.phone}`}}</div>-->
                                        <!--<el-button  @click="addFaxListIndex(item, index)" size="small" type="text"-->
                                        <!--:disabled="item.addOkIsNo =='添加' ? false:true"-->
                                        <!--:class="item.addOkIsNo =='添加' ? 'add':'added'">{{item.addOkIsNo}}</el-button>-->
                                        ...
                                        <el-card :class="{ cardModelDiv:changeIndex == index}" shadow="hover" :body-style="bodyStyle" style="box-shadow:0 11px 56px -11px rgba(54, 121, 225, 0.16)">
                                            <div slot="header">
                                                <div style="font-weight:bold;font-size:16px;color:#555555;">{{item.contactor || item.unit || item.phone || item.call || item.number || '暂无'}}</div>
                                                <el-button @click.stop="addFaxListIndex(item, index)" size="small" type="text" :disabled="item.addOkIsNo =='添加' ? false:true" :class="item.addOkIsNo =='添加' ? 'add':'added'">{{item.addOkIsNo}}</el-button>
                                                <div style="font-size:14px;color:#999999;padding-top: 8px">{{item.sendtime || '暂无'}}</div>
                                            </div>
                                            <div style="font-size:16px;color:#666666;">{{item.title || '暂无'}}</div>
                                        </el-card>
                                    </div>
                                </el-scrollbar>
                            </div>
...
export default {
    ...
    methods: {
        ...
        getSendFaxList() {
            ...
            if (this.faxTimeDate) {
                console.log(this.faxTimeDate);
                time1 = this.formatDateTime(this.faxTimeDate[0]) + ' 00:00:00' // 后端统计需要加上时间 author:hexinting date:2020-10-27
                time2 = this.formatDateTime(this.faxTimeDate[1]) + ' 23:59:59' // 后端统计需要加上时间 author:hexinting date:2020-10-27
            }
            let data = {
                type: '', //类型
                startTime: time1,
            ...
            this.$api.sendfaxGroupList(data).then((res) => {
                if (res.errorcode == 0) {
                    this.faxSeverList = res.data.data;
                    this.faxList = [];
                    let saveData = [...this.faxSeverList]
        ...
        getRelationFaxList() {
            let data = {
                id: this.faxAllForm.contactId,
                page: this.currentPage,
                size: this.pageSize
            }
            this.$api.findFaxListByContactId(data).then((res) => {
                if (res.errorcode == 0) {
                    this.faxSeverList = res.data.data;
                    this.faxList = [];
                    let saveData = [...this.faxSeverList]
        ...
        addFaxListIndex(item, i) {
            ...
            this.$api.previewFaxArrayBuff(data1).then((res) => {
                if (!res.errorcode) {
