<!--
  ...
  @description: 统计分析-值班统计分析-短信统计（优化版）。
-->
<template>
  <div>
    <div class="operation_bar">
      ...
      <div class="operation_bar_right_item">
        ...
        <!-- 统计 -->
        <el-button type="primary" @click="inquireBtnDisabled()">统计</el-button>

        <!-- 生成话务统计报表 -->
        <el-button type="primary" @click="createSmsExels">生成短信统计报表</el-button>
      </div>
    </div>
    <div class="contentStyle" >
      <echarts-text-statistics
        ref="echartsTextStatistics"
        :id="idTwo"
        :width="width"
        :height="height"
        :color-list="colorList"
        style="margin-bottom:14px;"
        class="echarts_text_statistics">
      </echarts-text-statistics>
      <echarte-node
        ref="echarteNode"
        :id="idOne"
...
<script>
    ...
    import echartsTextStatistics from "./echartsPages/echartsTextStatistics"
    import echarteNode from "./echartsPages/echarteNode"
    ...
    export default {
        ...
        methods:{
          createSmsExels(){//导出短信统计报表
            ...
            switch (type) {
              case "0":
                ...
                const request = {
                  method: 'get',
                  url: `/eos/statistics/eventStatController/smsExcel?startTime=${star}&&timeEnd=${end}&&operatorId=${this.watch}&&groupType=${this.exportLog.value}`,
          ...
          /*
          * @Author: qinjiaqi
          * @Date: 2020-09-16 11:08:13
          * @LastEditTime: 2020-09-16 11:08:13
          * @LastEditors: qinjiaqi
          * @Description: 阻止统计按钮重复触发
          * @FilePath: \石景山eos\src\view\statisticAnalysis\workOnDuty\page\textStatistics.vue
          */
          /** 
           * @param {type} 没有
           ...
           * @description：阻止统计按钮重复触发
           */          
          inquireBtnDisabled:debounce( function() {
              this.inquire();
          },1000),
          inquire() { //提交查询
            this.barSeries=[];
            ...
            this.$api.noteStat(linData).then((res) => {
              if(res.errorcode==0){
                this.lineXdata=res.data.time.reverse();//折线
                this.lineSeries=res.data.countMax;//折线
                let maxNum = Math.max.apply(null,this.lineSeries);
                maxNum =  Math.ceil(maxNum+Math.floor(maxNum*2));
                // this.legendData=res.data.name[0];
                this.barXdata=res.data.time.reverse();
                res.data.count.reverse()
                for(let i=0;i<res.data.count.length;i++){
                  this.barSeries.push({
                    name:res.data.count[i].name,
                    data:res.data.count[i].data,
                    type:'bar',
                  })
                }
                let barNum=(res.data.max)*2;
                this.$refs.echartsTextStatistics.drawLine( this.lineXdata,this.lineSeries,maxNum);
                this.$refs.echarteNode.drawLine(this.barXdata,this.barSeries,barNum);
              }
            });
