    ...
    <el-header>
      <div class="selectMode">
        <div class="selectMode-l">
          当月排班模式
          <br />
          <span @click="openChangeMode()">变更</span>
        </div>
...
import PersonSearch from "../ebs_Organization/paibanSearch";
import { http } from "../../assets/js/ebs_common";
export default {
  data() {
    return {
      loading: false,
  ...
  methods: {
    autoScheduling() {
      this.$confirm("是否自动排班", "提示", {
        ...
        .then(() => {
          ...
          this.$http("/duty/subordinate/automaticDuty", data).then((res) => {
            let data = res.data;
      ...
      if (todayDate - selectDate > 0) {
        ...
        this.$confirm("确认删除?", "提示", {
          ...
          .then(() => {
            this.$http({
              method: "post",
              url: `${window.g.ApiUrl}/duty/subordinate/deleteByDate`,
              data: {
                date: date,
              },
            })
    ...
    classAdmin() {
      ...
      this.$http({
        method: "post",
        url: `${window.g.ApiUrl}/duty/subordinate/saveAll`,
        data: {
          data,
        },
      })
    ...
    getTableDataByMonth(month) {
      ...
      this.$http({
        method: "get",
        url: `${window.g.ApiUrl}/duty/subordinate/getAllByDate?date=${month}`,
      })
        .then((res) => {
          this.loading = false;
    ...
    getClassByDay(date) {
      ...
      if (todayDate - selectDate > 0) {
        ...
        this.$http({
          method: "get",
          url: `${window.g.ApiUrl}/duty/subordinate/getByDate?date=${date}`,
        })
          .then((res) => {
    ...
    openChangeMode() {
      ...
      this.$http({
        method: "get",
        url: `${window.g.ApiUrl}/duty/subordinateMode/mode/vo`,
        data: {},
      })
        .then((res) => {
        ...
        .then((res) => {
          if (res.data.data == "") {
            ...
            if (res.data.errorcode == -1) {
              ...
              this.nowMonthModeData = res.data.data;
              this.modeArray = res.data.data.roleList;
              this.tempModeId = res.data.data.id;
    ...
    getOwnHistory2() {
      ...
      for (let i = 0; i < history.length; i++) {
        this.$http({
          method: "get",
          url: `${window.g.ApiUrl}/duty/subordinate/getClickHistory?roleId=${history[i]}`,
        })
          .then((res) => {
      ...
      if (peopleArr.length == 0) {
        ...
        for (let i = 0; i < peopleArr.length; i++) {
          let obj = {
            ...
            contacotrMobile:
              peopleArr[i].mobile == null || peopleArr[i].mobile == ""
                ? "-"
                : peopleArr[i].mobile,
    ...
    exportScheduling() {
      window.open(
        `${this.baseUrl}/duty/subordinate/getAllByDateExcel?date=${
          this.toYear
        }-${this.toMonth}&Authorization=${
