<template>
  ...
  <div class="allFax">
    <div class="left_top_box">
      ...
      ></el-date-picker>
      <el-input
        v-model="filtrateData.searchText"
        placeholder="请输入关键字"
    ...
    <div class="table_box">
      ...
      >
        ...
        <template slot="workUnitCall" slot-scope="data">
          ...
          <br />
          {{data.slotData.call || data.slotData.calling}}
        </template>
        <!--呼叫时间-->
        ...
        <template slot="faxType" slot-scope="data">
          <div v-if="data.slotData.direct === '已收'">
            <div
              class="comonFaxType unread"
              v-if="data.slotData.read === '未读'"
            >{{data.slotData.read}}</div>
            <div
              class="comonFaxType read"
              v-else-if="data.slotData.read === '已读'"
            >{{data.slotData.read}}</div>
          </div>
          <div v-else-if="data.slotData.direct === '已发'">
            <div
              class="comonFaxType succ"
              v-if="data.slotData.success === '成功'"
            >发送{{data.slotData.success}}</div>
            <div
              class="comonFaxType ban"
              v-else-if="data.slotData.success === '失败'"
            >发送{{data.slotData.success}}</div>
          </div>
        </template>
        <!--处置-->
        <template slot="processingName" slot-scope="data">
          <div
            style="color: #F46666"
            v-if="data.slotData.processingName === '默认处置中'"
          >{{data.slotData.processingName}}</div>
          <div
            style="color: #0091FF"
            v-else-if="data.slotData.processingName === '处置中'"
          >{{data.slotData.processingName}}</div>
          <div
            style="color: #999999"
            v-else-if="data.slotData.processingName === '无需处置'"
          >{{data.slotData.processingName}}</div>
          <div
            style="color: #0BD295"
            v-else-if="data.slotData.processingName === '已处置'"
          >{{data.slotData.processingName}}</div>
          <div v-else style="color: #666666">----</div>
        </template>
        <!--事件标题-->
    ...
    computed: {
        selectEventStyle() {
            return function(eventId) {
                ...
                // return 'color:#0091ff;background-color:rgba(0, 145, 255, 0.1);';
            };
        },
    },
    created() {
    ...
    methods: {
        ...
        handleStateSelectChange(val) {
            if (val === 'success' || val === 'fail') {
                this.typeList = [{ label: '已发', value: '1' }];
            } else if (val === 'read' || val === 'unread') {
                this.typeList = [{ label: '已收', value: '0' }];
            } else {
                this.typeList = [
                    { label: '已发', value: '1' },
                    { label: '已收', value: '0' },
        ...
        getFaxList() {
            ...
            this.$api
                ...
                .then(res => {
                    ...
                    res.data.data.forEach((e, i) => {
                        if (e.sendtime) {
                            ...
                            if (window.g.IsYangZhouProject) {
                                res.data.data[i].sendtime = res.data.data[
                                    i
                                ].sendtime.substring(
                                    0,
                                    res.data.data[i].sendtime.lastIndexOf(':')
                                );
                            }
                        }
                    });
                    ...
                    this.rowClick(
                        this.rowData ? this.rowData : res.data.data[0]
                    );
                    // 表格结束loading
        ...
        // 获取传真数据，并且不回到第一条 qinjiaqi 20210126
        getSaveUnitFaxList() {
            this.tableConfig.loading = true;
            this.$api
                .fax(this.filtrateData)
        ...
        confirmFaxDelete(id) {
            ...
            confirmDelete('放入回收站', '是否将该传真放入回收站?')
                .then(() => {
                    this.$api
                        .faxDelete(parmas)
                        .then(res => {
        ...
        rowClick(row) {
            ...
            this.rowData = {
                ...row,
                sendtime: row.sendtime.replace('<br />', ' '),
            };
            this.$emit('clickRowData', this.rowData);
            // }
        },
