<template>
  ...
  <div class="recycleFax">
    <div class="left_top_box">
      ...
      <el-button style="background:#3F92FE;padding:12px 9px; font-size:15px;color: white;border: none" @click="handleAllFaxDelete">清空回收站</el-button>
    </div>
    <div class="table_box">
      <myTable :tableColumn="tableColumn" :tableData="tableData" :tableConfig="tableConfig" @rowClick="rowClick" ref="refTable">
        ...
        <template slot="operate" slot-scope="data">
          ...
          <el-tooltip class="item" effect="dark" content="恢复" placement="top">
            <i class="el-icon-document-checked iconHandle" @click.stop="handleFaxRecover(data.slotData.id)"/>
          </el-tooltip>
        </template>
      </myTable>
    </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      // 获取传真数据
      getFaxDeleted(){
        this.$api.listFaxDeleted(this.searchData).then((res) => {
      ...
      handleFaxRecover(id){
        confirmDelete('恢复传真','是否恢复该条传真?').then(() =>{
          ...
          this.$api.faxRecover(parmas).then((res) => {     //faxRecover
            this.search()
            this.$message({
              type: 'success',
              message: '已恢复传真'
