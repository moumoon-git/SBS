  ...
  <div class="sms-tree relationFaxDialog">
      ...
      <el-col :span="14" style="border-right: 1px solid #E9EBF0;">
          ...
          <div style="padding: 20px 36px 10px; color: #666666;font-size:14px;border-bottom: 1px solid #E9EBF0">
            <template v-for="(item,i) in contactList">
              <div :class="{'shortcut_box':true,'mgb8':i===0}">
                <span :class="contactName === item.name? 'shortcut_active' : ''" @click="shortcutClick(item)">
        ...
        <el-col :span="14" style="border-left: 1px solid #E9EBF0;">
          <div style="text-align: center;padding-top: 10px" class="common_input">
            ...
            <el-input style="width: 60%;" :disabled="contactCode === 'recentcontact'" @input="handleSearch" @keyup.enter.native="handleSearch" placeholder="输入关键字搜索" v-model="searchData.searchText" class="mgb8">
          ...
          <div class="my_table" style="min-height: 623px;">
            <myTable ref="smsTable" :tableColumn="tableColumn" :tableData="tableData" :tableConfig="tableConfig" @rowClick="rowClick" @selectallrow="selectFaxAll" @selectrow="selectionFaxRow(arguments)" @selectionChange="selectionChange"></myTable>
          </div>
    ...
    >
      <div class="msg-dialog">
        <i class="el-icon-error" title="关闭" @click="msgDialogVisible = false"></i>
    ...
    methods: {
      ...
      changTab(tabName) {
        ...
        if (this.selectedName != tabName){
          ...
          this.selectedCode = groupCode; // 选中分组code
      ...
      nodeClick(nodeData,node){
        // 判断防止多次点击发送同一请求
        if (this.contactCode === 'recentcontact') {
          this.contactCode = '';
        }
      ...
      toggleRowSelection() {
        this.tableData.forEach(item => {
          this.$refs.smsTable.$refs.myTable.toggleRowSelection(item, false)
        })
      },
      /**
       * @author hexinting
       * @date 2021-01-28
       * @description 判断右侧已勾选人员，对应表格是否勾选
       */
      judgeIsSelected() {
        console.log(this.tableData, this.$refs.smsTable.$refs.myTable)
        var smsMember = this.tags;
        this.tableData.forEach(item => {
          let _index = smsMember.findIndex(el => el.id === item.id);
          console.log(_index)
          if (_index > -1) {
            this.$refs.smsTable.$refs.myTable.toggleRowSelection(item, true)
          }
        })
      },
      /**
      ...
      selectFaxAll(rows){
        ...
        if (this.contactCode === 'recentcontact') {
          ...
          var temporarySmsMember = this.tags;
          if (this.clickCount % 2 !== 0) { // 若点击次数是单数，则取消全选
            this.toggleRowSelection(); // 取消全选表格
            if(this.tableData.length > 0){
              this.tableData.forEach(item => {
                let _index = temporarySmsMember.findIndex(element => { return element.id == item.id }); // 下标 若右侧已勾选人员中对应到表格中有勾选，则取消勾选
                  if(_index !== -1){
                    temporarySmsMember.splice(_index, 1); // 取消选中
                    this.tags = temporarySmsMember;
                  }
              })
            }
          } else {
      ...
      selectionFaxRow(data){
        var rows = data[0]; // 表格勾选行
        var row = data[1] ? data[1] : null; // 当前选中或取消选中的一行
        if (row) {
          row = data[1]; // 单选的一行
          // 判断当前勾选状态改变的一行是否已存在右边已勾选队列
          if (rows.findIndex(item => item.id === row.id) < 0) {
            let delIndex = this.tags.findIndex(item => item.id === row.id)
            if (delIndex >= 0) {
        ...
        if (this.contactCode === 'recentcontact') {
          ...
          let inNumberArr = this.disposeFaxNumber(rows)[1] // 处理不存在传真号码并拿到存在的传真号码[1]的数组
          console.log('inNumberArr', inNumberArr)
          this.existFaxNumber(inNumberArr)
        }
        // if (rows.length === 0){
        //   return
