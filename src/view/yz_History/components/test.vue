...
<script>
  import mixin from '@/yz_integratedCommunication/icpPhone/icpPhonePage/mixin.js';
  ...
  export default {
    ...
    data() {
      return {
        ...
        // list: window.g.IsYangZhouProject ? [
        //   {//按钮切换
        //     name: '政务通信录',
        //     label: '1'
        //   },
    ...
    methods: {
      ...
      initContactorGroupsConfig() {
        ...
        if (this.list.filter(item => item && item.code === "phone").length === 0) { // 如果没有配置政务联络网
      ...
      changePage(){
        ...
        if(this.tabPosition==2){
          ...
          this.getDataAl();
          this.tabName = '常用联系人'
          this.tabCode = 'commonlyused'
        }else if(this.tabPosition==3){
          this.getRecentContactsTable()
          this.tabName = '最近联系人'
          this.tabCode = 'recentcontact'
      ...
      addList() {//新增
        this.$nextTick(() => {
          ...
          // this.$refs.ContactDetailsPopup.getTree(this.tabPosition, this.groupId, this.tabName);
           this.$refs.ContactDetailsPopup.getTree(this.tabPosition, this.groupId, this.tabCode);
        });
      }
      ,
      //修改方法
