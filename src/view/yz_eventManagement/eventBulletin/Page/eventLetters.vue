        ...
        <el-col :span="12">
          ...
          <div v-if="forceRefresh" :style="rightStyle">
            <el-scrollbar style="height: 100%">
              <historical-details
                v-show="selectedName == '事件快报'"
                :wordUrl="wordUrl"
                @getReceive="getReceive"
                @getEventHistorical="getEventHistorical"
                ref="historicalDetails"
              ></historical-details>
              <express-the-details
                v-show="selectedName == '快报历史'"
                ref="expressTheDetails"
              ></express-the-details>
            </el-scrollbar>
          </div>
        </el-col>
...
<script>
import navBar from "@/view/yz_eventManagement/eventReports/Page/criminalHistory/navbar";
import historicalDetails from "@/view/yz_eventManagement/eventBulletin/Page/historicalDetails";
import expressTheDetails from "@/view/yz_eventManagement/eventBulletin/Page/expressTheDetails";
import incidentReport from "@/view/yz_eventManagement/eventBulletin/Page/incidentReport";
import myTable from "@/yz_components/tables/tables";
document.write('<scr' + 'ipt src="' + window.SITE_CONFIG['cloudUrl'] + '/fileupload/pageoffice.js"></scr' + 'ipt>');
export default {
  name: "eventLetters",
  components: {
    navBar,
  ...
  data() {
    return {
      ...
      fileUrl: window.SITE_CONFIG['cloudUrl'],
      wordUrl: '',
    };
  },
  watch: {
    ...
    getReceive() {
      ...
      this.$api.eventLettersHistory(data).then((res) => {
        if (res.errorcode == 0) {
          ...
          if (res.data.length) {
            this.$http({
              url: window.g.ApiUrl + "/eos/event/bullitin/createEventBullitinAttach",
              method: "post",
              data: {
                datebullitinId: res.data[0].id,
              }
            }).then((res) => {
              this.wordUrl = `javascript:POBrowser.openWindowModeless(
                '${this.fileUrl}/fileupload/pageoffice/word?filePath=${res.data.data.filePath}
        ...
        if (res.errorcode == 0) {
          if (res.data.length > 0) {
            ...
            if (this.eventList.length) {
              this.$refs.incidentReport.initData(this.eventList[0]);
            }
          }
        }
      });
