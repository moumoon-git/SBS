...
<template>
  <div>
    <div>
      <el-col :span="24">
        <div style="padding: 20px;text-align: left">
          ...
          <el-select v-model="SelectValue" clearable placeholder="请选择" @change="monitorSelect">
            <el-option
    ...
    <div  class="contentMain">
      <div v-for="(item,index) in list" class="operationPlan" :key="index" @click="clickUpdate(item,index)">
        <div id="plans" :class="{ red:changeRed == index}">
          <div class="timePlan">
            <el-row>
              <el-col :span="5"><div class="reportingunit">报送单位:{{item.department}}</div></el-col>
              <el-col :span="4"><div class="submissionPeople">报送人:{{item.dealName}}</div></el-col>
              <el-col :span="7"><div class="submitPhoneNumber">报送号码:{{item.dealPhone}}</div></el-col>
              <el-col :span="8"><div class="submitTime">报送时间:{{item.dealTime}}</div></el-col>
            </el-row>
          </div>
          <!-- <div class="timePlan">{{item.department}} {{item.dealTime}}&nbsp;&nbsp;&nbsp;</div> -->
          <!-- <div class="contentPlan">{{item.description}}</div> -->
          <div class="contentPlan"> <el-row><el-col :span="24"><div class=" reportingunit">报送内容:{{item.description}}</div></el-col></el-row></div>
          ...
          <div class="upLoadtext" v-for="(item,index) in item.attachments" :key="index">
            <el-row>
              <el-col :span="24"><div class=" reportingunit"> 报送附件:{{item.fileName}}   <el-button size="mini" type="primary" @click.stop="downloadFile(item)">下载</el-button></div></el-col>
            </el-row>
          </div>
    ...
    <div>
        <div class="addButton">
          <el-button   type="primary" size="small" v-if="restShow" @click="addReport">新增续报</el-button>
          <el-button   type="primary" size="small" v-if="restShow" @click="updataReport">修改续报</el-button>
          <el-button   type="primary" size="small" v-if="restShow" @click="delectReport">删除续报</el-button>
          <el-button   type="primary" size="small" v-if="saveShow" @click="saveEvent('ruleForm')">保存续报</el-button>
          <el-button   type="primary" size="small" v-if="saveShow" @click="callOff">取消</el-button>
        </div>
    </div>

    <div id="formSave">
      <el-form style="padding: 0 20px;" label-width="100px" :model="ruleForm" :rules="disabled ? null : rules" ref="ruleForm" @submit.native.prevent  label-position="right">
        <el-row :gutter="gutter">
          <el-col :span="12">
            <el-form-item label="报送单位:" prop="submissionUnits" >
              <el-input type="text" :readonly="disabled"  v-model.trim="ruleForm.submissionUnits" show-word-limit></el-input>
            </el-form-item>
          </el-col>
          <!-- <el-row :gutter="gutter"> -->
          <el-col :span="12">
            <el-form-item label="报送人:" prop="submissionPeople" >
              <el-input type="text" :readonly="disabled"  v-model.trim="ruleForm.submissionPeople" show-word-limit></el-input>
            </el-form-item>
          </el-col>
          <!-- -->
        </el-row>
        ...
        <el-row :gutter="gutter">
          <el-col :span="12">
            <el-form-item label="报送号码:" prop="submissionNumber" >
              <el-input type="text" :readonly="disabled"  v-model.trim="ruleForm.submissionNumber" show-word-limit></el-input>
            </el-form-item>
          </el-col>
         <el-col :span="12">
        ...
        <el-row :gutter="gutter">
          <el-col :span="24">
            <el-form-item label="报送内容:" prop="submissionContent">
              <el-input type="textarea" :readonly="disabled" resize="none"  v-model.trim="ruleForm.submissionContent" show-word-limit :autosize="{ minRows:4, maxRows: 8}"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    export default {
        ...
        data() {
          return {
            ...
            rules:{
              ...
              submissionPeople: [
                {  message: '请输入报送人姓名', trigger: 'blur' },
              ],
              submissionNumber: [
                { message: '请输入报送号码', trigger: 'blur' },
              ],
              submissionTime: [
        ...
        methods:{
          ...
          updataReport(){//修改续报
            ...
            if(this.continueRow==''||this.continueRow==null||this.continueRow==[]){
              this.$message({
          ...
          saveEvent(ruleForm){//保存续报
          console.log(ruleForm,2222)
            if(this.eventId==''||this.eventId==null||this.eventId=='newID'){
              this.$message({
                message: '请先保存事件',
                type: 'warning'
          ...
          clickUpdate(item,index){//选中当前的续报
         ...
         onremove(file, fileList){
           ...
           this.uploadFileData = this.uploadFileData.filter(e => file.path !== e.filePath)
         },
         //文件上传成功时的钩子
          onsuccess(res) {
            this.uploadFileData.push(res.data)    //保存时请求参数
            this.uploadFile.push({name: res.data.fileName, path: res.data.filePath})
          },
         /**
         * @lastEditor hexinting
         * @lastDate 2021-02-07
