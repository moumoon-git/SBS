<template>
  <div class="submissionReview_wrap">
      ...
      <el-row style="height: 300px; margin-top: 20px">
        <div v-if="nameType == '短信'" style="height: 235px">
          <el-row>
            ...
            <el-col :span="8">
              ...
              >
                <eventImgOrVideo
                  resources-type="图片"
                  :img-or-video-data="submitData.checkImgArr"
                  @preview="preview"
                  @delImg="delImg"
                ...
                <eventImgOrVideo
                  resources-type="视频"
                  :img-or-video-data="submitData.checkVideoArr"
                  @preview="preview"
                  @delImg="delImg"
                  @showPicture="showPicture"
                ></eventImgOrVideo>
          ...
          <div
            ...
            v-if="auditSubmissionList.length > 0"
          >
            <div
            ...
            >
              <div class="auditSubmissionInfo">
                ...
                <div
                  ...
                  v-if="!item.eventSubmitApproval.status"
                >
                  未提交
                </div>
                <div
              ...
              <div class="submittedTo">
                报送对象：<span
                  v-for="(one, index) in item.eventSubmitApproval.sendContacts"
                  :key="index"
                  >{{
                    index == item.eventSubmitApproval.sendContacts.length - 1
                      ? one.name +
                        (one.position ? " (" + one.position + ")" : "")
                      : one.name +
                        (item.position ? " (" + one.position + ")" : "") +
                        "、"
                  }}</span
                >
              </div>
              ...
              <div class="auditSubmissionButton">
                <template v-if="!item.eventSubmitApproval.status">
    ...
    >
      ...
      >
        <el-checkbox-group
          v-model="checkImgList"
          v-if="resourcesType == '图片'"
        >
          <el-checkbox
            :label="item.url"
            v-for="item in imgOrVideoArr"
            :key="item.id"
          >
            <div style="width: 300px; height: 180px">
              <img
                :src="fileUrl + item.url"
                :onerror="baseUrl + item.url"
                alt
                style="width: 100%; height: 100%"
              />
            </div>
            <div class="eventImghover" @click.prevent="preview(item.url)">
        ...
        >
          ...
          >
            <div style="width: 300px; height: 180px">
              <video
                :src="fileUrl + item.path + item.name"
                :onerror="baseUrl + item.path + item.name"
                controls
                style="width: 100%; height: 100%"
              ></video>
            </div>
            <div
              class="eventImghover"
              @click.prevent="preview(item.path + item.name)"
            >
              <div class="preview"></div>
            </div>
          </el-checkbox>
...
import submittedDisopsition from "@/view/yz_eventManagement/eventReports/Page/share/submittedDisopsition"; //配置
...
import eventImgOrVideo from "./EventImgOrVideo";
...
export default {
  ...
  data() {
    return {
      ...
      idx: "", // 当前审核点击的索引值
      imgOrVideoArr: [], // 审核报送图片或图片
      previewVisible: false, // 预览图片视图
      previewUrl: "", // 预览URL
      eventImgVisible: false, // 添加事件全景图片视图
      checkImgList: [], // 选中的图片
      checkVideoList: [], // 选中的视频
      baseUrl: window.g.ApiUrl,
      fileUrl: window.g.FileUrl,
      resourcesType: "", // 判断加载的是图片还是视频数据
      isYangZhou: window.g.IsZengChengProject, // 是否扬州项目
    };
  },
  mounted() {
    this.getButton();
  ...
  methods: {
    ...
    getEventImg() {
      ...
      this.$api.eventImg(data).then((res) => {
        ...
        this.imgOrVideoArr = res.data;
        if (this.imgOrVideoArr.length > 0) {
          this.imgOrVideoArr.map((v, index) => {
            if (
              v.path
                .substring(v.path.indexOf("."), v.path.length)
                .toLowerCase()
                .match(/\.(jpg|jpeg|png|gif|svg)$/) == null &&
              v.filename
                .substring(v.filename.indexOf("."), v.filename.length)
                .toLowerCase()
                .match(/\.(jpg|jpeg|png|gif|svg)$/) == null
            ) {
              this.imgOrVideoArr.splice(index, 1);
            }
          });
        }

        if (this.imgOrVideoArr.length > 0) {
          this.imgOrVideoArr.map((v) => {
            v.url =
              v.path
                .substring(v.path.indexOf("."), v.path.length)
                .toLowerCase()
                .match(/\.(jpg|jpeg|png|gif|svg)$/) != null
                ? (v.path.substring(0, 1) == "/" ? "" : "/") + v.path
                : v.path +
                  (v.filename
                    .substring(v.filename.indexOf("."), v.filename.length)
                    .toLowerCase()
          ...
          console.log("this.imgOrVideoArr: ", this.imgOrVideoArr);
        }
      });
    },
    // 获取事件全景视频
    getEventVideo() {
      ...
      this.$api.eventVideo(data).then((res) => {
        if (res.errorcode != 0) {
          this.$message.error(res.msg);
          return;
        }
    ...
    missPlan(list, index) {
      ...
      if (this.buttons[index].dynamicContactor.code === 'yes') {
        if (this.area) {
          ...
          this.flowSetting = 1; // 石景山需求 默认预设人员
        } else {
          this.flowSettingArr.push(
            {
              name: "预设人员",
        ...
        if (this.area) {
          ...
          this.flowSetting = 0; // 石景山需求 默认预设人员
        } else {
          this.flowSettingArr.push({
            name: "预设人员",
            id: 0,
      ...
      this.processForm();
      console.log("flowSetting:", this.flowSetting);
      console.log("flowSettingArr:", this.flowSettingArr);
    },
    setClick(index) {
      //设置点击按钮
      console.log(index);
    ...
    clcikSend() {
      ...
      //             filename: '12345.pdf',
      //           };
      //           this.$api.previewFaxArrayBuffdownload(data).then((res) => {
      //             if(!res.errorcode=='0'){
      //               this.$message.error(res.msg);
      //             }
      //           })
      ...
      for (let i = 0; i < txtDom.length; i++) {
        ...
        if (curHeight > twoHeight) {
          this.$set(
            this.auditSubmissionList,
            i,
            Object.assign({}, this.auditSubmissionList[i], {
              stretch: true,
            })
          );
    ...
    editHandle(obj) {
      ...
      if (obj.addOrEdit) {
        ...
        this.submitData.checkImgArr = obj.attachmentList.pictures;
        // 只获取视频数据路径
        if (obj.attachmentList.videos.length > 0) {
          this.submitData.checkVideoArr;
          obj.attachmentList.videos.map((v) => {
            this.submitData.checkVideoArr.push(v.pictureUri);
          });
        } else {
          this.submitData.checkVideoArr = [];
        }
        ...
        //   });
        // });
        this.$refs.noteCheck.dataName = obj.eventSubmitApproval.sendContacts;
    ...
    // 删除选中图片
    delImg(resourcesType, item) {
      if (this.resourcesType == "图片") {
        this.submitData.checkImgArr = this.submitData.checkImgArr.filter(
          (v) => v != item
        );
      } else if (this.resourcesType == "视频") {
        this.submitData.checkVideoArr = this.submitData.checkVideoArr.filter(
          (v) => v != item
        );
      }
    },
    ...
    submit() {
      ...
      if (this.resourcesType == "图片") {
        this.submitData.checkImgArr = this.checkImgList;
      } else if (this.resourcesType == "视频") {
        this.submitData.checkVideoArr = this.checkVideoList;
      }
      // console.log('this.checkImgArr: ', this.submitData.checkImgArr);
    },
    // 显示事件全景图片或视频
    showPicture(resourcesType) {
      this.resourcesType = resourcesType;
      this.eventImgVisible = true;
      if (this.resourcesType == "图片") {
        this.getEventImg();
        if (this.submitData.checkVideoArr.length > 0) {
          this.checkImgList = this.submitData.checkImgArr;
        } else {
          this.checkImgList = [];
        }
      } else if (this.resourcesType == "视频") {
        this.getEventVideo();
        if (this.submitData.checkVideoArr.length > 0) {
          this.checkVideoList = this.submitData.checkVideoArr;
        } else {
          this.checkVideoList = [];
        }
      }
    ...
    /**
     * @description: 提交审核后初始化数据
     * @lastEditor : xuchuangxing
     * @updateTime : 2020-11-02 14:27
     * @param {*}
     * @return {*}
     */
    initData() {
...
/* 审核报送样式 */
.auditSubmissionData .submittedTo {
  text-align: left;
  padding: 10px;
  font-size: 15px;
  font-weight: bold;
