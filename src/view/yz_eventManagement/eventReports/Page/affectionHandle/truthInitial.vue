    ...
    <div style="display: flex; align-items: center; padding: 15px 0">
      <div style="flex: 2">
        <label v-if="(cpShow = false)">
          事件等级:
          <span style="margin-left: 30px">{{ tentative }}</span>
        </label>
        <el-radio-group
...
export default {
  ...
  watch: {
    Level: {
      handler(newV) {
        ...
        obj = this.LevelArr.find((item) => item.id === newV); // 筛选出匹配数据
        this.decisionPlan = obj ? obj.name : '';
      },
      deep: true,
    },
    ...
    initialPlan: {
      ...
      handler(newV) {
        let obj = {};
        // 这里的userList就是上面遍历的数据源
        obj = this.options.find((item) => item.id === newV); // 筛选出匹配数据
        this.decisionLevel = obj ? obj.name : '';
      },
      deep: true,
    },
  ...
  methods: {
      ...
      this.$api.getDefault(data).then((res) => {
        if (res.errorcode === 0) {
          ...
          if (
            ...
            if (String(sessionStorage.getItem('refreshTruti')) === 'true') {
              this.elSwitchValue = 1;
              this.cutPlan(res.data.preplans.id);
            }
    ...
    Steimlevel(e) {
      let obj = {};
      // 这里的userList就是上面遍历的数据源
      obj = this.LevelArr.find((item) => item.id === e); // 筛选出匹配数据
      this.decisionPlan = obj ? obj.name : '';
    ...
    setIme() {
      ...
      const phoneTime = sessionStorage.getItem('phoneExtend1');
      const smsTime = sessionStorage.getItem('smsExtend2');
      if (this.eventId == '' || this.eventId == null) {
        return;
      }
      if (phoneTime == 0 || phoneTime == '' || phoneTime === 'undefined') {
          ...
          if (res.errorcode === 0) {
            if (res.data.hasPhone) {
              ...
              if (res.data.phoneTimeout) {
                ...
                const overtime = res.data.phoneTime; // 超时
                this.phoneTime = overtime;
              } else {
                this.showPhoneGreen = true;
                this.showPhoneRed = false;
                const noOvertime = res.data.phoneTime;
            ...
            if (res.data.hasSms) {
              ...
              if (res.data.smsTimeout) {
                ...
                const overNotetime = res.data.smsTime; // 超时
                this.smsTime = overNotetime;
              } else {
                this.showNoteGreen = true;
                this.showNoteRed = false;
                const overNotetime = res.data.smsTime;
                this.smsTime = overNotetime;
              }
            } else {
              this.showNote = false;
            }
            const day = parseInt(this.phoneTime / 60 / 24);
            const hour = parseInt((this.phoneTime / 60) % 24);
            const min = parseInt(this.phoneTime % 60);
            const smsDay = parseInt(this.smsTime / 60 / 24);
            const smaHour = parseInt((this.smsTime / 60) % 24);
            const smsMin = parseInt(this.smsTime % 60);
      ...
      if (smsTime == 0 || smsTime === '' || phoneTime === 'undefined') {
    ...
    cutPlan(e) {
      let obj = {};
      obj = this.options.find((item) => item.id === e);
      this.decisionRank = obj ? obj.name : '';
      this.initialPlan = e;
      this.getplanLevel();
      ...
      this.$http({
        ...
        )}&preplanId=${String(e)}&eventDealId=${this.activeEvent.eventDealId}`,
        method: 'get',
      }).then((res) => {
        console.log('/eos/event/param/judgeEventLevel res', res);
        if (res.data.errorcode == 0) {
          if (res.data.data) {
            this.plan.forEach((eeeee, i) => {
              if (eeeee.eventLevelId === res.data.data.eventLevelId) {
                this.setClick(this.plan[i], i);
              }
            });
          }
          this.Level = res.data.data.eventLevelId;
