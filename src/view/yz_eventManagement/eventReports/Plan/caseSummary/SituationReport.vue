  ...
  <div>
    <div class="addButton">
      <el-button @click="upload" type="primary" size="small"
        >点击上传</el-button
      >
      <!-- <el-upload action="123" :headers="{Authorization: $window.localStorage.getItem('token') ? $window.localStorage.getItem('token') : ''}" :show-file-list="false" with-credentials :before-upload="beforeUpload">
...
export default {
  ...
  methods: {
    ...
    /**
     * @description: 删除文件服务器对应文件同时删除列表
     * @lastEditor : xuchuangxing
     * @updateTime : 2020-11-09 13:50
     * @param {*} row ：选中列表信息
     * @return {*}
     */
    handleDeleteItem(row) {
      ...
      this.$confirm("删除当前信息?", "提示", {
        ...
        .then(() => {
          ...
          axios({
            method: 'get',
            url: `/eos/event/sum/delFile?filePath=${row.url}`,
    ...
    handleDownloadItem(row) {
      const request = {
        method: 'get',
        url: `/eos/event/sum/downloadFile?fileName=${row.name}&filePath=${row.url}`,
        baseURL: window.SITE_CONFIG.fileupload,
        headers: {
      ...
      // window.location.href = `${window.SITE_CONFIG.fileupload}/eos/event/sum/downloadFile?fileName=${row.name}&filePath=${row.url}`;
    },
    upload() {
      console.log(Bus.$emit,123)
