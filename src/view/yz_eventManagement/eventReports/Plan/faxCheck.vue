    ...
    <div style="margin-bottom: 10px" id="formDom">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-position="left" @submit.native.prevent>
        <el-form-item prop="inputImport">
          <el-input @blur="blur('ruleForm')" v-model.number="ruleForm.inputImport" placeholder="添加号码,回车确认添加!"
    ...
    methods: {
      ...
      submitSendFax({
        ...
        for (let i = 0; i < this.dataName.length; i++) {
          faxMemberList.push({
            receiverFaxNo: this.dataName[i].value,
            contactType: 0,
            // contactId: '', // 手动输入的不用填
            // receiverPlatformId: '', // 手动输入的不用填
          });
          if (length <= 1) {
            memberList += this.dataName[i].value;
        ...
        this.$http({
          url: `${window.SITE_CONFIG["event"]}/fax/sendFax`,
          method: 'post',
       ...
       */
      promptConfirm() {
        this.sendFax(this.faxForm, this.faxFileName); // 发送传真
      },
      /**
       * @author hexinting
       * @date 2020-10-25
      ...
      preview() {//文件预览
        ...
        window.open(this.pdfPath+`?Authorization=${localStorage.getItem("token")? localStorage.getItem("token"): ""}`)
      },
      ...
      submitUpload(uploadForm, pdfName) {
        ...
        // 任务133 date:2020-10-25 author:hexinting
        if (this.IsYangZhouProject) {
          this.promptConfig.visible = true
          this.faxForm = uploadForm; // 存放传真表单数据
          this.faxFileName = pdfName; // 存放传真文件名字
        } else {
          this.sendFax(uploadForm, pdfName);
        }
      },
      /**
       * @param {obj} uploadForm 上传传真文件成功的数据
