      ...
      <el-row style="margin-bottom: 10px;background: #F0F6FF;line-height:28px;">
        ...
        <el-col :span="4" v-show="eShow">
          <el-upload  class="upload-demo" ref="upload" auto-upload with-credentials accept=".jpg,.jpeg,.png,.pdf,.ppt,.pptx,.doc,.docx,.txt,.xlsx,.xls"
                      ...
                      :show-file-list="false" :headers="{Authorization: $window.localStorage.getItem('token') ? $window.localStorage.getItem('token') : ''}">
...
<script>
    import tableList from '@/yz_components/table/tableList'
    import addressBookTree from '@/view/yz_eventManagement/eventRegistration/newModel/smsPhoneTree'
    import quickGroup from '@/view/yz_eventManagement/eventRegistration/newModel/quickGroup'
    import callLogModel from '@/view/yz_eventManagement/eventRegistration/newModel/callLogModel'
    import EditorBar from '@/view/yz_eventManagement/informationDisposal/editorChint'
    import EditorBar1 from '@/view/yz_eventManagement/informationDisposal/editorChint1'
        ...
        methods: {
            ...
            exportPdf(){
              this.$refs.editorBarData.editor.txt.html(this.editor.info)
                ...
                this.$api.convertHtmlPDF({html:this.baseData}).then((res) => {
                    if (res.errorcode == 0) {
                        this.dialogVisible3=false
                        this.PreviewPath=window.g.ApiUrl+'/fax/'+res.data
                    } else {
                        this.$message.error(res.msg)
                    }
                });
            ...
            submitUpload(){
                ...
                if (this.checkedList.length == 0) {
                  ...
                  // 扬州项目需求 任务133 date:2020-10-25 author hexinting
