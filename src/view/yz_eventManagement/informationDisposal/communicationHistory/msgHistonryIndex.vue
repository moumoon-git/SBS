  ...
  export default {
      ...
      methods:{
          ...
          minutes(time){
              ...
              //分钟
              if (secondNum>=60) {
                  minutes = parseInt(secondNum/60)>=10?parseInt(secondNum/60):'0'+parseInt(secondNum/60)
              }
              return minutes+'分'+ (secondNum%60>=10?secondNum%60:'0'+secondNum%60)+'秒'
          },
          //小时
          hourTimeNum (time) {
              //大于等于一个小时
              let hourStr = '00'
              if (time>=3600) {
                  hourStr = parseInt(time/3600)>=10?parseInt(time/3600):'0'+parseInt(time/3600)
