        ...
        <el-main style="margin-left:30px;">
          <!-- <el-scrollbar style="height: 90%"> -->
            <el-row>
              <el-col :span="12">
                <div class="head-class">
                  <img :src="contactForm.picture == '' ? imageUrl : this.$window.g.ApiUrl + contactForm.picture" alt="">
            ...
            <el-row>
              ...
              <el-col v-if="!IsYangZhouProject" :span="12">
                <span>紧急程度：</span>
...
export default {
  ...
  methods: {
    ...
    getTreeParentId(data, tabName, groupId) {
      ...
      let contactorGroups = JSON.parse(sessionStorage.getItem("contactorGroups")); // 分组名字
    ...
    showData(row) {
      this.$nextTick(() => {
        ...
        this.contactForm.picture=row.picture
    ...
    copyBusinessCardHandle() {
      if (
        ...
        this.copyBusinessCard +=
          ...
          (this.contactForm.fax ? "；传真号码：" + this.contactForm.fax : "") +
          "    ";
        if(window.g.IsZengChengProject) {
          this.copyBusinessCard = this.copyBusinessCard.replace('；性别：男','')
          this.copyBusinessCard = this.copyBusinessCard.replace('；性别：女','')
        }
