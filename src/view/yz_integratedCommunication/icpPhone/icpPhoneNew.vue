...
import phoneRight from "@/view/yz_integratedCommunication/icpPhone/icpPhonePage/phoneStep/phoneDial";
...
import meetingRight from "@/view/yz_integratedCommunication/icpPhone/icpPhonePage/meetingSep/meetingRight";
import talkRight from "@/view/yz_integratedCommunication/icpPhone/icpPhonePage/talkStep/talkRight";
...
export default {
  ...
  data() {
    return {
      ...
      navList: window.g.IsYangZhouProject ? [
        {
          name: "phone",
          title: "电话"
        },
  ...
  */      
  ...
  methods: {
     ...
     * @lastEditorTime 2021年2月24日13:38:13
     * @description 新增分组
     */
    closeDialog(a) {
      ...
      if(a.data.groupId <= 0) {
          ...
          if (window.g.IsYangZhouProject && a.data.groupId != -13 || !window.g.IsYangZhouProject) {
            this.$message({
    ...
    /** 
     * @LastEditTime: 2020-09-26 19:05
     * @LastEditors: hexinting
     * @param {type} 没有 
     * @return {type} undefined
     * @Description: 把路由存的发送类型传给changeNav
    */    
    getFaxToList(){
        const sendTypeName = this.$route.query.sendTypeName
        if (sendTypeName) {
          this.changeNav(sendTypeName)
        }
    },
    /**
     * @lastEditor hexinting
