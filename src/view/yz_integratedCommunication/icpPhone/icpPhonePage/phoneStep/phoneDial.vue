<template>
  <div class="phoneDial-right">
    <div :class="[phoneList.length>0 ?'planTop':phoneState==0 ? 'planTop' : 'planTop2']">
      <div v-if="phoneList.length==0" style="position: absolute;left: 50%;width: 150px;margin-left: -67px;text-align: center;top: 320px;
           font-weight: 500;font-size: 12px;color: rgba(202,213,232,1);position: absolute;z-index: 1;top: calc(50% + 50px);">暂无通话队列
      </div>
      <el-scrollbar style="height: 100%">
        <div class="my-fax-list-box">
          <div class="threePhone-list" v-if="phoneState==0" v-for="item in threePhoneList">
            <div>
              <div style="background:rgba(247,248,250,1);height:30px;padding:5px 10px">
                <span style="color:rgba(0,145,255,1);font-size: 12px">三方通话中</span>
                <span style="font-size: 12px">{{item.time}}</span>
              </div>
              <div style="font-size: 12px" v-for="(items, indexs) in item.content" :key="indexs">
                <div style="display: flex;padding:10px 5px;">
                  ...
                  <div style="flex: 5">
                    <div>
                      <span style="width: 60px;display: inline-block">{{items.name}}</span>
                      <span style="padding-left: 15px">{{items.position}}</span>
                    </div>
                    <div style="padding-top: 6px">{{items.workUnit}}</div>
                  </div>
                  <div style="flex: 3">
                    <div
                      :class='[items.state==1 || items.state==2  ? "blueSize" :  items.state== 0 ? "yellowSize" :"redSize"]'>
                      {{items.state==1 ? "通话中" : items.state==2 ? '呼叫中':items.state==3 ? '呼叫失败': items.state==
                      0?'本人':'已挂断'}}
                    </div>
                    <div style="padding-top: 6px">{{items.number}}</div>
                  </div>
                  <div style="flex: 1;display:flex;justify-content: center;align-items: center">
                    <i class="el-icon-close" v-if="items.state!=0" @click="closeThreeList(items, indexs)"></i>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <!-- bug 7309 author:hexinting date:2020-10-26 isCallState为false则表示不在通话状态，通话队列要移除该号码 -->
          <div class="my-fax-list1"
            ...
            <div class="my-fax-list1-left-box" @click="phoneListClick(item, index)">
              <div class="my-fax-list1-right">
                <div
                  style="width: 69%;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;display: inline-block;">
                  <span>{{item.content.name}}</span>
                  <span style="padding-left: 20px">{{item.content.workUnit}}</span>
                </div>
                <div style="display: inline;float: right;">
                       <span style="font-size: 12px;" :class="[item.myState ==4 ? 'redSize': 'blueSize']">
                      {{item.myState==0? '待呼出':item.myState==1?'通话中':item.myState==2?'保持中':item.myState==3?'呼叫中':item.myState==5?'转接中':'呼叫失败'}}
                       </span>
                  <span style="font-size: 12px;padding-left: 10px">{{item.time}}</span>
                </div>
              </div>
              <div class="my-fax-list1-right" style="padding-top: 12px;">
                <span style="color: #909399;font-size: 12px">{{item.content.position}}</span>
                <span style="float: right;font-size: 12px">{{item.content.number}}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <el-form ref="ruleForm" class="auto-call" label-width="80px" :model="autoCallForm" :rules="autoCallRules">
      ...
      <el-tooltip :content="isAutoCall" placement="top" style="margin-right:13px;">
    ...
    <div style="margin-top: 10px;">
      <div class="cardPlan">
        <div class="flexBox">
          <el-card v-for=" ( item , index ) in box"
                   :style="item.start ?'color:black;border-radius: 0;':'color:#909399;border-radius: 0;'"
                   :shadow="showFun" :class='[item.current==0 && item.start ? "cardShaShow" : "cardShaHide"]'
                   :key="index">
            <div @mouseover="changeMask(item, index)" class="dialBox"
                 @mouseleave="changeGoBack(item, index)" @click="chaneClick(item, index)"
            >
              <div style="padding-top: 20px">
                <div style="height: 61px;width: 61px"
                     :style="{ 'backgroundImage' : 'url(' + item.url + ')' , 'background-size':'61px 61px','background-position':'center center'}"></div>
        ...
        <transition name="el-zoom-in-bottom">
          <div v-show="showDialPlate" class="transition-box">
            <dial-plate @closeModel="closeModel" @dialing="dialing"></dial-plate>
          </div>
        </transition>

        <transition name="el-zoom-in-bottom">
          <div v-if="showAdaptor" class="transition-box">
            <through-connection-plane :threeCall="threeCall" @affirmModel="affirmModel" ref="throughConnection"
                                      @closeConnectionModel="closeConnectionModel"></through-connection-plane>
          </div>
        </transition>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    name: "phoneDial",
    components: {
      dialPlate, throughConnectionPlane
    },
    data() {
    ...
    watch: {
      ...
      nowSelectPhoneList: {
        handler(neW, olD) {
          console.log('nowSelectPhoneList', neW, olD)
          if (neW !== olD) {
            console.log(neW)
            if (neW.length <= 0) {
              this.changeRed = -1;
      ...
      autoCallSettings: {
        handler: function(newVal, oldVal) {
          ...
          if (newVal.phone !== oldVal.phone || newVal.agentId !== oldVal.agentId) {
            if (newVal.phone !== '') {
    ...
    mounted() {
      console.log("---------------",this.autoCallSettings.isAutoCall)
      // 当自动转接按钮关闭后，查询已配置的呼叫转发号码
    ...
    methods: {
      ...
      phoneListClick(item, index) {
        ...
        if (this.isCallPhone == 'hangUp') {  //未通话时，每次点击电话列表都把点击的数据放入全局, 未通话时可修改
          this.$store.commit("setNowSelectPhoneList", '');//先清空
          this.$store.commit("setNowSelectPhoneList", atPresentPhone);//添加到现在选中的通话
          this.changeRed = index;
        }
        if ((this.isCallPhone == 'call' || this.isCallPhone == 'talking') && this.isPhoneCallStatus) {  //通话中但是是转接或者三方保持都可修改
        ...
        if ([0, 3, 4].includes(item.myState) && [false].includes(item.isCallState)) { // 0 >=待呼出 1>= 通话中  2>= 保持中 3>= 呼叫中 4>= 呼叫失败
          console.log(item.myState);
          this.toState = false
          this.changeRed = index;
          let phoneList = [...this.phoneList];
          phoneList.forEach(items => {  //获取当前选中的列表是不是在保持中
            if (items.myState == 2) {
              myState = true
            }
          })
          if (!myState) { //把phoneList第二条变为拨号盘 ,且保持中是不能修改当前选中保存的这个区状态管理
            this.box[1].current = 0;
      ...
      changeMask(item, index) {
        if (item.start) {
          ...
          switch (caseData) {
            ...
            case 5:
              this.box[caseData].current = 0;
              this.box[caseData].url = '/static/img/phoneCallImg/keepCallingBlue.png'
              break;
            default:
              break;
          }
            ...
            switch (caseData) {
              ...
              case 1:
                if (this.box[caseData].text == '拨号盘') {
                  ...
                  this.box[caseData].url = '/static/img/phoneCallImg/callGray.png'
                }
                if (this.box[caseData].text == '挂断') {
                  this.box[caseData].current = 1;
                  this.box[caseData].url = '/static/img/phoneCallImg/hangup.png'
              ...
              case 2:
                this.box[caseData].current = 1
                this.box[caseData].url = '/static/img/phoneCallImg/recoverGray.png'
                break;
              case 3:
                this.box[caseData].current = 1
                this.box[caseData].url = '/static/img/phoneCallImg/switchGray.png'
                break;
              case 4:
                this.box[caseData].current = 1
                this.box[caseData].url = '/static/img/phoneCallImg/phoneBlue.png'
              ...
              case 5:
                this.box[caseData].current = 1
                this.box[caseData].url = '/static/img/phoneCallImg/keepCallingGray.png'
                break;
              default:
                break;
            }
      ...
      //鼠标点击
      chaneClick(item, index) { //0=>三方 1=>拨号 2=>恢复 3=>转接 4=>拨号盘 5=>保持

        if (item.start) {
          if ([0].includes(index)) {//三方通话
      ...
      async phoneTransform(number) {
        let data = {number: number, agentId: sessionStorage.getItem("agent_id") || ''};
        let phoneCall = ''
      ...
      affirmModel(state, speakToPeople) {
        ...
        this.phoneTransform(speakToPeople.mobile).then(result => {
          let self = this;
          if (!state) { //普通转接
            this.$icp19.TransferCallOneKey(Number(self.agent_id), speakToPeople.mobile);
          } else { //三方拨号
            this.$icp19.ThreeWayConf(Number(self.agent_id), Number(self.conn), speakToPeople.mobile, 1);
          }
        });
          ...
          this.$api.findCallForwarding(data).then(res => {
            if (res.errorcode === 0) {
              ...
              if (res.data) {
                ...
                this.isAutoCall = res.data.enabled === 1 ? '关' : '开'
                let autoCallSettings = {
                  // phone: this.isAutoCall ===  '关' ? '' : res.data.phone,
                  phone: res.data.phone,
                  // isAutoCall: res.data.enabled === 1 ? '开' : '关'
                  isAutoCall: res.data.enabled === 1 ? '关' : '开'
                }
                this.$store.commit('setAutoCallSettings', autoCallSettings)
                console.log('autoCallSettings', autoCallSettings, this.$store.state.autoCallSettings)
              }
            }
          })
        }
       ...
       * @lastAuthor hexinting
       * @lastDate 2020-10-15
       * @description 监听到agent更改，则调用ucs接口自动转接设置
       */
      AgentChangeStatus() {
        let agent_id = sessionStorage.getItem("agent_id")
        // let status = this.isAutoCall === '开' ? 1 : 0
        let status = this.isAutoCall === '开' ? 0 : 1
        let transferNum = this.autoCallForm.phone
        this.$icp19.AgentChangeStatus(agent_id, status, transferNum).then(res => {
      ...
      setForwardingPhone() {
        ...
        //   }
          ...
          let data = {
            ...
            agentId: agentId,
            enabled: this.isAutoCall === '开' ? '0' : '1',
            enabledSms: 0 // 是否在转发时候发送短信通知 1：通知 0：不通知
          }
      ...
      querySearchAsync(queryString, cb) {
        ...
        var results = queryString ? autoCallPhoneList.filter(this.createStateFilter(queryString)) : autoCallPhoneList;
        cb(results);
