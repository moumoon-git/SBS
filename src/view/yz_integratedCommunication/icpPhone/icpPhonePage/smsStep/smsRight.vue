  ...
  <div class="my-phone-right-main">
    <div class="my-phone-right-main-user">
      <div class="my-sms-box">
        <div
          ...
          class="my-fax-list-box"
        >
          <template v-if="IsZengChengProject">
            <div
              v-for="(item, index) in peopleList"
              :key="index"
              class="my-fax-list1"
            >
              <span v-if="item.workUnit == '' && item.position != ''">
                {{ '姓名: ' +
                  item.name +
                  ' 职务: ' +
                  item.position +
              ...
              <span v-else-if="item.position == '' && item.workUnit != ''">
                {{ '姓名: ' +
                  item.name +
                  ' 工作单位: ' +
                  item.workUnit +
      ...
      <div class="my-fax-head">
        ...
        <div class="my-fax-head-right">
          ...
          <span class="my-fax-head-right-line" />
    ...
    <div class="sendMsg">
      ...
      >
        <!--
            * @editorName: qinjiaqi
            * @Description: 给文本框加双击功能和切换功能
          -->

        <div v-if="IsZengChengProject">
          <el-tabs
            v-model="activeTab"
            type="card"
          ...
          >
            <el-tab-pane
              ...
              name="first"
            >
              <span @dblclick="OpenMsgDialog">
                <el-input
                  v-model="copyBusinessCard1"
                  type="textarea"
                  placeholder="请输入短信内容"
                  resize="none"
                  show-word-limit
        ...
        >
          <el-form-item label="定时短信:">
            <el-date-picker
              ...
              type="datetime"
              :format="
                IsYangZhouProject ? 'yyyy-MM-dd HH:mm' : 'yyyy-MM-dd HH:mm:ss'
              "
            ...
            <el-switch v-model="msgForm1.isTimingSMS" />
          </el-form-item>
        </el-row>
        <el-row
          :gutter="gutter"
...
export default {
  ...
  computed: {
    ...
    copyBusinessCard: {
      get() {
        // qinjiaqi 进入页面时在此处进行短信内容12的赋值。写在create和mount无效。不加null判断会打印出null
        this.copyBusinessCard1 = localStorage.getItem('copyBusinessCard1') == null ? '' : localStorage.getItem('copyBusinessCard1');
        this.copyBusinessCard2 = localStorage.getItem('copyBusinessCard2') == null ? '' : localStorage.getItem('copyBusinessCard2');
        return this.$store.state.copyBusinessCard;
      },
      set() {
        this.$store.commit('setCopyBusinessCard', this.clickMsg === 1 ? this.copyBusinessCard1 : this.copyBusinessCard2);
      },
    },
    selectEventStyle() {
  ...
  watch: {
    ...
    'msgForm1.isUrgeSMS'(newV) {
      if (newV) {
        // 当打开催报短信按钮时，自动选择催报短信类型
        this.msgForm1.msgTypeId = 1;
    ...
    /**
     * @authorName：qinjiaqi
     * @editorDate 2020-12-21
     * @description：监听双击文本框改变值
     */
    copyBusinessCard(newV) {
      if (this.clickMsg == 1) {
        this.copyBusinessCard1 = newV;
        localStorage.setItem('copyBusinessCard1', newV);
      } else {
        this.copyBusinessCard2 = newV;
        localStorage.setItem('copyBusinessCard2', newV);
      }
      this.$store.commit('setCopyBusinessCard', newV);
      console.log('当前短信的内容：', this.copyBusinessCard);
    },
    copyBusinessCard1(newV) {
      this.$store.commit('setCopyBusinessCard', newV);
      console.log('当前短信1的内容：', this.copyBusinessCard1);
    },
    copyBusinessCard2(newV) {
      this.$store.commit('setCopyBusinessCard', newV);
      console.log('当前短信2的内容：', this.copyBusinessCard2);
    },
  },
  created() {
  ...
  methods: {
    ...
    handleChooseTemplate(templateContent) {
      if (templateContent.length > 0) {
        if (this.clickMsg === 1) {
          this.copyBusinessCard1 = templateContent;
        } else {
          this.copyBusinessCard2 = templateContent;
        }
      }
    },
    setCaseId() {
      let data = {
        id: sessionStorage.getItem('newID'),
    ...
    OpenMsgDialog() {
      this.$nextTick(() => {
        ...
        if (this.clickMsg == 1)
          this.$refs.MsgDialog.copyBusinessCard = this.copyBusinessCard1;
    ...
    selectTemplate(item) {
      ...
      this.$store.commit('setCopyBusinessCard', item.text);
      if (this.clickMsg == 1) {
        this.copyBusinessCard1 = item.text;
        console.log('短信模板给内容1：', this.copyBusinessCard1);
      } else {
        this.copyBusinessCard2 = item.text;
        console.log('短信模板给内容2：', this.copyBusinessCard2);
      }
    ...
    getSmsType() {
      ...
      this.$api.smsType(data).then((res) => {
        ...
        this.msgForm1.msgTypeId = res.data[0].value;
      });
    },
    /**
     * @lastEditor hexinting
    ...
    sendMsg() {
      ...
      if (this.copyBusinessCard.length > this.maxlength) {
      ...
      if (this.peopleList.length == 0) {
        ...
        if (this.IsYangZhouProject) {
          ...
          this.promptConfig.visible = true; // 显示弹窗提醒
        } else {
          this.$confirm('确认发送短信?', {
     ...
     */
    addSmsQueue() {
      console.log('发送短信：', this.peopleList);
      let phones = [];
      let contactorIds = [];
      for (let i = 0; i < this.peopleList.length; i++) {
        phones.push(
          Number(
            this.peopleList[i].number ||
              ...
              this.peopleList[i].orderNum ||
              this.peopleList[i].otherTel ||
              this.peopleList[i].phone ||
              '',
          ),
        );
        contactorIds.push(this.peopleList[i].id);
    ...
    /**
     * @description 根据手机号码查询联系人信息
     * @param {string | number} 电话
     */
    getContactorByMobile(phone) {
      return new Promise((resolve, reject) => {
        const data = {
          phoneNum: phone,
        };
        this.$api
          .findContactorByMobile(data)
          .then((res) => {
            if (res.errorcode === 0) {
              resolve(res.mailContactor);
            } else {
              this.$message({
                message: `查询联系人信息失败，错误码：${res.errorcode}，错误信息：${res.msg}`,
              });
              resolve();
            }
          })
          .catch((error) => {
            this.$message({
              message: `查询联系人信息失败，错误码：${error.errorcode}，错误信息：${error.msg}`,
            });
            reject(error);
          });
      });
    ...
    /**
     * @description 通过号码查找联系人数据
     * @param {string | number} 电话
     * @returns 返回联系人的信息
     */
    setPhoneList(phone) {
      return new Promise((resolve, reject) => {
        this.getContactorByMobile(phone).then((res) => {
          if (res) {
            resolve(res);
          } else {
            const contactor = {
    ...
    getfaxDataN(phoneList) {
      // 去重
      const newPhoneList = [...new Set(phoneList)];
      let peopleList = this.peopleList;
      if (peopleList.length > 0) {
        newPhoneList.forEach((el) => {
          // 判断是否已经存在待发送号码队列里
          const filterArr = peopleList.filter((item) => {
            const mobile =
              item.mobile ||
              item.mobile1 ||
              item.mobile2 ||
          ...
          if (filterArr.length === 0) {
            this.setPhoneList(el).then((res) => {
              this.peopleList.push(res);
            });
          }
        });
      } else {
        ...
        this.$api.contactorList(data).then((res) => {
          if (res.errorcode == 0) {
            if (res.data.list.length > 0) {
              ...
              this.saveMessageContactorToGroup();
              // 注释原因：3.0用新接口来存短信分组了
    ...
    /**
     * @description 3.0 存为短信分组
     */
    saveMessageContactorToGroup() {
      const phoneNumber = this.peopleList.map(
        (item) =>
          item.mobile ||
          item.mobile1 ||
          item.mobile2 ||
      ...
      const userIds = this.peopleList.map(
        (item) => item.contactorId || item.id,
      );
      this.$http({
        url:
          window.SITE_CONFIG['baseUrl'] +
          '/message/messageGroup/saveMessageContactorToGroup',
        method: 'post',
        data: {
          id: this.keys, // 分组id
          userIds: userIds.join(','), // 人员id
          phone: phoneNumber.join(','), // 手机号码
        },
      }).then(({ data }) => {
        if (data.errorcode !== 0) {
          this.groupDialogVisible = true;
    ...
    getMsgOperatorSign() {
      ...
      //   this.msgDialogVisible = true;
      //   if (this.clickMsg == 1) {
      //     this.copyBusinessCard = this.copyBusinessCard1
      //   } else {
      //     this.copyBusinessCard = this.copyBusinessCard2
      //   }
...
.my-phone-right-main-user .my-sms-box {
  height: 300px;
  overflow: auto;
