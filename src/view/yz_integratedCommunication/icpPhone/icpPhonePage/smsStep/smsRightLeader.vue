      ...
      <div class="my-sms-box">
        <div class="my-fax-list-box" v-if="peopleList.length > 0">
          <div
            ...
            :key="index"
          >
              <span v-if="item.workUnit == '' && item.position != ''">{{"姓名: " + item.name + " 职务: " + item.position + " 手机号码: " + (item.number || item.mobile1)}}</span>
              <span v-else-if="item.position == '' && item.workUnit != ''">{{"姓名: " + item.name + " 工作单位: " + item.workUnit + " 手机号码: " + (item.number || item.mobile1)}}</span>
              <span v-else-if="item.position == '' && item.workUnit == ''">{{"姓名: " + item.name + " 手机号码: " + (item.number || item.mobile1)}}</span>
              <span v-else>{{"姓名: " + (item.name?item.name:'--') + " 工作单位: " + (item.workUnit?item.workUnit:'--') + " 职务: " + (item.position?item.position:'--') + " 手机号码: " + (item.number || item.mobile1)}}</span>
            <i
          ...
          <el-form-item label="短信类型:">
            <el-col :span="24">
              <el-radio-group
                ...
                :key="key"
              >
                <el-radio :label="item.id" style="margin-right: 10px">{{
                  item.name
                }}</el-radio>
              </el-radio-group>
            </el-col>
...
import { failSend } from '../../../../../assets/interface/interface';
export default {
  components: {
    sendPair,
    addContact,
  ...
  methods: {
    ...
    addPromptSmsQueue() {
      ...
      for (let i = 0; i < this.peopleList.length; i++) {
        if (this.peopleList[i].mobile1 || this.peopleList[i].mobile) {
          phones.push(
            Number(this.peopleList[i].mobile1 || this.peopleList[i].mobile)
          );
    ...
    addSmsQueue() {
      ...
      let contactorIds = [];
      for (let i = 0; i < this.peopleList.length; i++) {
        if (this.peopleList[i].mobile1 || this.peopleList[i].mobile || this.peopleList[i].call) {
          phones.push(
            Number(
        ...
        if (this.peopleList.filter((v) => v.mobile == o).length == 0) {
          ...
          this.$api.findContactorByMobile(data).then((res) => {
            ...
            if (!res.mailContactor) {
              res.mailContactor = {}
              res.mailContactor.mobile1 = o
            }
            res.mailContactor.number = o;
            peopleList.push(res.mailContactor);
            console.log('人员列表：', peopleList)
          });
    ...
    getMsgOperatorSign() {
      ...
      // getMsgOperatorSign这个接口竞翊说没了，这个功能可以考虑去掉了
