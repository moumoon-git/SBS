<template>
  <div class="my-radio-box">
    <!-- <div class="my-radio-box-main">
      <el-radio-group v-model="tabPosition1" @change="changePage1">
        <el-radio-button v-for="(item,index) in list1" :label="index+1" :key="index">{{item.name}}</el-radio-button>
      </el-radio-group>
    </div>
    <template v-if="tabPosition1==2">
      <div class="my-phone-main">
        <div class="my-table-box">
          <el-row>
            <pending-information></pending-information>
          </el-row>
        </div>
      </div>
    </template> -->
    <!-- <template v-if="tabPosition1==1"> -->
        <div class="my-radio-box-main" >
          <el-radio-group v-model="tabPosition" @change="changePage">
            <el-radio-button v-for="(item,index) in list.filter(element=>{return element!=null})" :label="Number(item.label)" :key="index">{{item.name}}
            </el-radio-button>
          </el-radio-group>
          <div style="position: absolute;right: 3%;top: 50%;transform: translateY(-50%);display: flex;align-items: center;cursor: pointer;" @click="manageVisible=!manageVisible">
            <img :src="require('@/assets/icpImg/communicationManage.png')" style="width:1.3vw;" v-if="manageVisible">
            <span style="font-size: 14px;color: #999999;line-height: 14px;margin-left: 4px;" v-if="manageVisible">管理</span>
            <i class="el-icon-circle-check" style="font-size: 20px;color:rgba(0,145,255,1);" v-if="!manageVisible"></i>
            <span style="font-size: 14px;color: #999999;line-height: 14px;margin-left: 4px;" v-if="!manageVisible">完成</span>
          </div>
        </div>
        <div class="my-phone-main" v-show="tabPosition==1||tabPosition==2||tabPosition==3">
          <div class="my-tree-box" style="box-sizing: border-box;overflow: auto;">
            <div style="width: 100%;height:3.5vh;display: flex;border-bottom: 1px solid rgba(233,235,240,1);">
              <!-- <div :style="{width: '100%',display: 'flex','align-items': 'center','margin-left':'11%',cursor: 'pointer',color:tabPosition==2?'rgba(0,145,255,1)':'rgba(153,153,153,1)'}" @click="tabPosition=2;getDataAl();">
                <i class="el-icon-star-off" style="width:0.91vw;"></i>
                <span style="font-size:12px;font-family:PingFangSC-Regular,PingFang SC;font-weight:400;margin-left:3px;">{{contactList[0].name || '常用联系人'}}</span>
              </div> -->
              <div :style="{width: '100%',display: 'flex','align-items': 'center','margin-left':'11%',cursor: 'pointer',color:tabPosition==3?'rgba(0,145,255,1)':'rgba(153,153,153,1)'}" @click="tabPosition=3;getRecentContactsTable();">
                <i class="el-icon-time" style="width:0.91vw;"></i>
                <span style="font-size:12px;font-family:PingFangSC-Regular,PingFang SC;font-weight:400;margin-left:3px;">{{contactList[1].name || '最近联系人'}}</span>
              </div>
            </div>
            <el-scrollbar class="my-tree-scroll">
              <el-input
                placeholder="输入关键字进行搜索"
                v-model="formData1.filterTree" class="treeSearch" style="padding: 10px;box-sizing: border-box;">
                <i slot="suffix" class="el-input__icon el-icon-search" @click="$refs.tree1.filter(formData1.filterTree)"></i>
              </el-input>
              <el-tree :data="formData1.treeData" v-loading="formData1.myLoading"
                @node-click="handleNodeClick" node-key="id" ref="tree1"  :filter-node-method="filterNode"
                highlight-current :props="defaultProps" draggable
                @node-drop="handleDrop"
                :allow-drop="allowDrop"
                :allow-drag="allowDrag"
                :show-checkbox="!manageVisible?true:false" @check-change="handleCheckChange" @check="handleTreeCheck">
                <span class="custom-tree-node" slot-scope="{ node, data }" :style="{display: 'flex','align-items': 'center',position: 'relative',width: '100%'}" @mouseover="treeBtnHoverIndex=data.id" @mouseout="treeBtnHoverIndex=-2">
                  <span>{{ node.label }}</span>
                  <span style="display: flex;position: absolute;top: 50%;left: 66%;transform: translateY(-50%);" v-show="treeBtnHoverIndex==data.id&&!manageVisible && data.id !=-1">
                    <div style="display: flex;align-items: center;cursor: pointer;" >
                      <el-tooltip class="item" effect="dark" content="编辑" placement="top">
                        <img :src="require('@/assets/icpImg/editTreeBtn.png')" style="width:1.3vw;" @click="editTreeItem(node, data)">
                      </el-tooltip>
                    </div>
                    <div style="display: flex;align-items: center;cursor: pointer;margin-left: 5px;" >
                      <el-tooltip class="item" effect="dark" content="删除" placement="top">
                        <img :src="require('@/assets/icpImg/deleteTreeBtn.png')" style="width:1.3vw;" @click="deleteTreeItem(node, data)">
                      </el-tooltip>
                    </div>
                  </span>
                </span>
              </el-tree>
            </el-scrollbar>
            <treeBtn v-if="!manageVisible" @changeDialog="closeDialog" @refreshData="refreshData" :treeDataItem="formData1.treeDataItem" ref="treeBtn1" :treeDataItems="formData1.treeMultipleSelection" tabPosition="zhengwulianluowang"
            :treeData="formData1.treeData" :formData="formData"  ></treeBtn>
            <!-- <el-alert title="温馨提示" type="info" description="可拖动修改同级分组排序" show-icon>
            </el-alert> -->
          </div>
          <div class="my-table-box" v-if="tabPosition==1" style="box-sizing: border-box;overflow: auto;">
            <btn-box v-if="!manageVisible" :btnArry="btnArry" v-bind="{
              add:{callback:addList},
              edit:{callback:handleEdit},
              del:{callback:handleDelete},
              sort:{callback:handleSort},
              download:{callback:handleExportContactor},
              document:{callback:handleExportm}
              }"
             :batchJoinIsShow="true"
             @handleBatchJoin="handleBatchJoin"
             @success="batchJoinSuccess"
             >
            </btn-box>
            <el-row class="my-table-box-search" style="margin:10px;">
              <el-col :span="14">
                <el-input
                  placeholder="请输入姓名/单位/职位/号码"
                  class="searchInput"
                  v-model="formData1.searchText"
                  @input="handleSearch"
                ></el-input>
              </el-col>
              <el-col :span="10">
                <el-button type="primary" @click="handleSearch" style="margin-left:10px;">搜索</el-button>
              </el-col>
            </el-row>
            <el-table
              ref="elTable1"
              class="my-table-main"
              @mousedown.native="handleMouseDown($event)"
              @mouseup.native="handleMouseUp($event)"
              :data="formData1.tableData"
              highlight-current-row
              :header-cell-style="{background:'#E6E6E6',color:'#606266'}"
              height="670"
              style="width: 100%; height: 670px; overflow: auto" 
              @row-dblclick="handleRowDbClick"
              @row-click="handleRowClick"
              @selection-change="handleSelectionChange"
              @select="handleSelect"
              @select-all="selectAll">
              <el-table-column type="selection"></el-table-column>
              <!-- <el-table-column type="index" width="50" :resizable="false" align="center"></el-table-column> -->
              <el-table-column align="center" prop="name" show-overflow-tooltip :resizable="false" label="姓名"></el-table-column>
              <el-table-column align="center" prop="workUnit" show-overflow-tooltip :resizable="false" label="单位"></el-table-column>
              <el-table-column align="center" prop="position" show-overflow-tooltip :resizable="false" label="职位"></el-table-column>
              <el-table-column align="center" prop="number" show-overflow-tooltip :resizable="false" label="联系号码">
                <!--<template slot-scope="scope" >-->
                  <!--<div  v-if="scope.row.number">-->
                    <!--<div v-if="scope.row.otherTel!=''||scope.row.officeTel!=''||scope.row.homeTel!=''||scope.row.otherTel2!=''" >-->
                      <!--<el-dropdown @command="handleCommand(scope.row)" placement="bottom" trigger="click"-->
                          <!--@visible-change="changeDropdown">-->
                        <!--<span class="el-dropdown-link">-->
                          <!--{{scope.row.number}}-->
                          <!--<i class="el-icon-arrow-down el-icon&#45;&#45;right"></i>-->
                        <!--</span>-->
                        <!--<el-dropdown-menu slot="dropdown">-->
                          <!--<el-dropdown-item command="a" v-if="scope.row.otherTel">-->
                            <!--<div @mouseover='elDropdownItem(1)' class="dropdownDiv">-->
                              <!--<span>手机2:  {{scope.row.otherTel}}</span>-->
                              <!--<span v-if="showSpanImg==1" style="display: inline-block;width:13px;padding-left: 30px">-->
                                <!--<img style="width: 100%;height: 100%" src="/static/img/icon/dailCall.png" />-->
                              <!--</span>-->
                            <!--</div>-->
                          <!--</el-dropdown-item>-->
                          <!--<el-dropdown-item command="b" v-if="scope.row.officeTel">-->
                            <!--<div @mouseover='elDropdownItem(2)' class="dropdownDiv" >-->
                              <!--<span>办公电话:  {{scope.row.officeTel}}</span>-->
                              <!--<span v-if="showSpanImg==2"  style="display: inline-block;width:13px;padding-left: 30px">-->
                            <!--<img style="width: 100%;height: 100%" src="/static/img/icon/dailCall.png" />-->
                          <!--</span>-->
                            <!--</div>-->
                          <!--</el-dropdown-item>-->
                          <!--<el-dropdown-item command="c" v-if="scope.row.homeTel">-->
                            <!--<div @mouseover='elDropdownItem(3)' class="dropdownDiv">-->
                              <!--<span> 应急电话:  {{scope.row.homeTel}}</span>-->
                              <!--<span v-if="showSpanImg==3" style="display: inline-block;width:13px;padding-left: 30px">-->
                              <!--<img style="width: 100%;height: 100%" src="/static/img/icon/dailCall.png" />-->
                            <!--</span>-->
                            <!--</div>-->
                          <!--</el-dropdown-item>-->
                          <!--<el-dropdown-item command="d" v-if="scope.row.otherTel2">-->
                            <!--<div @mouseover='elDropdownItem(4)' class="dropdownDiv">-->
                              <!--<span > 其他电话:  {{scope.row.otherTel2}}</span>-->
                              <!--<span v-if="showSpanImg==4" style="display: inline-block;width:13px;padding-left: 30px">-->
                              <!--<img style="width: 100%;height: 100%" src="/static/img/icon/dailCall.png" />-->
                            <!--</span>-->
                            <!--</div>-->
                          <!--</el-dropdown-item>-->
                        <!--</el-dropdown-menu>-->
                      <!--</el-dropdown>-->

                    <!--</div>-->
                    <!--<span v-else>{{scope.row.number}}</span>-->
                  <!--</div>-->
                  <!--<span v-else>{{scope.row.number}}</span>-->
                <!--</template>-->
              </el-table-column>
              <el-table-column label="操作" width="250px" align="center">
                <template slot-scope="scope">
                  <el-tooltip v-if="manageVisible" class="item" effect="dark" content="查看" placement="top">
                    <i class="el-icon-info" @click="watchContactDetails(scope.row)"  style="font-size: 24px;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <el-tooltip v-if="!manageVisible" class="item" effect="dark" content="编辑" placement="top">
                    <i class="el-icon-edit" @click="['formData' + tabPosition].rowData=scope.row;window.setTimeout(()=>{handleEdit()},500)"  style="font-size: 24px;margin-left: 1vw;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <el-tooltip v-if="!manageVisible" class="item" effect="dark" content="删除" placement="top">
                    <i class="el-icon-delete-solid" @click="['formData' + tabPosition].rowData=scope.row;window.setTimeout(()=>{handleDeleteItem()},500)" style="font-size: 24px;margin-left: 1vw;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <!-- <el-tooltip class="item" effect="dark" content="设为常用联系人" placement="top">
                    <i class="el-icon-star-on" @click="handSetToCommon(scope.row)" :style="{'font-size': '24px','margin-left': '1vw',cursor: 'pointer',color:scope.row.commonlyUsed==0?'#9FC8FE':'#FBC958'}"></i>
                  </el-tooltip> -->
                  <!-- <el-tooltip class="item" effect="dark" content="添加" placement="top">
                    <i class="el-icon-phone-outline" @click="addCallSms(scope.row)" style="font-size: 24px;margin-left: 1vw;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip> -->
                  <!-- <el-button type="primary" size="mini" @click="addCallSms(scope.row)">添加</el-button>
                  <el-button type="primary" size="mini" @click="handSetToCommon(scope.row)">设为常用</el-button> -->
                </template>
              </el-table-column>
            </el-table>
            <el-row>
              <el-pagination class="pagination" background :page-size="formData1.pageSize" :page-sizes="[10,20,30,40]"
                :total="formData1.total"
                :current-page="formData1.currentPage"
                @current-change="handCurrentChange"
                @size-change="handSizeChange"
                layout="total, prev, pager, next, sizes, jumper"></el-pagination>
            </el-row>
          </div>
          <div class="my-table-box" v-if="tabPosition==2" style="box-sizing: border-box;overflow: auto;">
            <btn-box v-if="!manageVisible"
              :btnArry="[{
                event: 'del',
                icon: 'el-icon-delete',
                title: '批量移除'
              }]"
              v-bind="{
                  del:{callback:handleDelete}
              }"
            ></btn-box>
            <el-row class="my-table-box-search" style="margin:10px;">
              <el-col :span="14">
                <el-input
                  placeholder="请输入姓名/单位/职位/号码"
                  class="searchInput"
                  v-model="formData2.searchText"
                  @input="handleSearch"
                ></el-input>
              </el-col>
              <el-col :span="10">
                <el-button type="primary" @click="handleSearch" style="margin-left:10px;">搜索</el-button>
              </el-col>
            </el-row>
            <el-table
              ref="elTable2"
              class="my-table-main"
              @mousedown.native="handleMouseDown($event)"
              @mouseup.native="handleMouseUp($event)"
              :data="formData2.tableData"
              highlight-current-row
              :header-cell-style="{background:'#E6E6E6',color:'#606266'}"
              height="670"
              style="width: 100%; height: 670px; overflow: auto" 
              @row-dblclick="handleRowDbClick"
              @row-click="handleRowClick"
              @selection-change="handleSelectionChange"
              @select="handleSelect"
              @select-all="selectAll"
            >
              <el-table-column type="selection"></el-table-column>
              <!-- <el-table-column type="index" width="50" show-overflow-tooltip  :resizable="false" align="center"></el-table-column> -->
              <el-table-column align="center" prop="name" show-overflow-tooltip :resizable="false" label="姓名"></el-table-column>
              <el-table-column align="center" prop="workUnit" show-overflow-tooltip :resizable="false" label="单位"></el-table-column>
              <el-table-column align="center" prop="position" show-overflow-tooltip :resizable="false" label="职位"></el-table-column>
              <el-table-column align="center" prop="number" show-overflow-tooltip :resizable="false" label="联系号码">
                <!--<template slot-scope="scope" >-->
                  <!--<div  v-if="scope.row.number">-->
                    <!--<div v-if="scope.row.otherTel!=''||scope.row.officeTel!=''||scope.row.homeTel!=''||scope.row.otherTel2!=''" >-->
                      <!--<el-dropdown @command="handleCommand(scope.row)" placement="bottom" trigger="click"-->
                                  <!--@visible-change="changeDropdown">-->
                    <!--<span class="el-dropdown-link">-->
                      <!--{{scope.row.number}}-->
                      <!--<i class="el-icon-arrow-down el-icon&#45;&#45;right"></i>-->
                    <!--</span>-->
                        <!--<el-dropdown-menu slot="dropdown">-->
                          <!--<el-dropdown-item command="a" v-if="scope.row.otherTel">-->
                            <!--<div @mouseover='elDropdownItem(1)' class="dropdownDiv">-->
                              <!--<span>手机2:  {{scope.row.otherTel}}</span>-->
                              <!--<span v-if="showSpanImg==1" style="display: inline-block;width:13px;padding-left: 30px">-->
                            <!--<img style="width: 100%;height: 100%" src="/static/img/icon/dailCall.png" />-->
                          <!--</span>-->
                            <!--</div>-->
                          <!--</el-dropdown-item>-->
                          <!--<el-dropdown-item command="b" v-if="scope.row.officeTel">-->
                            <!--<div @mouseover='elDropdownItem(2)' class="dropdownDiv" >-->
                              <!--<span>办公电话:  {{scope.row.officeTel}}</span>-->
                              <!--<span v-if="showSpanImg==2"  style="display: inline-block;width:13px;padding-left: 30px">-->
                            <!--<img style="width: 100%;height: 100%" src="/static/img/icon/dailCall.png" />-->
                          <!--</span>-->
                            <!--</div>-->
                          <!--</el-dropdown-item>-->
                          <!--<el-dropdown-item command="c" v-if="scope.row.homeTel">-->
                            <!--<div @mouseover='elDropdownItem(3)' class="dropdownDiv">-->
                              <!--<span> 应急电话:  {{scope.row.homeTel}}</span>-->
                              <!--<span v-if="showSpanImg==3" style="display: inline-block;width:13px;padding-left: 30px">-->
                              <!--<img style="width: 100%;height: 100%" src="/static/img/icon/dailCall.png" />-->
                            <!--</span>-->
                            <!--</div>-->
                          <!--</el-dropdown-item>-->
                          <!--<el-dropdown-item command="d" v-if="scope.row.otherTel2">-->
                            <!--<div @mouseover='elDropdownItem(4)' class="dropdownDiv">-->
                              <!--<span > 其他电话:  {{scope.row.otherTel2}}</span>-->
                              <!--<span v-if="showSpanImg==4" style="display: inline-block;width:13px;padding-left: 30px">-->
                              <!--<img style="width: 100%;height: 100%" src="/static/img/icon/dailCall.png" />-->
                            <!--</span>-->
                            <!--</div>-->
                          <!--</el-dropdown-item>-->
                        <!--</el-dropdown-menu>-->
                      <!--</el-dropdown>-->

                    <!--</div>-->
                    <!--<span v-else>{{scope.row.number}}</span>-->
                  <!--</div>-->
                  <!--<span v-else>{{scope.row.number}}</span>-->
                <!--</template>-->
              </el-table-column>
              <!-- <el-table-column label="操作" width="250px" align="center">
                <template slot-scope="scope">
                  <el-tooltip class="item" effect="dark" content="设为常用联系人" placement="top">
                    <i class="el-icon-star-on" @click="formData2.rowData=scope.row;window.setTimeout(()=>{handleDeleteItem()},500)" :style="{'font-size': '24px',cursor: 'pointer','margin-left': '1vw',color:'#FBC958'}"></i>
                  </el-tooltip>
                  <el-button type="primary" size="mini" @click="addPhoneCall(scope.row)">添加</el-button>
                  <el-button type="primary" size="mini" @click="talkCall(scope.row)">拨号</el-button>
                </template>
              </el-table-column> -->
            </el-table>
            <el-row>
              <el-pagination
                class="pagination"
                background
                :page-size="formData2.pageSize"
                :page-sizes="[10,20,30,40]"
                :total="formData2.total"
                :current-page="formData2.currentPage"
                @current-change="handCurrentChange"
                @size-change="handSizeChange"
                layout="total, prev, pager, next, sizes, jumper"
              ></el-pagination>
            </el-row>
          </div>
          <div class="my-table-box" v-if="tabPosition==3" style="box-sizing: border-box;overflow: auto;">
            <el-table
              ref="elTable3"
              class="my-table-main"
              @mousedown.native="handleMouseDown($event)"
              @mouseup.native="handleMouseUp($event)"
              :data="formData3.tableData"
              highlight-current-row
              :header-cell-style="{background:'#E6E6E6',color:'#606266'}"
              height="670"
              style="width: 100%; height: 670px; overflow: auto" 
              @row-click="handleRowClick"
              @selection-change="handleSelectionChange"
              @select="handleSelect"
              @select-all="selectAll"
            >
              <el-table-column type="selection"></el-table-column>
              <!-- <el-table-column type="index" width="50" show-overflow-tooltip  :resizable="false" align="center"></el-table-column> -->
              <el-table-column   show-overflow-tooltip align="center"
                                v-for="(item,key) in tableColumn3" :key="key" :label="item.label" :prop="item.prop"
                                :width="item.width" :resizable="false"></el-table-column>
              <!-- <el-table-column label="操作" width="250px" align="center">
                <template slot-scope="scope">
                  <el-button type="primary" size="mini" @click="addPhoneCall(scope.row)">添加</el-button>
                  <el-button type="primary" size="mini" @click="talkCall(scope.row)">拨号</el-button>
                </template>
              </el-table-column> -->
            </el-table>
            <el-row>
              <el-pagination
                class="pagination"
                background
                :page-size="100"
                :total="formData3.total"
                :current-page="formData3.currentPage"
                layout="total, prev, pager, next, jumper"
              ></el-pagination>
            </el-row>
          </div>

        </div>
        <!--<div class="my-phone-main" v-else-if="tabPosition==2">
          <div class="my-table-box">
            <el-table ref="singleTable" :data="formData1.tableData" highlight-current-row
                      height="586px"
                      :header-cell-style="{background:'#E6E6E6',color:'#606266'}"
                      style="width:100%;" @row-dblclick="handleRowDbClick" @row-click="handleRowClick">
              <el-table-column type="index" width="50" :resizable="false" align="center"></el-table-column>
              <el-table-column show-overflow-tooltip align="center"
                               v-for="(item,key) in tableColumn" :key="key" :label="item.label" :prop="item.prop"
                               :width="item.width" :resizable="false"></el-table-column>
              <el-table-column label="操作" width="250px" align="center">
                <template slot-scope="scope">
                  <el-button type="primary" size="mini" @click="addCallSms(scope.row)">添加</el-button>
                  <el-button type="primary" size="mini" @click="handSetToCommon(scope.row)">设为常用</el-button>
                </template>
              </el-table-column>
            </el-table>
            <el-row>
              <el-pagination class="pagination" background :page-size="formData1.pageSize" :page-sizes="[10,20,30,40]"
                             :total="formData1.total"
                             :current-page="formData1.currentPage"
                             @current-change="handCurrentChange"
                             @size-change="handSizeChange"
                             layout="total, prev, pager, next, sizes, jumper"></el-pagination>
            </el-row>
          </div>
        </div>-->
        <!-- <div class="my-phone-main" v-else-if="tabPosition==2">
          <div class="my-table-box">

            <btn-box :btnArry="btnArry1" v-bind="{
                            del:{callback:handleDelete},
                            edit:{callback:handleEdit},
                         }">
            </btn-box>
            <el-row class="my-table-box-search" style="margin:0 0 10px 8px;">
              <el-col :span="14">
                <el-input
                  placeholder="请输入姓名/单位/职位/号码"
                  class="searchInput"
                  v-model="formData2.searchText"
                  @input="handleSearch"
                ></el-input>
              </el-col>
              <el-col :span="10">
                <el-button type="primary" @click="handleSearch">搜索</el-button>
              </el-col>

            </el-row>
            <el-table ref="singleTable" :data="formData2.tableData" highlight-current-row
                      :header-cell-style="{background:'#E6E6E6',color:'#606266'}"
                      style="width:100%;height:600px;overflow: auto;" @row-dblclick="handleRowDbClick" @row-click="handleRowClick">
              <el-table-column type="index" width="50" :resizable="false" align="center"></el-table-column>
              <el-table-column show-overflow-tooltip align="center"
                               v-for="(item,key) in tableColumn" :key="key" :label="item.label" :prop="item.prop"
                               :width="item.width" :resizable="false"></el-table-column>
              <el-table-column label="操作" width="250px" align="center">
                <template slot-scope="scope">
                  <el-button type="primary" size="mini" @click="addCallSms(scope.row)">添加</el-button>

                </template>
              </el-table-column>
            </el-table>
            <el-row>
              <el-pagination class="pagination" background :page-size="formData2.pageSize" :page-sizes="[10,20,30,40]"
                             :total="formData2.total"
                             :current-page="formData2.currentPage"
                             @current-change="handCurrentChange"
                             @size-change="handSizeChange"
                             layout="total, prev, pager, next, sizes, jumper"></el-pagination>
            </el-row>
          </div>
        </div> -->

        <div class="my-phone-main" v-show="tabPosition==4">
          <div class="my-tree-box" style="box-sizing: border-box;overflow: auto;">
            <el-input
              placeholder="输入关键字进行搜索"
              v-model="formData4.filterTree" class="treeSearch" style="padding: 10px;box-sizing: border-box;">
              <i slot="suffix" class="el-input__icon el-icon-search" @click="$refs.tree4.filter(formData4.filterTree)"></i>
            </el-input>
            <div style="max-height: 711px;min-height: 711px;overflow: auto;">
              <el-tree :accordion="true"
                :filter-node-method="filterNode"
                :default-expand-all="isExpand" :data="formData4.treeData" v-loading="formData4.myLoading"
                @node-click="handleNodeClick" node-key="id" ref="tree4" highlight-current show-checkbox @check-change="handleCheckChange" @check="handleTreeCheck"
                :props="defaultProps" :auto-expand-parent="false" :check-strictly="true">
              </el-tree>
            </div>

          </div>
          <div class="my-table-box" style="box-sizing: border-box;overflow: auto;">
            <el-row class="my-table-box-search" style="margin:10px;">
              <el-col :span="14">
                <el-input
                  placeholder="请输入姓名/单位/职位/号码"
                  class="searchInput"
                  v-model="formData4.searchText"
                  @input="handleSearch"
                ></el-input>
              </el-col>
              <el-col :span="10">
                <el-button type="primary" @click="handleSearch" style="margin-left:10px;">搜索</el-button>
              </el-col>

            </el-row>
            <el-table
              ref="elTable4"
              class="my-table-main"
              @mousedown.native="handleMouseDown($event)"
              @mouseup.native="handleMouseUp($event)"
              :data="formData4.tableData"
              highlight-current-row
              :header-cell-style="{background:'#E6E6E6',color:'#606266'}"
              height="670"
              style="width: 100%; height: 670px; overflow: auto"
              @row-dblclick="handleRowDbClick"
              @row-click="handleRowClick"
              @selection-change="handleSelectionChange"
              @select="handleSelect"
              @select-all="selectAll">
              <el-table-column type="selection"></el-table-column>
              <!-- <el-table-column type="index" width="50" :resizable="false" align="center"></el-table-column> -->
              <el-table-column show-overflow-tooltip align="center"
                               v-for="(item,key) in tableColumn" :key="key" :label="item.label" :prop="item.prop"
                               :width="item.width" :resizable="false"></el-table-column>
              <!-- <el-table-column label="操作" width="250px" align="center">
                <template slot-scope="scope">
                  <el-button type="primary" size="mini" @click="addCallSms(scope.row)">添加</el-button>
                  <el-button type="primary" size="mini" @click="handSetToCommon(scope.row)">设为常用</el-button>
                  <el-tooltip class="item" effect="dark" content="设为常用联系人" placement="top">
                    <i class="el-icon-star-on" @click="handSetToCommon(scope.row)" :style="{'font-size': '24px','margin-left': '1vw',cursor: 'pointer',color:scope.row.commonlyUsed==0?'#9FC8FE':'#FBC958'}"></i>
                  </el-tooltip>
                </template>
              </el-table-column> -->
            </el-table>
            <el-row>
              <el-pagination class="pagination" background :page-size="formData4.pageSize" :page-sizes="[10,20,30,40]"
                :total="formData4.total"
                :current-page="formData4.currentPage"
                @current-change="handCurrentChange"
                @size-change="handSizeChange"
                layout="total, prev, pager, next, sizes, jumper"></el-pagination>
            </el-row>
          </div>
        </div>
        <div class="my-phone-main" v-show="tabPosition==5">
          <div class="my-tree-box" style="box-sizing: border-box;overflow: auto;">
            <el-scrollbar class="my-tree-scroll">
              <el-input
                placeholder="输入关键字进行搜索"
                v-model="formData5.filterTree" class="treeSearch" style="padding: 10px;box-sizing: border-box;">
                <i slot="suffix" class="el-input__icon el-icon-search" @click="$refs.tree5.filter(formData5.filterTree)"></i>
              </el-input>
              <el-tree :data="formData5.treeData" v-loading="formData5.myLoading" @node-click="handleNodeClick" node-key="id"
                       ref="tree5" highlight-current :props="defaultProps"
                       :filter-node-method="filterNode"
                       draggable
                       @node-drop="handleDrop"
                       :allow-drop="allowDrop"
                       :allow-drag="allowDrag" :show-checkbox="!manageVisible?true:false" @check-change="handleCheckChange" @check="handleTreeCheck">
                <span class="custom-tree-node" slot-scope="{ node, data }" :style="{display: 'flex','align-items': 'center',position: 'relative',width: '100%'}" @mouseover="treeBtnHoverIndex=data.id" @mouseout="treeBtnHoverIndex=-2">
                  <span>{{ node.label }}</span>
                  <span style="display: flex;position: absolute;top: 50%;left: 66%;transform: translateY(-50%);" v-show="treeBtnHoverIndex==data.id&&!manageVisible && data.id !=-1">
                    <div style="display: flex;align-items: center;cursor: pointer;" >
                      <el-tooltip class="item" effect="dark" content="编辑" placement="top">
                        <img :src="require('@/assets/icpImg/editTreeBtn.png')" style="width:1.3vw;" @click="editTreeItem(node, data)">
                      </el-tooltip>
                    </div>
                    <div style="display: flex;align-items: center;cursor: pointer;margin-left: 5px;" >
                      <el-tooltip class="item" effect="dark" content="删除" placement="top">
                        <img :src="require('@/assets/icpImg/deleteTreeBtn.png')" style="width:1.3vw;" @click="deleteTreeItem(node, data)">
                      </el-tooltip>
                    </div>
                  </span>
                </span>
              </el-tree>
            </el-scrollbar>
            <treeBtn v-if="!manageVisible" @changeDialog="closeDialog" @refreshData="refreshData" :treeDataItem="formData5.treeDataItem" :treeDataItems="formData5.treeMultipleSelection" tabPosition="jicenglianluowang"
            :treeData="formData5.treeData" :formData="treeBtnConfig" ref="treeBtn5"></treeBtn>
            <!-- <el-alert title="温馨提示" type="info" description="可拖动修改同级分组排序" show-icon>
            </el-alert> -->
          </div>
          <div class="my-table-box" style="box-sizing: border-box;overflow: auto;">
            <btn-box
              v-if="!manageVisible"
              :btnArry="btnArry"
              v-bind="{
                  add:{callback:addList},
                  edit:{callback:handleEdit},
                  del:{callback:handleDelete},
                  sort:{callback:handleSort},
                  download:{callback:handleExportContactor},
                  document:{callback:handleExportm}
              }"
              :batchJoinIsShow="true"
              @handleBatchJoin="handleBatchJoin"
              @success="batchJoinSuccess"
            ></btn-box>
            <el-row class="my-table-box-search" style="margin:10px;">
              <el-col :span="14">
                <el-input
                  placeholder="请输入姓名/单位/职位/号码"
                  class="searchInput"
                  v-model="formData5.searchText"
                  @input="handleSearch"
                ></el-input>
              </el-col>
              <el-col :span="10">
                <el-button type="primary" @click="handleSearch" style="margin-left:10px;">搜索</el-button>
              </el-col>
            </el-row>
            <el-table
              ref="elTable5"
              class="my-table-main"
              @mousedown.native="handleMouseDown($event)"
              @mouseup.native="handleMouseUp($event)"
              :data="formData5.tableData"
              highlight-current-row
              :header-cell-style="{background:'#E6E6E6',color:'#606266'}"
              height="670"
              style="width: 100%; height: 670px; overflow: auto"
              @row-dblclick="handleRowDbClick"
              @row-click="handleRowClick"
              @selection-change="handleSelectionChange"
              @select="handleSelect"
              @select-all="selectAll">
              <el-table-column type="selection"></el-table-column>
              <!-- <el-table-column type="index" width="50" :resizable="false" align="center"></el-table-column> -->
              <el-table-column show-overflow-tooltip align="center"
                               v-for="(item,key) in tableColumn" :key="key" :label="item.label" :prop="item.prop"
                               :width="item.width" :resizable="false"></el-table-column>
              <el-table-column label="操作" width="250px" align="center">
                <template slot-scope="scope">
                  <!-- <el-button type="primary" size="mini" @click="addCallSms(scope.row)">添加</el-button>
                  <el-button type="primary" size="mini" @click="handSetToCommon(scope.row)">设为常用</el-button> -->
                  <el-tooltip v-if="manageVisible" class="item" effect="dark" content="查看" placement="top">
                    <i class="el-icon-info" @click="watchContactDetails(scope.row)"  style="font-size: 24px;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <el-tooltip v-if="!manageVisible" class="item" effect="dark" content="编辑" placement="top">
                    <i class="el-icon-edit" @click="['formData' + tabPosition].rowData=scope.row;window.setTimeout(()=>{handleEdit()},500)"  style="font-size: 24px;margin-left: 1vw;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <el-tooltip v-if="!manageVisible" class="item" effect="dark" content="删除" placement="top">
                    <i class="el-icon-delete-solid" @click="['formData' + tabPosition].rowData=scope.row;window.setTimeout(()=>{handleDeleteItem()},500)" style="font-size: 24px;margin-left: 1vw;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <!-- <el-tooltip class="item" effect="dark" content="设为常用联系人" placement="top">
                    <i class="el-icon-star-on" @click="handSetToCommon(scope.row)" :style="{'font-size': '24px','margin-left': '1vw',cursor: 'pointer',color:scope.row.commonlyUsed==0?'#9FC8FE':'#FBC958'}"></i>
                  </el-tooltip> -->
                </template>
              </el-table-column>
            </el-table>
            <el-row>
              <el-pagination class="pagination" background :page-size="formData5.pageSize" :page-sizes="[10,20,30,40]"
                :total="formData5.total"
                :current-page="formData5.currentPage"
                @current-change="handCurrentChange"
                @size-change="handSizeChange"
                layout="total, prev, pager, next, sizes, jumper"></el-pagination>
            </el-row>
          </div>
        </div>
        <div class="my-phone-main" v-show="tabPosition==6">
          <div class="my-tree-box" style="box-sizing: border-box;overflow: auto;">
            <el-scrollbar class="my-tree-scroll">
              <el-input
                placeholder="输入关键字进行搜索"
                v-model="formData6.filterTree" class="treeSearch" style="padding: 10px;box-sizing: border-box;">
                <i slot="suffix" class="el-input__icon el-icon-search" @click="$refs.tree6.filter(formData6.filterTree)"></i>
              </el-input>
              <el-tree :data="formData6.treeData" v-loading="myLoading" @node-click="handleNodeClick" node-key="id"
                       :filter-node-method="filterNode"
                       ref="tree6" highlight-current :props="defaultProps"
                       draggable
                       @node-drop="handleDrop"
                       :allow-drop="allowDrop"
                       :allow-drag="allowDrag" :show-checkbox="!manageVisible?true:false" @check-change="handleCheckChange" @check="handleTreeCheck">
                <span class="custom-tree-node" slot-scope="{ node, data }" :style="{display: 'flex','align-items': 'center',position: 'relative',width: '100%'}" @mouseover="treeBtnHoverIndex=data.id" @mouseout="treeBtnHoverIndex=-2" >
                  <span>{{ node.label }}</span>
                  <span style="display: flex;position: absolute;top: 50%;left: 66%;transform: translateY(-50%);" v-show="treeBtnHoverIndex==data.id&&!manageVisible && data.id !=-1">
                    <div style="display: flex;align-items: center;cursor: pointer;" >
                      <el-tooltip class="item" effect="dark" content="编辑" placement="top">
                        <img :src="require('@/assets/icpImg/editTreeBtn.png')" style="width:1.3vw;" @click="editTreeItem(node, data)">
                      </el-tooltip>
                    </div>
                    <div style="display: flex;align-items: center;cursor: pointer;margin-left: 5px;" >
                      <el-tooltip class="item" effect="dark" content="删除" placement="top">
                        <img :src="require('@/assets/icpImg/deleteTreeBtn.png')" style="width:1.3vw;" @click="deleteTreeItem(node, data)">
                      </el-tooltip>
                    </div>
                  </span>
                </span>
              </el-tree>
            </el-scrollbar>
            <treeBtn v-if="!manageVisible" @changeDialog="closeDialog" @refreshData="refreshData" :treeDataItem="formData6.treeDataItem" :treeDataItems="formData6.treeMultipleSelection" tabPosition="beiqinlianluowang"
            :treeData="formData6.treeData" :formData="treeBtnConfig1" ref="treeBtn6"></treeBtn>
            <!-- <el-alert title="温馨提示" type="info" description="可拖动修改同级分组排序" show-icon>
            </el-alert> -->
          </div>
          <div class="my-table-box" style="box-sizing: border-box;overflow: auto;">
            <btn-box
              v-if="!manageVisible"
              :btnArry="btnArry"
              v-bind="{
                add:{callback:addList},
                edit:{callback:handleEdit},
                del:{callback:handleDelete},
                sort:{callback:handleSort},
                download:{callback:handleExportContactor},
                document:{callback:handleExportm}
              }"
              :batchJoinIsShow="true"
              @handleBatchJoin="handleBatchJoin"
              @success="batchJoinSuccess"
            ></btn-box>
            <el-row class="my-table-box-search" style="margin:10px;">
              <el-col :span="14">
                <el-input
                  placeholder="请输入姓名/单位/职位/号码"
                  class="searchInput"
                  v-model="formData6.searchText"
                  @input="handleSearch"
                ></el-input>
              </el-col>
              <el-col :span="10">
                <el-button type="primary" @click="handleSearch" style="margin-left:10px;">搜索</el-button>
              </el-col>
            </el-row>
            <el-table
              ref="elTable6"
              class="my-table-main"
              @mousedown.native="handleMouseDown($event)"
              @mouseup.native="handleMouseUp($event)"
              :data="formData6.tableData"
              highlight-current-row
              :header-cell-style="{background:'#E6E6E6',color:'#606266'}"
              height="670"
              style="width: 100%; height: 670px; overflow: auto"
              @row-dblclick="handleRowDbClick"
              @row-click="handleRowClick"
              @selection-change="handleSelectionChange"
              @select="handleSelect"
              @select-all="selectAll">
              <el-table-column type="selection"></el-table-column>
              <!-- <el-table-column type="index" width="50" :resizable="false" align="center"></el-table-column> -->
              <el-table-column show-overflow-tooltip align="center"
                               v-for="(item,key) in tableColumn" :key="key" :label="item.label" :prop="item.prop"
                               :width="item.width" :resizable="false"></el-table-column>
              <el-table-column label="操作" width="250px" align="center">
                <template slot-scope="scope">
                  <el-tooltip v-if="manageVisible" class="item" effect="dark" content="查看" placement="top">
                    <i class="el-icon-info" @click="watchContactDetails(scope.row)"  style="font-size: 24px;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <el-tooltip v-if="!manageVisible" class="item" effect="dark" content="编辑" placement="top">
                    <i class="el-icon-edit" @click="['formData' + tabPosition].rowData=scope.row;window.setTimeout(()=>{handleEdit()},500)"  style="font-size: 24px;margin-left: 1vw;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <el-tooltip v-if="!manageVisible" class="item" effect="dark" content="删除" placement="top">
                    <i class="el-icon-delete-solid" @click="['formData' + tabPosition].rowData=scope.row;window.setTimeout(()=>{handleDeleteItem()},500)" style="font-size: 24px;margin-left: 1vw;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <!-- <el-tooltip class="item" effect="dark" content="设为常用联系人" placement="top">
                    <i class="el-icon-star-on" @click="handSetToCommon(scope.row)" :style="{'font-size': '24px','margin-left': '1vw',cursor: 'pointer',color:scope.row.commonlyUsed==0?'#9FC8FE':'#FBC958'}"></i>
                  </el-tooltip> -->
                  <!-- <el-button type="primary" size="mini" @click="addCallSms(scope.row)">添加</el-button>
                  <el-button type="primary" size="mini" @click="handSetToCommon(scope.row)">设为常用</el-button> -->
                </template>
              </el-table-column>
            </el-table>
            <el-row>
              <el-pagination class="pagination" background :page-size="formData6.pageSize" :page-sizes="[10,20,30,40]"
                :total="formData6.total"
                :current-page="formData6.currentPage"
                @current-change="handCurrentChange"
                @size-change="handSizeChange"
                layout="total, prev, pager, next, sizes, jumper"></el-pagination>
            </el-row>
          </div>
        </div>
        <div class="my-phone-main" v-if="tabPosition==7">
          <div class="my-tree-box" style="box-sizing: border-box;overflow: auto;">
            <el-scrollbar class="my-tree-scroll">
              <el-input
                placeholder="输入关键字进行搜索"
                v-model="formData7.filterTree" class="treeSearch" style="padding: 10px;box-sizing: border-box;">
                <i slot="suffix" class="el-input__icon el-icon-search" @click="$refs.tree7.filter(formData7.filterTree)"></i>
              </el-input>
              <el-tree :data="formData7.treeData" v-loading="formData7.myLoading" @node-click="handleNodeClick"
                       :filter-node-method="filterNode"
                       node-key="id" ref="tree7" highlight-current :props="defaultProps" :show-checkbox="!manageVisible?true:false" @check-change="handleCheckChange" @check="handleTreeCheck">
                <span class="custom-tree-node" slot-scope="{ node, data }" :style="{display: 'flex','align-items': 'center',position: 'relative',width: '100%'}" @mouseover="treeBtnHoverIndex=data.id" @mouseout="treeBtnHoverIndex=-2" >
                  <span>{{ node.label }}</span>
                  <span style="display: flex;position: absolute;top: 50%;left: 66%;transform: translateY(-50%);" v-show="treeBtnHoverIndex==data.id&&!manageVisible && data.id !=-1">
                    <div style="display: flex;align-items: center;cursor: pointer;" >
                      <el-tooltip class="item" effect="dark" content="编辑" placement="top">
                        <img :src="require('@/assets/icpImg/editTreeBtn.png')" style="width:1.3vw;" @click="editTreeItem(node, data)">
                      </el-tooltip>
                    </div>
                    <div style="display: flex;align-items: center;cursor: pointer;margin-left: 5px;" >
                      <el-tooltip class="item" effect="dark" content="删除" placement="top">
                        <img :src="require('@/assets/icpImg/deleteTreeBtn.png')" style="width:1.3vw;" @click="deleteTreeItem(node, data)">
                      </el-tooltip>
                    </div>
                  </span>
                </span>
              </el-tree>
            </el-scrollbar>
            <treeBtn v-if="!manageVisible" @changeDialog="closeDialog" @refreshData="refreshData" :treeDataItem="formData7.treeDataItem" :treeDataItems="formData7.treeMultipleSelection" tabPosition="ziyuanlianluowang"
            :treeData="formData7.treeData" :formData="code" ref="treeBtn7"></treeBtn>
          </div>
          <div class="my-table-box" style="box-sizing: border-box;overflow: auto;">

            <btn-box
              v-if="!manageVisible"
              :btnArry="btnArry"
              v-bind="{
                add:{callback:addList},
                edit:{callback:handleEdit},
                del:{callback:handleDelete},
                sort:{callback:handleSort},
                download:{callback:handleExportContactor},
                document:{callback:handleExportm}
              }" 
              :batchJoinIsShow="true"
              @handleBatchJoin="handleBatchJoin"
              @success="batchJoinSuccess"
            ></btn-box>
            <el-row class="my-table-box-search" style="margin:10px;">
              <el-col :span="14">
                <el-input
                  placeholder="请输入姓名/单位/职位/号码"
                  class="searchInput"
                  v-model="formData7.searchText"
                  @input="handleSearch"
                ></el-input>
              </el-col>
              <el-col :span="10">
                <el-button type="primary" @click="handleSearch" style="margin-left:10px;">搜索</el-button>
              </el-col>

            </el-row>
            <el-table
              ref="elTable7"
              class="my-table-main"
              @mousedown.native="handleMouseDown($event)"
              @mouseup.native="handleMouseUp($event)"
              :data="formData7.tableData" highlight-current-row
              :header-cell-style="{background:'#E6E6E6',color:'#606266'}"
              height="670"
              style="width: 100%; height: 670px; overflow: auto"
              @row-dblclick="handleRowDbClick"
              @row-click="handleRowClick"
              @selection-change="handleSelectionChange"
              @select="handleSelect"
              @select-all="selectAll">
              <el-table-column type="selection"></el-table-column>
              <!-- <el-table-column type="index" width="50" :resizable="false" align="center"></el-table-column> -->
              <el-table-column show-overflow-tooltip align="center"
                               v-for="(item,key) in tableColumn" :key="key" :label="item.label" :prop="item.prop"
                               :width="item.width" :resizable="false"></el-table-column>
              <el-table-column label="操作" width="250px" align="center">
                <template slot-scope="scope">
                  <!-- <el-button type="primary" size="mini" @click="addCallSms(scope.row)">添加</el-button>
                  <el-button type="primary" size="mini" @click="handSetToCommon(scope.row)">设为常用</el-button> -->
                  <el-tooltip v-if="manageVisible" class="item" effect="dark" content="查看" placement="top">
                    <i class="el-icon-info" @click="watchContactDetails(scope.row)"  style="font-size: 24px;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <el-tooltip v-if="!manageVisible" class="item" effect="dark" content="编辑" placement="top">
                    <i class="el-icon-edit" @click="['formData' + tabPosition].rowData=scope.row;window.setTimeout(()=>{handleEdit()},500)"  style="font-size: 24px;margin-left: 1vw;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <el-tooltip v-if="!manageVisible" class="item" effect="dark" content="删除" placement="top">
                    <i class="el-icon-delete-solid" @click="['formData' + tabPosition].rowData=scope.row;window.setTimeout(()=>{handleDeleteItem()},500)" style="font-size: 24px;margin-left: 1vw;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <!-- <el-tooltip class="item" effect="dark" content="设为常用联系人" placement="top">
                    <i class="el-icon-star-on" @click="handSetToCommon(scope.row)" :style="{'font-size': '24px','margin-left': '1vw',cursor: 'pointer',color:scope.row.commonlyUsed==0?'#9FC8FE':'#FBC958'}"></i>
                  </el-tooltip> -->
                </template>
              </el-table-column>
            </el-table>
            <el-row>
              <el-pagination class="pagination" background :page-size="formData7.pageSize" :page-sizes="[10,20,30,40]"
              :total="formData7.total"
              :current-page="formData7.currentPage"
              @current-change="handCurrentChange"
              @size-change="handSizeChange"
              layout="total, prev, pager, next, sizes, jumper"></el-pagination>
            </el-row>
          </div>
        </div>
        <div class="my-phone-main" v-else-if="tabPosition==8">
          <div class="my-tree-box" style="box-sizing: border-box;overflow: auto;">

            <el-scrollbar class="my-tree-scroll">
              <el-input
                placeholder="输入关键字进行搜索"
                v-model="formData8.filterTree" class="treeSearch" style="padding: 10px;box-sizing: border-box;">
                <i slot="suffix" class="el-input__icon el-icon-search" @click="$refs.tree8.filter(formData8.filterTree)"></i>
              </el-input>
              <el-tree :data="formData8.treeData" v-loading="formData8.myLoading" @node-click="handleNodeClick"
                       :filter-node-method="filterNode"
                       node-key="id" ref="tree8" highlight-current :props="defaultProps" :show-checkbox="!manageVisible?true:false" @check-change="handleCheckChange" @check="handleTreeCheck">
                <span class="custom-tree-node" slot-scope="{ node, data }" :style="{display: 'flex','align-items': 'center',position: 'relative',width: '100%'}" @mouseover="treeBtnHoverIndex=data.id" @mouseout="treeBtnHoverIndex=-2">
                  <span>{{ node.label }}</span>
                  <span style="display: flex;position: absolute;top: 50%;left: 66%;transform: translateY(-50%);" v-show="treeBtnHoverIndex==data.id&&!manageVisible && data.id !=-1">
                    <div style="display: flex;align-items: center;cursor: pointer;" >
                      <el-tooltip class="item" effect="dark" content="编辑" placement="top">
                        <img :src="require('@/assets/icpImg/editTreeBtn.png')" style="width:1.3vw;" @click="editTreeItem(node, data)">
                      </el-tooltip>
                    </div>
                    <div style="display: flex;align-items: center;cursor: pointer;margin-left: 5px;" >
                      <el-tooltip class="item" effect="dark" content="删除" placement="top">
                        <img :src="require('@/assets/icpImg/deleteTreeBtn.png')" style="width:1.3vw;" @click="deleteTreeItem(node, data)">
                      </el-tooltip>
                    </div>
                  </span>
                </span>
              </el-tree>
            </el-scrollbar>
            <treeBtn v-if="!manageVisible" @changeDialog="closeDialog" @refreshData="refreshData"
                     :treeDataItem="formData8.treeDataItem" :treeDataItems="formData8.treeMultipleSelection"
                     tabPosition="duanxinfenzu" :treeData="formData8.treeData" :formData="formDataTree" ref="treeBtn8"></treeBtn>
          </div>
          <div class="my-table-box" style="box-sizing: border-box;overflow: auto;">
            <btn-box  v-if="!manageVisible" :btnArry="[{
            event: 'add',
            icon: 'el-icon-plus',
            title: '新增联系人'
            },{
            event: 'del',
            icon: 'el-icon-delete',
            title: '批量移除'
            },{
            event: 'select',
            title: '选择联系人'
            }]" v-bind="{
              add:{callback:addList},
              del:{callback:handleDelete},
              select:{callback:handleSelectContactor}
            }">
            </btn-box>
            <el-row class="my-table-box-search" style="margin:10px;">
              <el-col :span="14">
                <el-input
                  placeholder="请输入姓名/单位/职位/号码"
                  class="searchInput"
                  v-model="formData8.searchText"
                  @input="handleSearch"
                ></el-input>
              </el-col>
              <el-col :span="10">
                <el-button type="primary" @click="handleSearch" style="margin-left:10px;">搜索</el-button>
              </el-col>

            </el-row>
            <el-table
              ref="elTable8"
              class="my-table-main"
              @mousedown.native="handleMouseDown($event)"
              @mouseup.native="handleMouseUp($event)"
              :data="formData8.tableData"
              highlight-current-row
              :header-cell-style="{background:'#E6E6E6',color:'#606266'}"
              height="670"
              style="width: 100%; height: 670px; overflow: auto"
              @row-click="handleRowClick"
              @selection-change="handleSelectionChange"
              @select="handleSelect"
              @select-all="selectAll">
              <el-table-column type="selection"></el-table-column>
              <!-- <el-table-column type="index" width="50" :resizable="false" align="center"></el-table-column> -->
              <el-table-column show-overflow-tooltip align="center"
                               v-for="(item,key) in tableColumn" :key="key" :label="item.label" :prop="item.prop"
                               :width="item.width" :resizable="false"></el-table-column>
              <el-table-column label="操作" width="250px" align="center">
                <template slot-scope="scope">
                  <!-- <el-button type="primary" size="mini" @click="addCallSms(scope.row)">添加</el-button> -->
                  <el-tooltip v-if="manageVisible" class="item" effect="dark" content="查看" placement="top">
                    <i class="el-icon-info" @click="watchContactDetails(scope.row)"  style="font-size: 24px;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <el-tooltip v-if="!manageVisible" class="item" effect="dark" content="编辑" placement="top">
                    <i class="el-icon-edit" @click="['formData' + tabPosition].rowData=scope.row;window.setTimeout(()=>{handleEdit()},500)"  style="font-size: 24px;margin-left: 1vw;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <el-tooltip v-if="!manageVisible" class="item" effect="dark" content="删除" placement="top">
                    <i class="el-icon-delete-solid" @click="['formData' + tabPosition].rowData=scope.row;window.setTimeout(()=>{handleDeleteItem()},500)" style="font-size: 24px;margin-left: 1vw;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                </template>
              </el-table-column>
            </el-table>
            <el-row>
              <el-pagination class="pagination" background :page-size="formData8.pageSize" :total="formData8.total"
              :current-page="formData8.currentPage" layout="total, prev, pager, next, sizes, jumper"
                             @current-change="handCurrentChange"
                             @size-change="handSizeChange"></el-pagination>
            </el-row>
          </div>
        </div>

        <div class="my-phone-main" v-show="tabPosition==9">
          <div class="my-tree-box" style="box-sizing: border-box;overflow: auto;">
            <el-input
              placeholder="输入关键字进行搜索"
              v-model="formData9.filterTree" class="treeSearch" style="padding: 10px;box-sizing: border-box;">
              <i slot="suffix" class="el-input__icon el-icon-search" @click="$refs.tree9.filter(formData9.filterTree)"></i>
            </el-input>
            <div style="max-height: 711px;min-height: 711px;overflow: auto;">
              <el-tree :accordion="true"
                :filter-node-method="filterNode"
                :default-expand-all="isExpand" :data="formData9.treeData" v-loading="formData9.myLoading"
                @node-click="handleNodeClick" node-key="id" ref="tree9" highlight-current show-checkbox @check-change="handleCheckChange" @check="handleTreeCheck"
                :props="defaultProps">
              </el-tree>
            </div>
          </div>
          <div class="my-table-box" style="box-sizing: border-box;overflow: auto;">
            <el-row class="my-table-box-search" style="margin:10px;">
              <el-col :span="14">
                <el-input
                  placeholder="请输入姓名/单位/职位/号码"
                  class="searchInput"
                  v-model="formData9.searchText"
                  @input="handleSearch"
                ></el-input>
              </el-col>
              <el-col :span="10">
                <el-button type="primary" @click="handleSearch" style="margin-left:10px;">搜索</el-button>
              </el-col>

            </el-row>
            <el-table
              ref="elTable9"
              class="my-table-main"
              @mousedown.native="handleMouseDown($event)"
              @mouseup.native="handleMouseUp($event)"
              :data="formData9.tableData"
              highlight-current-row
              :header-cell-style="{background:'#E6E6E6',color:'#606266'}"
              height="670"
              style="width: 100%; height: 670px; overflow: auto"
              @row-dblclick="handleRowDbClick"
              @row-click="handleRowClick"
              @selection-change="handleSelectionChange"
              @select="handleSelect"
              @select-all="selectAll">
              <el-table-column type="selection"></el-table-column>
              <!-- <el-table-column type="index" width="50" :resizable="false" align="center"></el-table-column> -->
              <el-table-column show-overflow-tooltip align="center"
                               v-for="(item,key) in tableColumn" :key="key" :label="item.label" :prop="item.prop"
                               :width="item.width" :resizable="false"></el-table-column>
              <el-table-column label="操作" width="250px" align="center">
                <template slot-scope="scope">
                    <!-- 
                        @authorName: qinjiaqi
                        @description：查看按钮 
                    -->
                    <el-tooltip
                        v-if="manageVisible"
                        class="item"
                        effect="dark"
                        content="查看"
                        placement="top"
                    >
                        <i class="el-icon-info" @click="watchContactDetails(scope.row)"
                            style="font-size: 24px;color:#9FC8FE;cursor: pointer;">
                        </i>
                    </el-tooltip>
                </template>
              </el-table-column>
            </el-table>
            <el-row>
              <el-pagination class="pagination" background :page-size="formData9.pageSize" :page-sizes="[10,20,30,40]"
                :total="formData9.total"
                :current-page="formData9.currentPage"
                @current-change="handCurrentChange"
                @size-change="handSizeChange"
                layout="total, prev, pager, next, sizes, jumper"></el-pagination>
            </el-row>
          </div>
        </div>
        <div class="my-phone-main" v-if="tabPosition==10">
          <div class="my-tree-box" style="box-sizing: border-box;overflow: auto;">
            <el-scrollbar class="my-tree-scroll">
              <el-input
                placeholder="输入关键字进行搜索"
                v-model="formData10.filterTree" class="treeSearch" style="padding: 10px;box-sizing: border-box;">
                <i slot="suffix" class="el-input__icon el-icon-search" @click="$refs.tree10.filter(formData10.filterTree)"></i>
              </el-input>
              <el-tree :data="formData10.treeData" v-loading="formData10.myLoading" @node-click="handleNodeClick"
                       :filter-node-method="filterNode"
                       node-key="id" ref="tree10" highlight-current :props="defaultProps" :show-checkbox="!manageVisible?true:false" @check-change="handleCheckChange" @check="handleTreeCheck">
                <span class="custom-tree-node" slot-scope="{ node, data }" :style="{display: 'flex','align-items': 'center',position: 'relative',width: '100%'}" @mouseover="treeBtnHoverIndex=data.id" @mouseout="treeBtnHoverIndex=-2" >
                  <span>{{ node.label }}</span>
                  <span style="display: flex;position: absolute;top: 50%;left: 66%;transform: translateY(-50%);" v-show="treeBtnHoverIndex==data.id&&!manageVisible && data.id !=-1">
                    <div style="display: flex;align-items: center;cursor: pointer;" >
                      <el-tooltip class="item" effect="dark" content="编辑" placement="top">
                        <img :src="require('@/assets/icpImg/editTreeBtn.png')" style="width:1.3vw;" @click="editTreeItem(node, data)">
                      </el-tooltip>
                    </div>
                    <div style="display: flex;align-items: center;cursor: pointer;margin-left: 5px;" >
                      <el-tooltip class="item" effect="dark" content="删除" placement="top">
                        <img :src="require('@/assets/icpImg/deleteTreeBtn.png')" style="width:1.3vw;" @click="deleteTreeItem(node, data)">
                      </el-tooltip>
                    </div>
                  </span>
                </span>
              </el-tree>
            </el-scrollbar>
            <treeBtn v-if="!manageVisible" @changeDialog="closeDialog" @refreshData="refreshData" :treeDataItem="formData10.treeDataItem" :treeDataItems="formData10.treeMultipleSelection" tabPosition="ziyuanlianluowang"
            :treeData="formData10.treeData" :formData="treeBtnConfig2" ref="treeBtn10"></treeBtn>
          </div>
          <div class="my-table-box" style="box-sizing: border-box;overflow: auto;">
            <btn-box  v-if="!manageVisible" :btnArry="[{
              event: 'del',
              icon: 'el-icon-delete',
              title: '批量移除'
              },{
              event: 'select',
              title: '选择联系人'
              }]" v-bind="{
                add:{callback:addList},
                del:{callback:handleDelete},
                select:{callback:handleSelectContactor}
              }">
            </btn-box>
            <el-row class="my-table-box-search" style="margin:10px;">
              <el-col :span="14">
                <el-input
                  placeholder="请输入姓名/单位/职位/号码"
                  class="searchInput"
                  v-model="formData10.searchText"
                  @input="handleSearch"
                ></el-input>
              </el-col>
              <el-col :span="10">
                <el-button type="primary" @click="handleSearch" style="margin-left:10px;">搜索</el-button>
              </el-col>

            </el-row>
            <el-table
              ref="elTable10"
              class="my-table-main"
              @mousedown.native="handleMouseDown($event)"
              @mouseup.native="handleMouseUp($event)"
              :data="formData10.tableData"
              highlight-current-row
              :header-cell-style="{background:'#E6E6E6',color:'#606266'}"
              height="670"
              style="width: 100%; height: 670px; overflow: auto"
              @row-dblclick="handleRowDbClick"
              @row-click="handleRowClick"
              @selection-change="handleSelectionChange"
              @select="handleSelect"
              @select-all="selectAll">
              <el-table-column type="selection"></el-table-column>
              <!-- <el-table-column type="index" width="50" :resizable="false" align="center"></el-table-column> -->
              <el-table-column show-overflow-tooltip align="center"
                               v-for="(item,key) in tableColumn" :key="key" :label="item.label" :prop="item.prop"
                               :width="item.width" :resizable="false"></el-table-column>
              <el-table-column label="操作" width="250px" align="center">
                <template slot-scope="scope">
                  <!-- <el-button type="primary" size="mini" @click="addCallSms(scope.row)">添加</el-button>
                  <el-button type="primary" size="mini" @click="handSetToCommon(scope.row)">设为常用</el-button> -->
                  <el-tooltip v-if="manageVisible" class="item" effect="dark" content="查看" placement="top">
                    <i class="el-icon-info" @click="watchContactDetails(scope.row)"  style="font-size: 24px;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <el-tooltip v-if="!manageVisible" class="item" effect="dark" content="编辑" placement="top">
                    <i class="el-icon-edit" @click="['formData' + tabPosition].rowData=scope.row;window.setTimeout(()=>{handleEdit()},500)"  style="font-size: 24px;margin-left: 1vw;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <el-tooltip v-if="!manageVisible" class="item" effect="dark" content="删除" placement="top">
                    <i class="el-icon-delete-solid" @click="['formData' + tabPosition].rowData=scope.row;window.setTimeout(()=>{handleDeleteItem()},500)" style="font-size: 24px;margin-left: 1vw;color:#9FC8FE;cursor: pointer;"></i>
                  </el-tooltip>
                  <!-- <el-tooltip class="item" effect="dark" content="设为常用联系人" placement="top">
                    <i class="el-icon-star-on" @click="handSetToCommon(scope.row)" :style="{'font-size': '24px','margin-left': '1vw',cursor: 'pointer',color:scope.row.commonlyUsed==0?'#9FC8FE':'#FBC958'}"></i>
                  </el-tooltip> -->
                </template>
              </el-table-column>
            </el-table>
            <el-row>
              <el-pagination class="pagination" background :page-size="formData10.pageSize" :page-sizes="[10,20,30,40]"
              :total="formData10.total"
              :current-page="formData10.currentPage"
              @current-change="handCurrentChange"
              @size-change="handSizeChange"
              layout="total, prev, pager, next, sizes, jumper"></el-pagination>
            </el-row>
          </div>
        </div>

      <!--<div class="my-radio-box-main" style="margin-top:10px;">
        <el-radio-group v-model="tabPosition" @change="changePage">
          <el-radio-button v-for="(item,index) in list" :label="index+1" :key="index">{{item.name}}</el-radio-button>
        </el-radio-group>
      </div>-->
     <!-- <div class="my-phone-main" v-if="tabPosition==1">
       &lt;!&ndash; <div class="my-tree-box">
          <treeBtn @changeDialog="closeDialog" @refreshData="refreshData" :treeDataItem="formData1.treeDataItem"
                   :treeData="formData1.treeData" :formData="formData"></treeBtn>
          <el-scrollbar class="my-tree-scroll">
            <el-tree :data="formData1.treeData" v-loading="formData1.myLoading"
                     @node-click="handleNodeClick" node-key="id" ref="tree"
                     highlight-current :props="defaultProps"

            ></el-tree>
          </el-scrollbar>
          <el-alert title="温馨提示" type="info" description="可拖动修改同级分组排序" show-icon>
          </el-alert>
        </div>&ndash;&gt;
      </div>-->
    <!-- </template> -->
    <!-- <template v-if="tabPosition1==3">
      <urgent-information></urgent-information>
    </template> -->


    <!--联系人弹窗-->
    <contact-details-popup ref="ContactDetailsPopup" @getContactorList="getContactorList" ></contact-details-popup>
    <contact-sorting-popup ref="ContactSortingPopup" @getContactorList="getContactorList"></contact-sorting-popup>
    <watch-contact-details ref="watchContactDetails"></watch-contact-details>
    <!-- 通讯录 -->
    <add-contact
      ref="addContact"
      v-if="addContactVisible"
      @changeDialog="addContactVisible = false"
    ></add-contact>
    <!-- Author hexinting
      @Date  2021-1-18
      @goal  针对openLab项目，删除逻辑修改,1.删除当前分组联系人 2.删除所有分组的该联系人
      @deleteBtnFlag 控制弹框的显示
     -->
    <el-dialog
      title="联系人删除提示"
      :visible.sync="deleteBtnFlag"
      width="25%"
      center>
      <span>该联系人在系统中存在分组、预案等关联数据,删除数据将不可恢复，请您确认相关操作!</span>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="deleteGroupOne">从分组中删除</el-button>
        <el-button type="danger" @click="deleteGroupAll">彻底删除</el-button>
      </span>
    </el-dialog>
  </div>
</template>

<script>
  import mixin from './mixin.js';
  import treeBtn from '@/yz_components/phoneNew/treeBtn';
  import btnBox from '@/yz_components/phoneNew/btnBox';
  import ContactDetailsPopup from "@/view/yz_integratedCommunication/contactDetailsPopup";
  import watchContactDetails from "@/view/yz_integratedCommunication/watchContactDetails";
  import ContactSortingPopup from "@/view/yz_integratedCommunication/contactSortingPopup";
  import PendingInformation from '@/view/yz_integratedCommunication/icpSms/pendingInformation/pendingInformation';
  import UrgentInformation from '@/view/yz_integratedCommunication/icpSms/urgentInformation/urgentInformation';
  import addContact from "./smsStep/addContact/AddContact";
  import { mapGetters } from "vuex";
  import { areaList } from '@/assets/interface/interface';
  import bus from "@/yz_components/common/js/eventBus";
  import {replaceContactorGroupsConfig} from '@/assets/js/yz_common.js'

  export default {
    mixins: [mixin],
    components: {
      treeBtn,
      btnBox,
      ContactDetailsPopup,
      watchContactDetails,
      ContactSortingPopup,
      PendingInformation,
      UrgentInformation,
      addContact
    },
    data() {
      return {
        IsOpenLabProject: window.g.IsOpenLabProject, // openLab 项目
        IsYangZhouProject: window.g.IsYangZhouProject, // 扬州项目
        seletecdContacList:[],
        seletecdContacIds:[],     // 保存勾选的联系人id
        addContactVisible: false, //选择通讯录
        temporaryContactorName:'',
        treeBtnHoverIndex:-2,
        window:window,
        manageVisible:true, //图标管理功能切换
        showSpanImg:0,
        // tabName: window.g.IsYangZhouProject ? "政务通信录" : "政务联络网", // 标签名称
        // tabName: '政务联络网', // 标签名称
        tabName: "phone", // 标签名称 政务联络网
        formData1: {//政务联络网
          searchText: '',//搜索
          currentPage: 1,
          total: 0,//数据总数
          pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
          rowData: null,//表格选择数据
          tableData: [],//表格数据
          treeData: [],
          groupId: '',//分组id
          myLoading: false,
          filterTree:'', //树状列表输入框过滤
          tableMultipleSelection:[], //列表勾选的选项
          treeMultipleSelection:[] //树状分组勾选的选项
        },
        formData2: {//常用联系人
          searchText: '',//搜索
          currentPage: 1,
          total: 0,//数据总数
          pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
          rowData: null,//表格选择数据
          tableData: [],//表格数据
          treeData: [],
          groupId: 0,//分组id
          myLoading: false,
          filterTree:'', //树状列表输入框过滤
          tableMultipleSelection:[], //列表勾选的选项
          treeMultipleSelection:[] //树状分组勾选的选项
        },
        formData3: {//最近联系人
          searchText: '',//搜索
          currentPage: 1,
          total: 0,//数据总数
          pageSize: window.g.IsZengChengProject ? 40 : 20, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
          rowData: null,//表格选择数据
          tableData: [],//表格数据
          treeData: [],
          groupId: 0,//分组id
          myLoading: false,
          filterTree:'', //树状列表输入框过滤
          tableMultipleSelection:[], //列表勾选的选项
          treeMultipleSelection:[] //树状分组勾选的选项
        },
        formData4: {//应急联动网
          searchText: '',//搜索
          currentPage: 1,
          total: 0,//数据总数
          pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
          rowData: null,//表格选择数据
          tableData: [],//表格数据
          myLoading: false,
          treeData: [],
          treeDataItem: {},
          filterTree:'', //树状列表输入框过滤
          tableMultipleSelection:[], //列表勾选的选项
          treeMultipleSelection:[], //树状分组勾选的选项
          groupId:''
        },
        formData5: {//基层联络网
          searchText: '',//搜索
          currentPage: 1,
          total: 0,//数据总数
          pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
          rowData: null,//表格选择数据
          tableData: [],//表格数据
          myLoading: false,
          treeData: [],
          treeDataItem: {},
          filterTree:'', //树状列表输入框过滤
          tableMultipleSelection:[], //列表勾选的选项
          treeMultipleSelection:[], //树状分组勾选的选项
          groupId:''
        },
        formData6: {//备勤联络网
          searchText: '',//搜索
          currentPage: 1,
          total: 0,//数据总数
          pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
          rowData: null,//表格选择数据
          tableData: [],//表格数据
          myLoading: false,
          treeData: [],
          treeDataItem: {},
          filterTree:'', //树状列表输入框过滤
          tableMultipleSelection:[], //列表勾选的选项
          treeMultipleSelection:[], //树状分组勾选的选项
          groupId:''
        },
        formData7: {//资源联络网
          searchText: '',//搜索
          currentPage: 1,
          total: 0,//数据总数
          pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
          rowData: null,//表格选择数据
          tableData: [],//表格数据
          treeData: [],
          groupId: 0,//分组id
          myLoading: false,
          filterTree:'', //树状列表输入框过滤
          tableMultipleSelection:[], //列表勾选的选项
          treeMultipleSelection:[] //树状分组勾选的选项
        },
        formData8: {//短信分组
          searchText: '',//搜索
          currentPage: 1,
          total: 0,//数据总数
          pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
          rowData: null,//表格选择数据
          tableData: [],//表格数据
          treeData: [],
          groupId: 0,//分组id
          myLoading: false,
          filterTree:'', //树状列表输入框过滤
          tableMultipleSelection:[], //列表勾选的选项
          treeMultipleSelection:[] //树状分组勾选的选项
        },
        formData9: {//工业园区管委会通讯录
          searchText: '',//搜索
          currentPage: 1,
          total: 0,//数据总数
          pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
          rowData: null,//表格选择数据
          tableData: [],//表格数据
          myLoading: false,
          treeData: [],
          treeDataItem: {},
          filterTree:'', //树状列表输入框过滤
          tableMultipleSelection:[], //列表勾选的选项
          treeMultipleSelection:[], //树状分组勾选的选项
          groupId:''
        },
        formData10: {//快捷通讯录
          searchText: '',//搜索
          currentPage: 1,
          total: 0,//数据总数
          pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
          rowData: null,//表格选择数据
          tableData: [],//表格数据
          myLoading: false,
          treeData: [],
          treeDataItem: {},
          filterTree:'', //树状列表输入框过滤
          tableMultipleSelection:[], //列表勾选的选项
          treeMultipleSelection:[], //树状分组勾选的选项
          groupId:''
        },
        filterText: '',
        searchText: '',
        rowData: [],//行表格数据
        btnArry: [
          {
            event: 'add',
            icon: 'el-icon-plus',
            title: '新增'
          },
          //  {
          //   event: 'edit',
          //   icon: 'el-icon-edit',
          //   title: '修改'
          // },
           {
            event: 'del',
            icon: 'el-icon-delete',
            title: '批量移除'
          }, {
            event: 'document',
            icon: 'el-icon-document',
            title: '模板'
          }, {
            event: 'download',
            icon: 'el-icon-download',
            title: '导出联系人'

          }, {
            event: 'sort',
            icon: 'el-icon-sort',
            title: '排序'
          }],
        btnArry1: [
          {
            event: 'edit',
            icon: 'el-icon-edit',
            title: '修改'
          }, {
            event: 'del',
            icon: 'el-icon-delete',
            title: '批量移除'
          }],
        dialogVisible: true,
        formData: {
          pid: 0,
          code: 'phone',
          ref: 'sms',
          // name: window.g.IsYangZhouProject ? "政务通信录" : "政务联络网",
          name: '政务联络网',
        },
        code: {
          pid: 0,
          code: 'resource',
          ref: 'sms',
          name: '资源联络网',
        },
        formDataTree: {
          pid: 0,
          code: 'sms',
          ref: 'sms',
          name: '短信分组',
        },
        treeBtnConfig: {
          pid: 0,
          code: 'gliaison',
          ref: 'sms',
          name: '基层联络网',
        },
        treeBtnConfig1: {
          pid: 0,
          code: 'dutyGroup',
          ref: 'sms',
          name: '备勤联络网',
        },
        treeBtnConfig2: {
          pid: 0,
          code: 'quick',
          ref: 'sms',
          name: '快捷组',
        },
        treeDataItem: {},
        currentPage: 1,//当前页数
        groupId: -1,//分组id
        defaultProps: {
          children: 'children',
          label: 'name',
          plusIcon: true,
        },
        myLoading: false,
        treeData: [],//电话分组
        list: [
          {
              //按钮切换
              name: '政务联络网',
              label: '1',
              code: "phone",
              enabled: true // 是否显示
          },
          // {
          //   name: '常用联系人',
          //   label: '2',
          //   code: "commonlyused",
          //   enabled: true // 是否显示
          // }, {
          //   name: '最近联系人',
          //   label: '3',
          //   code: "recentcontact",
          //   enabled: true // 是否显示
          // },
          {
              name: '应急联动网',
              label: '4',
              code: "linkage",
              enabled: true // 是否显示
          },
          {
              name: '基层联络网',
              label: '5',
              code: "gliaison",
              enabled: true // 是否显示
          },
          {
              name: '备勤联络网',
              label: '6',
              code: "dutyGroup",
              enabled: true // 是否显示
          },
          {
              name: '资源联络网',
              label: '7',
              code: "resource",
              enabled: true // 是否显示
          },
          {
              name: '短信分组',
              label: '8',
              code: "sms",
              enabled: true // 是否显示
          },
          window.g.IsSuZhouPorject
              ? {
                  name: '工业园区管委会通讯录',
                  label: '9',
                  code: "suzhouGroup",
                  enabled: true // 是否显示
                }
              : null,
          {
              name: '快捷组',
              label: '10',
              code: "quick",
              enabled: true // 是否显示
          },
        ],
        // list: [
        //   {//按钮切换
        //     name: window.g.IsYangZhouProject ? "政务通信录" : "政务联络网",
        //     label: '1'
        //   },
        //   // {
        //   //   name: '常用联系人',
        //   //   label: '2'
        //   // }, {
        //   //   name: '最近联系人',
        //   //   label: '3'
        //   // },
        //   {
        //     name: window.g.IsYangZhouProject ? '应急通信录' : '应急联动网',
        //     label: '4'
        //   }, 
        //   window.g.IsYangZhouProject ? null : {
        //     name: "基层联络网",
        //     label: "5"
        //   },
        //   window.g.IsYangZhouProject ? null : {
        //     name: "备勤联络网",
        //     label: "6"
        //   },
        //   window.g.IsYangZhouProject ? null : {
        //     name: "资源联络网",
        //     label: "7"
        //   },
        //   {
        //     name: '短信分组',
        //     label: '8'
        //   },
        //   window.g.IsSuZhouPorject?
        //   {
        //     name: '工业园区管委会通讯录',
        //     label: '9'
        //   }:null,
        //   {
        //     name: '快捷组',
        //     label: '10'
        //   },
        //   ],

        tableColumn: [
          {
            prop: "name",
            label: "姓名"
          }, {
            prop: "workUnit",
            label: "单位"
          }, {
            prop: "position",
            label: "职位"
          }, {
            prop: "number",
            label: "联系号码"
          }
        ],//表头
        tableColumn1: [
          {
            prop: "name",
            label: "姓名"
          },
          {
            prop: "workUnit",
            label: "单位"
          },
          {
            prop: "position",
            label: "职位"
          },
          {
            prop: "number",
            label: "联系号码"
          },
          {
            prop: "updateTime",
            label: "更新时间"
          }
        ], //表头
        tableColumn3: [
          {
            prop: "name",
            label: "姓名"
          }, {
            prop: "workUnit",
            label: "单位"
          }, {
            prop: "position",
            label: "职位"
          }, {
            prop: "mobile",
            label: "联系号码"
          }
        ],//表头
        msgGroupTabColumn: [
          {
            prop: "name",
            label: "姓名"
          }, {
            prop: "workUnit",
            label: "单位"
          }, {
            prop: "position",
            label: "职位"
          }, {
            prop: "mobile",
            label: "联系号码"
          }
        ],//表头
        tabPosition: 1,
        tableData: [],//表格数据
        total: 0,//数据总数
        pageSize: 10, //总页数
        isExpand: false,
        tabPosition1: 1,
        list1: [
          {
            name: "信息发送",
            label: "1"
          },
          {
            name: "待发信息",
            label: "2"
          },
          {
            name: "催报信息",
            label: "3"
          }
        ],
        isSelectTree: false, // 应急联系人->树节点勾选
        clickCount: 1, //记录表格全选点击次数
        contactList: [
          {i:0, name:'常用联系人', icon: 'el-icon-star-off', code: 'commonlyused'},
          {i:1, name:'最近联系人', icon: 'el-icon-time', code: 'recentcontact'}
        ],

        mouseDownIndex: -1,
        mouseUpIndex: -1,
        deleteBtnFlag: false, // 控制删除对话框显示/隐藏
        deleteMoreorOne: '', // 打开的是删除单个的还是删除多个的  more多个，one一个
      }
    },
    created() {
      this.getPhoneGroupingTree();
      this.getContactorList();

      // this.$store.commit('getMsgContactsUpdate', false)
      // console.log(this.$store.state.msgContactsUpdate);
      // console.log(this.$store.state.faxContactsUpdate);
    },
    mounted(){
      this.$nextTick(()=>{
        // 3.0不要这个方法了
        // this.initContactorGroupsConfig(); // 初始化分组配置
        setTimeout(()=>{
          this.$refs['tree' + this.tabPosition].setCheckedNodes(this.smsTreeList['tree' + this.tabPosition])

          console.log("this.smsMember是什么",this.$store,this.smsTreeList,this.smsMember)
          this.smsMember.forEach(item=>{
            // console.log("mounted里勾选表格item",item,this['formData' + this.tabPosition].tableData.filter(element=>{return element.id==item.id}),this['formData' + this.tabPosition].tableData )
            if(this['formData' + this.tabPosition].tableData.filter(element=>{return element.id==item.id}).length!=0){
              this.$refs['elTable' + this.tabPosition].toggleRowSelection(this['formData' + this.tabPosition].tableData.filter(element=>{return element.id==item.id})[0],true);
            }

          })
        },800)
        const that = this
        bus.$off('getSmsContact')
        bus.$on('getSmsContact',(checkData)=>{
            if (checkData.length!=0) {
              let data = {
                  contactorIds: checkData.map(v=> {return v.id}),
                  groupId : this.groupId
                };
              this.$api.addContactorToGroup({data}).then(res => {
                if (res.errorcode != 0) {
                  this.$message.error(res.msg);
                  return;
                }
                this.getContactorList()
              });
          }
        })
      })
    },
    beforeDestroy() {
      sessionStorage.removeItem('store')
    },
    watch: {
      smsMember(newVal){
        this.$refs['elTable' + this.tabPosition].clearSelection();
        if (this.tabPosition == 3) {
          this.smsMember.forEach(item => {
            if(this['formData' + this.tabPosition].tableData.filter(
                element => {
                  return (
                      element.mobile ==
                      (item.number ? item.number : item.call ? item.call : item.mobile)
                  );
                }
              ).length != 0
            ) {
              this.$refs['elTable' + this.tabPosition].toggleRowSelection(
                this[
                  'formData' + this.tabPosition
                ].tableData.filter(element => {
                  return (
                    element.mobile ==
                    (item.number ? item.number : item.call ? item.call : item.mobile)
                  );
                })[0],
                true
              );
            }
          });
        } else {
          this.smsMember.forEach(item => {
            if (this['formData' + this.tabPosition].tableData.filter(
                element => {
                  return element.id == item.id;
                }
              ).length != 0
            ) {
              this.$refs['elTable' + this.tabPosition].toggleRowSelection(
                this[
                    'formData' + this.tabPosition
                ].tableData.filter(element => {
                    return element.id == item.id;
                })[0],
                true
              );
            }
          });
        }
        // 清空树节点选中
        if (newVal.length === 0 && this.tabPosition !== 3) {
          this.$refs['tree' + this.tabPosition].setCheckedKeys([]);
        }
      },
      filterText(val) {
        if (val == '' || val == null || val == undefined) {
          this.$refs.tree1.filter(val);
        }
      },
      /*'$store.state.msgContactsUpdate': {
        deep: true,
        handler: function(newVal, oldVal) {
          console.log(newVal, oldVal)
        }
      },*/
      handleMsgContactsUpdate(newVal, oldVal) {
        console.log(newVal, oldVal)
        if (newVal) {
          console.log('联系人更新')
          this.getContactorList()
          this.$store.commit('getMsgContactsUpdate', false) // 重置更新状态为false
        } else {
          console.log('newVal == false');
          console.log(this.$store.state.msgContactsUpdate);
        }
      }

    },
    computed: {
      handleMsgContactsUpdate() {
        console.log(this.$store.state.msgContactsUpdate);
        return this.$store.state.msgContactsUpdate
      },
      ...mapGetters({
        smsMember: "getSmsMember", //短信队列
        smsTreeList: "getSmsTreeList" //短信树状分组队列
      })
    },

    methods: {
      /*
       * @param  {type}    :   param
       * @return {type}    :   return
       * @lastEditor       :   yangqd
       * @lastDate         :   2020年12月22日 15:43:20
       * @descripttion     :   表格鼠标点击后，记录开始行
       */
      handleMouseDown(event){
        let ele = event.target;
        if(ele.className.indexOf("checkbox") !== -1){
          this.mouseDownIndex = -1
          this.mouseUpIndex = -1
          return
        }
        while(ele.className.indexOf("my-table-main") === -1 && ele.className.indexOf("el-table__row") === -1){
          ele = ele.parentNode
        }
        if(ele.className.indexOf("el-table__row") !== -1){
          let index = 0;
          while(ele.previousSibling != null){
            index++;
            ele = ele.previousSibling;
          }
          this.mouseDownIndex = index
        }
      },
      /*
       * @param  {type}    :   param
       * @return {type}    :   return
       * @lastEditor       :   yangqd
       * @lastDate         :   2020年12月22日 15:44:21
       * @descripttion     :   表格鼠标松开后，点击选中行
       */
      handleMouseUp(event){
        console.log("mouseUp", event)
        let ele = event.target;
        if(ele.className.indexOf("checkbox") !== -1){
          this.mouseDownIndex = -1
          this.mouseUpIndex = -1
          return
        }
        while(ele.className.indexOf("my-table-main") === -1 && ele.className.indexOf("el-table__row") === -1){
          ele = ele.parentNode
        }
        if(ele.className.indexOf("el-table__row") !== -1){
          let index = 0;
          while(ele.previousSibling != null){
            index++;
            ele = ele.previousSibling;
          }
          console.log("index", index)
          this.mouseUpIndex = index;

          if(this.mouseDownIndex !== -1 && this.mouseUpIndex !== -1){
            if(this.mouseDownIndex > this.mouseUpIndex){
              [this.mouseDownIndex, this.mouseUpIndex] = [this.mouseUpIndex, this.mouseDownIndex]
            }
            for(let i = 0;i <= this.mouseUpIndex;i++){
              if(i >= this.mouseDownIndex){
                let tmp = ele.firstChild.firstChild.firstChild
                tmp.click()
              }
              ele = ele.nextSibling
            }
          }
          this.mouseDownIndex = -1
          this.mouseUpIndex = -1
        }
      },
      /**
       * @author hexinting
       * @date 2020-11-05
       * @lastEditor hexinting
       * @lastDate 2020-11-05
       * @description 初始化分组配置
       */
      initContactorGroupsConfig() {
        let contactorGroups = JSON.parse(sessionStorage.getItem("contactorGroups")); // 后端返回的分组配置列表
        this.list = replaceContactorGroupsConfig(this.list); // 根据后端返回分组，替换联系人大分组名字配置
        this.list.forEach(item => {
            if (item) {
              if (item.code === "phone") { // 政务联络网
                  this.formData.name = item.name
              } else if (item.code === "resource") { // 资源联络网
                  this.code.name = item.name
              } else if (item.code === "sms") { // 短信分组
                  this.formDataTree.name = item.name
              }  else if (item.code === "gliaison") { // 基层联络网
                  this.treeBtnConfig.name = item.name
              } else if (item.code === "dutyGroup") { // 备勤联络网
                  this.treeBtnConfig1.name = item.name
              }else if (item.code === "quick") { // 快捷组
                  this.treeBtnConfig2.name = item.name
              }
            }
        })
        // 常用联系人、最近联系人
        this.contactList.forEach(item => {
          if (item) {
            let _index = contactorGroups.findIndex(el => el.code === item.code); // 根据code查找大分组名字
            if (_index !== -1) { // 存在
                item.name = contactorGroups[_index].name // 替换名字
            }
          }
        })
      },
      //获取工业园区管委会树状分组
      getSuzhouGroupList() {
        this.$http({
          url:window.g.ApiUrl +"/eos/communication/group/suzhouGroupLIst",
          method: "post",
          data: {}
        }).then(({data}) => {
          console.log("/eos/communication/group/suzhouGroupLIst",data); 
          this.formData9.treeData=data.data
        });
      },
      watchContactDetails(row){
        this.$refs.watchContactDetails.isDialogVisible = true;
        this.$refs.watchContactDetails.title = "查看联系人";
        this.$refs.watchContactDetails.getTree(this.tabPosition, this.groupId, this.tabName);
        this.$refs.watchContactDetails.showData(row);
      },
      handleCheckChange(data, checked, indeterminate) {
        console.log('已选联系人：', this.smsMember)
        // this['formData' + this.tabPosition].tableData = []
        var contactorNameArr = []
        var contactorNameString = ''
        if(this.tabPosition==2||this.tabPosition==3){
          this.tabPosition=1
        }
        console.log('handleCheckChange树状分组选中了什么',data, checked, indeterminate);
        if(checked){
          this['formData' + this.tabPosition].treeMultipleSelection.push(data)
          this.tableCheckStatus(true)
          this.isSelectTree = true;
        }else{
          this.isSelectTree = false;
          console.log(this.tabPosition);
          this.temporaryContactorName='';
          this['formData' + this.tabPosition].treeMultipleSelection=this['formData' + this.tabPosition].treeMultipleSelection.filter(function (elem) {
            return (elem.id != data.id);
          })
          // if (this['formData' + this.tabPosition].treeMultipleSelection.length === 0) {
          //   this.$store.commit('setSmsMember',[]) //当树状分组未被选中时，清空选中的数据
          // }

          // this.tableCheckStatus(false)
          // return

          this.$http({
              url: window.g.ApiUrl + '/eos/communication/group/findAllContactorInGroupAndLowerGroup',
              method: 'post',
              data:{
                type:'sms',
                groupIds:data.id
              }
            }).then(res => {
              var temporarySmsMember=[]
              Promise.resolve(()=>{
                this.smsMember.forEach(item=>{
                  res.data.data.forEach(element=>{
                    if(item.id!=element.id){
                      temporarySmsMember.push(item)
                    }
                  })
                })
              }).then(()=>{
                this.$store.commit("setSmsMember", temporarySmsMember);
              })
             
            })
        }
        if(this.manageVisible){
          console.log("this['formData' + this.tabPosition].treeMultipleSelection是什么",this['formData' + this.tabPosition].treeMultipleSelection)
          // this.$bus.$emit('clearNumber')
          if(this['formData' + this.tabPosition].treeMultipleSelection.length==0){
            // setTimeout(()=>{
            //   this.$bus.$emit('clearNumber')
            // },500)
            return false;
          }
          if(this.tabPosition==1||this.tabPosition==8||this.tabPosition==5||this.tabPosition==6||this.tabPosition==7||this.tabPosition==9||this.tabPosition==10){
            this.$http({
              url: window.g.ApiUrl + '/eos/communication/group/findAllContactorInGroupAndLowerGroup',
              method: 'post',
              data:{
                type:'sms',
                groupIds:this['formData' + this.tabPosition].treeMultipleSelection.map(function (elem) {
                  return elem.id
                })
              }
            }).then(res => {
              console.log("/eos/communication/group/findAllContactorInGroupAndLowerGroup",res)
              if(res.data.data&&res.data.data.length!=0){
                var temporarySmsMember=this.smsMember.filter(item=>{return item.isTableSelect==true})
                res.data.data.forEach(item=>{
                  // if(item.mobile){
                  //   this.$emit('findfaxDataS', item);
                  // }

                  if(temporarySmsMember.filter(element=>{return element.id==item.id}).length==0){
                    if(!item.number){
                      if(this.temporaryContactorName!=(item.name?item.name:item.contactor)&&checked){
                        this.temporaryContactorName=item.name||item.contactor
                        contactorNameArr.push(item.name||item.contactor)
                        contactorNameString = contactorNameArr.join()
                        this.$message.warning(`${contactorNameString}联系号码为空`)
                        // this.$message.warning(`${item.name||item.contactor}联系号码为空`)
                      }
                      
                    }else{
                      item.isTableSelect = true
                      temporarySmsMember.push(item)
                      console.log('aaaa已选择联系人：', temporarySmsMember)
                      this.$store.commit("setSmsMember", temporarySmsMember);
                    }
                  }

                })

              }

            })
          }
          if(this.tabPosition==4){
            this['formData' + this.tabPosition].treeMultipleSelection.forEach(item=>{
              this.$http({
                url: window.g.ApiUrl + '/eos/communication/preplan/getContactor',
                method: 'post',
                data:{
                  type:'sms',
                  id:item.id,
                  isTop:item.parentId==0?'true':'false'
                }
              }).then(res => {
                // console.log("/eos/communication/preplan/getContactor",res)
                if(res.data.data&&res.data.data.length!=0){
                  var temporarySmsMember=this.smsMember.filter(item=>{return item.isTableSelect==true})
                  res.data.data.forEach(item=>{
                    // if(item.mobile){
                    //   this.$emit('findfaxDataS', item);
                    // }

                    if(temporarySmsMember.filter(element=>{return element.id==item.id}).length==0){
                      if(!item.number){
                        if(this.temporaryContactorName!=(item.name?item.name:item.contactor)&&checked){
                          this.temporaryContactorName=item.name||item.contactor
                          contactorNameArr.push(item.name||item.contactor)
                          contactorNameString = contactorNameArr.join()
                          this.$message.warning(`${contactorNameString}联系号码为空`)
                          // this.$message.warning(`${item.name||item.contactor}联系号码为空`)
                        }
                       
                      }else{
                        temporarySmsMember.push(item)
                        this.$store.commit("setSmsMember", temporarySmsMember);
                      }
                    }
                  })
                }

              })
            })
          }

        }
      },
      // 每次树勾选状态改变都触发
      /* 获取导出联系人分组id
        @editor     xry
        @groupIds   勾选分组id
        @date  2020-9-14
      */
      handleTreeCheck(data,node){
        this.getContactor(data,node)
        console.log('handleTreeCheck树状分组选中了什么',data,node);
        var temporarySmsTreeList=this.smsTreeList
        temporarySmsTreeList['tree' + this.tabPosition]=node.checkedNodes
        console.log(temporarySmsTreeList)
        this.$store.commit("setSmsTreeList", temporarySmsTreeList);
        let groupIds = node.checkedKeys
        console.log('勾选到的分组id',groupIds)
         // 获取复选框分组id给子组件
         switch(this.tabPosition) {
                  case 1 :
                  this.$refs.treeBtn1.getGroupIds(groupIds)
                  break
                  case 5 :
                  this.$refs.treeBtn5.getGroupIds(groupIds)
                  break
                  case 6 :
                  this.$refs.treeBtn6.getGroupIds(groupIds)
                  break
                  case 7 :
                  this.$refs.treeBtn7.getGroupIds(groupIds)
                  break
                  case 8 :
                  this.$refs.treeBtn8.getGroupIds(groupIds)
                  break
                  case 10 :
                  this.$refs.treeBtn10.getGroupIds(groupIds)
                  break
            }
      },
      /*    获取选中联系人列表
            @editor     xry
            @seletecdContacList   勾选联系人列表
            @date  2020-9-14
      */	
      handleSelectionChange(val) {
        // console.log('列表选中了什么',val,this.smsMember)
        this.seletecdContacList = val
        this['formData' + this.tabPosition].tableMultipleSelection=val
        if(this.manageVisible){
          // this.$bus.$emit('clearNumber')
          // val.forEach(item=>{
          //   !item.mobile ? this.$message.warning("当前联系人电话为空") : this.$emit('findfaxDataS', item);
          // })
        }
      },
      handleSelect(selection, row){
        console.log('--当前列表行勾选了什么selection, row',selection, row, row.name, this.tabPosition)
        if (this.manageVisible) {
          this.disposeNumber(row)
          if(this.tabPosition==3){
            if(selection.filter(item=>{return item.mobile==row.mobile}).length==0){ //证明取消勾选
              var temporarySmsMember=this.smsMember.filter(item=>{return (item.number || item.call || item.mobile) != row.mobile})
              console.log('--temporarySmsMember ', temporarySmsMember)
              this.$store.commit("setSmsMember", temporarySmsMember);
            }else{
              var temporarySmsMember=this.smsMember
              if (!row.mobile || row.mobile.length !== 11) {
                this.$refs['elTable' + this.tabPosition].toggleRowSelection(row, false)
                return ;
              }
              if(temporarySmsMember.filter(element=>{return 
                (element.number ? element.number : element.call ? element.call : element.mobile) == row.mobile
                }).length==0)
              {
                if (row.mobile.length === 11) {
                  row.isTableSelect=true
                  temporarySmsMember.push(row)
                }
                this.$store.commit("setSmsMember", temporarySmsMember);
              }
            }
          }else{
            if(selection.filter(item=>{return item.id==row.id}).length==0){ //证明取消勾选
              var temporarySmsMember=this.smsMember.filter(item=>{return item.id!=row.id})
              this.$store.commit("setSmsMember", temporarySmsMember);
            }else{
              var temporarySmsMember=this.smsMember
              // let filter = temporarySmsMember.filter(item=>{
              //   return (item.id==row.id) || (item.number === row.number || row.call) || (item.call === row.number || row.call) 
              // })
              if(temporarySmsMember.filter(item=>{return item.id==row.id}).length==0){
                row.isTableSelect=true
                if(!row.number){
                  this.$refs['elTable' + this.tabPosition].toggleRowSelection(row, false)
                }else{
                  temporarySmsMember.push(row)
                  this.$store.commit("setSmsMember", temporarySmsMember);
                }
              }
            }
          }
        }
      },
      // 处理不存在号码情况并返回不存在的号码[0]和存在的号码[1]的数组
      disposeNumber(rows){
        console.log('--判断号码---', this.tabPosition, rows, rows.length)
        // 拿到存在的号码
        let inNumberArr = []
        // 处理不存在号码
        let notNumberArr = []
        let notNumberNameArr = []
        let notNumberNameArr2 = [] // notNumberNameArr2 存放是传真号码
        if (rows.length > 0) {
          rows.forEach(row =>{
            if (this.tabPosition === 3) {
              // 最近联系人
              if (row.mobile && row.mobile.length === 11) {
                // 电话号码
                inNumberArr.push(row);
              } else if (
                row.mobile && 
                row.mobile.length > 0 &&
                row.mobile.length !== 11
              ) {
                // 传真号码
                notNumberArr.push(row)
                if (row.name || row.name === ''){
                  notNumberNameArr2.push(row.name)
                } else {
                  notNumberNameArr2.push(row.contactor)
                }
              } else {
                // 没有号码
                notNumberArr.push(row)
                if (row.name || row.name === ''){
                  notNumberNameArr.push(row.name)
                } else {
                  notNumberNameArr.push(row.contactor)
                }
              }
              // 提示
              if (notNumberNameArr.length > 0 && notNumberNameArr2.length > 0){
                let notNumberString = notNumberNameArr.join()
                let notNumberString2 = notNumberNameArr2.join()
                console.log('手机号码为空:', notNumberString)
                console.log('号码格式不正确:', notNumberString2)
                this.$message({
                  type:'warning',
                  message:`(${notNumberString})手机号码为空;(${notNumberNameArr2})号码格式不正确`
                })
              } else if (notNumberNameArr.length > 0 && notNumberNameArr2.length === 0) {
                let notNumberString = notNumberNameArr.join()
                this.$message({
                  type:'warning',
                  message:`(${notNumberString})手机号码为空`
                })
              } else {
                let notNumberString2 = notNumberNameArr2.join()
                this.$message({
                  type:'warning',
                  message:`(${notNumberNameArr2})号码格式不正确`
                })
              }
            } else {
              if (row.number){
                inNumberArr.push(row)
              } else {
                notNumberArr.push(row)
                if (row.name || row.name === ''){
                  notNumberNameArr.push(row.name)
                } else {
                  notNumberNameArr.push(row.contactor)
                }
              }
              // 提示
              if (notNumberNameArr.length > 0){
                let notNumberString = notNumberNameArr.join()
                console.log('不存在电话号码的对象:', notNumberString)
                this.$message({
                  type:'warning',
                  message:`(${notNumberString})联系号码为空`
                })
              }
            }
          })
        } else {
          let row = rows
          if (this.tabPosition === 3) {
            // 最近联系人
            if (row.mobile && row.mobile.length > 0 && row.mobile.length !== 11) {
              // 传真号码
              this.$message({
                  type: 'warning',
                  message: `(${row.name ||
                      row.contactor})号码格式不正确`,
              });
              console.log('name:', name);
            } else if (!row.mobile || row.mobile === '') {
              // 没有号码
              this.$message({
                type:'warning',
                message:`(${row.name || row.contactor})手机号码为空`
              })
            }
          } else {
            if (!row.number || row.number === '') {
              console.log('---row:', row, row.name, row.contactor)
              // 提示
              this.$message({
                type:'warning',
                message:`(${row.name || row.contactor})手机号码为空`
              })
            }
          }
        }
        return [notNumberArr,inNumberArr]
      },
      toggleRowSelection() {
        if (this.tabPosition==3) {
          this['formData' + this.tabPosition].tableData.forEach(item => {
            if (!item.mobile || item.mobile.length !== 11) {
              this.$refs['elTable' + this.tabPosition].toggleRowSelection(item, false)
            }
          })
        } else {
          this['formData' + this.tabPosition].tableData.forEach(item => {
            if (!item.number) {
              this.$refs['elTable' + this.tabPosition].toggleRowSelection(item, false)
            }
          })
        }
      },
      /**
       * @LastEditor: hexinting
       * @LastEditorTime: 2020-9-14
       * @Description 获取表格数据时，将右侧已勾选的联系人 与 表格的联系人对应。若存在右侧已勾选联系人中，表格中对应的联系人勾选
       */
      selectedRow() {
        var arr = []; // 已存在右侧联系人中的表格数据
        var tableData = []; // 表格数据
        tableData = this['formData' + this.tabPosition].tableData;
        // console.log('smsMember:', this.smsMember);
        // console.log(
        //     'tableData:',
        //     this['formData' + this.tabPosition].tableData
        // );
        // console.log('tabPosition:', this.tabPosition);
        // 遍历右侧已勾选联系人，若表格中联系人的号码与右侧的相等，则表格行勾选
        this.smsMember.forEach(item => {
          for (let i in tableData) {
            // 因为有些接口返回的电话号码的参数名不一致，主要是 call,number,mobile(最近联系人)
            if (item.call && item.call === tableData[i].call ||
                item.number && item.number === tableData[i].call ||
                item.number && item.number === tableData[i].number ||
                item.mobile && item.mobile === tableData[i].number ||
                item.mobile && item.mobile === tableData[i].call ||
                item.mobile && item.mobile === tableData[i].mobile) {
                arr.push(tableData[i]);
            }
          }
        });
        // 表格行勾选
        this.$nextTick(() => {
          if (arr.length > 0) {
            arr.forEach(item => {
                this.$refs[
                    'elTable' + this.tabPosition
                ].toggleRowSelection(item, true);
            });
          }
        });
        console.log('常用联系人已勾选项：', arr);
      },
      // 应急联系人全选或取消全选
      selectTreeNodeContactor(selection) {
        this.toggleRowSelection()
        if (this.isSelectTree) {
          var temporarySmsMember=this.smsMember
          if (this['formData' + this.tabPosition].tableData.length > 0) {
            this.disposeNumber(this['formData' + this.tabPosition].tableData)
          }
          if(selection.length!=0){
            selection.forEach(item=>{
              if(temporarySmsMember.filter(element=>{return element.id==item.id}).length==0){
                item.isTableSelect=true
                if(!item.number){
                }else{
                  temporarySmsMember.push(item)
                  this.$store.commit("setSmsMember", temporarySmsMember);
                }
              }
            })
          }else{
            this['formData' + this.tabPosition].tableData.forEach(item=>{
              temporarySmsMember=temporarySmsMember.filter(element=>{return element.id!=item.id})
              this.$store.commit("setSmsMember", temporarySmsMember);
            })
          }
        } else {
          this.toggleRowSelection()
        }
        console.log("列表全选selection",selection)
        this.toggleRowSelection()
      },
      // 表格全选
      selectAll(selection){
        this.clickCount++;
        console.log('点击次数：', this.clickCount)
        console.log("列表全选selection",selection)
        if (this.manageVisible) {
          var temporarySmsMember=this.smsMember
          if (this.clickCount % 2 === 0) {
            this.toggleRowSelection()
            if (this['formData' + this.tabPosition].tableData.length > 0) {
              this.disposeNumber(this['formData' + this.tabPosition].tableData)
            }
            if(selection.length!=0){
              selection.forEach(item=>{
                if(this.tabPosition==3){
                  if(temporarySmsMember.filter(element => {
                    return (
                      (element.number || element.call || element.mobile) == item.mobile
                    );
                  }).length == 0){
                    item.isTableSelect=true
                    temporarySmsMember.push(item)
                    this.$store.commit("setSmsMember", temporarySmsMember);
                  }
                }else{
                  if(temporarySmsMember.filter(element=>{return element.id==item.id}).length==0){
                    item.isTableSelect=true
                    if(!item.number){
                      // this.$message.warning(`${item.name||item.contactor}联系号码为空`)
                    }else{
                      temporarySmsMember.push(item)
                      this.$store.commit("setSmsMember", temporarySmsMember);
                    }
                  }
                }

              })
            }else{
              if(this.tabPosition==3){
                this['formData' + this.tabPosition].tableData.forEach(item=>{
                  temporarySmsMember=temporarySmsMember.filter(element=>{return (element.number||element.call)!=item.id})
                  this.$store.commit("setSmsMember", temporarySmsMember);
                })
              }else{
                this['formData' + this.tabPosition].tableData.forEach(item=>{
                  temporarySmsMember=temporarySmsMember.filter(element=>{return element.id!=item.id})
                  this.$store.commit("setSmsMember", temporarySmsMember);
                })
              }
            }
          } else {
            this['formData' + this.tabPosition].tableData.forEach(item => {
              this.$refs['elTable' + this.tabPosition].toggleRowSelection(item, false);
              temporarySmsMember=temporarySmsMember.filter(element=>{return element.id != item.id})
              this.$store.commit("setSmsMember", temporarySmsMember);
            })
          }
        }
      },
      elDropdownItem(key) {
        if(key==1){
          this.showSpanImg=1
        }
        if(key==2){
          this.showSpanImg=2
        }
        if(key==3){
            this.showSpanImg=3
        }
        if(key==4){
            this.showSpanImg=4
        }
      },
      changeDropdown() {
        this.showSpanImg=0
      },
      handleCommand(row) { // 1>=手机2 row.otherTel  2>=办公电话 row.officeTel  3>=应急电话 row.homeTel 4>=其他电话 row.otherTel2
        let index=this.showSpanImg
        let number
        switch (index) {
          case 1:
              number = row.otherTel;
            break;
          case 2:
              number = row.officeTel;
            break;
          case 3:
            number = row.homeTel;
            break;
          case 4:
            number = row.otherTel2;
            break;
          default:
            number = '';
            break;
        }
        this['formData' + this.tabPosition].tableData.number=number
        this.$forceUpdate()
        // if(['',null].includes(number)){
        //   return
        // }
        // this.$confirm('确认拨出该号码？', '提示', {
        //     confirmButtonText: '确定',
        //     cancelButtonText: '取消',
        //     type: 'warning'
        //   }).then(() => {

        //     // this.$message({
        //     //   type: 'success',
        //     //   message: '确定!'
        //     // });

        //   }).catch(() => {
        //     this.$message({
        //       type: 'info',
        //       message: '已取消'
        //     });
        //   });
        console.log(number)
      },
      getRecentContactsTable(){
        //获取最近联系人
        this.$api.getRecentContactorList().then(res => {
          if (res.errorcode != 0) {
            this.$message({
              message: res.msg,
              error: "error"
            });
            return;
          }
          res.data.data.forEach(item=>{
            item.meetingType=''
          })
          setTimeout(()=>{
            console.log('执行定时器内容')
            this.formData3.tableData = res.data.data || [];
            this.formData3.total = res.data.totalElements;
            this.smsMember.forEach(item=>{
              if(this['formData' + this.tabPosition].tableData.filter(element=>{return element.mobile == (item.number ? item.number : item.call ? item.call : item.mobile)}).length!=0){
                this.$refs['elTable' + this.tabPosition].toggleRowSelection(this['formData' + this.tabPosition].tableData.filter(element=>{return element.mobile == (item.number ? item.number : item.call ? item.call : item.mobile)})[0],true);
              }
            })  
            this.selectedRow()
          },800)
        });
      },
      changePage1() {
        if (this.tabPosition1 == 2) {
          this.myData2 = {
            rowData: null,
            myLoading: false,
            tableData: [], //表格数据
            tableData1: [], //表格数据
            total: 0, //数据总数
            pageSize: 10 //总页数
          };
          // this.getTableData();
        }
      },
      gliaisonGroupList() {
        let data = {
          displayDefault: true
        };
        this.$api.gliaisonGroupList(data).then(res => {
          if (res.errorcode != 0) {
            this.$message({
              message: res.msg,
              type: "error"
            });
            return;
          }
          this.formData5.treeData = res.data || [];
        })
      },
      dutyGroupList() {
        let data = {
          displayDefault: true
        };
        this.$api.dutyGroupList(data).then(res => {
          if (res.errorcode != 0) {
            this.$message({
              message: res.msg,
              type: "error"
            });
            return;
          }
          this.formData6.treeData = res.data || [];
        })
      },
      getShortcutGroupData() {
        let data = {
          displayDefault: true //是否显示未分组联系人
        };
        this["formData" + this.tabPosition].myLoading = true;
        this.$api
          .shortcutGroupList(data)
          .then(res => {
            this["formData" + this.tabPosition].myLoading = false;
            if (res.errorcode != 0) {
              this.$message({
                message: res.msg,
                type: "error"
              });
              return;
            }
            this["formData" + this.tabPosition].treeData = res.data || [];
          })
      },
      addCallSms(row) {
        !row.mobile ? this.$message.warning("当前联系人电话为空") : this.$emit('findfaxDataS', row);
        // console.log(row)
      },
      refreshTable() {
        console.log('-------', this.tabPosition)
        if (this.tabPosition === 1 || this.tabPosition === 7) {
          this.getContactorList();
        } else if (this.tabPosition === 2) {
          this.getDataAl();
        } else if (this.tabPosition === 3) {
          this.getRecentContactsTable()
        } else if (this.tabPosition === 4) {
          this.getEmergencyAddressData();
        } else if (this.tabPosition === 5) {
          this.getContactorList();
        } else if (this.tabPosition === 6) {
          this.getContactorList();
        } else if (this.tabPosition === 9) {
          this.getContactorList();
        } else if (this.tabPosition === 10) {
          this.getContactorList();
        }
      },
      filterNode(value, data) {
        if (!value) return true;
        return data.name.indexOf(value) !== -1;
      },
      filterTexts() {
        this.$refs.tree1.filter(this.filterText);
      },
      /**
       * @lastEditor hexinting
       * @lastDate 2020-11-05
       * @description 切换分组
       */
      changePage(){
        this.manageVisible = true
        console.log("changePage切换 this.tabPosition",this.tabPosition,typeof this.tabPosition)
        if(this.tabPosition==2){
          this.formData2 ={//常用联系人
            searchText:'',//搜索
            currentPage:1,
            total: 0,//数据总数
            pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
            rowData:null,//表格选择数据
            tableData: [],//表格数据
            treeData: [],
            groupId: 0,//分组id
            myLoading: false,
            filterTree:'', //树状列表输入框过滤
            tableMultipleSelection:[], //列表勾选的选项
            treeMultipleSelection:[] //树状分组勾选的选项
          };
          this.getDataAl();
          // this.tabName = '常用联系人'
          this.tabName = 'commonlyused'
        }else if(this.tabPosition==3){
          this.formData3 ={//最近联系人
            searchText: '',//搜索
            currentPage: 1,
            total: 0,//数据总数
            pageSize: window.g.IsZengChengProject ? 40 : 20, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
            rowData: null,//表格选择数据
            tableData: [],//表格数据
            treeData: [],
            groupId: 0,//分组id
            myLoading: false,
            filterTree:'', //树状列表输入框过滤
            tableMultipleSelection:[], //列表勾选的选项
            treeMultipleSelection:[] //树状分组勾选的选项
          };
          this.getRecentContactsTable()
          // this.tabName = '最近联系人'
          this.tabName = 'recentcontact'
        }else if(this.tabPosition==8){
          this.formData8 ={//短信分组
            searchText: '',//搜索
            currentPage: 1,
            total: 0,//数据总数
            pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
            rowData: null,//表格选择数据
            tableData: [],//表格数据
            treeData: [],
            groupId: 0,//分组id
            myLoading: false,
            filterTree:'', //树状列表输入框过滤
            tableMultipleSelection:[], //列表勾选的选项
            treeMultipleSelection:[] //树状分组勾选的选项
          };
          this.getSmsGroupListTree(8);
          this.getDataRecent(8);
          // this.tabName = '短信分组'
          this.tabName = 'sms'
        }else if(this.tabPosition==4){
          this.formData4 = {//应急联动网
            searchText: '',//搜索
            currentPage: 1,
            total: 0,//数据总数
            pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
            rowData: null,//表格选择数据
            tableData: [],//表格数据
            myLoading: false,
            treeData: [],
            treeDataItem: {},
            filterTree:'', //树状列表输入框过滤
            tableMultipleSelection:[], //列表勾选的选项
            treeMultipleSelection:[] //树状分组勾选的选项
          }
          this.filterText ='';
          this.getEmergencyAddressBookTree()
          // this.tabName = this.IsYangZhouProject ? '应急通信录' : '应急联动网';
          this.tabName = 'linkage'
        }else if(this.tabPosition==5){
          this.formData5 = {//基层联络
            searchText: '',//搜索
            currentPage: 1,
            total: 0,//数据总数
            pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
            rowData: null,//表格选择数据
            tableData: [],//表格数据
            myLoading: false,
            treeData: [],
            treeDataItem: {},
            filterTree:'', //树状列表输入框过滤
            tableMultipleSelection:[], //列表勾选的选项
            treeMultipleSelection:[] //树状分组勾选的选项
          };
          this.gliaisonGroupList();
          // this.tabName = '基层联络网'
          this.tabName = 'gliaison'
        }else if(this.tabPosition==6){
          this.formData6 = {//备勤联络网
            searchText: '',//搜索
            currentPage: 1,
            total: 0,//数据总数
            pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
            rowData: null,//表格选择数据
            tableData: [],//表格数据
            myLoading: false,
            treeData: [],
            treeDataItem: {},
            filterTree:'', //树状列表输入框过滤
            tableMultipleSelection:[], //列表勾选的选项
            treeMultipleSelection:[] //树状分组勾选的选项
          };
          this.dutyGroupList();
          // this.tabName = '备勤联络网'
          this.tabName = 'dutyGroup'
        }else if(this.tabPosition==7){
          this.formData7 = {//资源联络网
            searchText: '',//搜索
            currentPage: 1,
            total: 0,//数据总数
            pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
            rowData: null,//表格选择数据
            tableData: [],//表格数据
            treeData: [],
            groupId: 0,//分组id
            myLoading: false,
            filterTree:'', //树状列表输入框过滤
            tableMultipleSelection:[], //列表勾选的选项
            treeMultipleSelection:[] //树状分组勾选的选项
          };
          this.getResourceGroupingTree(7);
          // this.tabName = '资源联络网'
          this.tabName = 'resource'
        } else if(this.tabPosition === 1) {
          this.formData1 = {//政务联络网
            searchText: '',//搜索
            currentPage: 1,
            total: 0,//数据总数
            pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
            rowData: null,//表格选择数据
            tableData: [],//表格数据
            treeData: [],
            groupId: 0,//分组id
            myLoading: false,
            filterTree:'', //树状列表输入框过滤
            tableMultipleSelection:[], //列表勾选的选项
            treeMultipleSelection:[] //树状分组勾选的选项
          };
          // this.tabName = this.IsYangZhouProject ? "政务通信录" : "政务联络网"
          this.tabName = 'phone'
          this.getPhoneGroupingTree();
          this.getContactorList()
        }else if(this.tabPosition==9){
          this.formData9 = {//工业园区管委会通讯录
            searchText: '',//搜索
            currentPage: 1,
            total: 0,//数据总数
            pageSize: window.g.IsZengChengProject ? 40 : 10, //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
            rowData: null,//表格选择数据
            tableData: [],//表格数据
            myLoading: false,
            treeData: [],
            treeDataItem: {},
            filterTree:'', //树状列表输入框过滤
            tableMultipleSelection:[], //列表勾选的选项
            treeMultipleSelection:[] //树状分组勾选的选项
          }
          this.filterText ='';
          this.getSuzhouGroupList()
          // this.tabName = '工业园区管委会通讯录'
          this.tabName = 'suzhouGroup'
        }else if(this.tabPosition==10){
          console.log(this.formData10)
          // this.formData10 = {//快捷组
          //   searchText: '',//搜索
          //   currentPage: 1,
          //   total: 0,//数据总数
          //   pageSize: 10,//总页数
          //   rowData: null,//表格选择数据
          //   tableData: [],//表格数据
          //   myLoading: false,
          //   treeData: [],
          //   treeDataItem: {},
          //   filterTree:'', //树状列表输入框过滤
          //   tableMultipleSelection:[], //列表勾选的选项
          //   treeMultipleSelection:[] //树状分组勾选的选项
          // }
          this.getShortcutGroupData()
          // this.tabName = '快捷组'
          this.tabName = 'quick'
        }
        setTimeout(() => {
          this.refreshTable(); // 修复禅道7057 author:hexinting date:2020-9-27
        }, 500)
      },
      handleSearch() {//搜索
        this['formData' + this.tabPosition].currentPage = 1//点击搜索就从0页开始查询
        this['formData' + this.tabPosition].groupId=''

        if (this.tabPosition == 1) {
          this.getContactorList('search');
        } else if (this.tabPosition == 2) {
          this.getDataAl()
        } else if(this.tabPosition == 3){
          this.getRecentContactsTable()
        } else if (this.tabPosition == 8) {
          this.getDataRecent(8);
        } else if (this.tabPosition == 4) {
          this.getEmergencyAddressData()
        } else if (this.tabPosition == 5) {
          this.getContactorList('search');
        } else if (this.tabPosition == 6) {
          this.getContactorList('search');
        } else if (this.tabPosition == 7) {
          this.getContactorList('search');
        } else if (this.tabPosition == 9) {
          this.getContactorList('search');
        } else if (this.tabPosition === 10) {
          this.getContactorList('search');
        }

      },
      addList() {//新增
        this.$nextTick(() => {
          this.$refs.ContactDetailsPopup.isDialogVisible = true;
          this.$refs.ContactDetailsPopup.title = "新增联系人";
          this.$refs.ContactDetailsPopup.getTree(this.tabPosition, this.groupId, this.tabName);
        });
      }
      ,
      //修改方法
      handleEdit() {
        if (!this['formData' + this.tabPosition].rowData) {
          this.$message({
            message: "请选中表格中任意行或双击任意行进行修改!",
            type: "warning"
          });
          return;
        } else {
          this.handleRowDbClick(this['formData' + this.tabPosition].rowData);
        }

      },
      handleBatchJoin() {
         let array = [
          {tabPosition: 1, value: 'governmentAffairs', remark: this.IsYangZhouProject ? "政务通信录" : "政务联络网"},
          {tabPosition: 5, value: 'primaryLevel', remark: '基层联络网'},
          {tabPosition: 6, value: 'spareTime', remark: '备勤联络网'},
          {tabPosition: 7, value: 'resource', remark: '资源通讯录'},
          {tabPosition: 8, value: 'sms', remark: '短信分组'},
          {tabPosition: 9, value: 'suzhou', remark: '工业园区管委会通讯录'},
          {tabPosition: 10, value: 'quick', remark: '快捷组'}
        ];
        let item = array.find(m => m.tabPosition === this.tabPosition);
        if (item) {
          this.$store.commit('setGroupType', item.value);
          let groupId = this[`formData${this.tabPosition}`].groupId || 0;
          this.$store.commit('setGroupId', groupId);
        }
      },
      // 导入联系人成功回调
      batchJoinSuccess() {
        this.refreshData()
      },
      editTreeItem(node, data){
        console.log("编辑什么node,data",node,data)
        if(this.tabPosition==2||this.tabPosition==3){
          this.tabPosition=1
        }
        this['formData' + this.tabPosition].currentPage = 1;//跳转到第一页
        this['formData' + this.tabPosition].groupId = data.id;
        this['formData' + this.tabPosition].searchText = "";
        this['formData' + this.tabPosition].treeDataItem = {
          groupId: data.id,
          currentGroupName: data.name
        }

        console.log(data);
        this.$store.commit('setGroupId', data.id)
        this.groupId = data.id

        setTimeout(()=>{
          this.$refs['treeBtn'+this.tabPosition].updateGroup()
        },500)
      },
      deleteTreeItem(node, data){
        console.log("删除什么node,data",node,data)
        if(this.tabPosition==2||this.tabPosition==3){
          this.tabPosition=1
        }
        this['formData' + this.tabPosition].currentPage = 1;//跳转到第一页
        this['formData' + this.tabPosition].groupId = data.id;
        this['formData' + this.tabPosition].searchText = "";
        this['formData' + this.tabPosition].treeDataItem = {
          groupId: data.id,
          currentGroupName: data.name
        }

        console.log(data);
        this.$store.commit('setGroupId', data.id)
        this.groupId = data.id

        setTimeout(()=>{
          this.$refs['treeBtn'+this.tabPosition].deleteGroup()
        },500)
      },
      getContactor(data, node) {
        if(this.tabPosition==2||this.tabPosition==3){
          this.tabPosition=1
        }
        this['formData' + this.tabPosition].currentPage = 1;//跳转到第一页
        this['formData' + this.tabPosition].groupId = data.id;
        this['formData' + this.tabPosition].searchText = "";
        this['formData' + this.tabPosition].treeDataItem = {
          groupId: data.id,
          currentGroupName: data.name
        }
        if (this.tabPosition == 1) {
          this.getContactorList()

        } else if (this.tabPosition == 4) {
          //如果没有父级节点，则查应急通讯录
          if (node.parent != null && node.parent.parent != null) {
            this['formData' + this.tabPosition].isTop = 'false';
          } else {
            this['formData' + this.tabPosition].isTop = 'true';
          }
          this.getContactorList()
          // this.getEmergencyAddressData()
        } else if (this.tabPosition == 5) {

          this.getDataRecent(5);

        } else if (this.tabPosition == 6) {

          this.getDataRecent(6);

        } else if (this.tabPosition == 7) {

          this.getContactorList();
        } else if (this.tabPosition == 8) {
          this.getDataRecent(8);
        } else if (this.tabPosition == 9) {
          this.getContactorList();
        } else if (this.tabPosition == 10) {
          this.getContactorList();
        }
      },
      handleNodeClick(data, node) {
        this.clickCount = 1
        this.isSelectTree = false
        this.getContactor(data, node) // 获取联系人
        this.tableCheckStatus(true) // 切换表格勾选状态
        console.log(data);
        this.$store.commit('setGroupId', data.id)
        this.groupId = data.id
        sessionStorage.setItem('PID', data.parentId);
      },
      //获取联系人数据
      getContactorList(type, row) {
        if (this.tabPosition !== 4) {
          if (!row) {
            this['formData' + this.tabPosition].rowData = null;
          }
          let data;
          var dictType
          switch(this.tabPosition){
            case 1:
              dictType='governmentAffairs'
              break;
            case 5:
              dictType='primaryLevel'
              break;
            case 6:
              dictType='spareTime'
              break;
            case 7:
              dictType='resource'
              break;
            case 8:
              dictType='sms'
              break;
            case 9:
              dictType='suzhouGroup'
              break;
            case 10:
            dictType='quick'
            break;
          }
          if (this.tabPosition == 1) {
            // bug9183 author:hexinting date:20201207
            data = window.g.IsZengChengProject ? {
              groupId: type?" ":this['formData' + this.tabPosition].groupId,
              page: this['formData' + this.tabPosition].currentPage,
              size: this['formData' + this.tabPosition].pageSize,
              searchText: this['formData' + this.tabPosition].searchText,
              type: 'sms',
              dictType:dictType,
              thisGroupOnly: true
            } : {
              groupId: type?" ":this['formData' + this.tabPosition].groupId,
              page: this['formData' + this.tabPosition].currentPage,
              size: this['formData' + this.tabPosition].pageSize,
              searchText: this['formData' + this.tabPosition].searchText,
              type: 'sms',
              dictType:dictType
            };
          } else {
            data = window.g.IsZengChengProject ? {
              groupId: type?" ":this['formData' + this.tabPosition].groupId,
              page: this['formData' + this.tabPosition].currentPage,
              size: this['formData' + this.tabPosition].pageSize,
              searchText: this['formData' + this.tabPosition].searchText,
              type:'sms',
              dictType:dictType,
              thisGroupOnly: true
            } : {
              groupId: type?" ":this['formData' + this.tabPosition].groupId,
              page: this['formData' + this.tabPosition].currentPage,
              size: this['formData' + this.tabPosition].pageSize,
              searchText: this['formData' + this.tabPosition].searchText,
              type:'sms',
              dictType:dictType
            };
          }

          this.$api.contactorList(data).then(res => {
            console.log(res)
            if (res.errorcode != 0) {
              this.$message({
                message: res.msg,
                type: "error"
              });
              return;
            }
            if (row) {
              setTimeout(() => {
                res.data.list.forEach(item => {
                  if (item.id === row.id) {
                    row.commonlyUsed = item.commonlyUsed
                  }
                })
              }, 100)
            } else {
              this['formData' + this.tabPosition].tableData = res.data.list || [];
              this['formData' + this.tabPosition].total = res.data.totalCount || 0;
            }
          })
        } else {
          this.getEmergencyAddressData();
        }
      },
      // 表格勾选状态 checked-> false:取消全选（有电话号码的联系人）,true:全选（有电话号码的联系人）
      tableCheckStatus(checked) {
        if(checked) {
          setTimeout(()=>{
            this.smsMember.forEach(item=>{
              if(this['formData' + this.tabPosition].tableData.filter(element=>{return element.id==item.id}).length!=0){
                this.$refs['elTable' + this.tabPosition].toggleRowSelection(this['formData' + this.tabPosition].tableData.filter(element=>{return element.id==item.id})[0],true);
              }
            })
          },800)
        } 
        //下面else 有逻辑错误
        // else {
        //   setTimeout(()=>{
        //     this['formData' + this.tabPosition].tableData.forEach(item => {
        //       this.$refs['elTable' + this.tabPosition].toggleRowSelection(item, false);
        //       this.smsMember.forEach((element, index) => {
        //         if (element.id==item.id) {
        //           this.smsMember.splice(index,1)
        //         }
        //       })
        //     })
        //   },800)
        // }
      },
      refreshData() {
        if (this.tabPosition == 1) {
          this.treeDataItem = {};
          this.getPhoneGroupingTree();
        } else if (this.tabPosition == 8) {
          this.formData8.treeDataItem = {};
          this.getSmsGroupListTree(8)

        } else if (this.tabPosition == 5) {
          this.formData5.treeDataItem = {};
          this.gliaisonGroupList();

        } else if (this.tabPosition == 6) {
          this.formData6.treeDataItem = {};
          this.dutyGroupList();

        } else if (this.tabPosition == 7) {
          this.formData7.treeDataItem = {};
          this.getResourceGroupingTree(7);
        } else if (this.tabPosition === 9) {
            this.formData9.treeDataItem = {};
            this.getSuzhouGroupList();
        } else if (this.tabPosition === 10) {
            this.formData10.treeDataItem = {};
            this.getShortcutGroupData();
        }
      },
      //应急通讯录架构树
      getEmergencyAddressBookTree() {
        let data = {
          id: 0,
        };
        let CallbackReload = this;
        this.formData4.myLoading = true;
        this.$api.getBriefsByClassId(data, CallbackReload).then(result => {
          this.formData4.myLoading = false;
          if (result) {
            this.formData4.treeData = [];
            for (let n in result.data) {
              this.formData4.treeData.push({
                id: result.data[n].id,
                name: result.data[n].name + '通讯录',
                versionId: result.data[n].versionId,
                children: []
              });
              let data = {
                id: result.data[n].versionId,
              };
              this.$api.getStructTree(data, this).then(res => {
                this.formData4.treeData[n].children = res.data;
              })
              // 默认选择第一个分组 禅道7050 author:hexinting date:2020-9-27
              this.formData4.isTop = true // 默认是一级分组
              this.formData4.treeDataItem.groupId = this.formData4.treeData[0].id;
              console.log('应急联动网id', this.formData4.treeDataItem.groupId)
            }
          }
        }).catch(err => {
          this.formData4.myLoading = false;
        })

      },
        /**
          * @author hexinting
          * @date 2021-1-22
          * @param {String} ids 联系人id
          * @param {String} groupId 联系人分组id
          * @param {String} deleteMoreorOne one的时候点开的是联系人后面的单选删除,more的时候是多选的删除
          * @param {Array} seletecdContacList 勾选中的联系人所有信息
          * @param {String} seletecdContacIds 处理好的联系人id
          * @description 删除分组关系联系人
          */
        deleteGroupOne() {
          let key = `formData${this.tabPosition}`
          console.log(this[key])
          if(this.deleteMoreorOne == 'one'){
              console.log('分组中删除')
              // 未分组联系人只能彻底删除,因为未分组
              if(this[key].rowData.groupId <= 0) {
                this.$message.error('未分组联系人没有分组关系只能彻底删除');
                return
              }
              let data = {
                  groupId:this[key].rowData.groupId,
                  ids: [this[key].rowData.id],
              };
              this.$api.deleteContactorGroupRelLower(data).then(res => {
                  if (res.errorcode !== 0) {
                      this.$message.error(res.msg);
                      this.deleteBtnFlag = false
                      return;
                  }
                  this.$message.success('删除成功!');
                  this.getContactorList();
                  this.deleteBtnFlag = false
              });
          }
          if(this.deleteMoreorOne == 'more'){
                // 处理联系人id
                this.seletecdContacIds = []
                if(this.seletecdContacList.length >= 1) {
                  this.seletecdContacList.map(ele=>{
                      this.seletecdContacIds.push(ele.id)
                  })
                }
                let data = {
                    groupId:this[key].groupId,
                    ids: this.seletecdContacIds,
                }
                this.$api.deleteContactorGroupRelLower(data).then(res => {
                    if (res.errorcode !== 0) {
                        this.$message.error(res.msg);
                        this.deleteBtnFlag = false
                        return;
                    }
                    this.$message.success('删除成功!');
                    this.getContactorList();
                    this.deleteBtnFlag = false
                });
          }
        },
        /**
        * @author hexinting
        * @date 2021-1-18
        * @param {String} deleteMoreorOne one的时候点开的是联系人后面的单选删除,more的时候是多选的删除
        * @description 彻底删除联系人
        */
        deleteGroupAll() {
          console.log('彻底删除')
          let key = `formData${this.tabPosition}`
          if(this.deleteMoreorOne == 'one'){
            console.log(this[key].rowData.id)
            let data = {
              data: { id: this[key].rowData.id },
              operate: 2,
            };
            this.$api.updateConcator(data).then(res => {
              if (res.errorcode !== 0) {
                this.$message.error(res.msg);
                this.deleteBtnFlag = false
                return;
              }
              this.$message.success('删除成功!');
              this.getContactorList();
              this.deleteBtnFlag = false
            });
          }
          if(this.deleteMoreorOne == 'more'){
            this.$http({
              url: `${window.g.ApiUrl}/eos/communication/contactor/deleteContactor`,
              method: 'post',
              data: {
                ids: this[key].tableMultipleSelection.map(
                    elem => elem.id
                ),
              },
            }).then(({ data }) => {
              console.log(
                '/eos/communication/contactor/deleteContactor',
                data
              );
              if (data.errorcode !== 0) {
                this.$message.error(data.msg);
                return;
              }
              this.$message.success('删除成功!');
              this.deleteBtnFlag = false
              if ([1, 5, 6, 7, 8, 9, 10].includes(this.tabPosition)){
                this.getContactorList();
              }
            })
          }
        },
      /**
       * @lastEditor hexinting
       * @date 2021-1-18
       * @description 删除单个列表选项
       */
      handleDeleteItem() {
        if (this.tabPosition == 1 || this.tabPosition == 8 || this.tabPosition == 5 || this.tabPosition == 6 || this.tabPosition == 7|| this.tabPosition == 9|| this.tabPosition == 10) {
          if (!this['formData' + this.tabPosition].rowData) {
            this.$message({
              message: "请选中表格中任意行进行删除!",
              type: "warning"
            });
            return;
          } else {
            // openLab项目 删除逻辑修改，删除前提示是“删除当前分组联系人” 还是“删除所有分组的该联系人”
            if (this.IsOpenLabProject) {
              this.deleteBtnFlag = true;
              this.deleteMoreorOne = 'one';
              return false;
            }

            let data = {
              data: {id: this['formData' + this.tabPosition].rowData.id},
              operate: 2,
            };
            this.$confirm('删除当前信息?', '提示', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
            }).then(() => {
              this.$api.updateConcator(data).then(res => {
                if (res.errorcode != 0) {
                  this.$message({
                    message: res.msg,
                    type: "error"
                  });
                  return;
                }
                this.$message({
                  message: "删除成功!",
                  type: "success"
                });
                this.getContactorList();
              });
            }).catch(() => {
              this.$message({
                type: 'info',
                message: '已取消删除!'
              });
            });
          }
        } else if (this.tabPosition == 2) {
          if (this.formData2.rowData == "" || this.formData2.rowData == null) {
            this.$message({
              message: "请选中表格中任意行进行取消收藏!",
              type: "warning"
            });
            return;
          } else {
            let data = {
              contactorId: this.formData2.rowData.id
            };

            this.$confirm('是否将该联系人从常用联系人中移出', '移出常用联系人', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
            }).then(() => {
              this.$api.removeCommonContactor(data).then(res => {
                if (res.errorcode != 0) {
                  this.$message({
                    message: res.msg,
                    type: "error"
                  });
                  return;
                }
                this.$message({
                  message: "移出成功!",
                  type: "success"
                });
                this.getDataAl();
              });
            }).catch(() => {
              this.$message({
                type: 'info',
                message: '已取消!'
              });
            });
          }
        }
      },
      /**
       * @lastEditor hexinting
       * @date 2021-1-18
       * @description 批量删除多个列表选项
       */
      handleDelete() {
        if (this.tabPosition == 1 || this.tabPosition == 8 || this.tabPosition == 5 || this.tabPosition == 6 || this.tabPosition == 7|| this.tabPosition == 9|| this.tabPosition == 10) {
          if (this['formData' + this.tabPosition].tableMultipleSelection.length==0) {
            this.$message({
              message: "请勾选表格中任一行进行删除!",
              type: "warning"
            });
            return;
          } else {
            // openLab项目 删除逻辑修改，删除前提示是“删除当前分组联系人” 还是“删除所有分组的该联系人”
            if (this.IsOpenLabProject) {
              this.deleteBtnFlag = true;
              this.deleteMoreorOne = 'more';
              return false;
            }

            this.$confirm('删除当前信息?', '提示', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
            }).then(() => {
              this.$http({
                url: window.g.ApiUrl + '/eos/communication/contactor/deleteContactor',
                method: 'post',
                data:{
                  ids:this['formData' + this.tabPosition].tableMultipleSelection.map(function (elem) {
                    return elem.id
                  })
                }
              }).then(({data}) => {
                console.log("/eos/communication/contactor/deleteContactor",data)
                if (data.errorcode != 0) {
                  this.$message({
                    message: data.msg,
                    type: "error"
                  });
                  return;
                }
                this.$message({
                  message: "删除成功!",
                  type: "success"
                });
                this.getContactorList();
              })


            }).catch(() => {
              this.$message({
                type: 'info',
                message: '已取消删除!'
              });
            });
          }
        } else if (this.tabPosition == 2) {
          if (this.formData2.tableMultipleSelection.length==0) {
            this.$message({
              message: "请选中表格中任一行进行删除!",
              type: "warning"
            });
            return;
          } else {
            this.$confirm('删除当前信息?', '提示', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
            }).then(() => {
              this.$http({
                url: window.g.ApiUrl + '/eos/communication/contactor/batchRemoveCommonContactor',
                method: 'post',
                data:{
                  contactorIds:this['formData' + this.tabPosition].tableMultipleSelection.map(function (elem) {
                    return elem.id
                  })
                }
              }).then(({data}) => {
                console.log("/eos/communication/contactor/batchRemoveCommonContactor",data)
                if (data.errorcode != 0) {
                  this.$message({
                    message: data.msg,
                    type: "error"
                  });
                  return;
                }
                this.$message({
                  message: "删除成功!",
                  type: "success"
                });
                this.getDataAl();
              })
            }).catch(() => {
              this.$message({
                type: 'info',
                message: '已取消删除!'
              });
            });
          }
        }
      },
      // 选择联系人
      handleSelectContactor() {
        this.addContactVisible = true;
        this.$nextTick(() => {
          this.$refs.addContact.getDataList('getSmsContact', this.smsMember);
          console.log('this.smsMember: ', this.smsMember);
        });
        this.$store.commit('setSmsOrFax', 'sms');
      },
      //排序方法
      handleSort() {
        this.$nextTick(() => {
          this.$refs.ContactSortingPopup.isDialogVisible = true;
          this.$refs.ContactSortingPopup.getTree();
        });
      },
      /* 导出联系人
        @Author xry
        @groupId 不是打勾，而是选中的分组id
        @seletecdContacList 勾选中的联系人所有信息
        @seletecdContacIds 处理好的联系人id
        @date 2020-9-14
      */
      handleExportContactor() {
        let key = `formData${this.tabPosition}`;
        console.log(this.seletecdContacList)
          // 2020-9-14修改接口后groupId为非必填项
          // if (!this[key].groupId) {
          //         this.$message.warning('请先选择一个分组');
          //         return;
          //     }
            if (this.seletecdContacList.length == 0) {
                this.$message.warning('请先选择联系人');
                return;
            }
            console.log(this[key].groupId);  // 最后一个点击分组的id,和复选框勾选无关 :导出联系人需要
            // 处理联系人id
            this.seletecdContacIds = []
            if(this.seletecdContacList.length >= 1) {
              this.seletecdContacList.map(ele=>{
                  this.seletecdContacIds.push(ele.id)
              })
            }
            console.log(this.seletecdContacIds)
            // 1.原方法
            // window.location.href = `${window.g.ApiUrl}/eos/communication/contactor/export?groupIds=${this[key].groupId}&contactorIds=${this.seletecdContacIds}`;
            // 2.携带验证的方法
            let exportURL = this[key].groupId ? `${window.g.ApiUrl}/eos/communication/contactor/export?groupIds=${this[key].groupId}&contactorIds=${this.seletecdContacIds}` 
            :  `${window.g.ApiUrl}/eos/communication/contactor/export?contactorIds=${this.seletecdContacIds}`
            var xhr = new XMLHttpRequest();
            xhr.open('GET', (exportURL), true);      
            xhr.withCredentials = true
            xhr.responseType = "blob";    // 返回类型blob
            let token = localStorage.getItem("token")
            if(token == null) {
              token = ''
            }
            xhr.setRequestHeader('Authorization', token)
            // 定义请求完成的处理函数，请求前也可以增加加载框/禁用下载按钮逻辑
            xhr.onload = function () {
                // 请求完成
                if (this.status === 200) {
                    // 返回200
                    var blob = this.response;
                    var reader = new FileReader();
                    reader.readAsDataURL(blob);    // 转换为base64，可以直接放入a标签href
                    reader.onload = function (e) {
                        // 转换完成，创建一个a标签用于下载
                        var a = document.createElement('a');
                        a.download = '联系人表.xlsx';
                        a.setAttribute('target','_blank')
                        a.setAttribute('target','_self')
                        a.href = e.target.result;
                        document.body.append(a);    // 修复firefox中无法触发click
                        console.log(a);
                        a.click();
                        // document.body.remove(a);
                    }
                }
            };
            xhr.send()
      },
      //表格行点击方法
      handleRowClick(row) {
        this['formData' + this.tabPosition].rowData = row;
      },
      closeDialog(key) {
        this.$emit('closeDialog', key);
      },
      //电话分组树
      getPhoneGroupingTree(key) {
        let data = {
          displayDefault: true //是否显示未分组联系人
        };

        this.$api.telephoneGroupingTree(data).then(res => {

          if (res.errorcode != 0) {
            this.$message({
              message: res.msg,
              type: "error"
            });
            return
          }
          if (key) {
            this['formData' + this.tabPosition].treeData = res.data || [];
          } else {
            this['formData' + this.tabPosition].treeData = res.data || [];
          }

        })
      },
      handSetToCommon(row) {
        if(row.commonlyUsed==0){
          let data = { contactorId: row.id };
          this.$api.usedContactor(data).then(res => {
            if (res.errorcode != 0) {
              this.$message.error(res.msg);
              return;
            }
            this.$message.success("已设为常用联系人!");
            if (this.tabPosition == 1) {
              this.getContactorList(0, row)
            } else if (this.tabPosition == 4) {
              this.getEmergencyAddressData()
            } else if (this.tabPosition == 8) {
              this.getDataRecent(8, 1);
            } else if (this.tabPosition == 5) {
              this.getDataRecent(5, 1);
            } else if (this.tabPosition == 6) {
              this.getDataRecent(6, 1);
            } else if (this.tabPosition == 7) {
              this.getContactorList(0, row);
            } else if (this.tabPosition == 9) {
              this.getContactorList(0, row);
            } else if (this.tabPosition == 10) {
              this.getContactorList(0, row);
            }
          });
        }else{
          let data = {
            contactorId: row.id
          };
          this.$confirm("是否将该联系人从常用联系人中移出", "移出常用联系人", {
            confirmButtonText: "确定",
            cancelButtonText: "取消",
            type: "warning"
          })
            .then(() => {
              this.$api.removeCommonContactor(data).then(res => {
                if (res.errorcode != 0) {
                  this.$message({
                    message: res.msg,
                    type: "error"
                  });
                  return;
                }
                this.$message({
                  message: "移出成功!",
                  type: "success"
                });

                if (this.tabPosition == 1) {
                  this.getContactorList(0, row)
                } else if (this.tabPosition == 4) {
                  this.getEmergencyAddressData()
                } else if (this.tabPosition == 8) {
                  this.getDataRecent(8, 1);
                } else if (this.tabPosition == 5) {
                  this.getDataRecent(5, 1);
                } else if (this.tabPosition == 6) {
                  this.getDataRecent(6, 1);
                } else if (this.tabPosition == 7) {
                  this.getContactorList(0, row);
                } else if (this.tabPosition == 9) {
                  this.getContactorList(0, row);
                } else if (this.tabPosition == 10) {
                  this.getContactorList(0, row);
                }
              });
            })
            .catch(() => {
              this.$message({
                type: "info",
                message: "已取消!"
              });
              return;
            });
        }


      },
      handleRowDbClick(row) {//双击修改

        this.$nextTick(() => {
          this.$refs.ContactDetailsPopup.isDialogVisible = true;
          this.$refs.ContactDetailsPopup.title = "修改联系人";
          this.$refs.ContactDetailsPopup.getTree();
          this.$refs.ContactDetailsPopup.showData(row);
        });
      },
      //改变页数查询
      handCurrentChange(val) {
        this['formData' + this.tabPosition].currentPage = val;
        if (this.tabPosition == 1) {
          this.getContactorList();
        } else if (this.tabPosition == 2) {
          this.getDataAl();
        } else if (this.tabPosition == 5) {
          this.getContactorList();
        } else if (this.tabPosition == 4) {
          this.getEmergencyAddressData()
        } else if (this.tabPosition == 6) {
          this.getContactorList();
        } else if (this.tabPosition == 7) {
          this.getContactorList();
        } else if(this.tabPosition == 8){
          this.getDataRecent(8)
        } else if(this.tabPosition == 9){
          this.getContactorList();
        } else if(this.tabPosition == 10){
          this.getContactorList();
        }
        this.tableCheckStatus(true)
      },
      //改变每页条数查询
      handSizeChange(val) {
        this['formData' + this.tabPosition].currentPage = 1;
        this['formData' + this.tabPosition].pageSize = val;
        if (this.tabPosition == 1) {
          this.getContactorList();
        } else if (this.tabPosition == 2) {
          this.getDataAl();
        } else if (this.tabPosition == 5) {
          this.getContactorList();
        } else if (this.tabPosition == 4) {
          this.getEmergencyAddressData()
        } else if (this.tabPosition == 6) {
          this.getContactorList();
        } else if (this.tabPosition == 7) {
          this.getContactorList();
        } else if(this.tabPosition == 8){
          this.getDataRecent(8)
        } else if(this.tabPosition == 9){
          this.getContactorList();
        } else if(this.tabPosition == 10){
          this.getContactorList();
        }
        this.tableCheckStatus(true)
      },
      getDataAl() {//获取常用联系人
        let data = {
          searchText: this.formData2.searchText,
          page: this.formData2.currentPage,
          size: this.formData2.pageSize,
          type: 'sms'
        };
        this.$api.commonContactorList(data).then(res => {
          if (res.errorcode != 0) {
            this.$message({
              message: res.msg,
              error: "error"
            });
            return;
          }
          this.formData2.tableData = res.data.data || [];
          this.formData2.total = res.data.totalElements || 0;
          setTimeout(()=>{
            this.smsMember.forEach(item=>{
              if(this['formData' + this.tabPosition].tableData.filter(element=>{return element.id==item.id}).length!=0){
                this.$refs['elTable' + this.tabPosition].toggleRowSelection(this['formData' + this.tabPosition].tableData.filter(element=>{return element.id==item.id})[0],true);
              }
            })
          },800)
        })
      },
      //获取短信分组
      getSmsGroupListTree(key) {
        let data = {
          displayDefault: true
        };
        this.$api.smsGroupList(data).then(res => {
          if (res.errorcode != 0) {
            this.$message({
              message: res.msg,
              type: "error"
            });
            return;
          }

          this['formData' + key].treeData = res.data || [];
        })
      },
      //获取应急通讯录联系人
      getEmergencyAddressData() {
        let data = {
          searchText: this.formData4.searchText,
          id: this.formData4.treeDataItem.groupId,
          page: this.formData4.currentPage,
          size: this.formData4.pageSize,
          isTop: this.formData4.isTop,
          type: "phone"
        };
        if (this.formData4.treeDataItem.groupId) {
          data.id = this.formData4.treeDataItem.groupId;
        } else {
          this.$message({
            message: "请先选择节点",
            type: "warning"
          });
          return;
        }

        this.$api.emergencyAddressBook(data).then(res => {
          if (res.errorcode != 0) {
            this.message({
              message: res.msg,
              type: "error"
            });
            return;
          }
          console.log(res.data.totalElements);
          console.log(res.data.data);
          this.formData4.tableData = res.data.data || [];
          this.formData4.total = res.data.totalElements || 0;
          this.selectTreeNodeContactor(res.data.data)
          this.selectedRow()
        });
      },
      // type: 设置常用联系人状态 1
      getDataRecent(key, type) {//获取短信分组
        console.log(key);
        var dictType
        switch(this.tabPosition){
          case 1:
            dictType='governmentAffairs'
            break;
          case 5:
            dictType='primaryLevel'
            break;
          case 6:
            dictType='spareTime'
            break;
          case 7:
            dictType='resource'
            break;
          case 8:
            dictType='sms'
            break;
          case 9:
              dictType='suzhouGroup'
              break;
          case 10:
            dictType='quick'
            break;
        }
        // bug9183 author:hexinting date:20201207
        let data = window.g.IsZengChengProject ? {
          searchText: this['formData' + key].searchText,
          groupId: this['formData' + key].groupId,
          page: this['formData' + key].currentPage,
          size: this['formData' + key].pageSize,
          type: 'sms',
          dictType:dictType,
          thisGroupOnly: true
        } : {
          searchText: this['formData' + key].searchText,
          groupId: this['formData' + key].groupId,
          page: this['formData' + key].currentPage,
          size: this['formData' + key].pageSize,
          type: 'sms',
          dictType:dictType
        };

        this.$api.contactorList(data).then(res => {
          if (res.errorcode != 0) {
            this.$message({
              message: res.msg,
              error: "error"
            });
            return;
          }
          if (type) {
            setTimeout(() => {
              res.data.list.forEach(item => {
                if (item.id === row.id) {
                  row.commonlyUsed = item.commonlyUsed
                }
              })
            }, 100)
          } else {
            this['formData' + key].tableData = res.data.list || [];
            this['formData' + key].total = res.data.totalCount || 0;
            setTimeout(()=>{

              this.smsMember.forEach(item=>{
                if(this['formData' + this.tabPosition].tableData.filter(element=>{return element.id==item.id}).length!=0){
                  this.$refs['elTable' + this.tabPosition].toggleRowSelection(this['formData' + this.tabPosition].tableData.filter(element=>{return element.id==item.id})[0],true);
                }
              })
            },800)
          }
        })
      },
      getResourceGroupingTree(key) {
        let data = {
          displayDefault: true //是否显示未分组联系人
        };
        this['formData' + this.tabPosition].myLoading = true;
        this.$api.resourceGroupList(data).then(res => {
          this['formData' + this.tabPosition].myLoading = false;
          if (res.errorcode != 0) {
            this.$message({
              message: res.msg,
              type: "error"
            });
            return
          }
          this['formData' + this.tabPosition].treeData = res.data || [];


        }).catch(err => {
          this['formData' + this.tabPosition] = false;
        })
      },

      // 拖动排序相关
      refreshTreeAfterDrag() {
        //刷新树数据
        if (this.tabPosition == 1) {
          // 政务联络网
          this.treeDataItem = {};
          this.getPhoneGroupingTree(1);
        } else if (this.tabPosition == 2) {
          // 常用联系人
        } else if (this.tabPosition == 8) {
          // 短信分组
        } else if (this.tabPosition == 4) {
          // 应急联动网
          this.filterText = "";
          this.getEmergencyAddressBookTree();
        } else if (this.tabPosition == 5) {
          // 基层联络网
          this.gliaisonGroupList();
        } else if (this.tabPosition == 6) {
          // 备勤联络网
          this.dutyGroupList();
        } else if (this.tabPosition == 7) {
          // 资源联络网
          this.getResourceGroupingTree(7);
        } else if (this.tabPosition === 8) {
          // 传真
          this.getFaxGroupData();
        } else if (this.tabPosition === 9) {
            // 工业园区管委会通讯录
            this.getSuzhouGroupList();
        } else if (this.tabPosition === 10) {
          // 快捷组
          this.getShortcutGroupData();
        }

        //选中
        this.$nextTick(function () {
          if (this.tabPosition == 1) {
            // 政务联络网
            this.$refs.tree.setCurrentKey(this.groupId);
          } else if (this.tabPosition == 2) {
            // 常用联系人
          } else if (this.tabPosition == 8) {
            // 短信分组
          } else if (this.tabPosition == 4) {
            // 应急联动网
            this.$refs.tree1.setCurrentKey(this.groupId);
          } else if (this.tabPosition == 5) {
            // 基层联络网
            this.$refs.tree5.setCurrentKey(this.groupId);
          } else if (this.tabPosition == 6) {
            // 备勤联络网
            this.$refs.tree6.setCurrentKey(this.groupId);
          } else if (this.tabPosition == 7) {
            // 资源联络网
            this.$refs.tree7.setCurrentKey(this.groupId);
          } else if (this.tabPosition === 8) {
            // 传真
            this.$refs.tree8.setCurrentKey(this.groupId);
          } else if (this.tabPosition === 10) {
            // 快捷组
            this.$refs.tree9.setCurrentKey(this.groupId);
          }
        })
      },
      handleDrop(draggingNode, dropNode, dropType, ev) {
        if (this.childrenlist) {
          let arrays = [];
          this.childrenlist.forEach(function (data) {
            if (data.id > 0) {
              arrays.push(data.id);
            }
          });
          let data = {
            ids: arrays //移动后数组ID
          };

          this.$api.updateContactorGroupOrder(data).then(res => {
            this.refreshTreeAfterDrag();
          })
        }
      },
      allowDrop(draggingNode, dropNode, type) { //拖拽时判定目标节点能否被放置
        this.groupId = draggingNode.data.id;
        if (draggingNode.data.parentId > 0) {
          this.childrenlist = draggingNode.parent.data.children;
        } else {
          this.childrenlist = draggingNode.parent.data;
        }

        if (draggingNode.data.parentId == dropNode.data.parentId) {
          return type !== 'inner';
        } else {
          return false;
        }
      },
      allowDrag(draggingNode) { //判断节点能否被拖拽
        return draggingNode.data.id > 0;
      }
    }
  }
</script>

<style scoped>
  /deep/ [role="treeitem"]{
    margin: 6px 0px;
  }
  .treeSearch /deep/ .el-input__inner{
    background: rgb(234, 234, 234);
    border-radius: 12px;
    opacity: 0.52;
    height: 3vh;
  }
  .el-table /deep/ thead th, .el-table /deep/ thead tr {
    background: rgba(249,252,255,1)!important;
  }
  .treeSearch /deep/ .el-input__suffix{
    height: initial;
    right: 16px!important;
    top: 50%!important;
    transform: translateY(-50%)!important;
  }
  .my-radio-box {
    height: 100%;
    position: relative;
  }
  .el-radio-button{

  }
  /*.my-radio-box .el-radio-button >>> .el-radio-button__inner {*/
  /*  padding: 15px 30px;*/
  /*  border-bottom: none;*/
  /*}*/
  .my-radio-box-main {
    position: relative;
    z-index: 30;
    margin-bottom: 10px;
    background: white;
    box-shadow:0px 3px 10px 2px rgba(54,121,225,0.09);
    border-radius:3px;
  }
  .my-radio-box  >>>.el-radio-button__inner {
    padding: 14px 22px;
    font-size: 14px;
    color: #999999;
    border: none;
    border-bottom: 2px white solid;
    background: white !important;
    box-shadow: none !important;
  }

  .my-radio-box .is-active >>>.el-radio-button__inner {
    border-bottom:2px #0091FF solid ;
    color: #0091FF;
    border-radius: 0;
  }
  >>>.el-radio-button:first-child .el-radio-button__inner{
    border-left:0;
  }

  .my-phone-main {
    box-sizing: border-box;
    height: calc(100vh - 175px);
    overflow: auto;
    display: flex;
  }

  .my-tree-box {
    height: initial;
    background-color: #fff;
    border-top: 1px solid rgba(211, 215, 219, 1);
    border-right: 1px solid rgba(211, 215, 219, 1);
    border-bottom: 1px solid rgba(211, 215, 219, 1);
    width: 26%;
    position: relative;
    min-width: 200px;
    box-shadow:0px 3px 10px 2px rgba(54,121,225,0.09);
    border-radius:3px;
  }

  .my-tree-scroll {
    height: 100%;
    box-sizing: border-box;
    position: relative;
    z-index: 20;
    max-height: 715px;
    padding-bottom: 40px;
  }
  .my-table-box {
    flex: 1;
    border-left: 10px solid #ebeef5;
    background: white;
    box-shadow:0px 3px 10px 2px rgba(54,121,225,0.09);
    border-radius:3px;
    display: flex;
    flex-direction: column;
  }
  .el-tree--highlight-current
    >>> .el-tree-node.is-current
    > .el-tree-node__content {
    background: #e5f4ff;
  }
  .pagination {
    padding: 16px 0;
    text-align: center;
  }
  .my-tree-input-box {
    padding: 10px 5px;
    position: absolute;
    top: -4px;
    left: 0;
    width: 100%;
    height: 40px;
    box-sizing: border-box;
    z-index: 30;
  }

  .filter-tree {
    width: 100%;
    box-sizing: border-box;
    padding-left: 10px;
    flex: 1;
  }
  .filter-tree /deep/ .el-tree-node__content {
    height: 32px !important;
  }
  .filter-tree /deep/ .el-tree-node__expand-icon {
    font-size: 17px;
    color: #8a8a8a;
    margin-right: 10px;
  }
  .filter-tree /deep/ .is-leaf.el-tree-node__expand-icon.el-icon-caret-right{
    margin-right: initial;
  }
  .filter-tree
    /deep/
    .el-tree-node
    .el-tree-node__content
    .el-tree-node__expand-icon:before {
    content: "";
    display: inline-block;
    width: 10px;
    height: 10px;
    background: url("data:image/png;base64,");
    background-size: 100% 100%;
  }

  .filter-tree
    /deep/
    .el-tree-node
    .el-tree-node__content
    .el-tree-node__expand-icon.is-leaf:before {
    width: 0;
  }
  /*一级菜单收缩显示*/
  /deep/
    .filter-tree
    > .el-tree-node
    > .el-tree-node__content
    > .el-tree-node__expand-icon.is-leaf:before {
    margin-right: 20px;
  }
  /deep/ .filter-tree > .el-tree-node > .el-tree-node__content {
    background: none !important;
  }
  /deep/ .filter-tree .el-tree-node__label {
    font-size: 12px;
    letter-spacing: 1px;
    margin-top: -2px;
  }
  /deep/
    .filter-tree
    > .el-tree-node
    > .el-tree-node__content
    > .el-tree-node__label {
    margin-left: 25px;
    position: relative;
    font-size: 12px;
    letter-spacing: 1px;
  }
  /deep/
    .filter-tree
    > .el-tree-node
    > .el-tree-node__content
    > .el-tree-node__label::before {
    content: "";
    position: absolute;
    left: -25px;
    width: 17px;
    height: 17px;
    background: url("data:image/png;base64,");
    background-size: 100% 100%;
  }
  /deep/
    .filter-tree
    > .el-tree-node
    > .el-tree-node__content
    > .el-tree-node__expand-icon.expanded
    ~ span::before {
    content: "";
    position: absolute;
    left: -25px;
    width: 17px;
    height: 17px;
    background: url("data:image/png;base64,") no-repeat;
    background-size: 100% 100%;
  }

  .filter-tree /deep/ .changeImgOften:before {
    content: "";
    position: absolute;
    top: 0;
    left: -25px;
    width: 23px;
    height: 23px;
    background: url("data:image/png;base64,") !important;
    background-size: 90% 90% !important;
  }
  .filter-tree /deep/ .changeImgLately:before {
    content: "";
    position: absolute;
    top: 0;
    left: -25px;
    width: 23px;
    height: 23px;
    background: url("data:image/png;base64,") !important;
    background-size: 90% 90% !important;
  }
  .filter-tree /deep/ .el-tree-node__expand-icon.is-leaf {
    color: transparent;
    cursor: default;
  }
  .filter-tree /deep/ .el-tree-node {
    margin-bottom: 5px;
  }
  .filter-tree /deep/ .el-tree-node__children .is-current::before {
    content: "";
    display: inline-block;
    width: 2px;
    height: 32px;
    background-color: #0091ff;
    position: absolute;
  }
  .filter-tree /deep/ .is-current > div:first-child {
    background: #ebf4fe;
    color: #0091ff;
  }
  .filter-tree /deep/ .is-current .changeImgOften:before {
    background: url("data:image/png;base64,") !important;
    background-repeat: no-repeat;
    height: 17px !important;
  }
  .my-tree-box >>> .el-scrollbar__wrap {
    overflow-x: hidden;
  }

  >>> .el-dropdown-menu__item {
    font-size: 12px;
  }

  .dropdownDiv{
    max-width:250px;
    text-align: left;
  }
</style>
