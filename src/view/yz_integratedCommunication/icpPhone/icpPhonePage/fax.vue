...
export default {
  ...
  methods: {
    ...
    initContactorGroupsConfig() {
      ...
      this.list.forEach((item) => {
        if (item) {
          if (item.code === "phone") {
            ...
            // 资源联络网
            this.otrData.name = item.name;
          } else if (item.code === "faxgroup") {
            // 传真分组
            this.formDataFax.name = item.name;
      ...
      let filterGroup = this.list.filter(
        (item) => item && item.code === "phone"
      ); // 查找是否配置政务联络网，selectedName和selectedCode更改默认值
      if (filterGroup.length === 0) {
        // 如果没有配置政务联络网
        this.tabPosition = Number(this.list[0].label); // 默认选中第一个
      } else {
        // 有配置政务联络网
        this.tabPosition = Number(filterGroup[0].label);
      }
      // 常用联系人、最近联系人
      ...
      let array = [
        {
          ...
          remark: this.IsYangZhouProject ? "政务通信录" : "政务联络网",
        },
