<template>
  <div class="my-radio-box">
    ...
    <div class="my-phone-main" v-if="tabPosition == 1">
      ...
      <div class="my-table-box" style="box-sizing: border-box; overflow: auto">
        <btn-box
          ...
          :btnArry="btnArry1"
          v-bind="{
            refresh: { callback: myRefresh1 },
            refresh1: { callback: myRefresh },
            add: { callback: addList },
            edit: { callback: myEdit },
            del: { callback: handleDelete },
            sel:{ callback:  selectVideo}
          }"
        ></btn-box>
    ...
    <select-video-dialog ref="selectVideoDialog"></select-video-dialog>
...
import watchVideoDialog from "@/view/yz_integratedCommunication/icpVideo/watchVideoDialog";
import TerminalDialog from "@/view/yz_integratedCommunication/icpVideo/terminalDialog";
import watchTerminalDialog from "@/view/yz_integratedCommunication/icpVideo/watchTerminalDialog";
...
import selectVideoDialog from "@/view/yz_integratedCommunication/icpVideo/selectVideo";
...
export default {
  ...
  data() {
    return {
      ...
      // phoneList: window.g.IsYangZhouProject ? [
      //   {
      //     name: "政务通信录",
      //     label: "3",
      //   },
  ...
  methods: {
    ...
    initContactorGroupsConfig() {
      ...
      if (this.confType === 0) {
        this.phoneList = replaceContactorGroupsConfig(this.phoneList); // 根据后端返回分组，替换联系人大分组名字配置
        this.phoneList.forEach((item) => {
          if (item) {
            if (item.code === "phone") {
              // 政务联络网
              this.myData.name = item.name;
            } else if (item.code === "resource") {
              // 资源联络网
              this.ropData.name = item.name;
            ...
            } else if (item.code === "VideoGroup") {
              // 监控视频
              this.formData1.name = item.name;
            } else if (item.code === "VideoTerminalGroup") {
              // 会场终端
              this.formData2.name = item.name;
            } else if (item.code === "GroupTalk") {
              // 集群分组
              this.formData3.name = item.name;
            }
          }
        });
      } else if (this.confType === 1) {
        this.shipinList = replaceContactorGroupsConfig(this.shipinList); // 根据后端返回分组，替换联系人大分组名字配置
      ...
      } else if (this.confType === 2) {
        this.clusterList = replaceContactorGroupsConfig(this.clusterList); // 根据后端返回分组，替换联系人大分组名字配置
    ...
    //视频点呼
    videoCall(row) {
      console.log("videoCall row", row);

      this.$confirm("是否确定要视频点呼", "提示", {
        ...
        .then(() => {
          this.$icp19
            ...
            .then((data) => {
              console.log("MakeCallEx data", data);
    ...
    myRefresh() {
      ...
      if (this.HWICP_Version == "HWICP-19.0") {
        ...
        let data = {
          ...
          topicName: sessionStorage.getItem("seat"),
        };
        console.log("----", data);
        this.btnArry1[0].disabled = true;
        this.$api.reloadDeviceInfo(data).then((res) => {
          if (res.errorcode === 0) {
    ...
    selectVideo(){
          this.$refs.selectVideoDialog.isDialogVisible = true;
          this.$refs.selectVideoDialog.title = "选择设备";
    },
    //表格行内单机方法
    handleRowClick(row) {
     ...
     */
    deleteGroupOne() {
      let key = `myData${this.tabPosition}`;
    ...
    videoremoveGroup() {
      ...
      var that = this;
      axios
        .post(`${window.g.ApiUrl}/eos/communication/video/delete`, {
          id: that["myData" + that.tabPosition].rowData.id,
        })
        .then(function (response) {
          console.log("/eos/communication/video/delete", response);
          if (response.data.errorcode == 0) {
            ...
            that.getVideoList();
            that["myData" + that.tabPosition].rowData = null;
          } else {
            this.$message({
              message: res.msg,
              type: "error",
    ...
    //查看设备信息
    Lookup(row) {
      this.$refs.VideoDialog.isSave = false;
      this.$refs.VideoDialog.isUpdateName = true;
      this.$refs.VideoDialog.isDialogVisible = true;
      this.$refs.VideoDialog.title = "视频设备信息";
      this.$refs.VideoDialog.getDisableVideoGroupTree();
    ...
    async talkCall1(row) {
      ...
      switch (
        ...
        case 1:
          if (row.indexCode == "" || row.indexCode == null) {
            ...
            content = {
              name: row.name,
              call: row.indexCode, // 针对监控视频，改为indexCode
      ...
      if (this.confStatus) {
        if (this.HWICP_Version == "HWICP-19.0") {
          ...
          row.myAuto = false;
          row.myVideo = this.$store.state.ConfBroadcastStatus;
          row.myState = 0;
    ...
    changePage() {
      ...
      this["myData" + this.tabPosition].pageSize = window.g.IsZengChengProject
        ? 40
        : 10; //总页数 bug 9359 增城定制化需求 author：何欣婷 date:2020-12-10
      this["myData" + this.tabPosition].rowData = null;
      ...
      switch (this.tabPosition) {
        case 1:
          this.myData1.groupId = -1; // 默认显示默认分组的联系人从-1开始查起 author: xuchuanxgxing lastUpdateTime: 2020-12-15 15:48
          this.getVideoGroupTree();
          break;
        case 2:
          this.myData2.groupId = -1; // 默认显示全部设备的联系人从-1开始查起 author: xuchuanxgxing lastUpdateTime: 2020-12-15 15:48
        ...
        case 6:
          ...
          break;
        case 7:
          this.myData7.groupId = -1; // 默认显示全部设备的联系人从-1开始查起 author: xuchuanxgxing lastUpdateTime: 2020-12-15 15:48
