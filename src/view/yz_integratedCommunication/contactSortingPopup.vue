...
export default {
    ...
    data() {
        return {
            ...
            IsOpenLabProject: window.g.IsOpenLabProject // 是否是openLab项目
        };
    },
    watch: {
    ...
    methods: {
            ...
            this.$api.groupCollection(data).then(res => {
                ...
                if (this.IsYangZhouProject) {
                    this.treeData = this.treeData.filter(item => {
                        if (item.name !== "基层联络网分组" && item.name !== "备勤联络网分组" && item.name !== "资源联络网分组") {
                        return item
                        }
                    });
                }
        ...
        findAllContactor() {
            ...
            let data = this.IsOpenLabProject ? {
                groupId: this.groupId,
                thisGroupOnly: false, // openLab项目添加这个参数
            ...
            this.$api.contactorOrderList(data).then(res => {
                console.log("contactorOrderList res",res)
                if (res.errorcode === 0) {
