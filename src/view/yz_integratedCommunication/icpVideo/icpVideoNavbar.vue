    ...
    <el-row v-if="searchBarIsShow">
      ...
      <el-col :span="12">
        ...
        <!--<div v-if="MapProjectUrl">-->
          <el-button v-if="updateIsShow && IsLanZhouProject " type="primary" icon="el-icon-refresh" @click="handleUpdateGroup">更新分组</el-button>
          <el-button v-if="updateTerminalIsShow && IsLanZhouProject" type="primary" icon="el-icon-refresh" @click="handleUpdateTerminal">更新终端</el-button>
        <!--</div>-->
...
<script>
  ...
  import RefreshButton from "@/yz_components/button/refreshButton";

  export default {
    components: {RefreshButton, TableButtonGroup, AddButton, DeleteButton, EditButton},
    props: ['placeholder'],
    name: "icpVideoNavbar",
    data() {
      return {
        ...
        updateIsShow:false,//是否显示更新分组按钮
        updateTerminalIsShow:false,//是否显示更新终端分组
        searchText: '',//搜索
        searchBarIsShow:true,//是否显示搜索框
        IsLanZhouProject:window.g.IsLanZhouProject,//是否为兰州项目
      }
    },
    methods:{
      //新增方法
      ...
      handleUpdateTerminal(){
        this.$emit("handleUpdateTerminal");
      },
      //搜索值改变方法
      searchTextChange() {
