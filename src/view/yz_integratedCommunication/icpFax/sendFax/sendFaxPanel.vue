...
<template>
  <div>
    ...
    <div class="sendFaxBox">
      <el-form v-model="faxForm" label-width="80px" @submit.native.prevent style="margin:32px" label-position="right">
  ...
  export default {
    ...
    mounted() {
      ...
      if (name != '' && fax_call != '' && name != null && fax_call != null) {
        this.peopleList = [{name: sessionStorage.getItem('fax_name'), fax: sessionStorage.getItem('fax_call')}]
    ...
    methods: {
      ...
      showData(data) {
        this.peopleList.push(
          {contactorId: data.id, name: data.name, fax: data.fax}
        );
