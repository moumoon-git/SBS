    ...
    <el-row style="display: flex; height: 100%">
      <el-col>
        ...
        >
          <div
            class="commonInfo"
            :style="{ height: activeAllModel == '电话' ? '695px' : '695px' }"
          >
            <div class="phoneHeader">
              ...
              <div class="phoneIndex_header" v-if="activeAllModel == '电话'">
                ...
                <div class="phoneIndex_header_Tab" @click="tabCommon">
                  <img
                    v-if="!tabObj.common"
                    src="/static/img/Communihistory/common.png"
                    alt=""
                  />
                  <img
                    v-else
                    src="/static/img/Communihistory/common1.png"
                    alt=""
                  />
                ...
                <div class="phoneIndex_header_Tab" @click="tabRecently">
                  ...
                  <img
                    v-else
                    src="/static/img/Communihistory/recently1.png"
                    alt=""
                  />
                  <span>最近联系人</span>
                </div>
              ...
              <div class="framework" v-if="eleFlagHumanFlag">
                ...
                <div class="framework_right">
                  <div class="framework_right_header">
                    <div>
                      <el-input
                        placeholder="请输入姓名/电话号码/职务/工作单位搜索"
                            ...
                            <el-dropdown-menu slot="dropdown">
                              <el-dropdown-item>
                                <div
                                  ...
                                  v-if="item.phoneAllObj.mobile"
                                >
                                  <span
                                    >手机号码：{{
                                      item.phoneAllObj.mobile
                                    }}</span
                                  >
                                  <img
                                    @click="
                                ...
                                <div
                                  class="selectPhoneCell"
                                  v-if="item.phoneAllObj.officeTel"
                                >
                                  <span
                                    >办公室电话：{{
                                      item.phoneAllObj.officeTel
                                    }}</span
                                  >
                                  <img
                                    @click="
                                ...
                                <div
                                  class="selectPhoneCell"
                                  v-if="item.phoneAllObj.homeTel"
                                >
                                  <span
                                    >家庭电话：{{
                                      item.phoneAllObj.homeTel
                                    }}</span
                                  >
                                  <img
                                    @click="
                                ...
                                <!-- 注释原因：修复bug12531 通过与通讯录管理里面，联系人信息字段的对比。3.0联系人电话的字段只有手机号码，家庭电话，办公室电话 -->
              ...
              <div class="commonAndRecently" v-if="!eleFlagHumanFlag">
                ...
                <div class="commonAndRecently_content">
                  ...
                  >
                    <div class="framework_right_content_list_left">
                      <div>
                        {{ item.contactName || item.contactor || call || '无' }}
                        {{ item.position || '无' }}
                      </div>
                      <div>{{ item.workUnit }}</div>
                    </div>
                        ...
                        >
                          ...
                          <el-dropdown-menu slot="dropdown">
                            <el-dropdown-item>
                              ...
                              >
                                <span
                                  >手机号码：{{ item.phoneAllObj.mobile }}</span
                                >
                                <img
                                  @click="
                                    callPhone(item.phoneAllObj.mobile, item.id)
                                  "
                                  src="/static/img/Communihistory/cell1.png"
                                  alt=""
                                />
                              ...
                              >
                                ...
                                <img
                                  @click="
                                    callPhone(item.phoneAllObj.homeTel, item.id)
                                  "
                                  src="/static/img/Communihistory/cell1.png"
                                  alt=""
                                />
...
export default {
  ...
  mounted() {
    ...
    this.tabCommon(); // 隐藏了快捷组，所以全部默认显示通讯录数据
    // } else {
    //   this.tabShortcutGroup();
    // }
    ...
    let wrap = document.getElementsByClassName('submissionReview_wrap')[0];
    if (wrap.parentNode) {
  ...
  methods: {
     ...
     * @author hexinting
     * @date 2020-12-11
     * @description 一键报送 将事故简况内容复制到短信内容框
     */
    giveDataClass() {
      this.activeAllModel = '短信';
      this.$nextTick(() => {
        if (this.$refs.smsRight.copyBusinessCard1.length === 0) {
          this.$refs.smsRight.copyBusinessCard1 = `${this.$store.getters.getOverallEvent}`;
          return;
        }
        this.$confirm(
          '已有短信内容, 是否覆盖?点击【取消】则自动生成的内容到末尾',
          {
          ...
          .catch(() => {
            this.$refs.smsRight.copyBusinessCard1 = `${this.$refs.smsRight.copyBusinessCard1}${this.$store.getters.getOverallEvent}`;
          });
      });
    },
    //初始化数据
    ...
    addressBook() {
      ...
      this.$http({
        ...
        if (res.data.code == 0) {
          ...
          this.reportingUnitData = res.data.data; // console.log("this.treeData: ", this.treeData);
    ...
    callPhoneAssign(item) {
      ...
      // }
      console.log(
        item.mobile || item.mobile1 || item.homeTel || item.officeTel,
      );
    ...
    getRecentlyListData() {
      this.$http({
        ...
        if (res.data.errorcode === 0 || res.data.code === 0) {
          ...
          this.theOrganizationData = res.data.data.data;
          console.log(
            '%c [ his.theOrganizationData ]',
            'font-size:13px; background:pink; color:#bf2c9f;',
            this.theOrganizationData,
          );
        } else {
          this.$message.error(res.msg);
        }
      ...
      //   console.log("/eos/communication/contactor/recentContactorList", res)
      //   if (res.errorcode == 0) {
      //     console.log(res.data)
      //     this.lately.total = res.data.totalElements
      //     if (res.data) {
    ...
    searchBtn() {
      if (
        ...
        if (this.gid === -1) {
          this.affairs({ id: -1 }, dictType,true);
        } else {
          this.affairs({}, dictType,true);
        }
      } else if (this.value == 4) {
    ...
    shortcutGroupList() {
      this.$api.shortcutGroupList({ displayDefault: true }).then((res) => {
        ...
        this.$http({
          ...
          console.log('/mail/mailcontactor/list', res);
          if (res.data.code != 0) {
            this.$message({
              message: res.msg,
              type: 'error',
          ...
          if (res.data) {
            this.settlePhone(res.data.data.list);
          }
          this.organizationData = res.data.data.list;
          this.organization.total = res.data ? res.data.data.totalCount : 0;
        });
      });
    },
    //应急联动网
    ...
    // 政务组织网
    affairs(obj, dictType,check = false) {
      let groupId;
...
.submissionReview_wrap >>> .tabsClass {
  background: #ffffff;
}

.submissionReview_wrap >>> .el-tabs__header.is-top {
  margin: 0;
  height: 52px;
  border-bottom: 0;
}

.submissionReview_wrap >>> .el-tabs__nav-wrap.is-top {
  color: #a2a2a2 !important;
  /* background: #f9fbff; */
  height: 52px;
}

.submissionReview_wrap >>> .el-tabs__nav-scroll {
  margin-top: 10px;
}

.submissionReview_wrap >>> .el-tabs__item {
  font-size: 17px;
  color: #a2a2a2;
}

.submissionReview_wrap >>> .el-tabs__item.is-active {
  color: #409eff;
  padding-left: 10px;
}

.submissionReview_wrap >>> .el-tabs__item:hover {
  color: #6b7280;
}
...
.submissionReview_wrap >>> .my-phone-right-main .sendMsg .operation {
  margin: 20px 0px !important;
}
.submissionReview_wrap .phoneHeaderRadio >>> .el-radio-button__inner {
  padding: 10px 20px;
}
>>> .MessageHandling > div {
  /* height: 100%;
...
.submissionReview_wrap >>> .my-phone-right-main-user {
  padding: 0 !important;
...
.submissionReview_wrap >>> .sendMsg {
  overflow: hidden;
  padding-bottom: 0;
}
.submissionReview_wrap >>> .sendMsg .el-tabs {
  /* margin: 0 20px; */
}
...
.submissionReview_wrap >>> .my-sms-box {
  height: 214px !important;
  padding-top: 0 !important;
}
.submissionReview_wrap >>> .my-sms-box .NoSMSList {
  height: 100%;
  font-size: 14px;
  font-weight: 500;
  line-height: 322px;
}
.submissionReview_wrap >>> .my-fax-head {
...
.submissionReview_wrap .phoneHeaderRadio {
  display: flex;
  margin: 10px 0;
}
...
.submissionReview_wrap .commonInfo {
  display: flex;
  flex-direction: column;
}
...
.submissionReview_wrap .framework_right_footer {
  position: relative;
  margin-bottom: 3px;
}

.submissionReview_wrap .framework_right_footer > span {
  position: absolute;
  z-index: 100;
  top: 4px;
  bottom: 0;
...
.submissionReview_wrap .framework_right_footer >>> .el-pagination {
  position: relative;
  text-align: center;
}

.submissionReview_wrap .framework_right_footer >>> .el-pagination__jump {
  font-size: 0;
  position: absolute;
  left: 34%;
  height: 25px;
...
.submissionReview_wrap .framework_right_footer >>> .el-pagination .btn-next,
.submissionReview_wrap .framework_right_footer >>> .el-pagination .btn-prev {
  position: relative;
  z-index: 99;
}
...
.submissionReview_wrap .MessageHandling >>> .relatedEvents {
  display: none;
  margin: 10px 10px;
}
.submissionReview_wrap .tabpanel >>> .el-scrollbar__wrap {
  overflow: hidden;
}
</style>
