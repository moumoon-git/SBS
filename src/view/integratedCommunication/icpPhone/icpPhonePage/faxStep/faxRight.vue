<template>
  <div class="my-phone-right-main">
    ...
    <div class="my-phone-right-main-user">
      ...
      <el-dialog title="存为传真分组" :visible.sync="dialogVisible" width="30%" :close-on-click-modal="false" :append-to-body="true">
        <div class="tree">
      ...
      >
        ...
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" @click="setType('overwrite')" size="medium"
            >覆 盖</el-button
          >
          <el-button
            type="primary"
      ...
      >
              ...
              <el-form-item>
                ...
                <div style="width: 100%; text-align: left">
                  <el-upload
                    ...
                    :http-request="upload"
                  >
                    <el-button class="btn-upload" slot="trigger" plain
                      >上传文件</el-button
                    >
          ...
          <!-- <el-row :gutter="gutter">
            <el-col :span="24">
              <el-form-item>
                ...
                <el-input
                  v-show="false"
                  v-model="faxForm.receiptFileName"
                  readonly
                ></el-input>
                <div style="width: 100%; text-align: left">
                  <el-upload
        ...
        <div class="fax-action">
          <el-row :gutter="gutter">
            ...
            <el-col :span="4">
              <el-button type="primary" size="mini" @click="submitUpload"
                >发送</el-button
              >
            </el-col>
          </el-row>
    ...
    >
      ...
      <div style="text-align: right; margin-top: 8px">
        <el-button
          ...
          @click="faxTemplateVisible = false"
          >取 消</el-button
        >
        <el-button
          style="padding: 10px 25px"
          type="primary"
          @click="faxTemplateEnter"
          >确 认</el-button
        >
      </div>
    </el-dialog>
        ...
        <div class="faxdetailright">
          ...
          <div class="faxDetailInform">
            ...
            <span style="margin-top: -4px">
              文件共
              <span style="font-size: 16px">{{ faxDetail.faxList.length }}</span
              >份
            </span>
            <div
          ...
          <div style="margin-top: 10px">
            ...
            >
              <el-tab-pane
                :label="'发送队列（' + faxDetail.waitFileList.length + '）'"
                name="first"
              >
                <div
                  class="sendList"
                  v-for="(item, index) in faxDetail.waitFileList"
                  :key="index"
                >
              ...
              <el-tab-pane
                :label="'发送成功（' + faxDetail.successFileList.length + '）'"
                name="second"
              >
                <div
                  class="sendList"
              ...
              <el-tab-pane
                :label="'发送失败（' + faxDetail.failedFileList.length + '）'"
                name="third"
              >
                <div
                  class="sendList"
                ...
                >
                  ...
                  <span style="float: right">
                    <el-button type="text" size="mini" @click="reSendFax(item)"
                      >重发</el-button
                    >
                  </span>
                </div>
      ...
      <div style="height: 700px">
        ...
        <div class="leftShow">
          ...
          <div class="pictureShow2">
            <div v-if="pictureList.length == 0" class="icon-smile">
              暂无上传图片
            </div>
          </div>
        </div>
    ...
    >
      ...
      <div style="height: 700px">
        <div class="leftShow">
          <div class="pictureShow1">
            <div v-if="faxScanStatus == 0" class="icon-scan3" @click="getScan">
              点击获取扫描件
            </div>
            <div v-else-if="faxScanStatus == 1" class="icon-scanloading">
              正在扫描
            </div>
    ...
    <div class="elDialog" v-show="selectUnitVisible">
      <selectUnitDialog
        v-if="selectUnitVisible"
        ref="selectUnitDialog"
        selectTitle="选择单位"
        :selectData="selectUnitVisible"
        @close="selectUnitVisible = false"
        @closeDialog="selectUnitVisible = false"
        @addUnit="addPerson"
      ></selectUnitDialog>
    </div>
  </div>
...
export default {
  ...
  methods: {
    addPerson(idArr, dataArr){
      ...
      let storeFaxData = JSON.parse(JSON.stringify(this.$store.state.faxMember))
      // 不删除和覆盖之前的数据，做数据合并
      let newArr = Array.from(new Set([...arr,...storeFaxData]))
      this.$store.commit("setFaxMember", newArr);
    },
    // 选择通讯录
    selectHandle() {
      ...
      data.forEach((o, i) => {
        ...
        if (this.findfaxNumber.filter((item) => o === item).length === 0) {
          ...
          this.$api.findContactorByMobile(arg).then((res) => {
            ...
            // } else {
            //   // if (res.data.fax === o) {
            //   //   storeFaxData.push(res.data);
            //   //   this.findfaxNumber.push(o);
            //   // }
            // }
            this.$store.commit("setFaxMember", storeFaxData);
    ...
    getFaxGroupData() {
      ...
      //   });
        ...
        this.$http({
          ...
          console.log('/event/cdrFaxGroup/getCdrFaxGroupList', data);
          if (data.errorcode === 0) {
            this.dialogVisible = true;
            this.faxTreeData = data.data || [];
            this.dialogVisible = true;
          } else {
            this.$message({
                message: data.msg,
    ...
    handleRemove(file, fileList) {
      ...
      this.previewPath = "";
      this.previewName = "";
      const index = this.faxFileList.findIndex((item) => item.id === file.id);
      this.faxFileList.splice(index, 1);
    },
    //上传传真文件前调用的方法
    beforeUpload(file) {
    ...
    //发送点击事件
    submitUpload() {
      if ( new Date(this.faxForm.sendingTime).valueOf() < new Date().valueOf() ) {
        this.$message.warning("发送时间不能早于当地时间!");
        return;
      }
      if (this.faxFileList == "") {
    ...
    sendFax() {
      ...
      this.$http({
        ...
        data: this.$http.adornData({
          senderFaxNo: '',
          faxName: this.faxForm.faxTitle,
          eventId: this.faxForm.caseId,
          sendTime: this.faxForm.sendingTime,
          appAttachmentId: this.faxFileList.map((file) => file.id).join(),
          faxName: this.faxFileList.map((file) => file.name).join(),
          faxFileUrl: this.faxFileList.map((file) => file.url).join(),
          memberList: this.peopleList.map((item) =>{ return { 'receiverFaxNo': item.fax, 'contactType': item.groupType==='messages'?3:2, 'contactId': item.id }}),
        }),
      }).then(({ data }) => {
        console.log('/event/fax/sendFax', data);
        if (data.errorcode === 0) {
          this.$message.success('发送传真成功');
        } else {
    ...
    getRemainTime() {
      ...
      if (TableData.length > 0) {
        for (let i in TableData) {
          ...
          TableData[i].sendTimeAfter = array[1];
