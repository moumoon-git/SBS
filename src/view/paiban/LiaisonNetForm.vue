...
<script>
import checkedGrouping from "./checkedGrouping";
import PositionTree from "./positionTree"; // 导入职务数据
import regular from './regular'
import ParentnodeTree from "./ParentnodeTree.js"; //传入当前节点id拿到父级节点数据
...
export default {
  ...
  data() {
    return {
      imageUrl: require("./img/head.png"), // 默认头像
      deleteUrl: require("./img/delete.png"), // 删除按钮
      uploadUrl: require("./img/upload.png"), // 上传按钮
      ...
      rules: {
        ...
        officeTel: [
          {
            //办公电话
            validator: regular.fixedTelephone,
            trigger: "blur",
          },
        ],
        homeTel: [
  ...
  created() {
    ...
    this.$http.post(window.g.ApiUrl + "/emergency/preparation/dictionary/getByParentCode", {
      pcode: "sex",
    }).then((result) => {
    ...
    this.$http.post(window.g.ApiUrl + "/common/getColumnLength",{
      table: "app_contactor",
    }).then((res) => {
  ...
  methods: {
    ...
    onSelectTree() {
      ...
      this.$http(window.g.ApiUrl + "/emergency/preparation/contactorGroup/topTree", data).then(
        (res) => {
          let data = [res.data];
          let type = "选择联系人分组";
          let treeData = {
