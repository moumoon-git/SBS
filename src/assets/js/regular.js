...
// 电话或固定电话
const mobileOrLandline = (rule, value, callback) => {
  const mobileRegExp = /^1(\d){10}$/;
  const landlineRegExp = /^((\d)+-)?(\d){8}$/;
  const officeRegExp = /^((\d)+-)?(\d){7}$/; // 七位座机
  if (value && !mobileRegExp.test(value) && !landlineRegExp.test(value) && !officeRegExp.test(value)) {
