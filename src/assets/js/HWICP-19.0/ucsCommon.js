...
ucsCommon.prototype.InitSDK = async function(sysConfig) {
  ...
  this.sysVideoType =sysConfig.sysVideoService;
  let doRet = await mqEvent.connect_server(
    sysConfig.listenMQServer,
    this.MQMyCode,
    this.MQDeCode,
    this.MQUser,
    this.MQPwd,
    sysConfig.logCallBack
  );

  this.startAgent();
  let timeStamp = sysConfig.sysConfService === "YouShiKang" ? 20 * 1000 : 1500;  
  this.isConnected = (sysConfig.sysConfService === 'YouShiKang' || sysConfig.sysVideoService === 'YouShiKang')?true:false;
...
ucsCommon.prototype.UnInitialize = async function() {
  ...
  if ("undefined" != this.heartInerval) {
...
ucsCommon.prototype.startAgent = function() {
  let tmp = document.getElementById("startEMTAgent");
  if (tmp === null && this.isFirstInit && (this.sysConfType !== "YouShiKang" || this.sysVideoType !== "YouShiKang")) {
    this.CloseEMTAgent();
    let dom = document.createElement("div");
    dom.setAttribute("id", "startEMTAgent");
    dom.setAttribute("style", "display:none;");
    dom.innerHTML =
      "<iframe scrolling='no' frameborder='0' marginheight='10' marginwidth='40%' width='500' height='500' allowTransparency src='" +
      "EMTProtocol://<Config>" +
      "<MQServer>" +
      this.MQServer +
      "</MQServer>" +
      ...
      "<MQDeCode>" +
      this.MQDeCode +
      "</MQDeCode><KillRunning>1</KillRunning>" +
      "</Config>'></iframe>";
...
ucsCommon.prototype.CloseEMTAgent = function() {
  ...
  axios
    .post(this.ucsServerUrl + "/ucs/communication/send", data)
    .then(function(data) {
...
ucsCommon.prototype.startCheckHeart = function(timeSpan) {
  ...
  this.heartInerval = setInterval(function() {
    ...
    if (
      that.heartBreakTick < 60 ||
      that.heartBreakTick > 100 ||
      that.sysConfType === "YouShiKang"
    ) {
      let requestHeader = that.setRequestHeader(1, 255, 270);
      ...
      if (that.sysConfType !== "YouShiKang") {
        axios
          .post(that.ucsServerUrl + "/ucs/communication/send", data)
          .then(function(data) {
            if (data.data.errorcode == 0) {
              console.log("发送坐席心跳");
            } else {
        ...
        axios
          .post(that.ucsServerUrl + "/ucs/communication/heartBreak", data)
          .then(function(data) {
            if (data.data.errorcode == 0) {
              console.log("发送坐席心跳");
            } else {
...
ucsCommon.prototype.HeartBreakAck = function(data) {
  if (
    "undefined" !== this.heartInerval &&
    this.heartInerval > 0 &&
    this.heartBreakTick < 60
  ) {
    clearInterval(this.heartInerval);
  ...
  if(this.isConnected)
  {
    ...
    switch (loginRet) {
      case 1:
      {
        ...
        if("undefined"!=this.heartInerval && this.heartInerval>0)
        {
          clearInterval(this.heartInerval);
        }
        mqEvent.dis_connect();
...
ucsCommon.prototype.SendCtiConfig = function() {
  ...
  axios
    .post(this.ucsServerUrl + "/ucs/communication/start", data)
    .then(function(data) {
      if (data.data.errorcode == 0) {
...
ucsCommon.prototype.AgentLogin = async function (user, passwd, license, agent_id1, agent_id2, token, userId,operatorId) {
  ...
  this.operatorId = operatorId?operatorId:0;
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/agentLogin",
    data
  ); //.then(function(data) {
...
ucsCommon.prototype.AgentLogout = async function() {
  let requestHeader = this.setRequestHeader(1, 1, 0);
  requestHeader["user"] = this.user;
  requestHeader["passwd"] = "";
  requestHeader["license"] = "";
  requestHeader["agent_id1"] = this.agentId;
  ...
  axios
    .post(this.ucsServerUrl + "/ucs/communication/agentLogout", data)
    .then(function(data) {
      if (data.data.errorcode == 0) {
 ...
 * @constructor
 */
ucsCommon.prototype.AgentChangeStatus = async function(
  agent_id,
  status,
  transferNum
) {
  ...
  let doRet = axios.post(
    this.ucsServerUrl + "/ucs/communication/agentChangeStatus",
    data
  ); //.then(function(data) {
 ...
 * @constructor
 */
ucsCommon.prototype.MakeCall = async function(
  agent_id,
  called,
  iddcode,
  assignObj = {}
  ...
  let that = this;
  // let transNumber = await axios.post(
  //   this.ucsServerUrl +
  //     "/ucs/communication/phone/validateNumber?number=" +
  //     called
  // );
  let transNumber =await this.TranslateNumber(called);
  var requestHeader = that.setRequestHeader(1, 5, 0);
  requestHeader["agent_id"] = agent_id;
  requestHeader["called"] =transNumber;
   // transNumber.status == 200 ? transNumber.data.data : called;
  ...
  let doRet = await axios.post(
    that.ucsServerUrl + "/ucs/communication/makeCall",
    data
  ); //.then(function(data) {
  // 打印返回信息在demo上
  // containerVue.textareaOut = JSON.stringify(data.data);
  if (doRet.data.errorcode == 0) {
    Message.success("正在发起呼叫！");
    ...
    Message.error(doRet.data.msg);
    store.commit("setNowSelectPhoneList", []); // 清空当前呼出电话信息
    store.commit("setIsCallPhone", ""); // 设置可关闭弹窗
    // 删除电话队列对应的信息
    let phoneList = JSON.parse(JSON.stringify(store.getters.getPhoneList));
    let index = phoneList.findIndex(
      m => String(m.content.number) === String(called)
    );
    if (index > -1) {
 ...
 * @constructor
 */
ucsCommon.prototype.MakeCallEx = async function(
  agent_id,
  number,
  iddcode,
  param_1,
  ...
  // );
  let transNumber = await this.TranslateNumber(called);
  let topicName = that.MQMyCode;
  if (topicName == "" || topicName == null || topicName == undefined) {
    console.error("坐席未登录!");
  }
  let requestHeader = this.setRequestHeader(1, 44, 0);
  requestHeader["agent_id"] = agent_id;
  requestHeader["called"] =transNumber;
    // transNumber.status == 200 ? transNumber.data.data : number;
  requestHeader["iddcode"] = iddcode;
  ...
  let doRet = await axios.post(
    that.ucsServerUrl + "/ucs/communication/makeCallEx",
    data
  );
  ...
  if (doRet.data.errorcode == 0) {
    ...
    store.commit("setCallStatus", true);
    store.commit("SET_ISSHOWWIN", false);
    store.commit("setCallee", requestHeader["number"]);
    sessionStorage.setItem("phoneCall", requestHeader["number"]);
  } else {
    Message.error(doRet.data.msg);
  }
  return doRet;
...
ucsCommon.prototype.DropCall = async function(agent_id, conn, key) {
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/dropCall",
    data
  ); //.then(function(data) {
  if (doRet.data.errorcode != 0) {
...
ucsCommon.prototype.HeldCall = async function(agent_id, conn) {
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/heldCall",
    data
  ); //.then(function(data) {
  if (doRet.data.errorcode == 0) {
...
ucsCommon.prototype.AlternateCall = async function(agent_id, conn_a, conn_b) {
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/alternateCall",
    data
  ); //.then(function(data) {
  if (doRet.data.errorcode == 0) {
...
ucsCommon.prototype.TransferCall = async function(agent_id, conn) {
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/transferCall",
    data
  ); //.then(function(data) {
  return doRet;
  // });
 ...
 * @constructor
 */
ucsCommon.prototype.PlayRecordFile = async function(
  agent_id,
  conn,
  recordfile
) {
  ...
  axios
    .post(this.ucsServerUrl + "/ucs/communication/playRecordFile", data)
    .then(function(data) {});
};
...
ucsCommon.prototype.CreateConf = async function(agent_id) {
  ...
  axios
    .post(this.ucsServerUrl + "/ucs/communication/createConf", data)
    .then(function(data) {});
};
/**
 ...
 * @constructor
 */
ucsCommon.prototype.CreateConfEx = async function(
  agent_id,
  memberNum,
  numType,
  winLocation
  ...
  let requestHeader = this.setRequestHeader(1, 255, 271);
  let that = this;
  let numberList = !memberNum ? [] : memberNum.split(",");
  let numTypeList = !numType ? [] : numType.split(",");
  // await numberList.forEach(async function (item, index, items) {

  for (let i = 0; i < numberList.length; i++) {
    let memInfo = numberList[i].split("|*|");
    if (memInfo.length > 1) {
      let tmpType = numTypeList[i];
      if (
        String(tmpType) === "1" &&
        memInfo[0].length >= 7 &&
        !isNaN(Number(memInfo[0]))
      ) {
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/createConfEx",
    data
  ); //.then(function(data) {
  if (doRet.data.errorcode == 0) {
    console.log("已发起创建会议！");
  ...
  //       store.commit("setConfStatus", true); //设置会议状态
  //     }
  //   });
};
...
ucsCommon.prototype.DeleteConf = async function(agent_id, conf_id) {
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/deleteConf",
    data
  ); //.then(function(data) {
...
ucsCommon.prototype.JoinConf = async function(agent_id, conf_id, conn) {
  ...
  axios
    .post(this.ucsServerUrl + "/ucs/communication/joinConf", data)
    .then(function(data) {});
};
 ...
 * @constructor
 */
ucsCommon.prototype.JoinConfEx = async function(
  agent_id,
  conf_id,
  conn,
  phoneNumber,
  ...
  axios
    .post(this.ucsServerUrl + "/ucs/communication/joinConfEx", data)
    .then(function(data) {});
};
 ...
 * @constructor
 */
ucsCommon.prototype.JoinConfEx3 = async function(
  agent_id,
  conf_id,
  conn,
  phoneNumber,
  ...
  if (Number(numType) < 8) {
    ...
    phoneNumber =
      transNumber.status == 200 ? transNumber.data.data : phoneNumber;
  }
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/joinConfEx3",
    data
  ); //.then(function(data) {
  if (doRet.data.errorcode == 0) {
    console.log("已发起加入会议！");
...
ucsCommon.prototype.RemoveFromConf = async function(agent_id, conf_id, conn) {
  ...
  axios
    .post(this.ucsServerUrl + "/ucs/communication/removeFromConf", data)
    .then(function(data) {
      if (data.data.errorcode == 0) {
        console.log("已发起移出成员！");
      }
 ...
 * @constructor
 */
ucsCommon.prototype.RemoveFromConfEx = async function(
  agent_id,
  conf_id,
  conn,
  number,
  ...
  if (String(numberType) === "1" && number.length >= 7) {
    ...
    number = transNumber.status == 200 ? transNumber.data.data : number;
  }
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/removeFromConfEx",
    data
  ); //.then(function(data) {
  if (doRet.data.errorcode == 0) {
...
ucsCommon.prototype.RequestConfMember = async function(agent_id, conf_id) {
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/requestConfMember",
    data
  );
  return doRet;
};
...
ucsCommon.prototype.RequestConf = async function(agent_id) {
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/requestConf",
    data
  );
  return doRet;
};
 ...
 * @constructor
 */
ucsCommon.prototype.OperateConfMember = async function(
  agent_id,
  conf_id,
  conn,
  member_type
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/operateConfMember",
    data
  );
  return doRet;
};
 ...
 * @constructor
 */
ucsCommon.prototype.OperateConfMemberEx = async function(
  agent_id,
  confId,
  conn,
  type,
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/operateConfMemberEx",
    data
  ); //.then(function(data) {
  if (doRet.data.errorcode == 0) {
    if (type == 2) {
 ...
 * @constructor
 */
ucsCommon.prototype.SendFax = async function(
  case_id,
  opr_name,
  fax_number,
  fax_name,
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/send",
    data
  );
  return doRet;
};
...
ucsCommon.prototype.TransferCallOneKey = async function(agent_id, mobile_to) {
  ...
  requestHeader["agent_id"] = agent_id;
  requestHeader["mobile_to"] =
    transNumber.status == 200 ? transNumber.data.data : mobile_to;
  var data = {
    data: {
      topicName: this.MQMyCode,
      text: requestHeader
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/transferCallOneKey",
    data
  );
  if (doRet.data.errorcode == 0) {
 ...
 * @constructor
 */
ucsCommon.prototype.ThreeWayConf = async function(
  agent_id,
  callTag,
  thirdNum,
  numType
  ...
  requestHeader["thirdNum"] =
    transNumber.status == 200 ? transNumber.data.data : thirdNum;
  requestHeader["numType"] = numType;
  var data = {
    data: {
      topicName: this.MQMyCode,
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/threeWayConf",
    data
  );
  if (doRet.data.errorcode == 0) {
 ...
 * @constructor
 */
ucsCommon.prototype.SetVideoWindow = async function(
  agent_id,
  localHandle,
  remoteHandle
) {
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/setVideoWindow",
    data
  );
  return doRet;
};
 ...
 * @constructor
 */
ucsCommon.prototype.ConfBoardCastMember = async function(
  agent_id,
  conf_id,
  mixType,
  phoneNum,
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/confBoardCastMember",
    data
  ); //.then(function(data) {
  if (doRet.data.errorcode == 0) {
    if (isCannel == 1) console.log("已发起取消广播画面功能");
    else if (isCannel == 0) console.log("已发起广播画面功能");
    // typeof call ==='function'&&call(data)
  } else {
 ...
 *@date 2020/1/2
 */
ucsCommon.prototype.ConfSetFloorSite = async function(
  agentId,
  confId,
  floorSite
) {
  ...
  axios
    .post(this.ucsServerUrl + "/ucs/communication/confSetFloorSite", data)
    .then(function(data) {
      if (data.data.errorcode == 0) {
        console.log("已发起点名发言");
      }
...
ucsCommon.prototype.ConfReleaseChair = async function(agentId, confId) {
  ...
  axios
    .post(this.ucsServerUrl + "/ucs/communication/confReleaseChair", data)
    .then(function(data) {
      if (data.data.errorcode == 0) {
...
ucsCommon.prototype.ConfReconnectSite = async function(agentId, confId, sites) {
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/confReconnectSite",
    data
  );
  if (doRet.data.errorcode == 0) {
    console.log("已发起重呼");
 ...
 * @constructor
 */
ucsCommon.prototype.ConfWatchMember = async function(
  agent_id,
  conf_id,
  mixType,
  phoneNum
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/confWatchMember",
    data
  );
  return doRet;
};
 ...
 * @constructor
 */
ucsCommon.prototype.ICPQueryGroupInfo = async function(
  agent_id,
  iType,
  iPageIndex,
  iPageSize,
 ...
 * @constructor
 */
ucsCommon.prototype.ICPQueryDeviceInfo = async function(
  agent_id,
  iType,
  iPageIndex,
  iPageSize,
 ...
 * @constructor
 */
ucsCommon.prototype.ICPStartRealPlay = async function(
  agent_id,
  iType,
  devName,
  windowHandle,
 ...
 * @constructor
 */
ucsCommon.prototype.ICPStopRealPlay = async function(
  agent_id,
  iType,
  playHandle
) {
 ...
 * @constructor
 */
ucsCommon.prototype.ICPPTZDevice = async function(
  agent_id,
  vcnId,
  devNum,
  ptzCommand,
 ...
 * @constructor
 */
ucsCommon.prototype.ConfAddDevice = async function(
  agent_id,
  iType,
  conf_id,
  devName
...
ucsCommon.prototype.AnswerCall = async function(agent_id, conn, isVideo) {
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/answerCall",
    data
  );
  if (doRet.data.errorcode == 0) {
 ...
 * @constructor
 */
ucsCommon.prototype.TalkingGroupSubcribe = async function(
  agent_id,
  expire,
  groupName
) {
 ...
 * @constructor
 */
ucsCommon.prototype.TalkingGroupMonitor = async function(
  agent_id,
  groupName,
  isMonitor,
  isAutoSubcribe
 ...
 * @constructor
 */
ucsCommon.prototype.TalkingGroupSelect = async function(
  agent_id,
  groupName,
  isSelect
) {
 ...
 * @constructor
 */
ucsCommon.prototype.TalkingGroupPTT = async function(
  agent_id,
  groupName,
  isPTT
) {
  ...
  //   if (data.data.errorcode == 0) {
  //     if (isPTT == 1)
  //       Message.success("抢权成功，你有1分钟说话时间");
  //     else if (isPTT == 0)
  //       Message.success("无线集群群组已放权！");
  //   }else{
  //     Message.error("抢权失败，请稍等片刻重试");
  //   }
  // });
};
 ...
 * @constructor
 */
ucsCommon.prototype.MixedGroupCall = async function(
  agent_id,
  groupNum,
  groupName
) {
  ...
  return await axios.post(
    this.ucsServerUrl + "/ucs/communication/mixedGroupCall",
    data
  ); //.then((data)=>{
  //   if(data.conf_id == -1){
  //     Message.error('连接失败，错误码：'+data.errcode)
 ...
 * @constructor
 */
ucsCommon.prototype.ConfAcceptInvite = async function(
  agent_id,
  conf_id,
  isAccept,
  confNumber
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/confAcceptInvite",
    data
  );
  return doRet;
};
 ...
 * @constructor
 */
ucsCommon.prototype.SnatchPickup = async function(
  agent_id,
  conn,
  agentTag,
  agentNum
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/snatchPickup",
    data
  );
  return doRet;
};
...
ucsCommon.prototype.ConfStartData = async function(agent_id, confId, isStart) {
  ...
  let doRet = await axios.post(
    this.ucsServerUrl + "/ucs/communication/confStartData",
    data
  );
  return doRet;
};
 ...
 * @constructor
 */
ucsCommon.prototype.ConfSetVideoSource = async function(
  agent_id,
  confId,
  isLock,
  dstMember,
  ...
  return await axios.post(
    this.ucsServerUrl + "/ucs/communication/confSetVideoSource",
    data
  );
};
 ...
 * @constructor
 */
ucsCommon.prototype.OperateBlackList = async function(
  agent_id,
  phoneNumber,
  iControl
) {
  ...
  return await axios.post(
    this.ucsServerUrl + "/ucs/communication/operateBlackList",
    data
  );
};
...
ucsCommon.prototype.AgentSendDtmf = async function(agent_id, dtmfNum) {
  ...
  return await axios.post(
    this.ucsServerUrl + "/ucs/communication/agentSendDtmf",
    data
  );
};
...
ucsCommon.prototype.AgentAccessConf = async function(agent_id, conf_id) {
  ...
  return await axios.post(
    this.ucsServerUrl + "/ucs/communication/agentAccessConf",
    data
  );
};
...
ucsCommon.prototype.FaxScan = async function(path) {
  ...
  axios
    ...
    .then(function(data) {
      if (data.data.errorcode == 0) {
        store.commit("setfaxScanStatus", 1);
        // store.commit('setscanFilePath','http://192.168.3.36:8088/eos/communication/fax/loadPDF/3ba3a980296545ca94f85e249a1c4fc1.pdf');
      }
    });
};
...
ucsCommon.prototype.ReverseNumber = async function(number) {
  ...
  let transNum = await axios.post(
    this.ucsServerUrl +
      "/ucs/communication/phone/reverseValidatedNumber?number=" +
      number +
      "&agentId=" +
      this.MQMyCode
  );
