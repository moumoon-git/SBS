...
import mqEvent from "./icpInterface19.0.js";
...
//return ;
  ...
  let agent = await axios.post(this.sysConfig.ucsServerUrl + '/ucs/communication/phone/agent', data);
  console.dir(agent);
  if (agent.data.errorcode == 0) {
    console.log('获取agent账号成功');
    let loginResult = await this.xwFcc.login({
...
xwSDK.prototype.GetTag = function () {
  let now = new Date();
  let starTime = new Date('2020-2-26');
  return now - starTime;
}
/**
 * 发起呼叫
...
xwSDK.prototype.MakeCallEx = async function (agent_id, number, iddcode, param_1, numType) {
  ...
  if (Number(numType) === 1 && number.length >= 8) {
    let doRet = await axios.post(this.sysConfig.ucsServerUrl + '/ucs/communication/phone/validateNumber?number=' + number+"&agentId="+this.sysConfig.agentNum);
...
xwSDK.prototype.HeldCall = async function (agent_id, conn) {
  ...
  let doRet = await this.xwFcc.hold({
    ishold: true
  });
  console.log(doRet);
  if (doRet.status === 200) {
...
xwSDK.prototype.TransferCallOneKey = async function (agent_id, mobile_to) {
  ...
  let transRet = await this.xwFcc.transfer({
    address: transNum,
    devicetype: numType,
    mode: 2
...
xwSDK.prototype.ThreeWayConf = async function (agent_id, callTag, thirdNum, numType) {
  ...
  if (thirdNum.length == 8 || thirdNum.length == 11) {
    let doRet = await axios.post(this.sysConfig.ucsServerUrl + '/ucs/communication/phone/validateNumber?number=' + thirdNum+"&agentId="+this.sysConfig.agentNum);
    transNum = doRet.status === 200 ? doRet.data.data : transNum;
    numType = 5;
  }
  let transRet = await this.xwFcc.confjoin({
    address: transNum,
    devicetype: numType,
    mode: 4
  });
...
xwSDK.prototype.CreateConfEx = async function (agent_id, memberNum, numType, winLocation) {
  ...
  for (let index = 0; index < numberList.length; index++) {
    ...
    let item = numberList[index];
    let memInfo = item.split("|*|");
    if (memInfo.length > 1) {
      let oType;
      let pType;
      let tmpType = numTypeList[index];
      if (tmpType === '1') {
        oType = '3';
        pType = '3';
        let transNumber = await axios.post(this.ucsServerUrl + '/ucs/communication/phone/validateNumber?number=' + memInfo[0]+"&agentId="+this.sysConfig.agentNum);
  ...
  let createRet = await this.xwFcc.createConf({
    meetingName: '视频会议',
    members: memberList,
...
xwSDK.prototype.JoinConfEx3 = async function (agent_id, conf_id, conn, phoneNumber, numType, displayName) {
  let oType;
  let pType;
  let tmpType = String(numType);
  if (tmpType === '1') {
    oType = '3';
    pType = '3';
    let transNumber = await axios.post(this.ucsServerUrl + '/ucs/communication/phone/validateNumber?number=' + phoneNumber+"&agentId="+this.sysConfig.agentNum);
  ...
  if (oType === '1') {
    ...
    //如果是视频监控，需要按@切割监控编码，分出id与sno
    let numInfo = phoneNumber.split('@');
    if (numInfo.length > 1) {
      memberList[0].OBJ_ID = numInfo[0];
      memberList[0].PART_SNO = numInfo[1];
    }
  }
  let doRet = await this.xwFcc.addMeetingMembers({
    members: memberList, meetingInfo: {
      MEETING_ID: conf_id,
...
xwSDK.prototype.ICPStartRealPlay = async function (agent_id, iType, devName, windowHandle, iX, iY, iWidth, iHeight) {
  let devInfo = !devName ? [] : devName.split('@');
  iX = !iX ? '0' : iX;
  iY = !iY ? '0' : iY;
  iWidth = !iWidth ? '600' : iWidth;
  iHeight = !iHeight ? '400' : iHeight;
  let playRet = await this.xwFcc.openMonitor({
    resId: devInfo[0],
    x: String(iX),
    y: String(iY),
    width: Number(iWidth),
...
xwSDK.prototype.OnAgentLoginAck = function (message) {
...
xwSDK.prototype.ReverseNumber = async function (number) {
  let transNum = await axios.post(this.sysConfig.ucsServerUrl + '/ucs/communication/phone/reverseValidatedNumber?number=' + number+"&agentId="+this.sysConfig.agentNum);
