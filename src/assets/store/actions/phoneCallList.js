...
export default {
    ...
    atPresentPhone.forEach((item, index) => {
      phoneList.forEach((items, indexs) => {
        if (item.content.number == items.content.number) {
          ...
          phoneList[indexs].isCallState = false; //是否是通话状态 // bug 7309 author:hexinting date:2020-10-28
        }
      });
    });
  ...
  calledSuccess({ commit, state }, data) {
    ...
    content = {
      ...
      state: 0,
      important: callinPhone.important,
        picture: callinPhone.picture
    };
    ...
    let atPresentPhone = store.getters.getNowSelectPhoneList,
      atPhoneList = store.getters.getPhoneList;
    atPresentPhone.forEach((item, index) => {
      atPhoneList.forEach((items, indexs) => {
        if (item.content.number == items.content.number) {
          atPhoneList[indexs].myState = 1; //改变状态成通话中
          atPhoneList[indexs].isCallState = true; //是否是通话状态  // bug 7309 author:hexinting date:2020-10-26
        } else {
          atPhoneList[indexs].isCallState = false; //是否是通话状态  // bug 7309 author:hexinting date:2020-10-28
        }
      });
    });
    ...
    atPresentPhone.forEach((item, index) => {
      phoneList.forEach((items, indexs) => {
        if (item.content.number == items.content.number) {
          ...
          phoneList[indexs].isCallState = false; //是否是通话状态  // bug 7309 author:hexinting date:2020-10-26
        }
      });
    });
    commit("setPhoneList", phoneList); //提交修改状态之后的通话列表
    ...
    atPresentPhone.forEach((item, index) => {
      phoneList.forEach((items, indexs) => {
        if (item.content.number == items.content.number) {
          ...
          phoneList[indexs].isCallState = true; // 是否通话状态 // bug 7309 author:hexinting date:2020-10-26
        } else {
          phoneList[indexs].isCallState = false; // 是否通话状态 // bug 7309 author:hexinting date:2020-10-28
        }
      });
    });
    commit("setPhoneList", phoneList); //提交修改状态之后的通话列表
  ...
  async callMultitap({ dispatch, commit, state }) {
    console.log('------------电话转接---------------')
    // bug 8392，8449 手动转接通话结束后弹出框未显示呼入人信息 author:hexinting date:2020-11-18
    let callinPhone = store.getters.getUserCall;
    let content;
    let phoneList = store.getters.getPhoneList; // 通话队列
    let isHavePhone = phoneList.filter(item => item.number === callinPhone.mobile); // 判断phoneList 中是否有当前通话的号码
    console.log('电话转接-callinPhone:', callinPhone)
    content = {
      name: callinPhone.name,
      number: callinPhone.mobile,
      workUnit: callinPhone.workUnit,
      position: callinPhone.position,
      state: 0,
      important: callinPhone.important,
    ...
    phoneList.forEach((items, indexs) => {
      ...
      phoneList[indexs].isCallState = false; // 是否通话状态 // bug 7309 author:hexinting date:2020-11-7
      phoneList[indexs].time = "00:00:00";
    });
    if (isHavePhone.length === 0) {  // 判断phoneList（通话队列） 中是否有当前通话的号码；没有就插入通话队列
      phoneList.push({
        myState: 0, // 0 >=待呼出 1>= 通话中  2>= 保持中 3>= 呼叫中 4>= 呼叫失败
        content: content,
        time: "00:00:00",
    ...
    commit("setNowSelectPhoneList", keepList);
    commit("setPhoneList", phoneList); //提交修改状态之后的通话列表
  ...
  async manyCalls({ commit, state }, data) {
    console.log('电话呼入添加到呼入弹窗队列',data,JSON.parse(JSON.stringify(store.getters.getIsManyCallsList)));
    ...
    let city;
    let tmpCity = await axios.post(
      Url + "/eos/communication/phone/getNumberArea",
      numberData
    );
  ...
  manyPeopleReply({ commit, state }, data) {
    ...
    // 修复控制台报错 author:hexinting date:2020-11-5
    console.log(store.getters.getIsManyCallsList, phoneList);
    if (callList.length > 0) {
      ...
      if (index > -1) {
        ...
        store.commit("setIsDialogType", 'small'); // author:hexinting date:2020-11-5 通话结束，改为小弹窗 显示通话进度条
        store.dispatch("answerCalls");
        // 删除电话队列对应的信息
        let phoneListIndex = phoneList.findIndex(
          m => String(m.content.number) === String(number)
        );
        if (phoneListIndex > -1) {
          phoneList.splice(phoneListIndex, 1);
        }
        store.commit("setPhoneList", phoneList);
        console.log('---更新通话队列', callList)
        commit("setIsmManyCallsList", callList);
      }
    }
  }
