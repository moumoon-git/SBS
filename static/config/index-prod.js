...
(function() {
    window.SITE_CONFIG = {};
    // api接口请求地址
    window.SITE_CONFIG["cloudUrl"] = "http://192.168.3.201:8055";
    // websocket 服务地址
    window.SITE_CONFIG["websocketUrl"] = "http://192.168.3.201:9999"
    // SOC服务接口
    window.SITE_CONFIG["baseUrl"] = window.SITE_CONFIG["cloudUrl"] + '/ser';
   // sockjs websocket服务接口
   window.SITE_CONFIG["websocket"] = window.SITE_CONFIG["websocketUrl"] + "/endpointOyzc";
    // 3.0模块文件服务接口
    window.SITE_CONFIG["fileupload"] = window.SITE_CONFIG["cloudUrl"] + '/fileupload';
    // 3.0模块事件服务接口
    ...
    // 微前端项目部署地址
    window.SITE_CONFIG['micro'] = 'http://192.168.3.201:9955';
    // 图片转文字服务
    window.SITE_CONFIG["proxyUrl"] = "http://192.168.3.48:8200";
    // 华为ASR访问录音的地址
    window.SITE_CONFIG['HWASR'] = 'http://222.189.189.181:9998';
    ...
    // SOC可视化页面跳转uri
    window.SITE_CONFIG["visual"] = window.SITE_CONFIG["cloudUrl"] + "/visualizion/index.html";
    // SOC原来的C端websocket地址
    window.SITE_CONFIG["websocket_phone"] = "ws://localhost:9529";
    // SOC自动登录后首页的路由地址
    window.SITE_CONFIG["autoLogin"] = "/PerformanceStatistics/WorkOrderOverview/WorkOrderOverview";
})();
...
window.g = {
    ...
    semanticsAnalysisUrl: "http://192.168.3.43:8092", // 固定话术语义分析访问地址（智能接报弹窗使用）
    ...
    IsShenzhenProject: true, //是否为深圳市政府项目（事件接报独立业务、信息处置（短信回拨）等业务）
